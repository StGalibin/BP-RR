...
Процедура ЗаполнитьПравилаОрганизации(Правила, Организация, Знач НачалоИнтервала, Знач КонецИнтервала, Отбор = Неопределено, МенеджерВременныхТаблиц = Неопределено, ТекущийПериод = Неопределено) Экспорт
	...
	СоздатьАктуальныеПравила(МенеджерВременныхТаблиц, НачалоИнтервала, КонецИнтервала, Организация, ОтборПоИмениЗадачи, ОтборПоДействию, ТекущийПериод);
...
//                             что относятся к уплате налога по задаче с этим именем
//
Процедура СоздатьАктуальныеПравила(МенеджерВременныхТаблиц, НачалоОбзора, КонецОбзора, Организация, ИмяЗадачи, Действие, ТекущийПериод = Неопределено)
