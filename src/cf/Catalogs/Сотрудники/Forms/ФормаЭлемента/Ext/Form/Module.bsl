...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДополнительныеПараметры.Вставить("Объект", ЭтотОбъект.Сотрудник);
	...
	Если УчетЗарплатыИКадровСредствамиБухгалтерии Тогда
		...
		Если НЕ ЗначениеЗаполнено(ФизическоеЛицо.ДатаРождения) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не указана дата рождения'"),
				,
		...
		Если Элементы.ДокументыФизическихЛицВидДокумента.Видимость Тогда
			Если НЕ ЗначениеЗаполнено(ДокументыФизическихЛиц.ВидДокумента) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Не указан вид документа, удостоверяющего личность'"),
					,
					"ДокументыФизическихЛиц.ВидДокумента",
					,
					Отказ);
			...
			Если НЕ ЗначениеЗаполнено(ДокументыФизическихЛиц.Серия) ИЛИ НЕ ЗначениеЗаполнено(ДокументыФизическихЛиц.Номер) Тогда
				Если (НЕ ЗначениеЗаполнено(ДокументыФизическихЛиц.ВидДокумента)
					...
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						...
						Отказ);
				Иначе
					Если НЕ ЗначениеЗаполнено(ДокументыФизическихЛиц.Серия) Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				...
				КонецЕсли;
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(ДокументыФизическихЛиц.КемВыдан) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru='Не указано кем выдан %1'"),
		...
		МассивДанных = ФизическоеЛицо.КонтактнаяИнформация.НайтиСтроки(
			Новый Структура("Вид",ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.АдресПоПропискеФизическиеЛица")));
		Источник = ?(МассивДанных.Количество()<> 0, МассивДанных[0], Неопределено);
		АдресПоПропискеЗаполнен = ?(Источник = Неопределено, Ложь, ЗначениеЗаполнено(Источник.Представление));
		Если НЕ АдресПоПропискеЗаполнен Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не указан адрес по прописке'"),
				,
...
Процедура Подключаемый_ВыполнитьКомандуПечати(Команда)
	...
	УправлениеПечатьюКлиент.ВыполнитьПодключаемуюКомандуПечати(Команда, ЭтотОбъект, Сотрудник);
...
Процедура КадровыеДокументы(Команда)
	...
	СотрудникиКлиент.ОткрытьДополнительнуюФорму(
		СотрудникиКлиентСервер.ОписаниеДополнительнойФормы("ЖурналДокументов.КадровыеДокументы.ФормаСписка"), ЭтотОбъект);
...
Процедура ОткрытьДополнительнуюФорму(ОписаниеДополнительнойФормы, Форма, ОписаниеОповещения = Неопределено, КлючНазначенияИспользования = "", ОписаниеДанных= "") Экспорт
	...
	ПараметрыФормы = СотрудникиКлиент.ПараметрыДополнительнойФормы(ОписаниеДополнительнойФормы, Форма);
...
Процедура ОбновитьПредставлениеНалогаНаДоходы(ИсточникДанных = Неопределено, Знач СтруктураВычетов = Неопределено)
	...
	ИначеЕсли СтатусФизическихЛицКакНалогоплательщиковНДФЛ.Статус = Нерезидент Тогда
...
Процедура ОбновитьПредставлениеАдресаПоПрописке(ИсточникДанных = Неопределено)
	...
	Если НЕ ЗначениеЗаполнено(КонтактнаяИнформацияПолеАдресПоПропискеФизическиеЛица) Тогда
		...
		Элементы.КонтактнаяИнформацияПолеАдресПоПропискеФизическиеЛица.ЦветТекста = ЦветСтиляПоясняющийОшибкуТекст;
	Иначе
		Элементы.КонтактнаяИнформацияПолеАдресПоПропискеФизическиеЛица.ЦветТекста = ЦветСтиляЦветГиперссылки;
	КонецЕсли;
...
Процедура ОбновитьПредставлениеВыплатыЗарплаты(ИсточникДанных = Неопределено)
	...
	Если НЕ ЗначениеЗаполнено(Источник.Период)
		ИЛИ (НЕ ЗначениеЗаполнено(Источник.ЗарплатныйПроект)
...
Процедура ОписаниеВспомогательныхРеквизитовПриЧтенииНаСервере()
	...
	Если УчетЗарплатыИКадровСредствамиБухгалтерии Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ГражданствоФизическихЛицСрезПоследних.Период,
		|	ГражданствоФизическихЛицСрезПоследних.Страна,
		|	ГражданствоФизическихЛицСрезПоследних.ИНН
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СтатусФизическихЛицКакНалогоплательщиковНДФЛСрезПоследних.Период,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СтатусыЗастрахованныхФизическихЛицСрезПоследних.Период,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СведенияОбИнвалидностиФизическихЛицСрезПоследних.Период,
		|	СведенияОбИнвалидностиФизическихЛицСрезПоследних.Инвалидность,
		|	СведенияОбИнвалидностиФизическихЛицСрезПоследних.ДействуетДо,
		|	СведенияОбИнвалидностиФизическихЛицСрезПоследних.ДатаВыдачи
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	БухучетЗарплатыСотрудниковСрезПоследних.Период,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЛицевыеСчетаСотрудниковПоЗарплатнымПроектамСрезПоследних.Период,
		...
		|				И Организация = &Организация) КАК ЛицевыеСчетаСотрудниковПоЗарплатнымПроектамСрезПоследних";
...
Процедура УстановитьПараметрыИностранногоГражданина()
	...
	Если НЕ ГражданинРФ Тогда
		Если НЕ ЗначениеЗаполнено(СтатусыЗастрахованныхФизическихЛиц.Период) Тогда
			ПрочитатьЗаписьСтатусыЗастрахованныхФизическихЛицДляРедактированияВФорме();
		КонецЕсли;
...
&НаСервере
Процедура ПрочитатьЗаписьСтатусыЗастрахованныхФизическихЛицДляРедактированияВФорме()
	...
	Если НЕ ЗначениеЗаполнено(СтатусыЗастрахованныхФизическихЛиц.Период) Тогда
		...
		Если НЕ МенеджерЗаписи.Выбран() Тогда
			МенеджерЗаписи.ФизическоеЛицо = ФизическоеЛицоСсылка;
		КонецЕсли;
		ЗначениеВРеквизитФормы(МенеджерЗаписи, "СтатусыЗастрахованныхФизическихЛиц");
	КонецЕсли;
