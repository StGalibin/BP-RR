...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.КлючНазначенияИспользования)
		...
		Если Параметры.КлючНазначенияИспользования = "ВыплатаЗарплаты" Тогда
			...
			Элементы.ЛицевыеСчетаСотрудниковПоЗарплатнымПроектамПериодСтрокой.Видимость = Ложь;
			Элементы.ЛицевыеСчетаСотрудниковПоЗарплатнымПроектамИстория.Заголовок       = НСтр("ru = 'История'");
...
Процедура ДействующийЛицевойСчетЗавершение(Ответ, ДополнительныеПараметры) Экспорт 
	...
	УстановитьСтраницуБанкСчетИнфо(ЭтотОбъект);
...
Процедура ПроинициализироватьФорму()
	...
	Если ЛицевыеСчетаСотрудниковПоЗарплатнымПроектамПрежняя = Неопределено Тогда
		...
		Если НЕ ЗначениеЗаполнено(ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.Период) Тогда
			Если ЗначениеЗаполнено(ДатаПриема) Тогда
				ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.Период = НачалоМесяца(ДатаПриема);
			Иначе
...
Процедура УстановитьДоступностьБанкСчет() Экспорт
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ЛицевыеСчетаСотрудниковПоЗарплатнымПроектамИстория",
		"Доступность",
		ОрганизацияЗаполнена И Не ФизическоеЛицоЛСВведенДокументом);
