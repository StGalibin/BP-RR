...
Процедура НаименованиеСокращенноеПриИзменении(Элемент)
	...
	Объект.КодОКОПФ = "";     // Будет заполнено перед записью - см. Справочники.Организации.СформироватьОКОПФ()
...
Процедура ПрочитатьДанныеРуководителя()
	...
	ЕстьОтветственныеЛица = Ложь;
	РуководительФизЛицо   = Неопределено;
	РуководительДолжность = Неопределено;
	РуководительПериод    = '0001-01-01';
	...
	Если НЕ Объект.Ссылка.Пустая() Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			РуководительДолжность  = Выборка.Должность;
			РуководительФизЛицо    = Выборка.ФизическоеЛицо;
			РуководительПериод     = Выборка.Период;
