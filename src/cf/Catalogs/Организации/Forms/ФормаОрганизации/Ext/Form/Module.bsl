...
Процедура СистемаНалогообложенияНажатие(Элемент, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ЭтоМедленныйРежимРаботы ИЛИ ЭтоВебКлиент Тогда
			ОткрытьФорму("ОбщаяФорма.ДлительнаяОперацияНалогиИОтчеты", ПараметрыФормы, ЭтаФорма, УникальныйИдентификатор, , , ОписаниеОповещения);
		Иначе
			ОткрытьФорму("ОбщаяФорма.НалогиИОтчеты", ПараметрыФормы, ЭтаФорма, УникальныйИдентификатор, , , ОписаниеОповещения);
		КонецЕсли;
...
Процедура КодОрганаФСГСПолучателяНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		ЮридическийАдресЗначенияПолей = ЗначенияПолейКонтактнойИнформации("КонтактнаяИнформацияПолеЮрАдресОрганизации");
		Если ЗначениеЗаполнено(ЮридическийАдресЗначенияПолей) Тогда
			КодРегиона = КодРегионаПоАдресу(ЮридическийАдресЗначенияПолей);
...
Процедура РазместитьКонтактнуюИнформацию()
	...
	УправлениеКонтактнойИнформацией.ПриСозданииНаСервере(ЭтотОбъект, Объект, "ГруппаКомпоновкиКонтактнойИнформации", ПоложениеЗаголовкаЭлементаФормы.Лево,
...
Процедура ПослеИзмененияСокращенногоНаименования()
	...
	Справочники.Организации.СформироватьОКОПФ(Объект);
	Справочники.Организации.СформироватьОКФС(Объект);
...
Процедура ИнициализироватьФорму()
	...
	ИначеЕсли ЭтоФизЛицо Тогда
		...
		Элементы.ИПИспользуетТрудНаемныхРаботников.РасширеннаяПодсказка.Заголовок = Новый ФорматированнаяСтрока(
			Новый ФорматированнаяСтрока(НСтр("ru='Флажок следует устанавливать, если есть наемные работники.
			|В этом случае доступен раздел ""Зарплата и кадры"", а также реквизиты, необходимые для расчета зарплаты и взносов с фонда оплаты труда.
			|'")),
			Новый ФорматированнаяСтрока(НСтр("ru='Подробнее'"),,,, "http://its.1c.ru/bmk/bp30/fillingorganisation#entemploy"));
...
Процедура ЗаполнитьСведенияОНалоговойИнспекцииПоАдресу(ЗначенияПолейАдреса, ОписаниеОшибки = "")
	...
	СведенияОНалоговомОрганеПоАдресу = СведенияОНалоговомОрганеПоАдресу(ЗначенияПолейАдреса);
...
Процедура ЗаполнитьСведенияОНалоговойИнспекцииПоКоду(ОписаниеОшибки = "", ПерезаполнятьСведения = Ложь)
	...
	КодРегионаРегистрации                        = Лев(Объект.КодНалоговогоОргана, 2);
	РегистрацияВНалоговомОргане.КодРегиона 		 = Лев(Объект.КодНалоговогоОргана, 2);
...
&НаСервере
Функция ЗначенияПолейКонтактнойИнформации(ИмяРеквизита)
	...
	Если ДанныеСтроки <> Неопределено Тогда
		...
		Возврат ДанныеСтроки.ЗначенияПолей;
...
Процедура ЗаполнитьРегистрационныеДанныеОрганизации(Объект, Реквизиты)
	...
	Если ЗначениеЗаполнено(ЮридическийАдресЗначенияПолей) Тогда
		ЗаполнитьСведенияОНалоговойИнспекцииПоАдресу(ЮридическийАдресЗначенияПолей);
