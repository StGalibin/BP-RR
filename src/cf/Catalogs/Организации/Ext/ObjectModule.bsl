...
Процедура ПередЗаписью(Отказ)
	...
	УправлениеКонтактнойИнформациейБП.ОбновитьИсториюИзмененийАдреса(ЭтотОбъект);
	...
	Если ПустаяСтрока(Наименование) Или Наименование = Справочники.Организации.НаименованиеПоУмолчанию() Тогда
		...
		Иначе
			НовоеНаименование = ОрганизацииФормыКлиентСервер.НаименованиеИндивидульногоПредпринимателя(ФамилияИП, ИмяИП, ОтчествоИП);
		КонецЕсли;
	...
	Если ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		...
		Если ПустаяСтрока(НаименованиеСокращенное) Тогда
			НаименованиеСокращенное = ОрганизацииФормыКлиентСервер.СокращенноеНаименованиеИндивидульногоПредпринимателя(ФамилияИП, ИмяИП, ОтчествоИП);
		КонецЕсли;
		
		Если ПустаяСтрока(НаименованиеПолное) Тогда
			НаименованиеПолное = ОрганизацииФормыКлиентСервер.ПолноеНаименованиеИндивидульногоПредпринимателя(ФамилияИП, ИмяИП, ОтчествоИП);
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(КодОКОПФ) Тогда
		Справочники.Организации.СформироватьОКОПФ(ЭтотОбъект);
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(КодОКФС) Тогда
		Справочники.Организации.СформироватьОКФС(ЭтотОбъект);
	КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	Если НЕ Отказ И ПолучитьФункциональнуюОпцию("НачалоРаботы") Тогда
		Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьСервисРегистрации") Тогда
			// Если используется сервис регистрации, то установка константы "Начало работы"
	...
	РегистрыСведений.ИсторияРегистрацийВНалоговомОргане.ОбновитьИсторию(Ссылка, РегистрацияВНалоговомОргане);
	РегистрыСведений.ИсторияРегистрацийВОрганеПФР.ОбновитьИсторию(Ссылка, РегистрационныйНомерПФР);
