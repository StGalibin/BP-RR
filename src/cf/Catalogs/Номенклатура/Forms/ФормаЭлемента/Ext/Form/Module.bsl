...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ЗаписанаСпецификацияНоменклатуры" Тогда
		...
		Если Объект.Ссылка = Параметр.НоменклатураВладелец Тогда
...
Процедура СтавкаНДСПриИзмененииНаСервере()
	...
	Если Объект.СтавкаНДС <> Перечисления.СтавкиНДС Тогда 
...
Процедура ПолучитьЗначениеЦеныПродажи()
	...
	Если ВыборкаДанных.Следующий() Тогда
		...
		Если Не ВыборкаДанных.ЦенаВключаетНДС Тогда
			...
			ЦенаПродажи    = ВыборкаДанных.Цена * (1 + УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(ВыборкаДанных.СтавкаНДС, Ложь)/100);	
		Иначе
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.КодРаздел7ДекларацииНДС.Видимость       = Форма.РаздельныйУчетНДСНаСчете19
		И Объект.СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.БезНДС");
	...
	Если ЗначениеЗаполнено(Объект.ПериодичностьУслуги) И ЗначениеЗаполнено(Объект.НаименованиеПолное) Тогда
