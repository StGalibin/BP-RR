...
Процедура ПолучитьНаборДанныхВычетНалоговыйАгент(Запрос, СтруктураВнешнихНаборов) 
	...
	|	НДСПредъявленный.СтавкаНДС КАК СтавкаНДС,
	|	СУММА(ЕСТЬNULL(НДСПредъявленный.СуммаБезНДСОстаток, 0) - ЕСТЬNULL(НДСПредъявленныйРеализация0.СуммаБезНДСОстаток, 0)) КАК Сумма,
	|	СУММА(ЕСТЬNULL(НДСПредъявленный.НДСОстаток, 0) - ЕСТЬNULL(НДСПредъявленныйРеализация0.НДСОстаток, 0)) КАК СуммаНДС,
	|	ЗадолженностьПоставщику.СуммаОстатокКт,
...
Процедура ПолучитьНаборДанныхНачислениеАвансыПолученныеУпрощенныйУчетНДС(Запрос, СтруктураВнешнихНаборов) 
	...
	|	ВТРеализацииДокументыОплаты.Контрагент 
	...
	|	КОНЕЦ КАК СтавкаНДС,
	|	СУММА(РасшифровкаПлатежа.СуммаПлатежа) КАК Сумма,
	...
	|	АвансыПолученные.ДоговорКонтрагента,
	|	АвансыПолученные.СтавкаНДС,
	|	АвансыПолученные.СуммаБезНДС - ЕСТЬNULL(ВТСоставКнигиПродаж.СуммаБезНДСОборот, 0) - ЕСТЬNULL(ВТЗачетАвансовПоРеализации.СуммаБезНДСОборот, 0),
	|	АвансыПолученные.СуммаНДС - ЕСТЬNULL(ВТСоставКнигиПродаж.НДСОборот, 0) - ЕСТЬNULL(ВТЗачетАвансовПоРеализации.НДСОборот, 0),
	|	ВТСоставКнигиПродаж.ВидЦенности,
	|	ВТСоставКнигиПродаж.Событие
	|ИЗ
	|	ВТАвансыПолученные КАК АвансыПолученные
	...
	|	СчетФактураВыданныйДокументыОснования.ДокументОснование.ПометкаУдаления,
	|	СчетФактураВыданныйДокументыОснования.Ссылка,
	|	СчетФактураВыданныйДокументыОснования.ДокументОснование КАК ДокументОснование,
	...
	|	ДанныеОтчета.ДоговорКонтрагента,
	|	ДанныеОтчета.Сумма,
...
Процедура ПолучитьНаборДанныхВычетАвансыВыданныеУпрощенныйУчетНДС(Запрос, СтруктураВнешнихНаборов) 
	...
	|	ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18_118) КАК СтавкаНДСПредположительная,
	|	ВЫРАЗИТЬ(ХозрасчетныйОстатки.СуммаОстатокДт - ХозрасчетныйОстатки.СуммаОстатокДт * 18 / 118 КАК ЧИСЛО(15, 2)) КАК СуммаБезНДС,
	|	ВЫРАЗИТЬ(ХозрасчетныйОстатки.СуммаОстатокДт * 18 / 118 КАК ЧИСЛО(15, 2)) КАК СуммаНДС
	...
	|	ВТОстаткиАвансовВыданных.ДоговорКонтрагента,
	|	ВТОстаткиАвансовВыданных.Контрагент,
	|	ВТОстаткиАвансовВыданных.СтавкаНДСПредположительная,
	...
	|	ВТАвансыВыданныеЗаПериод.СтавкаНДСПредположительная,
	|	ВТАвансыВыданныеЗаПериод.ДоговорКонтрагента,
	|	ВТАвансыВыданныеЗаПериод.Контрагент,
	|	ВТАвансыВыданныеЗаПериод.СуммаБезНДС,
	|	ВТАвансыВыданныеЗаПериод.СуммаНДС
	...
	|	РасшифровкаПлатежа.ДоговорКонтрагента,
	|	ВЫБОР
	|		КОГДА РасшифровкаПлатежа.ДоговорКонтрагента.ПредъявляетНДС
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18_118)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС)
	|	КОНЕЦ,
	|	СУММА(ВЫБОР
	|			КОГДА РасшифровкаПлатежа.ДоговорКонтрагента.ПредъявляетНДС
	|				ТОГДА ВЫРАЗИТЬ(РасшифровкаПлатежа.Сумма * 18 / 118 КАК ЧИСЛО(15, 2))
	|			ИНАЧЕ 0
	|		КОНЕЦ),
	|	СУММА(ВЫБОР
	|			КОГДА РасшифровкаПлатежа.ДоговорКонтрагента.ПредъявляетНДС
	|				ТОГДА ВЫРАЗИТЬ(РасшифровкаПлатежа.Сумма - РасшифровкаПлатежа.Сумма * 18 / 118 КАК ЧИСЛО(15, 2))
...
Процедура ПолучитьНаборДанныхВычетСАвансовПолученныхУпрощенныйУчетНДС(Запрос, СтруктураВнешнихНаборов) 
	...
	|ВЫБРАТЬ
	|	СчетФактураВыданныйДокументыОснования.Ссылка,
	|	СчетФактураВыданныйДокументыОснования.ДокументОснование.Ссылка КАК ДокументОснование,
	|	СчетФактураВыданныйДокументыОснования.ДокументОснование.ДоговорКонтрагента КАК ДоговорКонтрагента,
	...
	|	МАКСИМУМ(СчетФактураВыданныйДокументыОснования.ДатаДокумента) КАК ДатаДокумента
	|ПОМЕСТИТЬ КорректировочныеДаты
	|ИЗ
	...
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СчетФактураВыданныйДокументыОснования.ДатаДокумента) > 1
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_СчетФактураВыданныйДокументыОснования.Ссылка,
	|	ВТ_СчетФактураВыданныйДокументыОснования.ДокументОснование,
	|	ВТ_СчетФактураВыданныйДокументыОснования.ДоговорКонтрагента,
	|	ВТ_СчетФактураВыданныйДокументыОснования.ДатаДокумента,
	|	ЕСТЬNULL(КорректировочныеДаты.ДатаДокумента, ВТ_СчетФактураВыданныйДокументыОснования.ДатаДокумента) КАК ДатаКорректировки
	|ПОМЕСТИТЬ СчетФактураВыданныйДокументыОснования
	|ИЗ
	|	ВТ_СчетФактураВыданныйДокументыОснования КАК ВТ_СчетФактураВыданныйДокументыОснования
	|		ЛЕВОЕ СОЕДИНЕНИЕ КорректировочныеДаты КАК КорректировочныеДаты
	|		ПО ВТ_СчетФактураВыданныйДокументыОснования.ДокументОснование = КорректировочныеДаты.ДокументОснование
	|ГДЕ
	|	ВТ_СчетФактураВыданныйДокументыОснования.ДатаДокумента = ЕСТЬNULL(КорректировочныеДаты.ДатаДокумента, ВТ_СчетФактураВыданныйДокументыОснования.ДатаДокумента)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НДСЗаписиКнигиПокупок.СтавкаНДС КАК СтавкаНДС_Аванс,
	|	НДСЗаписиКнигиПокупок.Организация КАК Организация,
	...
	|	НДСЗаписиКнигиПокупок.ДоговорКонтрагента КАК ДоговорАванса,
	|	ИСТИНА КАК НаАванс,
	|	ЛОЖЬ КАК ВозвратЧерезКомиссионера,
	|	НДСЗаписиКнигиПокупок.КонтрагентПоСчетуФактуре,
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗаписиКнигиПокупок.СтавкаНДС_Аванс КАК СтавкаНДС,
	|	ЗаписиКнигиПокупок.ДокументРегистратор КАК ДокументРегистратор,
	|	ЗаписиКнигиПокупок.ДоговорАванса КАК ДоговорКонтрагента,
	|	ЕСТЬNULL(ТаблицаСчетаФактурыДокументы.СчетФактураДокументРасшифровка, ЗаписиКнигиПокупок.СчетФактура) КАК СчетФактура,
...
Процедура ПолучитьНаборДанныхНачислениеРеализация0(Запрос, СтруктураВнешнихНаборов) 
	...
	|	ПодтверждениеНулевойСтавки.ДокументОтгрузки
	...
	|	ВТОтгрузкаПоСтавке0.ДокументРегистратор,
	|	ВТОтгрузкаПоСтавке0.СтавкаНДС,
	|	ВТОтгрузкаПоСтавке0.Сумма,
	|	ВТОтгрузкаПоСтавке0.СуммаНДС,
	|	ВТОтгрузкаПоСтавке0.ПризнакНаличияОшибок,
	|	ВТОтгрузкаПоСтавке0.ПризнакОтраженияНДС,
	|	ВТОтгрузкаПоСтавке0.КоличествоДокументов,
	|	ВТОтгрузкаПоСтавке0.СуммаВключенные,
	|	ВТОтгрузкаПоСтавке0.СуммаНеВключенные,
	|	ВТОтгрузкаПоСтавке0.ВидЦенности,
	|	ВТОтгрузкаПоСтавке0.Событие,
	|	ВТОтгрузкаПоСтавке0.ЗаписьДополнительногоЛиста,
	|	ВТОтгрузкаПоСтавке0.НомерСтроки
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НалоговаяБазаПоОтгрузкам.ДокументРегистратор,
	|	НалоговаяБазаПоОтгрузкам.СтавкаНДС,
	|	НалоговаяБазаПоОтгрузкам.Сумма,
	|	НалоговаяБазаПоОтгрузкам.СуммаНДС,
	|	НалоговаяБазаПоОтгрузкам.ПризнакНаличияОшибок,
	|	НалоговаяБазаПоОтгрузкам.ПризнакОтраженияНДС,
	|	НалоговаяБазаПоОтгрузкам.КоличествоДокументов,
	|	НалоговаяБазаПоОтгрузкам.СуммаВключенные,
	|	НалоговаяБазаПоОтгрузкам.СуммаНеВключенные,
	|	НалоговаяБазаПоОтгрузкам.ВидЦенности,
	|	НалоговаяБазаПоОтгрузкам.Событие,
	|	НалоговаяБазаПоОтгрузкам.ЗаписьДополнительногоЛиста,
	|	НалоговаяБазаПоОтгрузкам.НомерСтроки,
	|	ЕСТЬNULL(СведенияТаможенныхДекларацийЭкспорт.КодОперации, ""1011410"") КАК КодВидаОперации
	|ИЗ
	...
	|		ПО НалоговаяБазаПоОтгрузкам.ДокументОтгрузки = СведенияТаможенныхДекларацийЭкспорт.ДокументОтгрузки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура ПолучитьНаборДанныхВычетПоПриобретеннымЦенностям0(Запрос, СтруктураВнешнихНаборов) 
	...
	|	И ТаможенныеДекларацииПоЭкспорту.ДокументОтгрузки В
	|			(ВЫБРАТЬ
	|				ВычетНДСПоСтавке0Предварительная.ДокументОтгрузки
	|			ИЗ
	|				ВычетНДСПоСтавке0Предварительная)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ТаможенныеДекларацииПоЭкспорту.ДокументОтгрузки
	|;
	...
	|ВЫБРАТЬ
	|	ЕСТЬNULL(СведенияТаможенныхДекларацийЭкспорт.КодОперации, ""1011410"") КАК КодВидаОперации,
	|	ВычетНДСПоСтавке0Предварительная.ДокументРегистратор,
	|	ВычетНДСПоСтавке0Предварительная.ДокументОтгрузки,
	|	ВычетНДСПоСтавке0Предварительная.СтавкаНДС,
	|	ВычетНДСПоСтавке0Предварительная.Сумма,
	|	ВычетНДСПоСтавке0Предварительная.СуммаНДС,
	|	ВычетНДСПоСтавке0Предварительная.ПризнакНаличияОшибок,
	|	ВычетНДСПоСтавке0Предварительная.ПризнакОтраженияНДС,
	|	ВычетНДСПоСтавке0Предварительная.КоличествоДокументов,
	|	ВычетНДСПоСтавке0Предварительная.СуммаВключенные,
	|	ВычетНДСПоСтавке0Предварительная.СуммаНеВключенные,
	|	ВычетНДСПоСтавке0Предварительная.НомерВходящегоДокумента,
	|	ВычетНДСПоСтавке0Предварительная.ДатаВходящегоДокумента,
	|	ВычетНДСПоСтавке0Предварительная.СчетФактура,
	|	ВычетНДСПоСтавке0Предварительная.Событие,
	|	ВычетНДСПоСтавке0Предварительная.ВидЦенности,
	|	ВычетНДСПоСтавке0Предварительная.ЗаписьДополнительногоЛиста,
	|	ВычетНДСПоСтавке0Предварительная.НомерСтроки
	|ИЗ
	|	ВычетНДСПоСтавке0Предварительная КАК ВычетНДСПоСтавке0Предварительная
	|		ЛЕВОЕ СОЕДИНЕНИЕ СведенияТаможенныхДекларацийЭкспорт КАК СведенияТаможенныхДекларацийЭкспорт
	|		ПО ВычетНДСПоСтавке0Предварительная.ДокументОтгрузки = СведенияТаможенныхДекларацийЭкспорт.ДокументОтгрузки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
