	...
	ИначеЕсли Не СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда 
		...
		Если ПравОбластьИмя = "А1" Или ПравОбластьИмя = "В1" Тогда 
			...
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				СмежнаяОбласть = ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(Область.Имя, "В1", "А1")];
				ОбластьДляВвода = ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(Область.Имя, "А1", "В1")];
