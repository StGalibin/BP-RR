...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "09/03/2017";
...
Функция ВыгрузкаВозможна(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ДанныеТаблица1["П000010000503"] = 0 Тогда
		...
		ЭтаФорма, НСтр("ru='В таблице 1, в строке 5 не указан размер страхового тарифа в соответствии с классом профессионального риска.'"),
...
Процедура ПроверитьКС_Т1_Ст1Гр03_4а(СтруктураРезультата)
	...
	УсловиеВыполнено = РасчетЗаКвартал(1);
	Если НЕ УсловиеВыполнено ИЛИ (УсловиеВыполнено И КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
...
Процедура ПроверитьКС_Т1_Ст1Гр03_4б(СтруктураРезультата)
	...
	УсловиеВыполнено = НЕ РасчетЗаКвартал(1);
	Если НЕ УсловиеВыполнено ИЛИ (УсловиеВыполнено И КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
...
Процедура ПроверитьКС_Т2_Ст10Гр03_1а(СтруктураРезультата)
	...
	Если ТекущийКвартал > 1 Тогда
		...
		КонецЕсли;
	Иначе
		Если ТекущийГод = 2017 Тогда //Если 1 квартал 2017 года
