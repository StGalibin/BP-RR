...
Процедура ЗаполнитьСведенияОбОрганизации(Форма, ИзмененаДатаПодписи = Ложь)
	...
	Если (ПрежнийКодПоМесту = "231" И ПоМесту <> "231") ИЛИ (ПрежнийКодПоМесту <> "231" И ПоМесту = "231") Тогда
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Раздел3_Прил2, "П000320000101_" + Формат(Инд, "ЧГ=")) Цикл
		...
		УзелКППИнУч = ПолучитьПодчиненныйЭлемент(УзелСведНалГодИ, "КППИнУч");
		Если ЗначениеЗаполнено(УзелКППИнУч.Значение) Тогда
			УзелСумНалИсч = ПолучитьПодчиненныйЭлемент(УзелСведНалГодИ, "СумНалИсч");
			УзелСумНалВыч = ПолучитьПодчиненныйЭлемент(УзелСведНалГодИ, "СумНалВыч");
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Раздел4, "П0004М3009003_" + Формат(Инд, "ЧГ=")) Цикл
		...
		УзелКодОпер = ПолучитьПодчиненныйЭлемент(УзелКорНБИзмЦен, "КодОпер");
		Если ЗначениеЗаполнено(УзелКодОпер.Значение) Тогда
			УзелКорНалБазаУв = ПолучитьПодчиненныйЭлемент(УзелКорНБИзмЦен, "КорНалБазаУв");
			УзелКорНалБазаУм = ПолучитьПодчиненныйЭлемент(УзелКорНБИзмЦен, "КорНалБазаУм");
	...
	Для Каждого ЛистРаздела5 Из Раздел5 Цикл
		...
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ДанныеЛистаРаздела5, "П000500003003_" + Формат(Инд, "ЧГ=")) Цикл
			...
			УзелКодОпер = ПолучитьПодчиненныйЭлемент(УзелСумОпер, "КодОпер");
			Если ЗначениеЗаполнено(УзелКодОпер.Значение) Тогда
				УзелНалБазаПод = ПолучитьПодчиненныйЭлемент(УзелСумОпер,   "НалБазаПод");
				УзелНалВычПод = ПолучитьПодчиненныйЭлемент(УзелСумОпер,    "НалВычПод");
				УзелНалБазаНеПод = ПолучитьПодчиненныйЭлемент(УзелСумОпер, "НалБазаНеПод");
				УзелНалВычНеПод = ПолучитьПодчиненныйЭлемент(УзелСумОпер,  "НалВычНеПод");
	...
	Если НЕ УзелПуст(УзелНалВычПред0) Тогда
		УзелСумВозмПдтв = ПолучитьПодчиненныйЭлемент(УзелНалВычПред0, "СумВозмПдтв");
		УзелСумВозмНеПдтв = ПолучитьПодчиненныйЭлемент(УзелНалВычПред0, "СумВозмНеПдтв");
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Раздел6, "П0006М3011003_" + Формат(Инд, "ЧГ=")) Цикл
		...
		// Элемент "КорНалБазаУв" обязателен при отсутствии элемента "КорНалБазаУм".
		// Элемент "КорНалБазаУм" обязателен при отсутствии элемента "КорНалБазаУв".
		УзелКодОпер = ПолучитьПодчиненныйЭлемент(УзелКорНалИзмЦен, "КодОпер");
		Если ЗначениеЗаполнено(УзелКодОпер.Значение) Тогда
			УзелКорНалБазаУв = ПолучитьПодчиненныйЭлемент(УзелКорНалИзмЦен, "КорНалБазаУв");
			УзелКорНалБазаУм = ПолучитьПодчиненныйЭлемент(УзелКорНалИзмЦен, "КорНалБазаУм");
			Если УзелКорНалБазаУм.Значение = "0" Тогда
				УзелКорНалБазаУв.Обязательность = "О";
			КонецЕсли;
		КонецЕсли;
...
Процедура ПроставитьНаВсехЛистах(Форма, Знач ЗначениеПоказателя)
	...
	Иначе
		...
		Если ЗаполнитьНачальноеЗначениеКБКРаздела1 Тогда
			Форма.СтруктураДанныхРаздел1.Вставить("П000100002003", "18210301000011000110");
		КонецЕсли;
	КонецЕсли;
...
Процедура ВывестиРазделРегистраУчетаНДС(ИмяРаздела, НомерСтраницы = 1) Экспорт
	...
	МакетРаздела = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(ИмяМакетаРаздела);
...
Процедура ПроверкаКС_1_5(СтруктураРезультата)
	...
	Для Каждого Страница Из мДанныеОтчета.Раздел3_Прил1 Цикл
		...
		Для Инд = 1 По КолвоСтрокВМногострочнойЧасти Цикл
			ИмяПоказателя = "П000310007004_" + Формат(Инд, "ЧГ=0");
			ИмяСтрокиГода = "П000310007001_" + Формат(Инд, "ЧГ=0");
	...
	СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), НСтр("ru='НК РФ ст.170, 171, 171.1'"));
