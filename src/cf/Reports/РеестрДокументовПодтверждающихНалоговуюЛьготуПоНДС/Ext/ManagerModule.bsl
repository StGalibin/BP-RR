...
Процедура СформироватьОтчет(Знач ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Если ПараметрыОтчета.НачалоПериода <> НачалоКвартала(ПараметрыОтчета.НачалоПериода)
		ИЛИ КонецДня(ПараметрыОтчета.КонецПериода) <> КонецКвартала(ПараметрыОтчета.КонецПериода) Тогда 
...
Функция ВывестиДанныеРеестраПодтверждающихДокументов(ПараметрыОтчета, ДокументРезультат, Макет)
	...
	|	ФормированиеЗаписейРаздела7ДекларацииНДСПодтверждающиеДокументы.НомерДокумента,
	...
	|	ЗаписиРеестраПредварительная.НаименованиеКонтрагента,
	|	ЗаписиРеестраПредварительная.ИННКонтрагента,
	|	ЗаписиРеестраПредварительная.КППКонтрагента,
	...
	Пока ВыборкаПоКодам.Следующий() Цикл
		...
		Пока ВыборкаПоВидам.Следующий() Цикл
			...
			КонецЕсли;
			ВыборкаПоОперациям = ВыборкаПоВидам.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
				...
				Пока ВыборкаЗаписи.Следующий() Цикл
					...
					Если ПорядокЗаписи = КоличествоЗаписей Тогда
						Если ПорядокОперации = КоличествоОпераций 
							И ПорядокВида = КоличествоВидов Тогда // значит последний вид по коду и последняя операция
							...
							Продолжить;
						ИначеЕсли ПорядокОперации = КоличествоОпераций Тогда // значит последняя операция по виду
