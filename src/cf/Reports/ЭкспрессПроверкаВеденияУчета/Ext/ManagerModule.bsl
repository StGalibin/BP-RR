...
Функция ПроверитьСоответствие90_НДС_БУ(СтрокаПроверки, ПараметрыОтчета)
	...
	|	Соответствие90_НДС_БУ_Предварительная.ДокументОтгрузки,
	|	Соответствие90_НДС_БУ_Предварительная.Контрагент,
	|	Соответствие90_НДС_БУ_Предварительная.ДокументПредставление,
	|	Соответствие90_НДС_БУ_Предварительная.ЭтоВозвратОтПокупателяВКнигуПокупок,
	|	Соответствие90_НДС_БУ_Предварительная.Дата,
	|	Соответствие90_НДС_БУ_Предварительная.СтавкаНДС,
	|	Соответствие90_НДС_БУ_Предварительная.СуммаВыручки,
	|	Соответствие90_НДС_БУ_Предварительная.СуммаСНДСРуб,
	|	Соответствие90_НДС_БУ_Предварительная.НДСНачисленный,
	|	Соответствие90_НДС_БУ_Предварительная.СуммаНДСпоОтгрузкеБезПереходаСобственности,
...
Функция ПроверитьСоответствие91_НДС_БУ(СтрокаПроверки, ПараметрыОтчета)
	...
	|	РезультатПоСуммовымРазницам.СтатьяПрочихДоходовИРасходов,
	...
	|	ВТ_РеализацияВВалютеВсе.СтатьяПрочихДоходовИРасходов,
	|	ВЫБОР
	|		КОГДА ВТ_РеализацияВВалютеВсе.Регистратор ССЫЛКА Документ.РеализацияОтгруженныхТоваров
	|				И ВТ_РеализацияВВалютеВсе.КорСчет В (&Счета62_76_УЕ)
	|			ТОГДА ВЫБОР
	...
	|	И ЕСТЬNULL(ВТ_НДСЗаписиКнигиПродаж.СтавкаНДС, ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18)) <> ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0)
	...
	|	РеализацияОтгруженныхТоваров.Ссылка,
	|	ЕСТЬNULL(ВТ_ВалютнаяВыручкаПоКурсуРеализации.ИсключитьДокументИзПроверкиСоответствия, ЛОЖЬ),
	|	ВЫБОР
	|		КОГДА ВТ_Обороты91.Регистратор ССЫЛКА Документ.ВозвратТоваровОтПокупателя
	|			ТОГДА ЕСТЬNULL(ВЫРАЗИТЬ(ВТ_Обороты91.Регистратор КАК Документ.ВозвратТоваровОтПокупателя).ОтразитьВКнигеПокупок, ЛОЖЬ)
	|					ИЛИ &ВерсияУчетаНДС > 1
...
Функция ПроверитьСоответствие90и91_НДС_НУ(СтрокаПроверки, ПараметрыОтчета)
	...
	|	Таблица.ИсключитьДокументИзПроверкиСоответствия
	...
	|			ИЛИ НЕ(ВТ_Данные.БазаНДСпоБУ + ВТ_Данные.БазаНДСпоОтгрузкеБезПереходаСобственности = 0
...
Функция ПроверитьНачислениеНДСсАвансов(СтрокаПроверки, ПараметрыОтчета)
	...
	|	НЕ(СУММА(ВТ_Данные.СуммаНДС) = 0
	|			И СУММА(ВТ_Данные.Сумма76_АВ_68_02) = СУММА(ВТ_Данные.Сумма68_02_76_АВ))
	|
	|УПОРЯДОЧИТЬ ПО
	|	Контрагент,
	|	Документ
...
Функция ПроверитьЕстьРаспределениеКосвРасходов(СтрокаПроверки, ПараметрыОтчета)
	...
	Иначе
		СтрокаПроверки.КомментарийРезультатаНетОшибок = НСтр("ru = 'Документы распределения НДС не нужны, так как в настройках учета НДС организации
...
Функция ПроверитьВсеКосвРасходыРаспределены(СтрокаПроверки, ПараметрыОтчета)
	...
	Если ДолжноБытьРаспределениеРасходов Тогда
		...
		КонецЦикла;
	Иначе
		СтрокаПроверки.КомментарийРезультата = НСтр("ru = 'Распределение НДС не требуется, так как в настройках учета НДС организации
														  |не включена возможность наличия операций без НДС или с НДС 0%'");
	КонецЕсли;
...
Функция ПроверитьЕстьВычетНДСсАвансов(СтрокаПроверки, ПараметрыОтчета)
	...
	Запрос.УстановитьПараметр("СчетаПоАвансамПокупателей", БухгалтерскийУчет.СформироватьМассивСубсчетовПоОтбору(СчетаПоАвансамПокупателей));
	...
	Для Каждого Документ Из СписокДокументов Цикл
		...
		Если ОтдельнаяНумерацияСчетовФактурНаАванс Тогда
			Если Документ.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаАванс
				ИЛИ Документ.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаАвансКомитента
				ИЛИ Документ.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаАвансКомитентаНаЗакупку Тогда
