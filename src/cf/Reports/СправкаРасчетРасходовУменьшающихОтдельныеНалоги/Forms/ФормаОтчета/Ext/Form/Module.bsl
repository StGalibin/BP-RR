...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	Если Форма.ПрименяетсяУСНДоходы И Форма.ПрименяетсяЕНВД Тогда
		ТипНалога = НСтр("ru = 'налог УСН и ЕНВД'");
	ИначеЕсли Форма.ПрименяетсяУСНДоходы Тогда
		ТипНалога = НСтр("ru = 'налог УСН'");
	ИначеЕсли Форма.ПрименяетсяЕНВД Тогда
		ТипНалога = НСтр("ru = 'ЕНВД'");
	Иначе
		ТипНалога = НСтр("ru = 'налог УСН и ЕНВД'");
	КонецЕсли;
	
	ПериодОтчета = БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(Отчет.НачалоПериода, Отчет.КонецПериода);
	...
	Форма.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ЗаголовокОтчета,
		ТипНалога, ПериодОтчета, ПредставлениеОрганизации);
...
Процедура ОбновитьНалог()
	...
	ПрименяетсяЕНВД = УчетнаяПолитика.ПлательщикЕНВД(Отчет.Организация, Отчет.НачалоПериода);
	ПрименяетсяУСНДоходы = УчетнаяПолитика.ПрименяетсяУСНДоходы(Отчет.Организация,  Отчет.НачалоПериода);
