...
Функция ТекстЗапросаТитульныйЛист(ПараметрыОтчета, НомераТаблиц)
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА Патенты.ПостановкаНаУчетВНалоговомОргане = ЗНАЧЕНИЕ(Перечисление.ПостановкаНаУчетВНалоговомОргане.ПоМестуНахожденияОрганизации)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ВыданПоМестуЖительства,
	|	Патенты.КодПоОКТМО КАК КодПоОКТМО,
	|	ЕСТЬNULL(Патенты.НалоговыйОрган.КодРегиона, """") КАК НалоговыйОрганКодРегиона
	|ИЗ
