...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел2" Тогда
		...
		Для каждого ВеткаР2 Из ДеревоДанных.Строки Цикл
			...
			Стр170 = 0;
			Для каждого Пок Из ВеткаР2.Данные Цикл
				Если СтруктЗависимыхЯчеек.Свойство("П000020017003") Тогда
					Продолжить;
				КонецЕсли;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Иначе
		...
		Если ЗапрещенныеСимволы56КПП.Найти(Символы56КПП) <> Неопределено Тогда
			СообщитьВТС("5-й и 6-й символы КПП налогоплательщика не должны принимать значения: "
	...
	Для каждого СтраницаРаздела Из ДеревоРаздела.Строки Цикл
		...
		Если ЗаполненаСтраницаРаздела Тогда
			...
			Если НЕ ЗначениеЗаполнено(КатегорЗем) Тогда
				...
				СтатусПроверки = Ложь;
			Иначе
				ВозможныеЗначения = ВозможныеЗначенияПоДаннымСпискаВыбора("П000020003003");
				Если ВозможныеЗначения.Количество() > 0 И ВозможныеЗначения.Найти(КатегорЗем) = Неопределено Тогда
					ВозможныеЗначенияДляСообщения = СтрСоединить(ВозможныеЗначения, ", ");
					СообщитьВТС("На странице " + НомСтраницы + " раздела 2 код категории земель должен принимать одно из значений: """
					+ СтрСоединить(ВозможныеЗначения, """, """) + """.", "Раздел2", "П000020003003", НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
	...
	Для Каждого Сочетание Из СочетаниеРазделовСвернутое Цикл
		...
		ИначеЕсли Сочетание.Счетчик1 = 0 Тогда
			...
			Если Сочетание.Счетчик2 <> 0
				...
				СообщитьВТС("По сочетанию КБК """ + Сочетание.ОКТМО + """ и кода по ОКТМО """
...
Процедура Проверить(Команда)
	...
	|от 10.05.2017 № ММВ-7-21/347@, будет реализована в будущих версиях программы.'"));
	Возврат;
