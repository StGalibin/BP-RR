...
Процедура ПолучитьНалоговуюНагрузкуУСН(СтруктураПараметров, ИсточникОтчета, ПоказателиОтчетов, 
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		СправкаРасчетУСН = ДанныеРасчетаУСН(СтруктураПараметров);
...
Функция ДанныеРасчетаУСН(Параметры)
	...
	ТаблицаРеквизитов.Колонки.Добавить("Период", Новый ОписаниеТипов("Дата", , ,
	...
	ТаблицаРеквизитов.Колонки.Добавить("НачалоГода", Новый ОписаниеТипов("Дата", , ,
													Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
	ТаблицаРеквизитов.Колонки.Добавить("КонДата", Новый ОписаниеТипов("Дата", , ,
												Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
	ТаблицаРеквизитов.Колонки.Добавить("ЭтоНачалоГода", Новый ОписаниеТипов("Булево"));
	ТаблицаРеквизитов.Колонки.Добавить("ЭтоКонецГода", Новый ОписаниеТипов("Булево"));
	
	ТаблицаРеквизитов.Колонки.Добавить("СтавкаНалогаУСН", Новый ОписаниеТипов("Число",
														Новый КвалификаторыЧисла(5, 2, ДопустимыйЗнак.Неотрицательный)));
	ТаблицаРеквизитов.Колонки.Добавить("ПрименяетсяУСН", Новый ОписаниеТипов("Булево"));
	ТаблицаРеквизитов.Колонки.Добавить("ПрименяетсяУСНДоходы", Новый ОписаниеТипов("Булево"));
	ТаблицаРеквизитов.Колонки.Добавить("ПрименяетсяУСНДоходыМинусРасходы", Новый ОписаниеТипов("Булево"));
	ТаблицаРеквизитов.Колонки.Добавить("ПлательщикТорговогоСбораЗаПериод", Новый ОписаниеТипов("Булево"));
	
	Реквизиты = ТаблицаРеквизитов.Добавить();
	Реквизиты.Период               = Параметры.КонецПериода;
	Реквизиты.Регистратор          = Документы.ДокументРасчетовСКонтрагентом.ПустаяСсылка();
	Реквизиты.Организация          = Параметры.Организация;
	Реквизиты.НачалоГода           = Параметры.НачалоПериода;
	Реквизиты.КонДата              = Параметры.КонецПериода;
	Реквизиты.ЭтоНачалоГода        = Истина;
	Реквизиты.ЭтоКонецГода         = Ложь;
	Реквизиты.СтавкаНалогаУСН      = УчетнаяПолитика.СтавкаНалогаУСН(Параметры.Организация, Параметры.КонецПериода);
	Реквизиты.ПрименяетсяУСН       = УчетнаяПолитика.ПрименяетсяУСН(Параметры.Организация, Параметры.КонецПериода);
	Реквизиты.ПрименяетсяУСНДоходы = УчетнаяПолитика.ПрименяетсяУСНДоходы(Параметры.Организация, Параметры.КонецПериода);
	Реквизиты.ПрименяетсяУСНДоходыМинусРасходы = 
		УчетнаяПолитика.ПрименяетсяУСНДоходыМинусРасходы(Параметры.Организация, Параметры.КонецПериода);
	Реквизиты.ПлательщикТорговогоСбораЗаПериод = УчетнаяПолитика.ПлательщикТорговогоСбораЗаПериод(
		Параметры.Организация,
		НачалоГода(ДобавитьМесяц(Параметры.НачалоПериода, -3)),
