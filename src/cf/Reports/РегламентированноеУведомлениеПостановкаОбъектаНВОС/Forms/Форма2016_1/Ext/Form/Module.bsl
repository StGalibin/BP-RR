...
Процедура ПриОткрытии(Отказ)
	ИдДляСвор = ПолучитьИзВременногоХранилища(СворачиваемыеЭлементы);
	Для Каждого Стр Из ДеревоСтраниц.ПолучитьЭлементы() Цикл
		СвернутьРек(ИдДляСвор, Стр);
	КонецЦикла;
...
&НаКлиенте
Процедура СвернутьРек(ИдДляСвор, ТекСтр)
	Если ИдДляСвор.Найти(ТекСтр.ПолучитьИдентификатор()) = Неопределено Тогда 
		Элементы.ДеревоСтраниц.Развернуть(ТекСтр.ПолучитьИдентификатор(), Ложь);
...
Процедура СохранитьДанные() Экспорт
	...
	ДанныеЛицензийТаблица = РеквизитФормыВЗначение("ДанныеЛицензий");
	ДанныеЛицензийТаблица.Колонки.Добавить("ДД");
	Для Каждого Стр Из ДанныеЛицензийТаблица Цикл 
		Если ЗначениеЗаполнено(Стр.ИмяФайла) Тогда 
			Стр.ДД = ПолучитьИзВременногоХранилища(Стр.АдресДД);
		КонецЕсли;
	КонецЦикла;
	ДанныеЛицензийТаблица.Колонки.Удалить("АдресДД");
	СтруктураПараметров.Вставить("ДанныеЛицензий", ДанныеЛицензийТаблица);
...
Процедура ЗагрузитьДанные(СсылкаНаДанные)
	...
	Для Каждого Стр Из СтруктураПараметров.ДанныеЛицензий Цикл 
		НовСтр = ДанныеЛицензий.Добавить();
		ЗаполнитьЗначенияСвойств(НовСтр, Стр);
		Если Стр.ДД = Неопределено Тогда 
			НовСтр.АдресДД = ПоместитьВоВременноеХранилище(Неопределено, Новый УникальныйИдентификатор);
		Иначе
			НовСтр.АдресДД = ПоместитьВоВременноеХранилище(Стр.ДД, Новый УникальныйИдентификатор);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ОткрытьФормуВводаЛицензий(ФильтрПоТипу)
	...
	ПараметрыДокументов = Новый Структура("Адрес, ФильтрПоТипу", ПарамСтр, ФильтрПоТипу);
