...
Процедура ЗаполнитьУзел(Узел, Стр, ИмяИД)
	Узел_АтмосфераТек = Документы.УведомлениеОСпецрежимахНалогообложения.НовыйУзелИзПрототипа(Узел);
	
	Документы.УведомлениеОСпецрежимахНалогообложения.УстановитьЗначениеЭлемента(Узел_АтмосфераТек, ИмяИД, СтрЗаменить(Строка(Стр.УИД), "-", ""));
	Документы.УведомлениеОСпецрежимахНалогообложения.УстановитьЗначениеЭлемента(Узел_АтмосфераТек, "DATE_DOC", Формат(Стр.ДатаВыдачи, "ДФ=yyyy-MM-dd"));
	Документы.УведомлениеОСпецрежимахНалогообложения.УстановитьЗначениеЭлемента(Узел_АтмосфераТек, "DATE_BEGIN", Формат(Стр.ДатаНачалаДействия, "ДФ=yyyy-MM-dd"));
	Документы.УведомлениеОСпецрежимахНалогообложения.УстановитьЗначениеЭлемента(Узел_АтмосфераТек, "DATE_END", Формат(Стр.ДатаОкончанияДействия, "ДФ=yyyy-MM-dd"));
	Документы.УведомлениеОСпецрежимахНалогообложения.УстановитьЗначениеЭлемента(Узел_АтмосфераТек, "NUM_DOC", Стр.НомерДокумента);
	Документы.УведомлениеОСпецрежимахНалогообложения.УстановитьЗначениеЭлемента(Узел_АтмосфераТек, "ISSUED_NAME", Стр.Орган);
	
	Если Стр.ДД <> Неопределено Тогда
		...
		ИмяПрисоединенногоФайла = КаталогВремФайлов + Стр.ИмяФайла;
		Стр.ДД.Записать(ИмяПрисоединенногоФайла);
		...
		КонецЦикла;
		Документы.УведомлениеОСпецрежимахНалогообложения.УстановитьЗначениеЭлемента(Узел_АтмосфераТек, "ATTACH", Base64СтрокаДанныеАрхива);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьДаннымиПриложенныхДокументов(Объект, Параметры, ДеревоВыгрузки)
	...
	Узел_Атмосфера = Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПодчиненныйЭлемент(Узел_Документ, "WASTE_DOC");
	Узел_Вода = Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПодчиненныйЭлемент(Узел_Документ, "WATER_DOC");
	Узел_Разрешения = Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПодчиненныйЭлемент(Узел_Документ, "EMISSION_DOC");
	...
	Для Каждого Стр Из Атмосфера Цикл
		ЗаполнитьУзел(Узел_Атмосфера, Стр, "ID_WSD");
	КонецЦикла;
	
	Для Каждого Стр Из Вода Цикл
		ЗаполнитьУзел(Узел_Вода, Стр, "ID_WD");
	КонецЦикла;
	
	Для Каждого Стр Из Разрешения Цикл
		ЗаполнитьУзел(Узел_Разрешения, Стр, "ID_ED");
	КонецЦикла;
