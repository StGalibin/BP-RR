		...
		Если ЗаполняемыеНастройки.Отбор Тогда
			...
			Для Каждого ЭлементОтбора Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
				Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
					Если СтрНайти(ЭлементОтбора.ЛевоеЗначение, "Субконто") > 0
						ИЛИ Строка(ЭлементОтбора.ЛевоеЗначение) = "Валюта" Тогда
			...
			Если ЕстьУчетПоПодразделениям И НЕ ПодразделениеЗаполнено Тогда
				...
				БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(
					...
					"Подразделение",
					Поле.Тип.ПривестиЗначение(Неопределено),
					,
					Ложь);
			...
			Для Индекс = 1 По КоличествоСубконто Цикл
				...
				ИначеЕсли НЕ ВестиУчетПоДоговорам 
					...
					Продолжить;
				КонецЕсли;
				НовыйЭлементОтбора = БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(
					Отчет.КомпоновщикНастроек,
					ИмяПоляПрефикс + Индекс,
					ДанныеСчета["ВидСубконто" + Индекс + "ТипЗначения"].ПривестиЗначение(Неопределено),
					,
					Ложь);
