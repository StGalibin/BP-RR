...
Функция ТекстЗапросаПоДокументам()
	...
	|	РАЗНОСТЬДАТ(ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, &Период), &Период, ДЕНЬ) - ЕСТЬNULL(ДоговорыКонтрагентов.СрокОплаты, СрокОплатыПокупателей.Значение) КАК Просрочка
	|ИЗ
	...
	|ГДЕ
	|	РАЗНОСТЬДАТ(ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, &Период), &Период, ДЕНЬ) > ЕСТЬNULL(ДоговорыКонтрагентов.СрокОплаты, СрокОплатыПокупателей.Значение)";
...
Функция ПечатьСправкаРасчет(МассивОбъектов, ОбъектыПечати)
	...
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_РасчетПени_СправкаРасчет";
...
Процедура ВключитьИспользованиеДополнительныхРеквизитовИСведений() Экспорт
	...
	НаборДополнительныхРеквизитовИСведенийСсылка = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.НаборыДополнительныхРеквизитовИСведений.Документ_НачислениеПеней");
