...
&НаКлиенте
Процедура ПодтвердитьЗапросНаОтменуПроведения(Команда)
	
	НачатьВыгрузкуДокумента(ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ПодтверждениеЗапросаНаОтменуПроведенияТТН"));
	
КонецПроцедуры

&НаКлиенте
Процедура ОтказатьсяОтЗапросаНаОтменуПроведения(Команда)
	
	НачатьВыгрузкуДокумента(ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ОтказОтЗапросаНаОтменуПроведенияТТН"));
...
Процедура ДоступностьЭлементовФормы()
	...
	СтатусыПодтвержденияЗапроса = Новый Массив;
	СтатусыПодтвержденияЗапроса.Добавить(Перечисления.СтатусыОбработкиТТНИсходящейЕГАИС.ПринятЗапросНаОтменуПроведения);
	СтатусыПодтвержденияЗапроса.Добавить(Перечисления.СтатусыОбработкиТТНИсходящейЕГАИС.ОшибкаПередачиПодтвержденияЗапросаНаОтменуПроведения);
	СтатусыПодтвержденияЗапроса.Добавить(Перечисления.СтатусыОбработкиТТНИсходящейЕГАИС.ОшибкаПередачиОтказаОтЗапросаНаОтменуПроведения);
	...
	Элементы.ФормаГруппаКомандыАктаРасхождений.Видимость = СтатусыПодтвержденияАкта.Найти(Объект.СтатусОбработки) <> Неопределено;
	Элементы.ФормаГруппаКомандыЗапросаНаОтменуПроведения.Видимость = СтатусыПодтвержденияЗапроса.Найти(Объект.СтатусОбработки) <> Неопределено;
