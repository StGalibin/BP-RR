...
Процедура ДобавитьКомандыОтправки(КомандыОтправки) Экспорт
	...
	КомандаОтправки = КомандыОтправки.Добавить();
	КомандаОтправки.Идентификатор = "АктСверкиСПечатьюИПодписью";
	КомандаОтправки.Представление = НСтр("ru = 'Акт сверки (с печатью и подписью)'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	// Печать акта сверки с печатью и подписью
	КомандаПечати = КомандыПечати.Добавить();
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктСверкиСПечатьюИПодписью") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АктСверкиСПечатьюИПодписью", НСтр("ru = 'Акт сверки взаиморасчетов'"),
			ПечатьАктаСверки(МассивОбъектов, ОбъектыПечати, Истина), , "Документ.АктСверкиВзаиморасчетов.ПФ_MXL_АктСверки");
	КонецЕсли;
...
Процедура ДополнитьТаблицуСчетамиФактурами(ТаблицаПоДаннымОрганизации, СтруктураПараметров)
	...
	ШаблонСчетФактуры = НСтр("ru='Счет-фактура (%1 от %2)'");
	...
	Для Каждого СтрокаТаблицы Из ТаблицаПоДаннымОрганизации Цикл
		...
		Для Каждого СтрокаСчетаФактуры Из СтрокиСчетовФактур Цикл
			...
			Если СтрокаСчетаФактуры.ВидСчетаФактуры = "Выданный" Тогда
				НомерСчетаФактуры = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(СтрокаСчетаФактуры.Номер, Истина, Ложь);
			Иначе
				НомерСчетаФактуры = СтрокаСчетаФактуры.Номер;
			КонецЕсли;
			
			ПредставлениеСчетаФактуры = ПредставлениеСчетаФактуры + Символы.ПС + СтрШаблон(
				ШаблонСчетФактуры,
				НомерСчетаФактуры,
				Формат(СтрокаСчетаФактуры.Дата, "ДЛФ=D"));
...
Функция ПредставлениеДокументаПоДаннымКонтрагента(ПредставлениеДокумента)
...
Функция ПолучитьТекстЗапросаДляФормированияПечатнойФормыАктСверки()
	...
	|	ДанныеДокумента.ДатаКонтр,
	|	ДанныеДокумента.ПредставлениеКонтр,
...
//
Функция ПечатьАктаСверки(МассивОбъектов, ОбъектыПечати, СПечатью = Ложь)
		...
		Если СПечатью Тогда
			...
			Если ЗначениеЗаполнено(Шапка.ПредставительОрганизации) Тогда
				...
				Если ОтветственныеЛицаОрганизации.Руководитель = Шапка.ПредставительОрганизации Тогда
					ДвоичныеДанныеПодписьРуководителя = 
						Справочники.Организации.ДвоичныеДанныеКартинкиОрганизации(Шапка.Организация, "ФайлПодписьРуководителя");
				...
				ИначеЕсли ОтветственныеЛицаОрганизации.ГлавныйБухгалтер = Шапка.ПредставительОрганизации Тогда
					ДвоичныеДанныеПодписьГлавногоБухгалтера = 
...
Функция НачальноеСальдоПоДоговорам(Шапка)
	...
	ТаблицаДоговоров = Шапка.АктСверки.ПоДаннымОрганизации.Выгрузить(,"Договор");
	ТаблицаДоговоров.Свернуть("Договор");
	МассивДоговоров = ТаблицаДоговоров.ВыгрузитьКолонку("Договор");
	
	// Дополним массив договорами из табличной части контрагента.
	ТаблицаДоговоровКонтрагента = Шапка.АктСверки.ПоДаннымКонтрагента.Выгрузить(,"Договор");
	...
	ЗапросОстаткиДоговоров.УстановитьПараметр("АктСверкиВВалюте",
		Шапка.ВалютаДокумента <> ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета());
	...
	|	ВЫБОР
	|		КОГДА ХозрасчетныйОстатки.Субконто1.Валютный
	|					И ХозрасчетныйОстатки.Субконто1.ОплатаВВалюте
