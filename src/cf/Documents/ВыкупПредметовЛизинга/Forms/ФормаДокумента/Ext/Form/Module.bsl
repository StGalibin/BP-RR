...
Процедура УстановитьУсловноеОформление()
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.ОС.СуммаНДС", ВидСравненияКомпоновкиДанных.НеРавно, 0);

	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.ОС.СпособУчетаНДС", ВидСравненияКомпоновкиДанных.НеЗаполнено);
...
Процедура ДатаОбработатьИзменение()
	...
	Если Объект.НДСНеВыделять <> ПрименениеУСНДоходы Тогда
		Объект.НДСНеВыделять = ПрименениеУСНДоходы;
...
Функция ДополнительныеСведенияОС(ОсновныеСредства)
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Сведения.СтавкаНДС = ?(Объект.НДСНеВыделять, Перечисления.СтавкиНДС.БезНДС, Перечисления.СтавкиНДС.НДС18);
