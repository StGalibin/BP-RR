...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам Тогда
		...
		Если СуммаУслуг = 0 И РасшифровкаПлатежа.Количество() = 1
			...
			РасшифровкаПлатежа[0].СуммаУслуг = ЭквайрингСуммаУслугБанкаПоДоговору(
...
Функция ЭквайрингСуммаУслугБанкаПоДоговору(Организация, ДоговорКонтрагента, СуммаДокумента)
	...
	|	МАКСИМУМ(ВидыОплатОрганизацийКомиссияБанка.СуммаОперацийОт) КАК СуммаОперацийОт
	|ПОМЕСТИТЬ ВТ_МаксимальнаяВыручка
	|ИЗ
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ВидыОплатОрганизацийКомиссияБанка.Ссылка
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
	|	СуммаОперацийОт
	...
	|ВЫБРАТЬ
	|	ВидыОплатОрганизацийКомиссияБанка.Ссылка,
	|	ВидыОплатОрганизацийКомиссияБанка.СуммаОперацийОт,
	|	ВидыОплатОрганизацийКомиссияБанка.ПроцентБанковскойКомиссии
	|ИЗ
	|	Справочник.ВидыОплатОрганизаций.КомиссияБанка КАК ВидыОплатОрганизацийКомиссияБанка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_МаксимальнаяВыручка КАК ВТ_МаксимальнаяВыручка
	|		ПО ВидыОплатОрганизацийКомиссияБанка.Ссылка = ВТ_МаксимальнаяВыручка.Ссылка
	|			И ВидыОплатОрганизацийКомиссияБанка.СуммаОперацийОт = ВТ_МаксимальнаяВыручка.СуммаОперацийОт";
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	// Расчет суммы услуг выполняем при условии, что одному договору соответствует 1 вид оплаты.
	Если Выборка.Количество() = 1 Тогда
		Выборка.Следующий();
		СуммаУслуг = Окр(Выборка.ПроцентБанковскойКомиссии * СуммаДокумента / (100 - Выборка.ПроцентБанковскойКомиссии), 2);
	КонецЕсли;
...
Функция ЭквайрингПроцентУслугБанкаПоВидуОплаты(ВидОплаты, СуммаДокумента)
	...
	|	И ВидыОплатОрганизацийКомиссияБанка.СуммаОперацийОт < &СуммаОперацийОт
	|	И ВидыОплатОрганизацийКомиссияБанка.ПроцентБанковскойКомиссии > 0
	|
	|СГРУППИРОВАТЬ ПО
	|	ВидыОплатОрганизацийКомиссияБанка.Ссылка
...
Процедура ЗаполнитьПоДокументуОснованию(Основание)
	...
	ИначеЕсли ТипДокументаОснования = ТипОплатаПлатежнойКартой Тогда
		...
		Если ЗначениеЗаполнено(ДанныеОснования.ВидОплаты) Тогда
			...
			ПроцентБанка = ЭквайрингПроцентУслугБанкаПоВидуОплаты(ДанныеОснования.ВидОплаты, ДанныеОснования.СуммаДокумента);
