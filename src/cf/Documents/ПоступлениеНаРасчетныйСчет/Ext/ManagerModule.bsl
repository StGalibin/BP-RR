...
Процедура ПодготовитьСодержаниеПереквалификацииКУДиР(КУДиР)
	...
	ОписаниеДоходовПоВидам.Вставить("Графа5",
		НСтр("ru='доходы УСН'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	ОписаниеДоходовПоВидам.Вставить("ДоходЕНВД",
		НСтр("ru='доходы ЕНВД'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	ОписаниеДоходовПоВидам.Вставить("ДоходТорговыйСбор",
		НСтр("ru='доходы по деятельности на торговом сборе'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	ОписаниеДоходовПоВидам.Вставить("ДоходПатент",
		НСтр("ru='доходы по деятельности на патенте'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	ОписаниеДоходовПоВидам.Вставить("ДоходКомитент",
		НСтр("ru='выручка комитента'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Функция ПодготовитьТаблицыРасчетовДляУСН(ТаблицаРеквизиты, ТаблицаВзаиморасчетов, Отказ) Экспорт
	...
	ПодготовленныеТаблицы = Новый Структура("ТаблицаВзаиморасчетыУСН, ТаблицаВозвратыПоВзаиморасчетамУСН, КУДиРПереквалификация, КУДиРПатент, ТаблицаПрочихРасчетовУСН",
		Параметры.Взаиморасчеты, Неопределено, НовыйСтруктураКУДиР(), НовыйТаблицаКУДиРПатент(), Неопределено);
	...
	Если Реквизиты.ВидОперации <> Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам
		Или Не (Реквизиты.УчитыватьЗадолженностьУСН Или Реквизиты.УчитыватьЗадолженностьУСНПатент) Тогда
		Возврат ПодготовленныеТаблицы;
	КонецЕсли;
	...
	ОстаткиСчетовУСНРеализация.Индексы.Добавить("РасчетныйДокумент, Подразделение");
	...
	Для каждого СтрокаТаблицыВзаиморасчеты Из НераспределенныеВзаиморасчеты Цикл
		...
		Для каждого СтрокаПрочихРасчетов Из НайденныеСтрокиПрочихРасчетов Цикл
			...
			ОтборОстатковПоСчетамУСН = Новый Структура("РасчетныйДокумент, Контрагент, Договор",
				СтрокаПрочихРасчетов.РасчетныйДокумент, СтрокаПрочихРасчетов.Контрагент, СтрокаПрочихРасчетов.ДоговорКонтрагента);
