...
Процедура КнигаПокупокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗапретитьРедактированиеСтрокиСДругимПериодом(Элемент, СтандартнаяОбработка) Тогда
		Возврат; 
	КонецЕсли;
...
&НаСервере
Процедура УстановитьУсловноеОформлениеСтрокиСДругимПериодом(ИмяЭлемента, ПутьКДанным)
	
	// Вносить исправления можно только в регистрационные записи проверяемого периода
	// Следовательно, записи по другим кварталам носят справочный характер и не должны редактироваться, а также не должны переноситься в ответ.
	
	Если ЗначениеЗаполнено(ПериодОтраженияЗаписи) Тогда
			
		// Не даем редактировать поле "результат проверки"
		ЭлементУО = УсловноеОформление.Элементы.Добавить();
		
		ДобавитьОформляемоеПоле(ЭлементУО.Поля, ИмяЭлемента);
		
		ГруппаИ = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(ЭлементУО.Отбор.Элементы, "", ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
		
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаИ, ПутьКДанным, ВидСравненияКомпоновкиДанных.НеРавно, ПериодОтраженияЗаписи);
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаИ, ПутьКДанным, ВидСравненияКомпоновкиДанных.Заполнено);
		
		ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст", "<Справочно. Для исправления подайте уточненную декларацию за соответствующий период>");
		ЭлементУО.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
		ЭлементУО.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Ложь);
		ЭлементУО.Оформление.УстановитьЗначениеПараметра("ЦветТекста", Новый Цвет(128, 122, 89));
...
&НаКлиенте
Функция ЗапретитьРедактированиеСтрокиСДругимПериодом(Элемент, СтандартнаяОбработка)
	
	Если Элемент.ТекущиеДанные <> Неопределено 
		И ЗначениеЗаполнено(Элемент.ТекущиеДанные.ПериодОтраженияЗаписи)
		И Элемент.ТекущиеДанные.ПериодОтраженияЗаписи <> ПериодОтраженияЗаписи Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ПоказатьПредупреждение(,НСтр("ru = 'Период записи отличается от периода декларации. 
                                      |При необходимости исправления сведений в данной строке 
                                      |подайте уточненную декларацию за соответствующий период.'"));
		Возврат Истина; 
	КонецЕсли;
...
Процедура ПодготовитьФормуНаСервере()
	...
	ПериодОтраженияЗаписи 			= КонтекстЭДОСервер.ПериодОтраженияЗаписиПоясненияКДекларацииНДС(Объект.Требование);
...
Процедура УстановитьУсловноеОформлениеКнигаПокупок()
	...
	#Область ЗапретРедактированияЗаписейСДругимПериодом
		УстановитьУсловноеОформлениеСтрокиСДругимПериодом("КнигаПокупокРезультатПроверки", "Объект.КнигаПокупок.ПериодОтраженияЗаписи");
	#КонецОбласти
...
Процедура УстановитьУсловноеОформлениеКнигаПокупокДл()
	...
	#Область ЗапретРедактированияЗаписейСДругимПериодом
		УстановитьУсловноеОформлениеСтрокиСДругимПериодом("КнигаПокупокДлРезультатПроверки", "Объект.КнигаПокупокДл.ПериодОтраженияЗаписи");
	#КонецОбласти
...
Процедура УстановитьУсловноеОформлениеКнигаПродаж()
	...
	#Область ЗапретРедактированияЗаписейСДругимПериодом
		УстановитьУсловноеОформлениеСтрокиСДругимПериодом("КнигаПродажРезультатПроверки", "Объект.КнигаПродаж.ПериодОтраженияЗаписи");
	#КонецОбласти
...
Процедура УстановитьУсловноеОформлениеКнигаПродажДл()
	...
	#Область ЗапретРедактированияЗаписейСДругимПериодом
		УстановитьУсловноеОформлениеСтрокиСДругимПериодом("КнигаПродажДлРезультатПроверки", "Объект.КнигаПродажДл.ПериодОтраженияЗаписи");
	#КонецОбласти
...
Процедура УстановитьУсловноеОформлениеЖурналУчетаВыставленныхСчетовФактур()
	...
	#Область ЗапретРедактированияЗаписейСДругимПериодом
		УстановитьУсловноеОформлениеСтрокиСДругимПериодом("ЖурналУчетаВыставленныхСчетовФактурРезультатПроверки", "Объект.ЖурналУчетаВыставленныхСчетовФактур.ПериодОтраженияЗаписи");
	#КонецОбласти
...
Процедура УстановитьУсловноеОформлениеВыставленныеСчетаФактуры()
	...
	#Область ЗапретРедактированияЗаписейСДругимПериодом
		УстановитьУсловноеОформлениеСтрокиСДругимПериодом("ЖурналУчетаПолученныхСчетовФактурРезультатПроверки", "Объект.ЖурналУчетаПолученныхСчетовФактур.ПериодОтраженияЗаписи");
	#КонецОбласти
...
Процедура УстановитьВидимостьТабличныхЧастей()
	...
	ЭтоВерсия5_03 = ВерсияФоматаТребования = "5.03";
	
	Если ЭтоВерсия5_03 ИЛИ ПояснениеОтправлено Тогда
...
Процедура УстановитьСвойстваКолонокТабличныхЧастей()
	...
	Иначе
		Элементы.СведКСГруппаИнформация.Видимость = Ложь;
	КонецЕсли;
...
Функция КодыВидовОперацийВыданныхСчетовФактур(ЭтоЖурнал = Ложь, ЭтоНетКнигаПрод = Ложь)
	...
	Если НЕ ЭтоЖурнал И НЕ ЭтоНетКнигаПрод Тогда
...
Процедура ПроверитьНетКнигаПрод(Отказ)
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.НетКнигаПрод Цикл
		...
		Если ДоступноРедактированиеДанныхФНС ИЛИ ВерсияФоматаТребования <> "5.03" Тогда
