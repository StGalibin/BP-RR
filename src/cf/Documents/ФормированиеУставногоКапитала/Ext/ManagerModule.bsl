...
Функция ПечатьСписокУчредителей(МассивОбъектов, ОбъектыПечати)
	...
	|		НомерСтроки,
	|		ВЫБОР
	|			КОГДА ФормированиеУставногоКапитала.СформированПомощникомРегистрации
	|				ТОГДА ФормированиеУставногоКапитала.Учредители.Учредитель ССЫЛКА Справочник.Контрагенты
	|			ИНАЧЕ ФормированиеУставногоКапитала.Учредители.Контрагент.ЮридическоеФизическоеЛицо = &ЮридическоеЛицо
	|		КОНЕЦ КАК ЭтоЮридическоеЛицо,
	|		НЕ ФормированиеУставногоКапитала.СформированПомощникомРегистрации
	|			ИЛИ ФормированиеУставногоКапитала.Учредители.Учредитель ССЫЛКА Справочник.Контрагенты КАК ЭтоКонтрагент,
	|		ВЫБОР
	|			КОГДА ФормированиеУставногоКапитала.СформированПомощникомРегистрации
	|				ТОГДА ФормированиеУставногоКапитала.Учредители.Учредитель
	|			ИНАЧЕ ФормированиеУставногоКапитала.Учредители.Контрагент
	|		КОНЕЦ КАК Учредитель,
	|		СуммаВзноса
	|	)
	|ИЗ
...
Функция ОписаниеУчредителя(Учредитель)
	...
	ИначеЕсли Учредитель.ЭтоКонтрагент Тогда
	
		СведенияОбУчредителе = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(Учредитель.Учредитель);
		ОписаниеУчредителя   = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 (%2, зарегистрирован по адресу: %3)'"),
			СведенияОбУчредителе.ПолноеНаименование,
			СведенияОбУчредителе.ДокументУдостоверяющийЛичность,
			СведенияОбУчредителе.ЮридическийАдрес);
...
Функция ТекстЗапросаУчредители(НомераТаблиц)
	...
	|	Учредители.Контрагент КАК Учредитель,
...
Процедура СформироватьПроводки(ТаблицаРеквизиты, ТаблицаУчредители, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаУчредители Цикл
		...
		Если СтрокаТаблицы.СуммаВзноса > 0 Тогда
			...
			Проводка.СчетКт = СчетУставногоКапитала;
		Иначе
			Проводка.Сумма  = СтрокаТаблицы.СуммаВзноса;
			Проводка.СчетДт = СчетУставногоКапитала;
			Проводка.СчетКт = СчетРасчетов;
		КонецЕсли;
		
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Контрагенты", СтрокаТаблицы.Учредитель);
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Контрагенты", СтрокаТаблицы.Учредитель);
...
Функция ПолучитьУставныйКапиталДоИзменения(ТаблицаРеквизиты)
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&ДатаОстатков, Счет = &СчетУчетаУставногоКапитала, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты), Организация = &Организация) КАК ХозрасчетныйОстатки";
...
Процедура ВключитьИспользованиеДополнительныхРеквизитовИСведений() Экспорт
	...
	НаборДополнительныхРеквизитовИСведенийСсылка = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент(
		"Справочник.НаборыДополнительныхРеквизитовИСведений.Документ_ФормированиеУставногоКапитала");
