...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ СформированПомощникомРегистрации Тогда
		НепроверяемыеРеквизиты.Добавить("Учредители.Учредитель");
		НепроверяемыеРеквизиты.Добавить("Учредители.ТипУчредителя");
	Иначе
		НепроверяемыеРеквизиты.Добавить("Учредители.Контрагент");
		Для Индекс = 0 По Учредители.Количество() - 1 Цикл
			...
			Иначе
				...
				Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.Контрагент) Тогда
					ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
						"Колонка", , НСтр("ru = 'Контрагент'"), НомерСтроки, ИмяСписка);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, 
						Ссылка, ПутьКСтроке + "Контрагент", , Отказ);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
