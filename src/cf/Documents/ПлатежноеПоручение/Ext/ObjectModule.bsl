...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ДанныеЗаполнения <> Неопределено Тогда
		...
		ИначеЕсли ТипДанныхЗаполнения = Тип("Массив")
			...
			КонецЕсли;
			Если ТипЗнч(ДанныеЗаполнения[0]) = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыВБанк") Тогда
...
Процедура ЗаполнитьПоСписаниюСРасчетногоСчета(Основание)
	...
	Если Основание.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.СнятиеНаличных Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Платежное поручение не вводится на основании ""Списания с расчетного счета"" с видом операции ""%1"".'"), Перечисления.ВидыОперацийСписаниеДенежныхСредств.СнятиеНаличных);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	...
	ИначеЕсли ДанныеЗаполнения.Количество() = МассивСчетов.Количество() Тогда
		...
		Для Каждого ЭлементМассива Из МассивСчетов Цикл
			...
			Если НЕ ПустаяСтрока(ПараметрыСчета.НомерВходящегоДокумента) И НЕ ПустаяСтрока(ПараметрыСчета.ДатаВходящегоДокумента) Тогда
				...
				ТекстНазначениеСчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Счет № %1 от %2'"),
		...
		Иначе
			ТекстНазначение = ТекстОплата + "по счетам: ";
			Для Каждого ЭлементМассива Из МассивТекстНазначенияСчета Цикл
