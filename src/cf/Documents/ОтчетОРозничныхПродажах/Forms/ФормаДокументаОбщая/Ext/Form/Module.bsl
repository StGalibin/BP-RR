...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ТоварыНоменклатураПриИзмененииНаСервере(ДанныеСтроки, Контекст, ПараметрыЗаполненияСчетовУчета.КЗаполнению);
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ФОРМЫ ПодарочныеСертификаты
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ОтражениеДоходов.Видимость = Форма.ПрименяетсяУСНПатент
		Или Форма.ПрименяетсяТолькоУСНПатент
		Или Форма.РаздельныйУчетТорговыйСборПриУСН;
...
Процедура ОбработкаОповещенияОбработкиТабличнойЧастиТоварыНаСервере(Параметры)
	...
	СчетаУчетаВДокументах.ЗаполнитьСтроки(ДобавленныеСтроки, "Товары", ДанныеОбъекта, Документы.ОтчетОРозничныхПродажах);
...
Процедура ТоварыНомерГТДНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтруктураПараметров = Новый Структура("Отбор, Дата, ВыборПоОстаткам", Отбор, Объект.Дата, Истина);
	ОткрытьФорму("Справочник.НомераГТД.ФормаВыбора", СтруктураПараметров, ЭтотОбъект,,,, ОписаниеОповещения);
...
&НаСервере
Процедура ОтражениеДоходовПриИзмененииНаСервере() Экспорт
...
&НаСервере
Процедура ЗаполнитьСписокВыбораОтражениеДоходов() Экспорт
	...
	Если РаздельныйУчетТорговыйСборПриУСН И Не НТТВПродажныхЦенах Тогда
...
&НаСервере
Процедура УстановитьОтражениеДоходов()
	СписокВыбора = Элементы.ОтражениеДоходов.СписокВыбора;
	
	Если ПустаяСтрока(ОтражениеДоходов) И СписокВыбора.Количество() > 0 Тогда
