...
Функция ВыполнитьОперацию(БлокировкаУстановленаНеявно = Ложь) Экспорт
	...
	ИмяСобытия       = ЗакрытиеМесяцаКлиентСервер.СобытиеЖурналаРегистрации() + "." + Строка(ВидОперации);
	...
	ДлительностьВыполнения = ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоВыполнения;
	ЗаписьЖурналаРегистрации(ИмяСобытия,
							 УровеньЖурналаРегистрации.Информация,
							 Метаданные.Документы.РегламентнаяОперация,
							 Ссылка,
...
Процедура ПроверитьИсходныеДанныеИНастройки(СтруктураШапкиДокумента, Отказ, Ошибки)
	...
	Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетНалогаНаПрибыль Тогда
		...
		|ИЗ
		|	РегистрСведений.НастройкиУчетаНалогаНаПрибыль.СрезПоследних(&Дата, Организация = &Организация) КАК СтавкиНалогаНаПрибыльСрезПоследних";
		
		Результат = Запрос.Выполнить();
		Если Результат.Пустой() Тогда
			ШаблонСообщения = НСтр("ru = 'Не указана ставка налога на прибыль.
				|Отсутствуют записи в регистре сведений ""Налог на прибыль"" на %1.
				|Установите настройки учета налога на прибыль.'");
			
			СсылкаДляПерехода = "e1cib/data/РегистрСведений.НастройкиУчетаНалогаНаПрибыль";
	...
	Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетРасходовУменьшающихОтдельныеНалоги Тогда
	...
	Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетЕНВД Тогда
	
		УчетЕНВД.ВыявитьУчетныеОшибкиРасчетаЕНВД(
			Ошибки,
			СтруктураШапкиДокумента.КонДата,
			СтруктураШапкиДокумента.Организация);
...
Процедура СформироватьДвижения(СтруктураШапкиДокумента, Отказ, Ошибки)
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетЕНВД Тогда
		НачислитьЕНВД(СтруктураШапкиДокумента, Отказ);
...
Процедура ВыполнитьРасчетНалогаНаПрибыль(СтруктураШапкиДокумента, Отказ)
	...
	ТаблицаДолейБазыНалогаНаПрибыль = Неопределено;
	НалоговыйУчетОбособленныхПодразделений.ПодготовитьТаблицуРасчетДолейБазыНалогаНаПрибыль(
	...
	Если ПрименяютсяРазныеСтавкиНалогаНаПрибыль Тогда
		...
		Если ОбщаяСуммаНалога = 0 Тогда
			...
			РасчетнаяСтавкаНалогаЗаМесяц = 0;
			НалоговыйУчетОбособленныхПодразделений.ПолучитьРасчетнуюСтавкуНалогаНаПрибыль(
	...
	Если СтруктураШапкиДокумента.ПоддержкаПБУ18 Тогда
		...
		РасчетОНА_ОНО_ПНА_ПНО = РасчетНалогаНаПрибыль.НовыйРасчетОНА_ОНО_ПНА_ПНО(
			ПараметрыПроведения.ТаблицаРеквизитыРасчетНалога,
			СтруктураШапкиДокумента.СписокОрганизаций);
			
		РасчетНалогаНаПрибыль.СоздатьВременныеТаблицыДляРасчетаОНА_ОНО_ПНА_ПНО(РасчетОНА_ОНО_ПНА_ПНО);
...
Процедура НачислитьЕНВД(СтруктураШапкиДокумента, Отказ)
	
	// ПОДГОТОВКА ПРОВЕДЕНИЯ ПО ДАННЫМ ДОКУМЕНТА
	
	ПараметрыПроведения = Документы.РегламентнаяОперация.ПодготовитьПараметрыНачисленияЕНВД(
		СтруктураШапкиДокумента, Отказ);
	...
	ТаблицаПроводок = УчетЕНВД.ПодготовитьТаблицуНачисленияЕНВД(
		ПараметрыПроведения.ТаблицаРеквизиты); 
