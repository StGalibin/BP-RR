...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидОперации = Перечисления.ВидыОперацийРозничнаяПродажа.Возврат 
		...
		ОтчетОРозничныхПродажах = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Основание, "ОтчетОРозничныхПродажах");
		...
		Если ЗначениеЗаполнено(ОтчетОРозничныхПродажах) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Корректность", 
				НСтр("ru = 'Основание'"),,, НСтр("ru = 'Возврат денег возможен по чекам на основании которых еще не введен документ ""Отчет о розничных продажах""'"));
				
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Основание", "Объект", Отказ);
		КонецЕсли;
