...
Процедура УправлениеФормой(Форма)
	...
	ЭтоКомиссионнаяОперация = Форма.ОтражатьСуммыВЖурнале И Объект.СуммаДокументаКомиссия <> 0;
	// Если это комиссионная операция, то в журнал попадает вся сумма по счету-фактуре,
	// если операция без комиссии, отображаем пустую сумму для журнала.
	Элементы.СуммаДокументаПолностью.Видимость = ЭтоКомиссионнаяОперация;
	Элементы.СуммаКомиссия.Видимость = НЕ ЭтоКомиссионнаяОперация;
	...
	ИначеЕсли КоличествоОснований > 1 Тогда
		...
		Форма.НадписьДокументыОснования = УчетНДСКлиентСервер.ПредставлениеСсылкиПредмета(
