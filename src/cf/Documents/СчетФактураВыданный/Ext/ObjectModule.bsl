...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не ЭтоНовый() И Не УправлениеДоступомБП.ЕстьПравоИзмененияНаУровнеЗаписи(Ссылка) Тогда
		ТекстСообщения = НСтр("ru='У пользователя недостаточно прав на операцию с базой данных.'");
...
Процедура ЗаполнитьНаОснованииСчетаФактуры(Основание)
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, Основание, ,
		"Номер, Дата, СформированПриВводеНачальныхОстатковНДС, Проведен, ПометкаУдаления, ИдентификаторГосКонтракта");
...
Процедура ЗаполнитьСчетФактуруКорректировочный(Основание, Перезаполнить = Ложь)
	...
	КодВидаОперации = ПолучитьКодВидаОперации(ПараметрыЗаполнения.КодВидаОперацииОснования);
	
	ВерсияКодовВидовОпераций = УчетНДСКлиентСервер.ВерсияКодовВидовОпераций(Дата);
	Если ВерсияКодовВидовОпераций > 1 Тогда
		Если ПараметрыЗаполнения.КодВидаОперацииОснованияУменьшение <> "" Тогда 
			КодВидаОперацииНаУменьшение = ПараметрыЗаполнения.КодВидаОперацииОснованияУменьшение;
		Иначе
