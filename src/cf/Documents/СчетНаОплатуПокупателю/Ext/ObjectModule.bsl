...
Процедура ЗаполнитьИзТаблицыНоменклатуры(АдресТаблицыНоменклатуры, СуммаВключаетНДС)
	...
	Для каждого СтрокаТаблчнойЧасти Из Товары Цикл
		СтрокаТаблчнойЧасти.СтавкаНДС = СтрокаТаблчнойЧасти.Номенклатура.СтавкаНДС;
		СтрокаТаблчнойЧасти.СуммаНДС  = УчетНДСКлиентСервер.РассчитатьСуммуНДС(СтрокаТаблчнойЧасти.Сумма,
			СуммаВключаетНДС,
			УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(СтрокаТаблчнойЧасти.СтавкаНДС));
	КонецЦикла;
