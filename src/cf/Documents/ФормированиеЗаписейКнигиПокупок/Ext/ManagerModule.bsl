...
Функция ПодготовитьДанныеРазделаВычетПоПриобретеннымЦенностям(СтруктураПараметров) Экспорт
	...
	Для Каждого СтрокаСчетФактура Из ТаблицаНДСкВычету Цикл
		...
		Если СписокСчетовФактур.НайтиПоЗначению(СтрокаСчетФактура.СчетФактура) <> Неопределено 
			...
			И ТипЗнч(СтрокаСчетФактура.СчетФактура) <> Тип("ДокументСсылка.ВозвратТоваровОтПокупателя")) Тогда
...
Функция ПараметрыПоискаСчетовФактур(ПараметрыФормированияКниги, ДокументыОснования)
	...
	Результат.СчетФактураПроведен = Истина;
...
//
Процедура СоздатьСписокСчетовФактур(ТаблицаИсточник, МенеджерВТ_СписокСчетовФактур, ОтборСтрок = Неопределено)
...
Функция ПодготовитьДанныеРазделаВычетНДСПоНалоговомуАгенту(СтруктураПараметров) Экспорт
	...
	ВычетНДСПоНалоговомуАгенту.Свернуть("СчетФактураДата,СчетФактура,ДатаОплаты,ВидЦенности,Поставщик,ДоговорКонтрагента,СтавкаНДС,ДокументОплаты,
		|ДокументОплатыНДС,ДокументОтгрузки,Состояние,ЗаписьДополнительногоЛиста,КорректируемыйПериод,СчетУчетаНДС,ИсправленныйСчетФактура",
		"СуммаБезНДС,НДС");
...
Функция ОпределитьНаличиеСчетовФактурНалоговогоАгента(СтруктураПараметров, ТаблицаДокументов)
	...
	ТаблицаСчетовФактур = ТаблицаДокументов.Скопировать(, "ДокументОплаты");
	ТаблицаСчетовФактур.Колонки.ДокументОплаты.Имя = "СчетФактура";
	МенеджерВТ_СписокСчетовФактур = Новый МенеджерВременныхТаблиц;
	СоздатьСписокСчетовФактур(ТаблицаСчетовФактур, МенеджерВТ_СписокСчетовФактур);
...
Процедура ПолучитьДанныеОДокументахОплаты(СтруктураПараметров, ТаблицаРезультатов)
	...
	|					ИЗ
	|						ОборотыРасчеты)
	|				И (Поставщик, ДокументОплаты, ДоговорКонтрагента) В
	|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
...
// Устанавливает реквизит "Код вида операции" документов "Формирование записей книги покупок"  в значение "99"
...
Процедура УстановитьКодВидаОперацииНалоговыеНакладные() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СчетФактураПолученныйДокументыОснования.ДокументОснование КАК ДокументОснование
	|ПОМЕСТИТЬ ВТ_НалоговыеНакладные
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ФормированиеЗаписейКнигиПокупок.Ссылка КАК РегламентныйДокумент,
	|	ФормированиеЗаписейКнигиПокупокВычетПоПриобретеннымЦенностям.СчетФактура КАК ДокументОснование,
	|	ФормированиеЗаписейКнигиПокупокВычетПоПриобретеннымЦенностям.НомерСтроки - 1 КАК ИндексСтроки,
	|	ВЫБОР
	|		КОГДА ФормированиеЗаписейКнигиПокупокВычетПоПриобретеннымЦенностям.КодВидаОперации = ""99""
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК КодЗаполнен
	|ИЗ
	|	Документ.ФормированиеЗаписейКнигиПокупок КАК ФормированиеЗаписейКнигиПокупок
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ФормированиеЗаписейКнигиПокупок.ВычетПоПриобретеннымЦенностям КАК ФормированиеЗаписейКнигиПокупокВычетПоПриобретеннымЦенностям
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_НалоговыеНакладные КАК ВТ_НалоговыеНакладные
	|			ПО ФормированиеЗаписейКнигиПокупокВычетПоПриобретеннымЦенностям.СчетФактура = ВТ_НалоговыеНакладные.ДокументОснование
	|		ПО ФормированиеЗаписейКнигиПокупок.Ссылка = ФормированиеЗаписейКнигиПокупокВычетПоПриобретеннымЦенностям.Ссылка
	|ГДЕ
	|	НЕ ФормированиеЗаписейКнигиПокупок.ПометкаУдаления
	|	И ФормированиеЗаписейКнигиПокупок.Дата >= ДАТАВРЕМЯ(2015, 1, 1)
	|	И НЕ ФормированиеЗаписейКнигиПокупок.ПредъявленНДСКВычету0
	|
	|СГРУППИРОВАТЬ ПО
	|	ФормированиеЗаписейКнигиПокупок.Ссылка,
	|	ФормированиеЗаписейКнигиПокупокВычетПоПриобретеннымЦенностям.СчетФактура,
	|	ВЫБОР
	|		КОГДА ФормированиеЗаписейКнигиПокупокВычетПоПриобретеннымЦенностям.КодВидаОперации = ""99""
	|			ТОГДА 1
	|		ИНАЧЕ 0
	...
	Если ВыборкаИтоги.Следующий()
		...
		Пока ВыборкаДокументы.Следующий() Цикл
			...
			ФормированиеЗаписейДокумент = ВыборкаДокументы.РегламентныйДокумент.ПолучитьОбъект();
			ВыборкаСтроки = ВыборкаДокументы.Выбрать();
			
			Пока ВыборкаСтроки.Следующий() Цикл
				ФормированиеЗаписейДокумент.ВычетПоПриобретеннымЦенностям[ВыборкаСтроки.ИндексСтроки].КодВидаОперации = "99";
			КонецЦикла;
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					...
					ФормированиеЗаписейДокумент.Метаданные(),
					ФормированиеЗаписейДокумент.Ссылка, 
					ОписаниеОшибки);
			КонецПопытки;
