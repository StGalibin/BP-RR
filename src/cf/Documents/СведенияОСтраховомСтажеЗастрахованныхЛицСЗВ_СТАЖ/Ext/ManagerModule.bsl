...
Функция СформироватьВыходнойФайл(Ссылка)
	...
	Если ВыборкаПоДаннымДНП.Количество() > 0 Тогда 
		...
		Пока ВыборкаПоДаннымДНП.Следующий() Цикл 
			...
			НаборЗаписейОУТ.ИС__Код = СокрЛП(ВыборкаПоДаннымДНП.ОснованиеВыслугиЛетКод);
			НаборЗаписейОУТ.ИС__ПозицияСписка = СокрЛП(ВыборкаПоДаннымДНП.КодПозицииСпискаКод);
