...
Функция ТоварыПоДаннымПоступления(ДанныеОбъекта, ДокументыПоступления) Экспорт
	...
	|	ЕСТЬNULL(СправочникНоменклатура.СтавкаНДС, ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.ПустаяСсылка)) КАК СтавкаНДС,
	|	ВЫБОР
	...
	|	ЕСТЬNULL(СправочникНоменклатура.СтавкаНДС, ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.ПустаяСсылка)),
	|	ВЫБОР
...
Функция ПодготовитьТаблицыЗаявленияОВвозеТоваровДляВыгрузкиПечати(МассивДокументов) Экспорт
	...
	|	ВЫБОР
	|		КОГДА Реквизиты.Организация.ВариантНаименованияДляПечатныхФорм = ЗНАЧЕНИЕ(Перечисление.ВариантыНаименованияДляПечатныхФорм.СокращенноеНаименование)
	|			ТОГДА ПОДСТРОКА(Реквизиты.Организация.НаименованиеСокращенное, 1, 250)
	|		ИНАЧЕ ПОДСТРОКА(Реквизиты.Организация.НаименованиеПолное, 1, 250)
...
Процедура СформироватьАдресИзСтрокиВФорматеДевятьЗапятых(ЗначениеКонтактнойИнформации, ПредставлениеКонтактнойИнформации, РоссийскийАдрес)
	...
	Если СтрЧислоВхождений(ПредставлениеКонтактнойИнформации, ",") <> 9 Тогда 
		...
		ПредставлениеКонтактнойИнформации = УправлениеКонтактнойИнформацией.СведенияОбАдресе(ЗначениеКонтактнойИнформации);
...
Функция АнализироватьПоляСтавки(СтруктураСтавок)
	...
	Иначе 	
		...
		СтруктураСтавокМодиф.Вставить("СтавкаНДС", ?(СтруктураСтавок.Ст18 = "", 0, ?(СтруктураСтавок.Ст18 = "10/110", 10, ?(СтруктураСтавок.Ст18 = "18/118", 18, Число(СтруктураСтавок.Ст18)))));
...
// Устанавливает в документе количество в единице измерения по ТНВЭД равное количеству в единице по первичному документу.
Процедура ЗаполнитьКоличествоПоТНВЭДОтложенно(Параметры) Экспорт
	...
	|	И ЗаявлениеОВвозеТоваровТовары.Количество <> 0
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗаявлениеОВвозеТоваровТовары.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЗаявлениеОВвозеТоваровТовары.Ссылка.Дата УБЫВ";
	...
	Для каждого СтрокаРезультата Из РезультатЗапроса Цикл
		...
		Попытка
			...
			ЗаполнитьКоличествоПоТНВЭД(СтрокаРезультата);
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), 
				...
				Метаданные.Документы.ЗаявлениеОВвозеТоваров, СтрокаРезультата.Ссылка, ТекстСообщения);
		КонецПопытки;
	...
	Если ОбъектовОбработано = 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Процедуре ЗаполнитьКоличествоПоТНВЭДОтложенно
				|не удалось обработать некоторые документы Заявление о ввозе товаров (пропущены): %1'"),
				ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), 
			УровеньЖурналаРегистрации.Информация,
			Метаданные.Документы.ЗаявлениеОВвозеТоваров,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура ЗаполнитьКоличествоПоТНВЭДОтложенно
					|обработала очередную порцию документов Заявление о ввозе товаров: %1'"), ОбъектовОбработано));
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьКоличествоПоТНВЭД(СтрокаПоДокументу)
	...
	Попытка
		...
		Для Каждого СтрокаДокумента Из ДокументОбъект.Товары Цикл
			Если ЗначениеЗаполнено(СтрокаДокумента.КоличествоПоТНВЭД) Тогда
				// Документ уже обработан.
				ОтменитьТранзакцию();
				Возврат;
			Иначе
				СтрокаДокумента.КоличествоПоТНВЭД = СтрокаДокумента.Количество;
			КонецЕсли;
		КонецЦикла;
