...
Процедура ЗаполнитьСтоимостьНУПриСпособеПоступленияПоДоговоруЛизинга() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПринятиеКУчетуОС.Ссылка,
	|	ХозрасчетныйОбороты.СуммаНУОборот КАК ПервоначальнаяСтоимостьНУ
	|ИЗ
	|	Документ.ПринятиеКУчетуОС КАК ПринятиеКУчетуОС
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(, , Регистратор, Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ОсновныеСредства)), ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства), , , ) КАК ХозрасчетныйОбороты
	|		ПО ПринятиеКУчетуОС.Ссылка = ХозрасчетныйОбороты.Регистратор
	|ГДЕ
	|	ПринятиеКУчетуОС.СпособПоступления = ЗНАЧЕНИЕ(Перечисление.СпособыПоступленияАктивов.ПоДоговоруЛизинга)
	|	И ПринятиеКУчетуОС.ПервоначальнаяСтоимостьНУ = 0";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		
		ДокументОбъект = Выборка.Ссылка.ПолучитьОбъект();
		ДокументОбъект.ПервоначальнаяСтоимостьНУ = Выборка.ПервоначальнаяСтоимостьНУ;
