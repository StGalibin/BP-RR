...
Процедура ПриОткрытии(Отказ)
	...
	Статус = ПредопределенноеЗначение("Перечисление.СтатусыСпецификаций.ВРаботе");
...
&НаКлиенте
Процедура РедактироватьНомер(Команда)
	
Ответ = Вопрос("Номер спецификации устанавливается автоматически. Если вы хотите установить номер вручную, убедитесь в том, что нет другого документа с нужным вам номером." + 
Символы.ПС + "Если вам необходимо установить ""дробный"" номер, то используйте символ ""/"". Например 9/1."+Символы.ПС+ "Включить возможность изменения?", РежимДиалогаВопрос.ДаНет);

Если Ответ = КодВозвратаДиалога.Да Тогда
	Элементы.НомерСпецификации.ТолькоПросмотр = Ложь;
Иначе
	Элементы.НомерСпецификации.ТолькоПросмотр = Истина;
	КонецЕсли;
