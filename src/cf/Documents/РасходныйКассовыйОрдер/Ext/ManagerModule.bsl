...
Функция СобратьДанныеЧеков(ДокументСсылка, НомерСекции, ВерсияФормата, СообщениеОбОшибке) Экспорт
	...
	ТипРасчета = ?(Реквизиты.ВидОперации = Перечисления.ВидыОперацийРКО.ВозвратПокупателю,
...
Функция ПодготовитьСообщенияПроверкиЗаполненияРасшифровкаПлатежа(Объект, ФормаРасшифровкаПлатежа = Неопределено, Отказ, ПроверяемыеРеквизиты, ПроверятьСчетаУчета = Истина) Экспорт
	...
	Если ЕстьРасшифровкаПлатежа(ВидОперации) Тогда
		...
		Для каждого СтрокаПлатежа Из РасшифровкаПлатежа Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаПлатежа.ДоговорКонтрагента)
...
Функция ТекстЗапросаРегистрацияОтложенныхРасчетовСКонтрагентами(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ВидОперации = Перечисления.ВидыОперацийРКО.ОплатаПоставщику
		ИЛИ Реквизиты.ВидОперации = Перечисления.ВидыОперацийРКО.ВозвратПокупателю Тогда
		ТекстЗапроса = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТаблицаРасшифровкаПлатежа.РеквизитыКонтрагент КАК Контрагент,
		|	ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента КАК ДоговорКонтрагента,
...
Функция ТекстЗапросаРеквизитыПечатиЧека(НомераТаблиц)
	НомераТаблиц.Вставить("ВТ_ДоговораПлатежногоАгента", НомераТаблиц.Количество());
	...
	|	РасходныйКассовыйОрдер.СтавкаНДС,
	|	РасходныйКассовыйОрдер.Выдать,
	|	РасходныйКассовыйОрдер.Основание,
...
// при возврате по документу "Возврат товаров от покупателя"
Функция ТекстЗапросаДанныеРасшифровкаПлатежаВозвратОтПокупателя(НомераТаблиц)
	НомераТаблиц.Вставить("ВТ_РасшифровкаПлатежа", НомераТаблиц.Количество());
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	РасходныйКассовыйОрдерРасшифровкаПлатежа.Ссылка,
	...
	|	РасходныйКассовыйОрдерРасшифровкаПлатежа.СчетНаОплату,
...
Функция ТекстЗапросаДанныеРасшифровкаПлатежаВозвратПоСчету(НомераТаблиц)
	...
	|		ПО ПриходныйКассовыйОрдерРасшифровкаПлатежа.Ссылка = РасходныйКассовыйОрдер.ДокументОснование
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СуммыВозвратаПоСтавкамНДС КАК ВТ_СуммыВозвратаПоСтавкамНДС
	|		ПО ПриходныйКассовыйОрдерРасшифровкаПлатежа.СтавкаНДС = ВТ_СуммыВозвратаПоСтавкамНДС.СтавкаНДС
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СуммыРеализацииПоСтавкамНДС КАК ВТ_СуммыРеализацииПоСтавкамНДС
	|		ПО ПриходныйКассовыйОрдерРасшифровкаПлатежа.СтавкаНДС = ВТ_СуммыРеализацииПоСтавкамНДС.СтавкаНДС
	|ГДЕ
	|	РасходныйКассовыйОрдер.Ссылка = &РасходныйКассовыйОрдер
...
Функция ПодготовитьПараметрыПечатиЧека(ДокументСсылка)
	...
	Если ТипЗнч(ДокументСсылка.ДокументОснование) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") Тогда
	...
	Иначе
		Запрос.Текст = Запрос.Текст 
			+ ТекстЗапросаДанныеРасшифровкаПлатежаВозвратОтПокупателя(НомераТаблиц) 
...
Процедура ЗаполнитьТаблицуНалоговыйАгентНДСОтложенно(Параметры) Экспорт
	...
	|ГДЕ
	|	РасходныйКассовыйОрдер.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога)
