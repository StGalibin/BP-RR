...
Процедура ОбновитьСчетаФактуры(РежимЗаписи)
	...
	Для каждого КлючИЗначение Из ВсеПодчиненныеСчетаФактуры Цикл
		...
		ЭлементБлокировки = Блокировка.Добавить("Документ." + ИмяТаблицы);
		ЭлементБлокировки.ИсточникДанных = КлючИЗначение.Значение;
...
Функция ПолучитьТаблицыПодчиненныхСчетовФактур(ДокументСсылка)
	...
	|	СчетФактураВыданный.Ссылка.СводныйКомиссионный КАК СводныйСФ
	|ИЗ
	...
	|	НАЧАЛОПЕРИОДА(СчетФактураПолученный.Ссылка.Дата, ДЕНЬ) КАК ДатаСФ,
