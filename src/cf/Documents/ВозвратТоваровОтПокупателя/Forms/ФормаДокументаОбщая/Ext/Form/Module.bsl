...
Процедура ПодготовитьФормуНаСервере()
	...
	УстановитьОтражениеДоходов(ЭтаФорма);
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ОтражениеДоходов.Видимость = Не ЗначениеЗаполнено(Объект.Сделка)
		И (Форма.ПрименяетсяТолькоУСНПатент Или Форма.ПрименяетсяУСНПатент Или Форма.РаздельныйУчетТорговыйСборПриУСН);
...
Процедура ЗаполнитьСписокВыбораОтражениеДоходов()
	...
	Если Не ПустаяСтрока(ОтражениеДоходов)
		И СписокВыбора.НайтиПоЗначению(ОтражениеДоходов) = Неопределено 
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьОтражениеДоходов(Форма)
	...
	Если ПустаяСтрока(Форма.ОтражениеДоходов) И СписокВыбора.Количество() > 0 Тогда
		Если Форма.Параметры.Ключ.Пустая() И Форма.Параметры.ЗначениеКопирования.Пустая() Тогда
			Форма.ОтражениеДоходов = СписокВыбора[0].Значение;
		Иначе // определяем вид деятельности по реквизитам объекта
...
Процедура УстановитьРеквизитыСделки(Форма)
	...
	ИначеЕсли Форма.ЭтоВозвратОтРозничногоПокупателя Тогда
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц()
	...
	ЗаполнитьПризнакМаркируемойПродукцииТаблицыТовары(Объект.Товары, ВестиУчетМаркировкиПродукцииВГИСМ);
...
&НаСервереБезКонтекста
Процедура ЗаполнитьПризнакМаркируемойПродукцииТаблицыТовары(Товары, ВестиУчетМаркировкиПродукцииВГИСМ)
	
	Если ВестиУчетМаркировкиПродукцииВГИСМ Тогда
...
Процедура ОбработкаОповещенияОбработкиТабличнойЧастиТоварыНаСервере(Параметры)
	...
	ЗаполнитьПризнакМаркируемойПродукцииТаблицыТовары(СтрокиДляДополнительнойОбработки, ВестиУчетМаркировкиПродукцииВГИСМ);
