...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц) Экспорт
	...
	|	Реквизиты.СведенияОТранспортировкеИГрузе,
	|	Реквизиты.ОтветственныйЗаОформление,
	|	Реквизиты.СопроводительныеДокументы,
	|	Реквизиты.Перевозчик,
	|	Реквизиты.ПеревозкаАвтотранспортом
	|ИЗ
...
Функция ПолучитьДоступныеВидыЭдПоВидуОперации(ВидОперации, Организация, Контрагент, ДоговорКонтрагента) Экспорт
	...
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ВЫБОР
	|		КОГДА СоглашенияОбИспользованииЭД.ДоговорКонтрагента = &ДоговорКонтрагента
	|			ТОГДА 0
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК Порядок,
	|	СоглашенияОбИспользованииЭД.ИсходящиеДокументы.(
	|		ВерсияФормата,
	|		ИсходящийДокумент
	|	),
	|	СоглашенияОбИспользованииЭД.ИспользоватьУПД
	|ИЗ
	...
	ВсеУПД = Истина;
	УжеЕстьУПД = Ложь;
	ВыборкаДокументов = Выборка.ИсходящиеДокументы.Выбрать();
	
	Если ВидОперации <> Перечисления.ВидыОперацийРеализацияТоваров.Услуги Тогда
		
		ВерсияФормата = "";
		ВыборкаДокументов.Сбросить();
		Если ВыборкаДокументов.НайтиСледующий(Перечисления.ВидыЭД.ТОРГ12Продавец, "ИсходящийДокумент") Тогда
			
			ВерсияФормата = ВыборкаДокументов.ВерсияФормата;
			
		КонецЕсли;
		Если СтрНайти(ВыборкаДокументов.ВерсияФормата, "УПД") = 0 Тогда
		...
		Иначе
			...
			УжеЕстьУПД = Истина;
			Результат.Добавить(Перечисления.ВидыЭД.ТОРГ12Продавец, "Универсальный передаточный документ");
	...
	Если ВидОперации <> Перечисления.ВидыОперацийРеализацияТоваров.Товары
		...
		ВерсияФормата = "";
		ВыборкаДокументов.Сбросить();
		Если ВыборкаДокументов.НайтиСледующий(Перечисления.ВидыЭД.АктИсполнитель, "ИсходящийДокумент") Тогда
		...
		ИначеЕсли Не УжеЕстьУПД Тогда
			
			УжеЕстьУПД = Истина;
			Результат.Добавить(Перечисления.ВидыЭД.АктИсполнитель, "Универсальный передаточный документ");
	...
	ВерсияФормата = "";
	ВыборкаДокументов.Сбросить();
	Если ВыборкаДокументов.НайтиСледующий(Перечисления.ВидыЭД.АктНаПередачуПрав, "ИсходящийДокумент") Тогда
	...
	ИначеЕсли Не УжеЕстьУПД Тогда
		...
		УжеЕстьУПД = Истина;
		Результат.Добавить(Перечисления.ВидыЭД.АктНаПередачуПрав, "Универсальный передаточный документ");
...
Функция ПолучитьВидЭдПоУмолчанию(ВидОперации) Экспорт
	...
	Если ЗначениеЗаполнено(ВидыЭДПоУмолчанию)
		И ЗначениеЗаполнено(ВидыЭДПоУмолчанию.Получить(ВидОперации)) Тогда
		Возврат ВидыЭДПоУмолчанию.Получить(ВидОперации);
	КонецЕсли;
...
Функция ОпределитьВидОперацииПоДокументуОснованию(Основание) Экспорт
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
		...
		|	ДокОснование.Ссылка = &Ссылка
		|";
	ИначеЕсли  ТипЗнч(Основание) = Тип("ДокументСсылка.СпецификацияКДоговору") Тогда
	Возврат Перечисления.ВидыОперацийРеализацияТоваров.Товары;	
	КонецЕсли;
...
Функция ПодготовитьСтруктуруТаблицУСН(ТаблицаРеквизиты, ТаблицаДокумента, ТаблицаВзаиморасчеты) Экспорт
	...
	Если НЕ ЕстьАвансыОплаченныеПлатежнойКартой(Взаиморасчеты) Тогда
		Возврат СтруктураВозврата;
	КонецЕсли;
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	Реквизиты.Вставить("ВедетсяРаздельныйУчетНДС", УчетнаяПолитика.РаздельныйУчетНДС(Реквизиты.Организация, Реквизиты.Период));
...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц, ЭтоОтложенноеПроведение)
	...
	|	Реквизиты.Патент,
	|	Реквизиты.СчетНаОплатуПокупателю,
...
Функция ТекстЗапросаВременныеТаблицыДокумента(НомераТаблиц, Реквизиты)
	...
	Если Реквизиты.ЕстьТовары Тогда
		...
		Если Реквизиты.ВедетсяРаздельныйУчетНДС Тогда
			ТекстЗапроса = ТекстЗапроса +
			"ВЫБРАТЬ
...
Функция ТекстЗапросаРасчетыВВалютеТовары()
	...
	|	ВременнаяТаблицаТовары.СырьевойТовар,
	|	ВременнаяТаблицаТовары.КодРаздел7ДекларацииНДС,
	|	ВременнаяТаблицаТовары.КодСоответствуетСт149НКРФ,
	|	ВременнаяТаблицаТовары.КодВключаетсяВРеестр
	|ПОМЕСТИТЬ ТаблицаТовары
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.СчетНаОплатуПокупателю) Тогда
		...
		КомандаПечати.СписокФорм     = "ФормаСписка,ФормаВыбора";
		КомандаПечати.ДополнительныеПараметры.Вставить("ИдентификаторВКомплекте","СчетЗаказСПечатьюКомплект");
		КомандаПечати.ДополнительныеПараметры.Вставить("СокращенноеСообщениеОбОшибке", Ложь);
...
Функция ПолучитьТекстЗапросаДляФормированияТаблицыСведенийТОРГ12()
	...
	|	РеализацияТоваровУслуг.ЗаГлавногоБухгалтераНаОсновании,
...
Функция СформироватьРеализациюТоваровИУслугВXML(МассивОбъектов) Экспорт
	...
	Для Каждого СтрокаДанных Из ТаблицаСсылок Цикл
		...
		Если ТипЗнч(СтрокаДанных.Ссылка) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
			...
			ТекстОшибки = "";
			ОбменСКонтрагентамиПереопределяемый.ПолучитьАдресСтруктурой(СтруктураАдресаКонтрагента,
			...
			ОбменСКонтрагентамиПереопределяемый.ПолучитьАдресСтруктурой(СтруктураАдресаОрганизации,
...
Функция ПрочитатьДанныеКарточкиЭД(ДвоичныеДанные, ВидДокумента = "")
	...
	Если СтруктураДанных.Показывать Тогда
		ТабличныйДокумент = ОбменСКонтрагентамиВнутренний.СформироватьПечатнуюФормуЭД(
			ИмяФайлаДанных, Перечисления.НаправленияЭД.Входящий, Новый Структура("ИД", УникальныйИдентификатор));
...
Функция ПодготовитьДанныеДляЗаполненияДокументов(МассивСсылокНаОбъект, СчетаФактурыРеализаций = Неопределено) Экспорт
	...
	Для Каждого СсылкаНаОбъект Из МассивСсылокНаОбъект Цикл
		...
		Если ТипЗнч(СсылкаНаОбъект) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
			...
			НастройкиОбмена.Вставить("ВерсияФормата", "ФНС 5.02");
			
		Иначе
			
			НастройкиОбмена.Вставить("ВерсияФормата", "CML 2.08");
	...
	Для Каждого СтруктураОбмена Из МассивСтруктурОбмена Цикл
		ПолноеИмяФайла = ЭлектронноеВзаимодействиеБП.ПолучитьПолноеИмяФайлаЭД(СтруктураОбмена);
...
Функция СформироватьЭДПрисоединенныйФайлПакетаТакском(СтруктураОбмена, СчетаФактурыРеализаций) 
	...
	АдресКаталога = ЭлектронноеВзаимодействиеБП.РабочийКаталог("Send");
	ВладелецЭД =  СтруктураОбмена.СтруктураЭД.ДокументыОснования[0];
...
Функция ПодготовитьТаблицуНДСПоОтгрузкеСУчетомКурсаАвансов(ТаблицаНДСПоОтгрузке, ТаблицаВзаиморасчеты, ТаблицаРеквизиты) Экспорт
	...
	Если ТаблицаРеквизиты = Неопределено
		...
		ИЛИ ТаблицаНДСПоОтгрузке = Неопределено 
		ИЛИ ТаблицаНДСПоОтгрузке.Количество() = 0 Тогда
...
Функция ПолучитьТаблицуСведенийСчетаНаОплату(МассивОбъектов, ДокументыБезСчетовНаОплату = Неопределено) Экспорт
	...
	Для каждого СтрокаСоответствия Из СоответствиеДокументовИСчетов Цикл
		...
		Иначе
			ДокументыБезСчетовНаОплату.Добавить(СтрокаСоответствия.Ключ);
		КонецЕсли;
	КонецЦикла;
...
Функция ТекстЗапросаТоварыУслугиКРеализацииПоСчету(НомераТаблиц)
	...
	|	ВТ_РеализованнаяНоменклатура.Количество
	|ИЗ
