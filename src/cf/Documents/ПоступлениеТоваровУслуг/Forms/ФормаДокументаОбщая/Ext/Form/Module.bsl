...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ Объект.ТТНВходящаяЕГАИС.Пустая() 
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(Объект.ТТНВходящаяЕГАИС) Тогда
		ОповеститьОбИзменении(Объект.ТТНВходящаяЕГАИС);
		Оповестить("СозданоПоступлениеПоТТНЕГАИС", Объект.ТТНВходящаяЕГАИС);
	КонецЕсли;
...
Процедура ЗаполнитьТоварыПоЕГАИСНаКлиенте()
	...
	ПараметрыФормы = Новый Структура("ТТНВходящаяЕГАИС", Объект.ТТНВходящаяЕГАИС);
...
Процедура ЗаполнитьТоварыПоЕГАИСНаСервере(АдресВременногоХранилища);
	ТаблицаТоваров = ПолучитьИзВременногоХранилища(АдресВременногоХранилища);
...
Процедура УстановитьСтатусДокумента()
	...
	РегистрыСведений.СтатусыДокументов.УстановитьСтатусыДокумента(Объект.Ссылка, ?(СтатусДокумента,
		Перечисления.СтатусыДокументовПоступления.ОригиналПолучен, Перечисления.СтатусыДокументовПоступления.ОригиналНеПолучен));
