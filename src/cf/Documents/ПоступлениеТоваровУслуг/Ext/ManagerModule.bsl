...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц, ЭтоОтложенноеПроведение)
	...
	|	ЕСТЬNULL(Реквизиты.ДоговорКонтрагента.ПредъявляетНДС, ЛОЖЬ) КАК ПредъявляетНДС,
	...
	|	Реквизиты.МОЛ,
	|	Реквизиты.МестонахождениеОС,
	|	Реквизиты.СпособОтраженияРасходовПоАмортизации,
	|	Реквизиты.СчетНаОплатуПоставщика,
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ОС1") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ОС1",
			"Акт о приеме-передаче объекта ОС (ОС-1)", ПечатьОС1(МассивОбъектов, ОбъектыПечати),,
...
Функция ПолучитьДополнительныеРеквизитыДляРеестра() Экспорт
	...
	Результат = Новый Структура("Информация, НомерВходящегоДокумента, ДатаВходящегоДокумента, Оригинал",
		"Контрагент", "НомерВходящегоДокумента", "ДатаВходящегоДокумента", "СтатусыДокументов.Статус");
...
Функция ТоварыУслугиКПоступлениюПоСчету(СчетНаОплату, ПоступленияПоСчету) Экспорт
	...
	|	ИЗ
	|		Документ.ПоступлениеТоваровУслуг.Товары КАК ПолученныеТовары
	|	ГДЕ
	|		ПолученныеТовары.Ссылка В(&ПоступленияПоСчету)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ЛОЖЬ КАК ЭтоУслуга,
	|		ПолученноеОборудование.Номенклатура КАК Номенклатура,
	|		"""" КАК Содержание,
	|		ПолученноеОборудование.Количество КАК Количество
	|	ИЗ
	|		Документ.ПоступлениеТоваровУслуг.Оборудование КАК ПолученноеОборудование
	|	ГДЕ
	|		ПолученноеОборудование.Ссылка В(&ПоступленияПоСчету)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		КОНЕЦ
	|	ИЗ
	|		Документ.ПоступлениеТоваровУслуг.Услуги КАК ПолученныеУслуги
	|	ГДЕ
	|		ПолученныеУслуги.Ссылка В(&ПоступленияПоСчету)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		КОНЕЦ
	|	ИЗ
	|		Документ.ПоступлениеТоваровУслуг.АгентскиеУслуги КАК ПолученныеАгентскиеУслуги
	|	ГДЕ
	|		ПолученныеАгентскиеУслуги.Ссылка В(&ПоступленияПоСчету)) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.ЭтоУслуга,
	|	ВложенныйЗапрос.Номенклатура,
	...
	ВыборкаТоваров = Результат[2].Выбрать();
