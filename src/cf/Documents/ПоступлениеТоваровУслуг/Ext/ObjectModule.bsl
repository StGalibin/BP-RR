...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Оборудование Тогда
		...
		Для каждого СтрокаТаблицы Из Оборудование Цикл
			...
			// Проверка номера ГТД.
			Если НЕ ПоставщикРезидентТаможенногоСоюза
				И УчетТоваров.НеУказанаСтранаПроисхождения(СтрокаТаблицы.НомерГТД, СтрокаТаблицы.СтранаПроисхождения) Тогда
...
Процедура ЗаполнитьПоДокументуОснованию(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ТТНВходящаяЕГАИС") Тогда
		...
		КонтрагентыПоДаннымЕГАИС = Обработки.СопоставлениеДанныхЕГАИС.СоответствиеКонтрагентовЕГАИС(МассивКонтрагентов);
		ОрганизацияПоДаннымЕГАИС = Обработки.СопоставлениеДанныхЕГАИС.ПолучитьОрганизациюИСклад(Основание.Грузополучатель);
		
		Грузоотправитель = КонтрагентыПоДаннымЕГАИС[Основание.Грузоотправитель];
		Грузополучатель  = КонтрагентыПоДаннымЕГАИС[Основание.Грузополучатель];
		
		Организация		 = ОрганизацияПоДаннымЕГАИС.Организация;
		Склад			 = ОрганизацияПоДаннымЕГАИС.Склад;
		
		Контрагент       = ?(Основание.Поставщик.Пустая(), Грузоотправитель, КонтрагентыПоДаннымЕГАИС[Основание.Поставщик]);
		...
		ЗаполнитьТоварыЕГАИС();
		
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьТоварыЕГАИС()
	...
	|	ВЫБОР
	|		КОГДА ТТНВходящаяЕГАИСТовары.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиТТНВходящейЕГАИС.ПередаетсяАктРасхождений)
	|				ИЛИ ТТНВходящаяЕГАИСТовары.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиТТНВходящейЕГАИС.ПереданАктРасхождений)
	...
	|			И ТТНВходящаяЕГАИСТовары.ИдентификаторУпаковки = СоответствиеНоменклатурыЕГАИС.ИдентификаторУпаковки
	|ГДЕ
