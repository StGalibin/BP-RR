	...
	Пока ВыборкаПоПоставщикам.Следующий() Цикл
		...
		Пока ВыборкаПоПартиям.Следующий() Цикл
			...
			Пока ВыборкаПоНоменклатуре.Следующий() Цикл
				...
				МассивКоличество = ОбщегоНазначенияБПКлиентСервер.РаспределитьПропорционально(ВыборкаПоНоменклатуре.КоличествоОстаток, МассивКРаспределениюКоличество);
...
Процедура ОбновитьСчетаФактуры(РежимЗаписи)
	...
	Исключение
		Отказ = Истина;
		ШаблонСообщения = НСтр("ru = 'Не удалось заблокировать документы %1'");
