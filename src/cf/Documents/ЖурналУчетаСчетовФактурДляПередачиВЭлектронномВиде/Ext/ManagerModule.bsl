...
Процедура ПодготовитьДанныеДляЗаполнения(ПараметрыДляЗаполнения, АдресХранилища) Экспорт
	...
	Если НЕ ПараметрыДляЗаполнения.ФорматПоПостановлению735 Тогда
		...
		Пока ВыборкаГруппировка.Следующий() Цикл
			...
			Пока ВыборкаЗаписи.Следующий() Цикл
				...
				Отчеты.ЖурналУчетаСчетовФактур.ЗаполнитьСтрокуЖурналаУчетаСчетовФактур(НоваяСтрока, ВыборкаЗаписи, ПараметрыДляЗаполнения);
	...
	Иначе
		...
		ПараметрыРаздела.Вставить("НалоговыйПериод",                     ПараметрыДляЗаполнения.НалоговыйПериод);
