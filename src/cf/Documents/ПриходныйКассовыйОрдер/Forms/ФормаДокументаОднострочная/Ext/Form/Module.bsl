...
&НаКлиенте
Процедура РозничнаяВыручкаОтражениеДоходовПриИзменении(Элемент)
	
	РозничнаяВыручкаОтражениеДоходовПриИзмененииСервер();
...
Процедура НапечататьЧекВыполнить()
	...
	ПриходныйКассовыйОрдерФормыКлиент.НапечататьЧек(ЭтотОбъект);
...
Процедура ОткрытьФормуТаблицы()
	...
	Шапка = Новый Структура("Ссылка, Дата, ВидОперации, Организация, ВалютаДокумента, Контрагент, ВалютаДокумента, СуммаДокумента, 
	|СчетУчетаРасчетовСКонтрагентом, Графа4_УСН, Графа5_УСН, ВыручкаСНТТ, ДеятельностьНаТорговомСборе");
	ЗаполнитьЗначенияСвойств(Шапка, Объект);
	РеквизитыФормы = "СвойстваПлатежа, ПорядокОтраженияАвансаСоответствиеЗначений, ПлательщикНДС, ПлательщикЕНВД, РозничнаяТорговляОблагаетсяЕНВД,
		|ПрименениеУСН, ПрименениеУСНДоходы, ПрименяетсяТолькоУСНПатент, ПрименяетсяОсобыйПорядокНалогообложения, ПрименяетсяУСНПатент, НТТНаЕНВД,
...
&НаСервере
Процедура РозничнаяВыручкаОтражениеДоходовПриИзмененииСервер()
	
	ПриходныйКассовыйОрдерФормы.РозничнаяВыручкаОтражениеДоходовПриИзмененииНаСервере(ЭтотОбъект);
