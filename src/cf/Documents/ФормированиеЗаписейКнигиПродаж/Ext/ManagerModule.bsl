...
Функция ПолучитьТекстЗапросаВосстановленПоАвансамУпрощенныйУчетНДС()
	...
	|ГДЕ
	|	НДСЗаписиКнигиПокупокОбороты.СуммаБезНДС - ЕСТЬNULL(НДСЗаписиКнигиПродажОбороты.СуммаБезНДС, 0) > 0
	|	И НДСЗаписиКнигиПокупокОбороты.НДС - ЕСТЬNULL(НДСЗаписиКнигиПродажОбороты.НДС, 0) > 0
	|
	|УПОРЯДОЧИТЬ ПО
	|	СчетФактура,
	|	Поставщик
