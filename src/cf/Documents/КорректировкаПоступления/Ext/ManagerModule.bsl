	...
	Пока ВыборкаСвязанныхДокументов.Следующий() Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Обновление реквизитов связанных документов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц)
	...
	|	ЕСТЬNULL(Реквизиты.ДоговорКонтрагента.ПредъявляетНДС, ИСТИНА) КАК ПоставщикПредъявляетНДС
	|ПОМЕСТИТЬ Реквизиты
	|ИЗ
	|	Документ.КорректировкаПоступления КАК Реквизиты
	|
	...
	|	Реквизиты.ВосстановитьНДС,
	|	Реквизиты.ПоставщикПредъявляетНДС КАК ПоставщикПредъявляетНДС
	|ИЗ
...
Функция ТекстЗапросаТаблицСторнирующихНДСЗаписей(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|ГДЕ
	|	ЗаписиКнигиПокупок.Организация = &Организация
	|	И ЗаписиКнигиПокупок.СчетФактура = &ИсправляемыйСчетФактура
	|	И ЗаписиКнигиПокупок.ИсправленныйСчетФактура = НЕОПРЕДЕЛЕНО
	|	И (ЗаписиКнигиПокупок.НДС >= 0
	|			ИЛИ ЗаписиКнигиПокупок.СуммаБезНДС >= 0)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаСобытия
	|;
	...
	|	ДатыСобытий.ДатаСобытия КАК ДатаСобытия,
	|	ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА ДатыСобытий.ДатаСобытия < ДАТАВРЕМЯ(2015, 1, 1)
	|			ТОГДА """"
	|		КОГДА ЗаписиКнигиПокупок.СчетФактура ССЫЛКА Документ.СчетФактураПолученный
	|				И ВЫРАЗИТЬ(ЗаписиКнигиПокупок.СчетФактура КАК Документ.СчетФактураПолученный).БланкСтрогойОтчетности
	|			ТОГДА ""23""
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК КодВидаОперации,
	|	ВЫБОР
	|		КОГДА ДатыСобытий.ДатаСобытия < ДАТАВРЕМЯ(2014, 10, 1)
	|			ТОГДА """"
	|		КОГДА ЗаписиКнигиПокупок.СчетФактура ССЫЛКА Документ.СчетФактураПолученный
	|				И ВЫРАЗИТЬ(ЗаписиКнигиПокупок.СчетФактура КАК Документ.СчетФактураПолученный).БланкСтрогойОтчетности
	...
	|		ПО ЗаписиКнигиПокупок.Организация = ДанныеПервичныхДокументов.Организация
	|			И (ВЫБОР
	|				КОГДА ЗаписиКнигиПокупок.СчетФактура ССЫЛКА Документ.СчетФактураПолученный
	|					ТОГДА ВЫРАЗИТЬ(ЗаписиКнигиПокупок.СчетФактура КАК Документ.СчетФактураПолученный).ДокументОснование = ДанныеПервичныхДокументов.Документ
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ)
	|;
	|
...
Процедура СформироватьДвиженияВыделениеАванса(ТаблицаВзаиморасчеты, ТаблицаРеквизиты, Движения, Отказ, ЭтоОтложенноеПроведение = Ложь) Экспорт
	...
	Если ПроведениеСервер.ИспользуетсяОтложенноеПроведение(Реквизиты.Организация, Реквизиты.Период)
		И НЕ ЭтоОтложенноеПроведение Тогда
