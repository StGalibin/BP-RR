...
Функция ОбновитьВторичныеДанныеДокумента(ДанныеОснования = Истина, ДанныеОрганизации = Истина, ДанныеСотрудника = Истина, ДанныеБанка = Истина) Экспорт 
	...
	Если ДанныеОснования И ЗаполнитьДанныеДокументаОснования() Тогда
	...
	Если ДанныеБанка И ЗаполнитьРеквизитыБанка() Тогда
		Модифицирован = Истина;
	КонецЕсли;
...
#Область ЗаполнитьРеквизитыБанка

Функция ЗаполнитьРеквизитыБанка()
	
	Модифицирован = Ложь;
	
	ДопРеквизитыБанка = РеквизитыБанка();
	Запрос = ПрямыеВыплатыПособийСоциальногоСтрахования.ЗапросПоСтруктуре(ДопРеквизитыБанка);
	Запрос.Выполнить();
	
	МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ЭтотОбъект.Метаданные().ПолноеИмя());
	ПараметрыФиксацииВторичныхДанных = МенеджерОбъекта.ПараметрыФиксацииВторичныхДанных(ЭтотОбъект);
	
	Модифицирован = ФиксацияВторичныхДанныхВДокументах.ОбновитьВторичныеДанные(Запрос.МенеджерВременныхТаблиц, ЭтотОбъект, , , ПараметрыФиксацииВторичныхДанных);
...
Функция РеквизитыБанка()
	РеквизитыБанка = Новый Структура("НаименованиеБанка,БИКБанка");
	
	Если ЗначениеЗаполнено(Банк) Тогда
		СтрокаДопРеквизитыБанка = "Наименование, Код";
		ДопРеквизитыБанка  = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Банк, СтрокаДопРеквизитыБанка);
		РеквизитыБанка.НаименованиеБанка = ДопРеквизитыБанка.Наименование;
		РеквизитыБанка.БИКБанка = ДопРеквизитыБанка.Код;
	КонецЕсли;
...
#Область ЗаполнитьДанныеСотрудника
...
Функция ЗаполнитьДанныеСотрудника()
	...
	Если ЗначениеЗаполнено(Сотрудник) Тогда
		...
		НеобходимыеКадровыеДанныеСотрудника = НеобходимыеКадровыеДанныеСотрудника();
		...
		Если ТЗКадровыеДанныеСотрудника.Количество() > 0 Тогда
			...
			ТЗДанныеОЛицевыхСчетах = ОбменСБанкамиПоЗарплатнымПроектам.ЛицевыеСчетаСотрудников(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Сотрудник),
			Истина,
			Организация,
			Дата);
			
			Если ТЗДанныеОЛицевыхСчетах.Количество() > 0 Тогда
				ДанныеОЛицевомСчете = ТЗДанныеОЛицевыхСчетах[0];
				РеквизитыДокумента.Вставить("СпособВыплатыПособия", Перечисления.СпособыВыплатыПособия.ЧерезБанк);
				РеквизитыДокумента.Вставить("Банк", ДанныеОЛицевомСчете.Банк);
				ДопРеквизитыБанка = РеквизитыБанка();
				РеквизитыДокумента.Вставить("НаименованиеБанка", ДопРеквизитыБанка.НаименованиеБанка);
				РеквизитыДокумента.Вставить("БИКБанка", ДопРеквизитыБанка.БИКБанка);
				РеквизитыДокумента.Вставить("НомерЛицевогоСчета", ДанныеОЛицевомСчете.НомерЛицевогоСчета);
			Иначе
...
Функция НеобходимыеКадровыеДанныеСотрудника()
	...
	Возврат НеобходимыеКадровыеДанныеСотрудника;	
...
Функция ЗаполнитьДанныеДокументаОснования()
	
	Возврат ПрямыеВыплатыПособийСоциальногоСтрахования.ЗаполнитьЗаявлениеСотрудникаНаВыплатуПособияПоОснованию(ЭтотОбъект);
