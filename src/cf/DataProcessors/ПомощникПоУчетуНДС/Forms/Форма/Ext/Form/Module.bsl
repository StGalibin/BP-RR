...
Процедура ПерепровестиДокументы(НомерМесяцаВКвартале)
	...
	Результат  = ЗапуститьПерепроведениеДокументов(НомерМесяцаВКвартале);
	...
	ИначеЕсли ЗначениеЗаполнено(ИдентификаторЗадания) Тогда
		...
		ОписаниеПроцесса.Месяц = ДобавитьМесяц(Объект.ПериодРегистрации, НомерМесяцаВКвартале - 1);
...
&НаСервере
Функция ЗапуститьПерепроведениеДокументов(НомерМесяцаВКвартале)
	...
	Если ПервыйНеисключенныйМесяц <= ТекущийМесяц Тогда
...
Процедура ОбработатьРезультатПерепроведенияДокументовНаКлиенте(РезультатПерепроведения)
	...
	Если ТипЗнч(РезультатПерепроведения) = Тип("Структура") Тогда
		...
		Если РезультатПерепроведения.Свойство("ВывестиИнформациюУведомлений")
			И РезультатПерепроведения.ВывестиИнформациюУведомлений Тогда
			// Покажем подробный отчет об ошибках.
			ОбщегоНазначенияБПКлиент.ОткрытьФормуОшибокПерепроведения(ЭтотОбъект, РезультатПерепроведения.АдресХранилищаСОшибками);
		КонецЕсли;
