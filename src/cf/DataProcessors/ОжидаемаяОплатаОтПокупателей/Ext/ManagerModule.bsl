...
Функция ТекстЗапросаПлатежи(ДляКалендаря, ДоступныОстаткиПоДокументамРасчетов, ДоступныОстаткиПоСчетамПокупателя)
	...
	Если ДоступныОстаткиПоСчетамПокупателя Тогда
		...
		|			ТОГДА 0
		|		ИНАЧЕ НеоплаченныеСчетаВВалюте.СуммаВВалюте
		...
		|			ТОГДА НеоплаченныеСчетаВВалюте.СуммаВВалюте * КурсыВалютДляСчетов.Курс / КурсыВалютДляСчетов.Кратность
		|		ИНАЧЕ 0
		|	КОНЕЦ - ЕСТЬNULL(ОплатаСчетов.СуммаОборот, 0) КАК Сумма,
