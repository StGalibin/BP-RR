...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Правило_НДФЛ             = ТекущееПравилоЗадачи("НДФЛ_Агент_Уплата");
	Правило_Взносы           = ТекущееПравилоЗадачи("СтраховыеВзносы");
	...
	Если ЭтоПредприниматель Тогда
		ЕстьБанковскийСчет = Справочники.БанковскиеСчета.ПолучитьКоличествоПодчиненныхЭлементовПоВладельцу(Объект.Организация) <> 0;
	КонецЕсли;
	...
	Если Аванс Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Аванс за %1 г. (%2)'"), ПредставлениеПериодаРегистрации, Объект.Организация.НаименованиеСокращенное);
	Иначе
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Зарплата, НДФЛ и взносы за %1 г. (%2)'"), ПредставлениеПериодаРегистрации, Объект.Организация.НаименованиеСокращенное);
	КонецЕсли;
	
	ВедетсяРасчетЗарплатыДляНебольшихОрганизаций = УчетЗарплаты.РасчетЗарплатыДляНебольшихОрганизаций();
	Элементы.ГруппаРасчетКоманды.Видимость          = ВедетсяРасчетЗарплатыДляНебольшихОрганизаций;
	Элементы.ГруппаРасчетПодвал.Видимость           = ВедетсяРасчетЗарплатыДляНебольшихОрганизаций;
	Элементы.ГруппаРасчетШапка.ОтображениеПодсказки = ?(ВедетсяРасчетЗарплатыДляНебольшихОрганизаций, ОтображениеПодсказки.Кнопка, ОтображениеПодсказки.Нет);
	...
	Если Аванс Тогда
		...
		ТекстШапки = НСтр("ru = 'Выплата аванса'");
		Элементы.ДекорацияДокументы.Заголовок = ТекстШапки;
...
Процедура ПриОткрытии(Отказ)
	...
	СтруктураРазделов = СформироватьСтруктуруРазделов(Истина, Истина, Истина, Истина, Истина, Параметры.Аванс);
			...
			Если Параметр.Свойство("Ведомость") Тогда
				...
				Если НайденныеВедомости.Количество() = 1 Тогда
					...
					Если НЕ ЗначениеЗаполнено(Платеж.ПлатежныеДокументы)
						...
						Платеж.Оплачено                        = Параметр.Оплачено;
						Платеж.ПлатежныеДокументыПредставление = ПредставлениеПлатежногоДокумента(Параметр.Ссылка, Параметр.Оплачено);
					Иначе
						...
						Платеж.ПлатежныеДокументыПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Несколько платежных документов (%1)'"),
...
Процедура ДействиеПлатежныйДокумент(Команда)
	...
	Если ТекущийДокумент <> Неопределено Тогда
		Если ТипЗнч(ТекущийДокумент.ПлатежныеДокументы) = Тип("СписокЗначений") Тогда
			...
			ОткрытьФорму("Обработка.ВыплатаЗарплатыПлатежнымиПоручениями.Форма", ПараметрыЗаполнения, ЭтаФорма,,,, ОткрытьДокументОповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Иначе
			...
			ИмяФормыПлатежногоДокумента = ?(ТипЗнч(ТекущийДокумент.ПлатежныеДокументы) = Тип("ДокументСсылка.РасходныйКассовыйОрдер"),
				"Документ.РасходныйКассовыйОрдер.ФормаОбъекта", "Документ.ПлатежноеПоручение.ФормаОбъекта");
			ОткрытьФорму(ИмяФормыПлатежногоДокумента, Новый Структура("Ключ", ТекущийДокумент.ПлатежныеДокументы),ЭтаФорма,,,, ОткрытьДокументОповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	Иначе
		...
		Если ТипЗнч(ПараметрыВедомости.Ведомость) = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыВКассу") Тогда
			...
			ОткрытьФорму("Документ.РасходныйКассовыйОрдер.ФормаОбъекта", ПараметрыЗаполнения, ЭтаФорма,,,, ОткрытьДокументОповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		ИначеЕсли ТипЗнч(ПараметрыВедомости.Ведомость) = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыВБанк") Тогда
			Если ЗначениеЗаполнено(ПараметрыВедомости.ЗарплатныйПроект) Тогда
				...
				ПараметрыЗаполнения = Новый Структура("ЗначенияЗаполнения",  ЗначенияЗаполнения);
				ОткрытьФорму("Документ.ПлатежноеПоручение.ФормаОбъекта", ПараметрыЗаполнения, ЭтаФорма,,,, ОткрытьДокументОповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
			Иначе
				ПараметрыЗаполнения  = Новый Структура();
				ПараметрыЗаполнения.Вставить("Организация",        Объект.Организация);
				ПараметрыЗаполнения.Вставить("СчетОрганизации",    Объект.СчетОрганизации);
...
Процедура ДействиеВыплата(Команда)
	...
	Если ТекущийДокумент <> Неопределено Тогда
		...
		ВозвращаемыеПараметры = Новый Структура("Ссылка, ВерсияДанных", ТекущийДокумент, ПараметрыВедомости.ВедомостьВерсияДанных);
		ОткрытьДокументОповещение = Новый ОписаниеОповещения("ОбработатьРедактированиеВедомости", ЭтотОбъект, ВозвращаемыеПараметры);
		ОткрытьФорму(ИмяФормыПлатежногоДокумента, Новый Структура("Ключ", ТекущийДокумент),ЭтаФорма,,,, ОткрытьДокументОповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
...
Процедура ДействиеРасчет(Команда)
	...
	Если ТекущийДокумент <> Неопределено Тогда
		...
		ОткрытьДокументОповещение = Новый ОписаниеОповещения("ОбработатьДобавлениеРасчетногоДокумента", ЭтотОбъект, Новый Структура("СведенияОДокументе", СведенияОДокументе));
		ОткрытьФорму("Документ." + ИмяОбъекта + ".ФормаОбъекта", Новый Структура("Ключ", ТекущийДокумент),ЭтаФорма,,,,ОткрытьДокументОповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ДобавитьОтпуск(Команда)
	...
	ПараметрыДокумента = Новый Структура("Организация, ПериодРегистрации", Объект.Организация, НачалоМесяца(Объект.ПериодРегистрации));
	ОткрытьФорму("Документ.Отпуск.ФормаОбъекта", Новый Структура("ЗначенияЗаполнения", ПараметрыДокумента), ЭтаФорма,,,, ДобавитьДокументОповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ДобавитьБольничный(Команда)
	...
	ПараметрыДокумента = Новый Структура("Организация, ПериодРегистрации", Объект.Организация, НачалоМесяца(Объект.ПериодРегистрации));
	ОткрытьФорму("Документ.БольничныйЛист.ФормаОбъекта", Новый Структура("ЗначенияЗаполнения", ПараметрыДокумента), ЭтаФорма,,,, ДобавитьДокументОповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ДобавитьВедомостьБанк(Команда)
	...
	ПараметрыДокумента = Новый Структура("Организация, ПериодРегистрации", Объект.Организация, НачалоМесяца(Объект.ПериодРегистрации));
	ОткрытьФорму("Документ.ВедомостьНаВыплатуЗарплатыВБанк.ФормаОбъекта", Новый Структура("ЗначенияЗаполнения", ПараметрыДокумента), ЭтаФорма,,,, ДобавитьДокументОповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ДобавитьВедомостьКасса(Команда)
	...
	ПараметрыДокумента = Новый Структура("Организация, ПериодРегистрации", Объект.Организация, НачалоМесяца(Объект.ПериодРегистрации));
	ОткрытьФорму("Документ.ВедомостьНаВыплатуЗарплатыВКассу.ФормаОбъекта", Новый Структура("ЗначенияЗаполнения", ПараметрыДокумента), ЭтаФорма,,,, ДобавитьДокументОповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаСервере
Функция ТекущееПравилоЗадачи(ИдентификаторЗадачи)
...
Функция СформироватьСтруктуруРазделов(СводРасчетов, РасчетныеДокументы, Ведомости, Платежи_НДФЛ, Платежи_Взносы, Аванс)
	...
	СтруктураРазделов.Вставить("Аванс",              Аванс);
...
Функция СохранилисьВерсииДанных(СписокНовый, СписокИсходный)
	...
	Для Каждого ДокументИсходый ИЗ СписокИсходный Цикл
		...
		Если ДокументНовый = Неопределено Тогда
			Возврат Ложь;
		КонецЕсли;
		Если ДокументИсходый.Представление <> ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументНовый.Значение, "ВерсияДанных") Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ОбработатьДобавлениеРасчетногоДокументаНаСервере(СведенияОДокументе)
	...
	Если СведенияОДокументе <> Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.БольничныйЛист")
			...
			РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "Проведен, Сотрудник, ПериодРегистрации, Начислено");
	...
	СтруктураРазделов = СформироватьСтруктуруРазделов(Истина, Истина, Ложь, ПеречитатьНДФЛ, Истина, Параметры.Аванс);
...
&НаКлиенте
Процедура ОбработатьДобавлениеРасчетногоДокумента(РезультатЗакрытия = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
...
&НаКлиенте
Процедура ОбработатьРедактированиеВедомости(РезультатЗакрытия = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	СтруктураРазделов = СформироватьСтруктуруРазделов(Истина, Ложь, Истина, Ложь, Ложь, Параметры.Аванс);
...
&НаКлиенте
Процедура ОбработатьРедактированиеПлатежногоДокументаНаНДФЛ(РезультатЗакрытия = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	СтруктураРазделов = СформироватьСтруктуруРазделов(Истина, Ложь, Ложь, Истина, Ложь, Параметры.Аванс);
...
&НаКлиенте
Процедура ОбработатьРедактированиеПлатежногоДокументаНаВзносы(РезультатЗакрытия = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	СтруктураРазделов = СформироватьСтруктуруРазделов(Истина, Ложь, Ложь, Ложь, Истина, Параметры.Аванс);
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ДекорацияОписаниеДокументыАванс.Видимость   = Форма.Параметры.Аванс И НЕ Форма.ЕстьВедомостьВБанк И НЕ Форма.ЕстьВедомостьВКассу;
	Элементы.ГруппаСобытиеДокументыСуммаАванса.Видимость = Форма.Параметры.Аванс И (Форма.ЕстьВедомостьВБанк ИЛИ Форма.ЕстьВедомостьВКассу);
...
Процедура ОбновитьИзображение(НомерРаздела = 0)
	...
	Для Каждого СтрокаРасчета Из СводРасчетов Цикл
		Если СтрокаРасчета.Порядок = 1 Тогда
			...
			Элементы.СуммаЗадолженность.Заголовок   = ОбщегоНазначенияБПВызовСервера.ФорматСумм(СтрокаРасчета.Сумма, , , " "); // ЧРГ=' '
			Прервать;
		КонецЕсли;
	КонецЦикла;
		...
		Если ЗначениеЗаполнено(Платеж.ПлатежныеДокументы) Тогда
			...
			Если НЕ ЕстьПлатежныеПоручения И ТипЗнч(Платеж.ПлатежныеДокументы) = Тип("СписокЗначений") Тогда
				Для Каждого СтрокаСписка ИЗ Платеж.ПлатежныеДокументы Цикл
					Если НЕ СтрокаСписка.Значение.Оплачено Тогда
						ЕстьПлатежныеПоручения = Истина;
						Прервать;
					КонецЕсли;
				КонецЦикла;
	...
	Если НомерРаздела = 2 ИЛИ НомерРаздела = 0 Тогда
		...
		Для Каждого Ведомость Из Ведомости Цикл
			...
			Если Элементы.Найти("ОписаниеВыплата" + НомерКонтейнера) = Неопределено Тогда
				КопироватьЭлементСПодчиненными(ЭтаФорма, Элементы.ОписаниеВыплата, Элементы.СобытиеВыплата, НомерКонтейнера);
			КонецЕсли;
	...
	Если НомерРаздела = 5 ИЛИ НомерРаздела = 0 Тогда
		...
		Для Каждого Показатель Из СводРасчетов Цикл
			...
			Если Элементы.Найти("ОписаниеДокументы" + НомерКонтейнера) = Неопределено Тогда
				КопироватьЭлементСПодчиненными(ЭтаФорма, Элементы.ОписаниеДокументы, Элементы.СобытиеДокументы, НомерКонтейнера);
			КонецЕсли;
...
Процедура ДействиеПлатеж(Команда, СуффиксТаблицы, ИмяПлатежногоДокумента = Неопределено)
	...
	ТекущийДокумент = ПараметрыФормыПлатежногоДокумента(Идентификатор, , СуффиксТаблицы);
	
	ВозвращаемыеПараметры = Неопределено;
	Если ТекущийДокумент.Свойство("Ключ") Тогда
		ВозвращаемыеПараметры = Новый Структура("Ссылка, ВерсияДанных", ТекущийДокумент.Ключ, ТекущийДокумент.ВерсияДанных);
	КонецЕсли;
...
&НаКлиенте
Процедура ДекорацияПлатежныеВедомостиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если МассивДокументов.Количество() > 0 Тогда
		УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.ВедомостьНаВыплатуЗарплатыВКассу", "Т53", МассивДокументов, ЭтотОбъект);
	КонецЕсли;
		
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияВыдачаНаличныхОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если МассивДокументов.Количество() > 0 Тогда
		УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.РасходныйКассовыйОрдер", "РКО", МассивДокументов, ЭтотОбъект);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияСпискиПеречисленийОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если МассивДокументов.Количество() > 0 Тогда
		УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.ВедомостьНаВыплатуЗарплатыВБанк", "СписокПеречислений", МассивДокументов, ЭтотОбъект);
	КонецЕсли;
		
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияПлатежныеПорученияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
