...
Процедура ОтобразитьВопросТеста(НомерВопроса)
	...
	Элементы.НадписьПояснение.Заголовок	 = ВопросТеста.Информация;
...
Процедура ЗарегистрироватьОтвет(Ответ)
	...
	СписокВопросов = ДанныеТеста.Вопросы;
	ВопросТеста = СписокВопросов[ТекущийВопрос - 1];
	РезультатТеста.Вставить("Вопрос" + Строка(ТекущийВопрос), ?(Ответ, ВопросТеста.ЕслиДа, ВопросТеста.ЕслиНет));
