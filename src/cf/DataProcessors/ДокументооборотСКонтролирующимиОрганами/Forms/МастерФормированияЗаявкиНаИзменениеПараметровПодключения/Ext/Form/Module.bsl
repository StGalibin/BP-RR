...
Процедура ПодписатьЗаявлениеИОтправить(Команда)
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите скан-копию паспорта владельца сертификата'"), ,"СканПаспорт");
	КонецЕсли;
...
Процедура КоманднаяПанельМастерДалее(Команда, БезбумажноеПродление = Ложь)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ОсновнаяПанель.ПодчиненныеЭлементы.ПолучениеРезультатаОтправкиЗаявления Тогда
		...
		Если ДокументЗаявление.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаявленияАбонентаСпецоператораСвязи.Отклонено") Тогда
			...
			ПараметрыОткрытияФормы.Вставить("ВидЗаявления", ПредопределенноеЗначение("Перечисление.ТипыЗаявленияАбонентаСпецоператораСвязи.Изменение"));
			
			ПрограммноеЗакрытие = Истина;
			
			// Закрываем это заявление и на основе него готовим еще одно (после закрытия) в процедуре 
			// ОткрытьОтложенноФормуМастераЗаявленияНаПодключениеПослеПолученияКонтекстаЭДОЗавершение
			Закрыть(ПараметрыОткрытияФормы);
...
Процедура ПроверитьЗаполнениеЗаявления(МастерДалее)
	...
	Если ИзменитьРеквизитыПодключенияК1СОтчетности Тогда
		Если ПустаяСтрока(СтрЗаменить(АдрЮР,",","")) Тогда
			...
			ТекстОшибки 		= НСтр("ru = 'Заполните юридический адрес'");
			ВывестиОшибку(ТекстОшибки);
		...
		Иначе	
			...
			Если НЕ ПустаяСтрока(СтрокаОшибкаАдреса) Тогда
				...
				ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.АдресЮридический");
				ТекстОшибки 		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Юридический адрес: %1'"),СтрокаОшибкаАдреса);
				ВывестиОшибку(ТекстОшибки);
...
Процедура УстановитьГалкуИзменитьВладельцаСертификата()
	...
	Иначе
		...
		Если (ПодразделениеОтличается ИЛИ ДолжностьОтличается) И ЗначениеЗаполнено(ВладелецЭЦП) Тогда
