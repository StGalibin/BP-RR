...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ЭлектроннаяВыпискаБанка")
		...
		Объект.НачалоПериода = ДатаСообщения;
		Объект.КонецПериода = ДатаСообщения;
...
Процедура КомандаЗагрузить(Команда)
	...
	Иначе
		АктуализироватьДокументыКИмпорту(РезультатЗагрузки.ПодготовленныеДанные);
	КонецЕсли;
...
Процедура ФайлЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбменСБанкомКлиент.ВыбратьИЗагрузитьФайл(ЭтаФорма);
...
Процедура ФайлЗагрузкиОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	...
	Если НЕ ПустаяСтрока(Текст) Тогда
		...
		ОбменСБанкомКлиент.ЗагрузитьВыбранныйФайл(Объект.ФайлЗагрузки, ЭтаФорма);
	КонецЕсли;
...
Процедура ДокументыКИмпортуСчетКонтрагентаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТипЗнч(ТекущиеДанные.Контрагент) = Тип("СправочникСсылка.Организации") Тогда
		Возврат;
	КонецЕсли;
	...
	ПараметрыОтбора.Вставить("Владелец",    ТекущиеДанные.Контрагент);
	ПараметрыОтбора.Вставить("Организация", Объект.Организация);
	ПараметрыОтбора.Вставить("Валютный",    (ВалютаДенежныхСредств <> ВалютаРегламентированногоУчета));
	Если ПараметрыОтбора.Валютный Тогда
		ПараметрыОтбора.Вставить("ВалютаДенежныхСредств", ВалютаДенежныхСредств);
	КонецЕсли;
...
Функция ПрочитатьФайлВыпискиНаСервере(ОписаниеФайла, Кодировка)
	...
	СтруктураПараметров.Вставить("ИмяФайла",  ОписаниеФайла.Имя);
...
Процедура ЗапроситьЭлектроннуюВыпискуБанка()
	...
	ОбменСБанкамиКлиент.ПолучитьВыпискуБанка(
		Объект.СоглашениеПрямогоОбменаСБанками, Объект.НачалоПериода, Объект.КонецПериода, ЭтотОбъект, ПолучитьНомерСчета(Объект.БанковскийСчет));
...
&НаСервере
Процедура РазместитьНаФормеРаспознанныеДанныеИзБанка(АдресХранилища, ВыводитьСообщения = Истина)
	
	Если ПустаяСтрока(АдресХранилища) Тогда
		Возврат;
	КонецЕсли;
	
	РаспознанныеДанныеИзБанка = ПолучитьИзВременногоХранилища(АдресХранилища);
...
Процедура РазместитьПредупреждение(СтрокаТаблицы, ПредупрежденияРаспознавания = Неопределено, РезультатСозданияДокументов = Неопределено)
	...
	Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.Документ)
		И ТребуетсяКонтрагент
		...
		МассивПредупреждений.Добавить(НСтр("ru='Не заполнен расчетный счет контрагента'"));
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция СоответствиеВыпискиОтбору(Знач АдресХранилища, Знач БанковскийСчет, Знач Организация = Неопределено)
...
Функция РаспознатьДанныеИзБанкаПовторноНаСервере()
	...
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		СтруктураПараметров,
		НСтр("ru = 'Повторное чтение данных из банка'"));
...
&НаСервере
Процедура ОбработатьРезультатРаспознатьДанныеИзБанкаПовторно(АдресХранилища)
	
	АдресХранилищаРаспознанныеДанныеИзБанка = АдресХранилища;
	РазместитьНаФормеРаспознанныеДанныеИзБанка(АдресХранилища, Ложь);
...
Процедура ДобавитьКонтрагентаВДеревоКонтрагентов(СтрокаКонтрагент, КонтрагентДляСозданиеНенайденного, УчастникиОпераций)
	...
	Иначе
		...
		ИмяКолонкиНаименование = "НаименованиеПолное";
	...
	Иначе
		...
		СтруктураРеквизитов.Наименование = НовыйКонтрагент[ИмяКолонкиНаименование];
	КонецЕсли;
...
Процедура Подключаемый_РаспознатьДанныеБанкаПовторно()
	...
	Результат = РаспознатьДанныеИзБанкаПовторноНаСервере();
	
	Если НЕ Результат.ЗаданиеВыполнено Тогда
		ДлительныеОперацииКлиент.ИнициализироватьПараметрыОбработчикаОжидания(ПараметрыОбработчикаОжидания);
		ПодключитьОбработчикОжидания("Подключаемый_ПроверитьВыполнениеЗаданияРаспознатьДанныеИзБанкаПовторно", 1, Истина);
		ФормаДлительнойОперации = ДлительныеОперацииКлиент.ОткрытьФормуДлительнойОперации(ЭтотОбъект, ИдентификаторЗадания);
		
		ИдентификаторЗадания = Результат.ИдентификаторЗадания;
		АдресХранилища       = Результат.АдресХранилища;
	Иначе
		ОбработатьРезультатРаспознатьДанныеИзБанкаПовторно(Результат.АдресХранилища);
		ВыполняетсяПовторноеРаспознаваниеДанныхИзБанка = Ложь;
...
Процедура ПрочитатьФайлЗавершение(ПодключеноРасширениеРаботыСФайлами, ДополнительныеПараметры) Экспорт
	...
	Если (Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.ГруппаЗагрузка) 
		...
		И НЕ ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() // Не запускаем чтение файла в веб-клиенте, так как требуется ращрешение от пользователя
...
&НаКлиенте
Процедура Подключаемый_ПроверитьВыполнениеЗаданияРаспознатьДанныеИзБанкаПовторно()
	
	Попытка
		Если ЗаданиеВыполнено(ИдентификаторЗадания) Тогда
			ДлительныеОперацииКлиент.ЗакрытьФормуДлительнойОперации(ФормаДлительнойОперации);
			ОбработатьРезультатРаспознатьДанныеИзБанкаПовторно(АдресХранилища);
			ВыполняетсяПовторноеРаспознаваниеДанныхИзБанка = Ложь;
			ВосстановитьТекущуюСтроку();
		Иначе
			ДлительныеОперацииКлиент.ОбновитьПараметрыОбработчикаОжидания(ПараметрыОбработчикаОжидания);
			ПодключитьОбработчикОжидания(
				"Подключаемый_ПроверитьВыполнениеЗаданияРаспознатьДанныеИзБанкаПовторно",
				ПараметрыОбработчикаОжидания.ТекущийИнтервал,
				Истина);
		КонецЕсли;
	Исключение
