...
Процедура СинхронизироватьТаблицы_Капитал()
	...
	ТаблицаКапитал.Свернуть("Контрагент, СчетУчета, СчетУчетаУчредители", "СуммаВклада, СуммаОплачено");
	...
	ТаблицаКапитал_Вспомогательная.Свернуть("Ссылка, Контрагент, СчетУчета", "Сумма, СуммаКт");
	...
	Для Каждого СтрокаТаблицы ИЗ ТаблицаКапитал_Вспомогательная Цикл
		Если ТаблицаКапитал.Найти(СтрокаТаблицы.Контрагент, "Контрагент") <> Неопределено Тогда
...
Процедура РаспределитьСуммыПоТаблице(ТаблицаИсточник, ТаблицаБаза, ИмяИсходнойКолонки, ИмяРезультирующейКолонки, ИмяКолонкиСчет = "СчетУчета")
	...
	Отбор = Новый Структура("Контрагент, СчетУчета");
