...
Функция ВыполнитьПерепроведение(Параметры) Экспорт
	...
	ИмяСобытия       = ЗакрытиеМесяцаКлиентСервер.СобытиеЖурналаРегистрации()
		+ "." + ЗакрытиеМесяца.ПредставлениеОперацииПерепроведениеДокументов(ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	ДанныеДляПроверки.Раздел = ПланыВидовХарактеристик.РазделыДатЗапретаИзменения.БухгалтерскийУчет;
			...
			Если НЕ ВыполнятьТолькоАктуализациюРасчетовСКонтрагентами Тогда
				...
				Пока ВыборкаПоДокументам.Следующий() Цикл
					...
					Иначе
						...
						Успешно = ПровестиПервичныйДокумент(ВыборкаПоДокументам, СтрокаОрганизации, ТаблицаСообщений);
	...
	// СтандартныеПодсистемы.ОценкаПроизводительности
	ОценкаПроизводительности.ЗакончитьЗамерВремени("ЗакрытиеМесяцаПерепроведениеДокументов", ВремяНачала);
...
Функция ИсключаемыеИзПерепроведенияДокументыБезусловно() Экспорт
	...
	Результат.Вставить("ВозвратИзТорговогоЗалаЕГАИС");
	Результат.Вставить("ОстаткиЕГАИС");
	Результат.Вставить("ПередачаВТорговыйЗалЕГАИС");
...
Функция ТекстыЗапросовПоПервичнымДокументам(Параметры, НомераТаблиц, НомерТаблицОтложенноеПроведение)
	...
	|
	|ВЫБРАТЬ
	|	КОНЕЦПЕРИОДА(Журнал.ПериодРегистрации, МЕСЯЦ),
	|	Журнал.Ссылка,
	|	Журнал.Организация,
	|	ЛОЖЬ,
	|	ВЫБОР
	...
	|
	|ВЫБРАТЬ
	|	КОНЕЦПЕРИОДА(Журнал.МесяцНачисления, МЕСЯЦ),
	|	Журнал.Ссылка,
	|	Журнал.Организация,
	|	ЛОЖЬ,
	|	ВЫБОР
...
Функция ПровестиПервичныйДокумент(ВыборкаПоДокументам, СостояниеПоОрганизации, ТаблицаСообщений)
