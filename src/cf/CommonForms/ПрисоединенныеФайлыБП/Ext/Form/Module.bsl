...
Процедура Отправить(Команда)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "ФормированиеПисьмаПрисоединенныеФайлы");
	...
	Если ЗначениеЗаполнено(ПараметрыПисьма) Тогда
		ФайловыеФункцииСлужебныйКлиент.ОтправитьФайлыПоПочте(
			ФайлыДляОтправки, УникальныйИдентификатор, ПараметрыПисьма);
	КонецЕсли;
...
Функция ПодготовитьПараметрыЭлектронногоПисьма(Знач ФайлыДляОтправки, Знач ВладелецФайла, Знач УникальныйИдентификатор)
	...
	МетаданныеВладельцаФайла = ВладелецФайла.Метаданные();
	...
	Если Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(ВладелецФайла)) И МетаданныеВладельцаФайла.ДлинаНомера > 0 Тогда
	...
	Для Каждого ФайлДляОтправки Из ФайлыДляОтправки Цикл
		ДанныеФайла = ПрисоединенныеФайлы.ПолучитьДанныеФайла(
				ФайлДляОтправки, УникальныйИдентификатор, Истина);
	...
	ПараметрыПисьма = ОтправкаПочтовыхСообщений.НовыеПараметрыПисьма();
