...
Процедура ДатаИзмененияСтрокойНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИмяРегистра = СтрЗаменить(ТекущийЭлемент.Имя, "ДатаИзмененияСтрокой_","");
	
	СтандартнаяОбработка = Ложь;
	ОписаниеОповещения = Новый ОписаниеОповещения("ПериодЗавершениеВыбора", ЭтотОбъект, ИмяРегистра);
...
Процедура СнижениеСтавкиНалогаНаИмуществоПриИзменении(Элемент)
	...
	СтавкиНалогаНаИмуществоФормыКлиент.СнижениеСтавкиНалогаНаИмуществоПриИзменении(ЭтотОбъект, Элемент);
	ОтобразитьПериоды("СтавкиНалогаНаИмущество");
	
КонецПроцедуры

&НаКлиенте
Процедура СниженнаяСтавкаНалогаНаИмуществоПриИзменении(Элемент)
	
	СтавкиНалогаНаИмуществоФормыКлиент.СниженнаяСтавкаНалогаНаИмуществоПриИзменении(ЭтотОбъект, Элемент);
	ОтобразитьПериоды("СтавкиНалогаНаИмущество");
...
&НаКлиенте
Процедура СниженнаяСтавкаНалогаНаИмуществоИзменениеТекстаРедактирования(Элемент, Текст, СтандартнаяОбработка)
...
Процедура История(Команда)
	...
	ИмяРегистра = СтрЗаменить(ТекущийЭлемент.Имя, "_История","");
	...
	Иначе
		ОткрытьФорму("РегистрСведений." + ИмяРегистра + ".Форма.ФормаСписка", Новый Структура("Отбор, ВедущийОбъект, ТекущийПериод", Отбор, Организация, ТекущаяДата));
	КонецЕсли;
...
Процедура _ОК(Команда)
	...
	ИмяРегистра = СтрЗаменить(ТекущийЭлемент.Имя, "ОК_","");
...
Процедура ОписатьНастройкиУчета()
	...
	Если СкрытыеОтчеты <> 0 Тогда
		НоваяСтрока = Список.Добавить();
		НоваяСтрока.Наименование = СтрШаблон(НСтр("ru = 'Все налоги и отчеты (ещё %1)'"), СкрытыеОтчеты);
...
Процедура ВопросПередСменойОрганизации(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат = КодВозвратаДиалога.Да Тогда
		...
		Если Элементы.Список.ТекущиеДанные <> Неопределено Тогда
			...
			ЗаписатьИзмененияРаздела(ИмяРаздела);
		КонецЕсли;
...
Процедура ЗаписатьИзменения(ИмяРегистра)
	...
	Если ИмяРегистра = "СтраховыеВзносы" Тогда
		...
		МенеджерЗаписи.Записать(Истина);
	Иначе
		Если ДатаИзменения = ЭтотОбъект[ИмяРегистра].Период Тогда
			МенеджерЗаписи = РеквизитФормыВЗначение(ИмяРегистра);
		Иначе
			МенеджерЗаписи = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
...
&НаСервере
Процедура ЗаписатьИзмененияРаздела(ИмяРаздела)
...
Процедура ОтменитьИзмененияИстории(Команда)
	...
	ИмяРаздела = СтрЗаменить(ТекущийЭлемент.Имя, "Отменить_","");
	Элементы["Страницы" + ИмяРаздела + "История"].ТекущаяСтраница = Элементы["Страница" + ИмяРаздела + "Периоды"];
...
Процедура ОтобразитьРеквизитыЗависимыеОтСистемыНалогообложения()
	...
	// Налог на имущество
	Если НастройкиСистемыНалогообложения.ПрименяетсяУСН Тогда
		ВидимостьНалогаНаИмущество = Ложь;
		Если СписокНедействующихНастроек.НайтиПоЗначению("ПлательщикНалогаНаПрибыль") <> Неопределено Тогда
			ВидимостьНалогаНаИмущество = Истина;
		КонецЕсли;
	Иначе
		ВидимостьНалогаНаИмущество = Истина;
	КонецЕсли;
	
	Элементы.СтраницыСтавкиНалогаНаИмуществоИстория.Видимость = ВидимостьНалогаНаИмущество;
	Элементы.ГруппаСтавкиНалогаНаИмущество.Видимость          = ВидимостьНалогаНаИмущество;
...
Процедура ОтобразитьРеквизитыЗависимыеОтТекущихНастроек(Форма)
	...
	Элементы.ГиперссылкаМорскиеСуда.Доступность
...
Процедура ПериодЗавершениеВыбора(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора <> Неопределено Тогда
		...
		ЭтотОбъект.ДатаИзмененияСтрокой = БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(
		...
		Если ДополнительныеПараметры = "НастройкиУчетаУСН" Тогда
