...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ДоговорЕстьРеквизит И НЕ КурсДокументаЕстьРеквизит Тогда	
		...
		Если РасчетыВУЕ И ВалютаУчета <> ВалютаРасчетов Тогда // у.е.
			...
			Элементы.КурсРасчетов.Доступность = (ВалютаДокумента <> ВалютаУчета);
	...
	Если ЗначениеЗаполнено(ВалютаДокумента) И (НЕ КурсДокументаЕстьРеквизит ИЛИ ПересчитатьВалютуДокумента) Тогда
		...
		Иначе
			Если ЗначениеЗаполнено(МассивКурсКратность) Тогда
				Курс = МассивКурсКратность[0].Курс;
				Кратность = МассивКурсКратность[0].Кратность;
				// Установим отображение курса расчетов по валюте расчетов на текущий день.
				МассивКурсКратностьРасчетов = КурсыВалют.НайтиСтроки(Новый Структура("Валюта", ВалютаРасчетов));
				Если ЗначениеЗаполнено(МассивКурсКратностьРасчетов) Тогда
					КурсРасчетов = МассивКурсКратностьРасчетов[0].Курс;
				КонецЕсли;
			Иначе
				Курс = 0;
				Кратность = 0;
