...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Завершение индексирования объектов журнала Отчетность" Тогда
...
Процедура ОтчетыПриОбновленииСоставаПользовательскихНастроекНаСервере(СтандартнаяОбработка)
	...
	ПользовательскиеНастройкиДинамическогоСписка = Отчеты.КомпоновщикНастроек.ПользовательскиеНастройки;
	...
	Для Каждого ЭлементПользовательскойНастройки Из ПользовательскиеНастройкиДинамическогоСписка.Элементы Цикл
		Если ТипЗнч(ЭлементПользовательскойНастройки) = Тип("ОтборКомпоновкиДанных")
			И ЭлементПользовательскойНастройки.ИдентификаторПользовательскойНастройки =
				Отчеты.КомпоновщикНастроек.Настройки.Отбор.ИдентификаторПользовательскойНастройки Тогда
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если (СтрНайти(ТекущиеДанныеНаименованиеОтчета, "Прил. 13: Объем собранного винограда для производства винодельческой продукции") > 0
			...
			Если ВыбраннаяФорма = "ФормаОтчета2016Кв2" Тогда
				...
				ТекстПредупреждения = НСтр("ru = 'Проверка выгрузки, выгрузка и отправка редакции формы, утвержденной ПП РФ №411 от 13.05.2016, 
			...
			ДлитОперация = ОбщийМодульАЛКОКлиент.ПолучитьПеречислениеВидыДлительныхОперацийРеглОтчетов("ОтправкаВКонтролирующийОрган");				
		...
		// 11-я и 12 алкодекларация.
		Если (( СтрНайти(ТекущиеДанныеНаименованиеОтчета, "Прил. 11: Розничная продажа алкогольной") > 0 ) 
			или ( СтрНайти(ТекущиеДанныеНаименованиеОтчета, "Прил. 12: Розничная продажа пива и пивных напитков") > 0 ))
			И ( ТипЗнч(ТекущиеДанные.РегламентированныйОтчет) = Тип("ДокументСсылка.РегламентированныйОтчет") ) Тогда
...
Процедура ПроверитьВыгрузку(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			...
			Если СтрНайти(ТекущиеДанныеНаименованиеОтчета, "Прил. 11: Розничная продажа алкогольной") > 0
				 или
			 	 СтрНайти(ТекущиеДанныеНаименованиеОтчета, "Прил. 12: Розничная продажа пива и пивных напитков") > 0			
				 Тогда
...
Процедура ОтправитьУведомления(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТекущиеДанные.Наименование = "Заявление о зачете налога"
			ИЛИ ТекущиеДанные.Наименование = "Заявление о возврате налога" Тогда
			
			ПредупредитьОбОграничениях(Закладка, НСтр("ru = 'Отправка из списка не предусмотрена'"));
			...
			Для Каждого ЭлементОтбора Из ЭлементПользовательскойНастройки.Элементы Цикл
				Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
					Если ЗначениеЗаполнено(ЭлементОтбора.ПредставлениеПользовательскойНастройки) Тогда
						...
						ЭлементОтбора.ПредставлениеПользовательскойНастройки = Строка(ЭлементОтбора.ЛевоеЗначение);
					КонецЕсли
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Процедура ОбновитьКоличествоНепрочтенныхОбъектов(Форма)
	...
	Для каждого НепрочтенныеОбъектыРаздела Из НепрочтенныеОбъектыПоРазделам Цикл
		...
		Форма.Элементы["Гиперссылка" + ИмяСтраницы(СтраницаЖурнала)].Заголовок = ИмяСтраницы(СтраницаЖурнала) + НадписьКоличество;
...
Процедура ОнлайнПроверка(Закладка)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		// АлкоОтчеты с реализацией проверки с использованием длительных опреаций,
		// виноградные декларации 13, 14, 15.
		Если (СтрНайти(Представление, "Прил. 13: Объем собранного винограда для производства винодельческой продукции") > 0
			или
			СтрНайти(Представление, "Прил. 14: Объем винограда, использованного для производства вина, игристого вина (шампанского)") > 0
			или
			СтрНайти(Представление, "Прил. 15: Объем винограда, использованного для производства винодельческой продукции с защищенным географическим указанием") > 0)
			И ( ТипЗнч(ПроверяемыйОбъект) = Тип("ДокументСсылка.РегламентированныйОтчет") ) Тогда
			
			Если НЕ ОбщегоНазначенияКлиент.ПодсистемаСуществует("РегламентированнаяОтчетность.ОтчетностьПоАлкогольнойПродукции") Тогда
				Возврат;
			КонецЕсли;
			
			ДокументСсылкаАлкоОтчет = ПроверяемыйОбъект;
			Описание = ОписаниеРегОтчета(ДокументСсылкаАлкоОтчет);
			ВыбраннаяФорма = Описание.ВыбраннаяФорма;
			...
			ДлитОперация = ОбщийМодульАЛКОКлиент.ПолучитьПеречислениеВидыДлительныхОперацийРеглОтчетов("ПроверкаВыгрузкиВИнтернете");				
		...
		// АлкоОтчеты с реализацией выгрузки с использованием длительных опреаций,
		// 11-я и 12-я алкодекларация.
		Если (( СтрНайти(Представление, "Прил. 11: Розничная продажа алкогольной") > 0 ) 
			или ( СтрНайти(Представление, "Прил. 12: Розничная продажа пива и пивных напитков") > 0 ))
			И ( ТипЗнч(ПроверяемыйОбъект) = Тип("ДокументСсылка.РегламентированныйОтчет") ) Тогда
		...
		Если ЗначениеЗаполнено(ИмяТипаДокументаРеестрДанныхЭЛН)
			И ТипЗнч(ПроверяемыйОбъект) = Тип("ДокументСсылка." + ИмяТипаДокументаРеестрДанныхЭЛН) Тогда
			
			ТекстПредупреждения = СтрШаблон(НСтр("ru = 'Для отчета ""%1"" проверка в интернете не предусмотрена.'"), Представление);
			ПоказатьПредупреждение(, ТекстПредупреждения);
			Возврат;
		КонецЕсли;
