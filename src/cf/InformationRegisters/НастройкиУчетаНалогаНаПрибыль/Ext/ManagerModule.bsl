...
// Возвращает сведения о порядоке уплаты авансовых платежей
//
// Параметры:
...
Функция ПорядокУплатыАвансовДействующейНастройки(Организация, Период = Неопределено) Экспорт
	...
	Иначе
		ПорядокУплатыАвансовПоНалогуНаПрибыль = Неопределено;
	КонецЕсли;
...
Процедура ОтключитьПрименениеРазныхСтавокНалогаНаПрибыль() Экспорт
	...
	|ГДЕ
	|	НастройкиУчетаНалогаНаПрибыль.ПрименяютсяРазныеСтавкиНалогаНаПрибыль";
	...
	Для Каждого Запись Из Набор Цикл
		Запись.ПрименяютсяРазныеСтавкиНалогаНаПрибыль = Ложь;
	КонецЦикла;
