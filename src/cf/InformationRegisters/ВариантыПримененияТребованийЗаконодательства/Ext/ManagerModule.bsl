...
// Некоторые настройки были перемещены из регистра
// ВариантыПримененияТребованийЗаконодательства в другие регистры.
//
Процедура УдалитьПеренесенныеНастройки() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр(
		"УсловияПроверяемыеПоДругимНастройкам",
		УсловияПроверяемыеПоДругимНастройкам());
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Настройки.Организация КАК Организация,
	|	Настройки.Условие КАК Условие
	|ИЗ
	|	РегистрСведений.ВариантыПримененияТребованийЗаконодательства КАК Настройки
	|ГДЕ
	|	Настройки.Условие В(&УсловияПроверяемыеПоДругимНастройкам)";
	...
	// Удаляем записи по одной, так как записей к удалению заведомо немного,
	// много меньше, чем записей в регистре.
	// ОбновлениеИнформационнойБазы.УдалитьДанные() не может использоваться для менеджера записи,
	// поэтому используем набор записей, состоящий из одной записи.
	НаборЗаписей = РегистрыСведений.ВариантыПримененияТребованийЗаконодательства.СоздатьНаборЗаписей();
	
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		НаборЗаписей.Отбор.Организация.Установить(Выборка.Организация);
		НаборЗаписей.Отбор.Условие.Установить(Выборка.Условие);
