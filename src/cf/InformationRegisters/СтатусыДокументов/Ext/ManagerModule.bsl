...
//
Функция УстановитьСтатусыДокумента(Документ, Статус = Неопределено, ДополнительныйСтатус = Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(Документ) Тогда
		МассивДокументов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Документ);
		СтатусИзменен = УстановитьСтатусыДокументов(МассивДокументов, Статус, ДополнительныйСтатус);
	Иначе
		СтатусИзменен = Ложь;
	КонецЕсли;
...
//
Функция УстановитьСтатусыДокументов(МассивДокументов, Статус = Неопределено, ДополнительныйСтатус = Неопределено) Экспорт
	...
	Если Не ИзменитьСтатус И Не ИзменитьДополнительныйСтатус Тогда
		Возврат СтатусИзменен;
	КонецЕсли;
	...
	Если ИмяДокумента = "СчетНаОплатуПоставщика" Тогда
		Запрос.Параметры.Вставить("ДополнительныйСтатусПоУмолчанию", Перечисления.СтатусыПоступленияПоСчету.СтатусНовогоДокумента());
	Иначе
	...
	Пока Выборка.Следующий() Цикл
		...
		Если (Не ИзменитьСтатус Или Выборка.Статус = Статус)
			И (Не ИзменитьДополнительныйСтатус Или Выборка.ДополнительныйСтатус = ДополнительныйСтатус) Тогда
			Продолжить;
		КонецЕсли;
