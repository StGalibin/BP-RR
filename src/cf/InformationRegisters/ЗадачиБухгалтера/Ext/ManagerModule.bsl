...
//                 что относятся к уплате налога по этой задаче
...
Процедура ЗаполнитьРасписаниеПоНалогамИСборам(Расписание, Организация, Знач НачалоОбзора, Знач КонецОбзора, Отбор = Неопределено, Правила = Неопределено, ОбновитьСписокОтчетов = Истина)
	...
	УчетЗарплаты.СоздатьПравилоВыплатаЗарплатыУплатаНДФЛ(МенеджерВременныхТаблиц, Организация); // отдельное правило для удержанного НДФЛ
...
//
Функция РасписаниеПоНалогамОтчетамЗаПериод(Организация, НачалоИнтервала, КонецИнтервала) Экспорт
	
	Расписание = НовыйРасписание();
	Расписание.Колонки.Добавить("ПеренестиНаРабочийДень", Новый ОписаниеТипов("Число"));

	ЗаполнитьРасписаниеПоНалогамИСборам(Расписание, Организация, НачалоИнтервала, КонецИнтервала,,, Ложь);
...
Процедура ЗаполнитьСтатусы() Экспорт
	...
	|	РегистрСведений.ЗадачиБухгалтера КАК ЗадачиБухгалтера
	|ГДЕ
	|	ЗадачиБухгалтера.Правило ССЫЛКА Справочник.ПравилаПредставленияОтчетовУплатыНалогов
	|	И ЗадачиБухгалтера.Правило.Действие = ЗНАЧЕНИЕ(Перечисление.ВидыДействийКалендаряБухгалтера.Отчет)";
	
	ЗадачиОтчеты = Запрос.Выполнить().Выгрузить();
	
	ВыполнениеЗадачБухгалтера.ДобавитьСтатусыЗадач(ЗадачиОтчеты);
	
	КлючиЗаписи = Новый Структура("Организация, Правило, ПериодСобытия, РегистрацияВНалоговомОргане");
	
	Для Каждого Задача Из ЗадачиОтчеты Цикл
		
		Если ПустаяСтрока(Задача.Статус) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ПеренестиДанныеИзРегистраВыполненныеЗадачиНачалаРаботы() Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	УдалитьВыполненныеЗадачиНачалаРаботы.Организация КАК Организация,
...
Процедура ОтметитьЗадачуУплатыСтраховыхВзносовИПЗаТекущийГодКакНевыполненную() Экспорт
	
	Период = ТекущаяДатаСеанса();
	
	Запрос = Новый Запрос;
	Запрос.Параметры.Вставить("Идентификатор",  "СтраховыеВзносы_Предприниматель");
	Запрос.Параметры.Вставить("КонецЭтогоГода", НачалоДня(КонецГода(Период)));
	Запрос.Параметры.Вставить("ЭтотГод",        Год(Период));
	Запрос.Параметры.Вставить("ФизическоеЛицо", Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо);
	...
	|	И ЗадачиБухгалтера.Срок = &КонецЭтогоГода
	|	И НЕ ЗадачиБухгалтера.Организация.ПометкаУдаления
	|	И ЗадачиБухгалтера.Организация.ЮридическоеФизическоеЛицо = ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо)
	|	И ГОД(ЗадачиБухгалтера.Организация.ДатаРегистрации) = &ЭтотГод
	|	И ЗадачиБухгалтера.Правило.Владелец.Код = &Идентификатор";
	...
	Пока Выборка.Следующий() Цикл
		...
		НаборЗаписей.Отбор.ПериодСобытия.Установить(Выборка.ПериодСобытия);
...
Процедура УдалитьЗадачиПоМестнымНалогамБезУказанияНалоговыхОрганов() Экспорт
	...
	Запрос.Параметры.Вставить("НалогиУплачиваемыеНаМестах", НалогиУплачиваемыеНаМестах);
	Запрос.Параметры.Вставить("РегистрацияВНалоговомОргане", Справочники.РегистрацииВНалоговомОргане.ПустаяСсылка());
	...
	|	РегистрСведений.ЗадачиБухгалтера КАК ЗадачиБухгалтера
	|ГДЕ
	|	ЗадачиБухгалтера.Правило.Владелец В(&НалогиУплачиваемыеНаМестах)
	|	И ЗадачиБухгалтера.РегистрацияВНалоговомОргане = &РегистрацияВНалоговомОргане
	|	И ЗадачиБухгалтера.ПериодСобытия >= ДАТАВРЕМЯ(2016, 1, 1)";
...
// Обработка обновления ИБ. Заполняет добавленный реквизит НаименованиеСокращенное
//
Процедура ЗаполнитьСокращенноеНаименование() Экспорт
	...
	ПравилаРегулярныхПлатежей = Тип("СправочникСсылка.ПравилаРегулярныхПлатежей");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(Выборка.Правило) = ПравилаРегулярныхПлатежей Тогда
			Запись.НаименованиеСокращенное = СократитьНаименованиеРегулярногоПлатежаПриОбновленииИБ(Выборка.Наименование);
		Иначе
			Запись.НаименованиеСокращенное = Выборка.Наименование;
		КонецЕсли;
...
// Вспомогательная функция обновления ИБ. Преобразует наименование заданное по 
// шаблону 'Оплата [СуммаСКопейками] руб [КонтрагентНаименование]' в сокращенное наименование,
// по шаблону '[СуммаБезКопеек] руб [КонтрагентНаименование]'
// См. также ЗаполнитьЗаписьРасписанияРегулярныеПлатежи()
//
Функция СократитьНаименованиеРегулярногоПлатежаПриОбновленииИБ(Знач Наименование)
	
	СловоОплата        = НСтр("ru ='Оплата '");
	НаименованиеВалюты = НСтр("ru =' руб '");
	
	ПозицияВалюты = СтрНайти(Наименование, НаименованиеВалюты);
	
	Если Не СтрНачинаетсяС(Наименование, СловоОплата) Или ПозицияВалюты = 0 Тогда
		Возврат Наименование;
	КонецЕсли;
	
	НаименованиеКонтрагента = Сред(Наименование, ПозицияВалюты + СтрДлина(НаименованиеВалюты));
	
	СуммаСтрокой      = Сред(Лев(Наименование, ПозицияВалюты - 1), СтрДлина(СловоОплата) + 1);
	ОписаниеТипаЧисло = ОбщегоНазначения.ОписаниеТипаЧисло(15, 0);
	СуммаЧислом       = ОписаниеТипаЧисло.ПривестиЗначение(СуммаСтрокой);
	Если СуммаЧислом <> 0 Тогда
		СуммаСтрокой = Формат(СуммаЧислом, "ЧДЦ=");
	КонецЕсли;
	
	НаименованиеСокращенное = СтрШаблон("%1 - %2", СуммаСтрокой, НаименованиеКонтрагента);
	
	Возврат НаименованиеСокращенное;
	
КонецФункции

// Удаление задач начала работы при обновлении ИБ
//
Функция УдалитьУстаревшиеЗадачиНачалаРаботы() Экспорт
	
	УдаляемыеПравила = Новый Массив;
	УдаляемыеПравила.Добавить(Перечисления.ЗадачиНачалаРаботы.УдалитьДоступКОрганизациям);
	УдаляемыеПравила.Добавить(Перечисления.ЗадачиНачалаРаботы.УдалитьРеквизитыОрганизации);
	УдаляемыеПравила.Добавить(Перечисления.ЗадачиНачалаРаботы.УдалитьБыстрыйСтарт);
	УдаляемыеПравила.Добавить(Перечисления.ЗадачиНачалаРаботы.УдалитьВидеоурок);
	УдаляемыеПравила.Добавить(Перечисления.ЗадачиНачалаРаботы.УдалитьПараметрыУчета);
	УдаляемыеПравила.Добавить(Перечисления.ЗадачиНачалаРаботы.УдалитьУчетнаяПолитика);
	УдаляемыеПравила.Добавить(Перечисления.ЗадачиНачалаРаботы.УдалитьНачальныеОстатки);
	УдаляемыеПравила.Добавить(Перечисления.ЗадачиНачалаРаботы.УдалитьНастройкаТорговыхТочек);
	
	Для Каждого Правило Из УдаляемыеПравила Цикл
...
Функция ПорядокПредоставленияОтчетаУплатыНалогаЗаПериод(Организация, ВидНалога, Период, Действие = Неопределено) Экспорт
	...
	НачалоРасписания = Макс(НачалоГода(ДобавитьМесяц(Период, -6)), ДатаНачалаДеятельности) - 1;
	...
	Для Индекс = 0 По Расписание.Количество() - 1 Цикл
		...
		Если СтрокаРасписания.Срок > СтрокаРасписания.ПериодСобытия Тогда
			...
			Если СтрокаРасписания.ПериодСобытия <= НачалоДня(Период) Тогда
				...
				КонецЕсли;
				Если ДействиеПравила = Перечисления.ВидыДействийКалендаряБухгалтера.УплатаНалога И ИндексСобытияУплаты = Неопределено Тогда
	...
	Если ИндексСобытияУплаты <> Неопределено Тогда
