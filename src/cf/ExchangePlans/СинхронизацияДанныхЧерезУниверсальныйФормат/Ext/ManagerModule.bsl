...
Процедура ОпределитьНастройки(Настройки, ИдентификаторНастройки) Экспорт
	...
	Если ИдентификаторНастройки = "ОбменУТБП" Тогда
		Настройки.Вставить("ЗаголовокКомандыДляСозданияНовогоОбменаДанными", НСтр("ru = '1С:Управление торговлей базовая, редакция 11'"));
		Настройки.Вставить("ЗаголовокПомощникаСозданияОбмена",               НСтр("ru = 'Настройка синхронизации с программой ""1С:Управление торговлей базовая, редакция 11""'"));
		Настройки.Вставить("ЗаголовокУзлаПланаОбмена",                       НСтр("ru = 'Синхронизация с программой ""1С:Управление торговлей базовая, редакция 11""'"));
		Настройки.Вставить("НаименованиеКонфигурацииКорреспондента",         НСтр("ru = '1С:Управление торговлей базовая, редакция 11'"));
	ИначеЕсли ИдентификаторНастройки = "ОбменУТ11" Тогда
	...
	ИначеЕсли ИдентификаторНастройки = "ОбменУНФ" Тогда
		Настройки.Вставить("ЗаголовокКомандыДляСозданияНовогоОбменаДанными", НСтр("ru = '1С:Управление нашей фирмой 1.6'"));
		Настройки.Вставить("ЗаголовокПомощникаСозданияОбмена",               НСтр("ru = 'Настройка синхронизации с программой ""1С:Управление нашей фирмой 1.6""'"));
		Настройки.Вставить("ЗаголовокУзлаПланаОбмена",                       НСтр("ru = 'Синхронизация с программой ""1С:Управление нашей фирмой 1.6""'"));
		Настройки.Вставить("НаименованиеКонфигурацииКорреспондента",         НСтр("ru = '1С:Управление нашей фирмой 1.6'"));
	ИначеЕсли ИдентификаторНастройки = "ОбменКасса" Тогда
		Настройки.Вставить("ЗаголовокКомандыДляСозданияНовогоОбменаДанными", НСтр("ru = '1С:Касса'"));
		Настройки.Вставить("ЗаголовокПомощникаСозданияОбмена",               НСтр("ru = 'Настройка синхронизации с программой ""1С:Касса""'"));
		Настройки.Вставить("ЗаголовокУзлаПланаОбмена",                       НСтр("ru = 'Синхронизация с программой ""1С:Касса""'"));
		Настройки.Вставить("НаименованиеКонфигурацииКорреспондента",         НСтр("ru = '1С:Касса'"));
	...
	ОписаниеВариантаНастройкиОбмена = Настройки.ОписанияВариантовНастроекОбмена.Добавить();
	ОписаниеВариантаНастройкиОбмена.ИдентификаторНастройки = "ОбменУТ11";
	ОписаниеВариантаНастройкиОбмена.ИмяКорреспондента      = "УправлениеТорговлей";
	
	ОписаниеВариантаНастройкиОбмена = Настройки.ОписанияВариантовНастроекОбмена.Добавить();
	ОписаниеВариантаНастройкиОбмена.ИдентификаторНастройки = "ОбменУПБП";
	ОписаниеВариантаНастройкиОбмена.ИмяКорреспондента      = "УправлениеПредприятием";
	...
	Настройки.Вставить("ИмяПланаОбменаДляПереходаНаНовыйОбмен", "_ДемоСинхронизацияДанныхЧерезУниверсальныйФормат");
...
Функция ПолучитьПредставлениеКонфигурацииКорреспондента(КраткоеИмяКонфигурации) Экспорт
	...
	ИначеЕсли КраткоеИмяКонфигурации = "БухгалтерияПредприятияКОРП" Тогда
		Возврат НСтр("ru = 'Бухгалтерия предприятия КОРП, ред.3.0'");
	...
	ИначеЕсли КраткоеИмяКонфигурации = "УправлениеНебольшойФирмой" Тогда
		Возврат НСтр("ru = 'Управление нашей фирмой, ред.1.6'");
	ИначеЕсли КраткоеИмяКонфигурации = "Касса" Тогда
		Возврат НСтр("ru = '1С:Касса, редакция 1.0'");
	Иначе
		Возврат "";
	КонецЕсли;
КонецФункции
...
Функция МассивДоступныхВариантовНастроекОбмена()
	...
	МассивНастроек.Добавить("ОбменУТ11");
	МассивНастроек.Добавить("ОбменУПБП");
	Если Метаданные.Имя <> "БухгалтерияПредприятияКОРП" Тогда
		МассивНастроек.Добавить("ОбменУНФ");
	КонецЕсли;
	МассивНастроек.Добавить("ОбменКасса");
...
Процедура ПолучитьВерсииФорматаОбмена(ВерсииФормата) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		ВерсииФормата.Вставить("1.4", ВнешняяОбработкаМенеджераОбмена);
	Иначе
...
Функция ВыполнитьПереходНаСинхронизациюДанныхЧерезУниверсальныйФормат(НастройкаСинхронизацииДанных) Экспорт
	...
	НоваяНастройкаОбъект.ВерсияФорматаОбмена = "1.0";
