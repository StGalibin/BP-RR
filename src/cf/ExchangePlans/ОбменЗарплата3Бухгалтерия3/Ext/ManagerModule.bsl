...
Процедура ОпределитьНастройки(Настройки, ИдентификаторНастройки) Экспорт
	...
	Настройки.ЗаголовокПомощникаСозданияОбмена  = НСтр("ru = 'Настройка обмена данными с программой Зарплата и управление персоналом, ред. 3'");;
	Настройки.ОтображатьЗначенияПоУмолчаниюНаУзле = Ложь;
	Настройки.ОтображатьЗначенияПоУмолчаниюНаУзлеБазыКорреспондента = Ложь;
	
	ЗаголовокФормыПомощника = НСтр("ru='Синхронизация данных с Зарплата и управление персоналом, ред. 3 (настройка)'");
...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	Если НастройкаОтборовНаУзле.НеВыгружатьПерсональныеДанныеФизическихЛиц Тогда
		ОграничениеВыгружатьПерсональныеДанные = НСтр("ru = 'Данные для формирования проводок будут загружаться сводно.'");;
	Иначе
		ОграничениеВыгружатьПерсональныеДанные = НСтр("ru = 'Данные для формирования проводок будут загружаться с детализацией по сотрудникам.'"); 
	КонецЕсли;
