...
//
Процедура ОбработатьРезультатПоискаИННПриИзмененииПоляПоиска(ПоискИННОтвет, ОписаниеОповещения) Экспорт
	
	ОбработатьРезультатПоискаЕГР(ПоискИННОтвет, ОписаниеОповещения, Ложь); // Заполнение без нажатия кнопки, поэтому поменьше говорим с пользователем
...
Процедура ЗаполнитьРеквизитыПоПолюПоискаИНН(ПоискИННЗапрос, ПоискИННОтвет, ОписаниеОповещения, ТекущийЭлемент, ЭлементПолеВводаИНН) Экспорт
	...
	Если ЗначениеЗаполнено(ПоискИННОтвет) Тогда
		ОбработатьРезультатПоискаЕГР(ПоискИННОтвет, ОписаниеОповещения, Истина);
		Возврат;
	КонецЕсли;
...
//
// Параметры:
//  Форма						 - УправляемаяФорма - форма справочника Организации: 
...
Процедура ПовторноЗаполнитьРеквизитыПоИНН(РезультатПодключения, ПараметрыЗаполнения) Экспорт
	...
	ОбработатьРезультатПоискаЕГР(ПоискИННОтвет, ПараметрыЗаполнения.ОбработчикЗавершенияЗаполнения, Истина);
...
//  Форма					 - УправляемаяФорма - форма, отображающая Объект (при успешном выборе в ней будет установлена модифицированность)
...
//
Процедура ВыбратьКодИзКлассификатора(ИмяКлассификатора, ИмяРеквизитаКод, ИмяРеквизитаНаименование, Форма, Объект, СтандартнаяОбработка) Экспорт
...
Процедура ВыбратьКодИзКлассификатораЗавершение(ВыбранныйКод, ПараметрыЗавершенияВыбора) Экспорт
	...
	ПараметрыЗавершенияВыбора.Объект[ПараметрыЗавершенияВыбора.ИмяРеквизитаНаименование] = ОрганизацииФормыДляОтчетностиВызовСервера.НаименованиеПоКлассификатору(
...
//  Форма					  - УправляемаяФорма - форма, отображающая представление системы налогообложения
...
Процедура ОбработатьРезультатПоискаЕГР(ПоискИННОтвет, ЗакончитьЗаполнениеДаннымиЕГР, АктивныйДиалогСПользователем)
...
Процедура НачатьЗаполнениеДаннымиЕГР(ПоискИННОтвет, АктивныйДиалогСПользователем, ЗакончитьЗаполнениеДаннымиЕГР)
	...
	Если Не ПустаяСтрока(ПоискИННОтвет.Ошибка.Код) Тогда
		...
		Если АктивныйДиалогСПользователем Тогда
			...
			Если ПоискИННОтвет.Ошибка.Код = "НеУказаныПараметрыАутентификации"
				Или ПоискИННОтвет.Ошибка.Код = "НеУказанПароль" Тогда 
				ПараметрыЗаполнения = Новый Структура;
				ПараметрыЗаполнения.Вставить("Запрос",                         ПоискИННОтвет.Запрос);
				ПараметрыЗаполнения.Вставить("ОбработчикЗавершенияЗаполнения", ЗакончитьЗаполнениеДаннымиЕГР);
			...
			ИначеЕсли ПоискИННОтвет.Ошибка.Код = "Сервис1СКонтрагентНеПодключен" Тогда
				ПараметрыФормы = Новый Структура;
			...
			ИначеЕсли ПоискИННОтвет.Ошибка.Код = "НеСоответствуетТребованиям" Тогда
				ПоказатьПредупреждение(, НСтр("ru = 'Введите корректный ИНН'"));
			Иначе
				// Не обрабатываемый код ошибки
				ПоказатьПредупреждение(, НСтр("ru = 'Не удалось заполнить реквизиты.'"));
			КонецЕсли;
