...
Функция РазделитьСообщенияПоТипам(СообщенияОтчетности)
	...
	ТипыСообщений = СообщенияОтчетности.Скопировать(, "ТипСообщения");
	ТипыСообщений.Свернуть("ТипСообщения");
	ТипыСообщений.Колонки.Добавить("Приоритет", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(1)));
	...
	Для Каждого ТипСообщений Из ТипыСообщений Цикл
		ТипСообщений.Приоритет = ПриоритетСобщенийОтчетности(ТипСообщений.ТипСообщения);
...
Функция ПриоритетСобщенийОтчетности(ТипСообщений)
...
Процедура ОстаткиДенежныхСредств(Параметры) Экспорт
	...
	ИначеЕсли ВалютныеОстатки.Количество() <= БлокиИнформационнойПанелиКлиентСервер.КоличествоВалютныхОстатков() Тогда
...
Процедура ДобавитьПоддержкаСервиса(ТаблицаБлоков)
	...
	Добавить(ТаблицаБлоков,
		...
		БлокиИнформационнойПанелиКлиентСервер.ТекстЗаголовкаПоддержкаСервиса(ОбщегоНазначенияБП.ДомашняяСтраницаСервиса()),
...
Процедура ПоддержкаСервиса(Параметры) Экспорт
	...
	Результат.Вставить("ПоддержкаСервиса_Заголовок",
		...
		ОбщегоНазначенияБП.ДомашняяСтраницаСервиса())));
