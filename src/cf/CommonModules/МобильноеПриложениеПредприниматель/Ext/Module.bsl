...
Процедура ПодготовитьДанныеДляВыгрузки(СтруктураПараметров, СообщениеОбОшибке) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МобильноеПриложениеПредприниматель.ПодготовитьДанныеДляВыгрузки'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			СтрШаблон(НСтр("ru = 'Не удалось удалить файл %1: %2'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
				ПолноеИмяФайла,
...
Процедура ОтправитьУведомление(ДанныеУведомления, ВариантОтправки, ДанныеАутентификации, Получатели)
	...
	Исключение
		ИмяСобытия  = НСтр("ru='Отправка Push-уведомлений'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ТекстСколькоОсталось(Срок, Сегодня)
	...
	Иначе
		ТекстОсталось = СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
			ОсталосьДней, НСтр("ru='Остался,Осталось,Осталось'"), Ложь);
		ТекстДней = СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
			ОсталосьДней, НСтр("ru='день,дня,дней'"));
		Возврат ТекстОсталось + " " + ТекстДней;
	КонецЕсли;
