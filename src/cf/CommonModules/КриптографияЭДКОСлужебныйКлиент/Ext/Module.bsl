...
Процедура ПроверитьСертификат(ОповещениеОЗавершении, Сертификат, Проверки, ВыводитьСообщения, МенеджерКриптографии) Экспорт
	...
	ВходящиеПараметры = ПараметрыВМассив(2, Сертификат, Проверки);
	...
	Если Результат.Выполнено И Результат.СертификатНайден Тогда
		...
		Если ВходящийКонтекст.ВходящиеПараметры[1] = "ПроверитьТолькоПоСпискуУстановленныхУЦ" Тогда
			...
			ИЛИ ТекДата - 49 * 3600 < Результат.СвойстваСертификата.ДействителенС ИЛИ ТекДата + 49 * 3600 > Результат.СвойстваСертификата.ДействителенПо Тогда
				ВыполнитьОбработкуОповещения(
					ВходящийКонтекст.ОповещениеОЗавершении,
					ПодготовитьРезультат(Истина, "Валиден", Истина, ВходящийКонтекст));
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПроверитьПодписьPKCS7(ОповещениеОЗавершении, ЭтоЭлектроннаяПодписьВМоделиСервиса, ФайлПодписи, ФайлДанных, ИмпортироватьСертификаты, ВыводитьСоообщения, МенеджерКриптографии) Экспорт
	
	ВходящиеПараметры = ПараметрыВМассив(4, ЭтоЭлектроннаяПодписьВМоделиСервиса, ФайлПодписи, ФайлДанных, ИмпортироватьСертификаты);
	...
	Если ЭтоЭлектроннаяПодписьВМоделиСервиса Тогда
		Попытка
			...
			Если НЕ ПодписьВалидна И ЗначениеЗаполнено(КомментарийПоОшибке) Тогда
				ВыполнитьОбработкуОповещения(
					ОповещениеОЗавершении,
					ПодготовитьРезультат(Ложь, "ОписаниеОшибки", КомментарийПоОшибке));
		...
		Исключение
			ВыполнитьОбработкуОповещения(
				ОповещениеОЗавершении,
				ПодготовитьРезультат(Ложь, "ОписаниеОшибки", ОписаниеОшибкиНеУдалосьПроверитьПодпись()));
		КонецПопытки;
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьПодписьPKCS7ПослеПолученияФайлаПодписи(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Иначе
			ВыполнитьОбработкуОповещения(
				Оповещение,
				ПодготовитьРезультат(Истина, "ИмяФайла", ВходящийКонтекст.ВходящиеПараметры[2])); 
		КонецЕсли;	
	Иначе
		ВыполнитьОбработкуОповещения(
			ВходящийКонтекст.ОповещениеОЗавершении, 
...
Процедура ПроверитьПодписьPKCS7ПослеПолученияИмениКаталога(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		ПолучитьПодписантовPKCS7(
			Оповещение, ВходящийКонтекст.ИмяФайлаПодписи, ВходящийКонтекст.КаталогСертификатов, ВходящийКонтекст.ВыводитьСообщения,
...
Процедура ПроверитьПодписьPKCS7ПослеПолученияПодписантов(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Если ИмпортироватьСертификаты Тогда
			...
			НайтиСертификаты(Оповещение, ВходящийКонтекст.Подписанты, Ложь, ВходящийКонтекст.ВыводитьСообщения,
				ВходящийКонтекст.МенеджерКриптографии, Истина);
		Иначе
...
Процедура ПроверитьПодписьPKCS7ПослеИмпортаСертификата(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		Результат = ПодготовитьРезультат(Ложь, "ОписаниеОшибки", Результат.ОписаниеОшибки);
...
Процедура ПроверитьПодписьPKCS7ПроверкаПодписей(ВходящийКонтекст, ПодписьВалидна = Истина)
	...
	Результат = ПодготовитьРезультат(Истина, "ПодписьВалидна", ПодписьВалидна, ВходящийКонтекст);
	Результат.Вставить("Подписанты", ВходящийКонтекст.Подписанты);
	Для ИндексПодписанта = 0 По Результат.Подписанты.Количество() - 1 Цикл
		Результат.Подписанты[ИндексПодписанта] = Новый ФиксированнаяСтруктура(Результат.Подписанты[ИндексПодписанта]);
	КонецЦикла;
...
Процедура ПроверитьПодписьPKCS7ПослеВыполнения(Результат, Параметры, ВходящийКонтекст) Экспорт
	...
	Если Результат Тогда
		...
		ПроверитьСертификат(Оповещение, ВходящийКонтекст.Подписанты[ВходящийКонтекст.ТекущийПодписант], "ПроверитьТолькоПоСпискуУстановленныхУЦ",
			ВходящийКонтекст.ВыводитьСообщения, ВходящийКонтекст.МенеджерКриптографии);
	Иначе
...
Процедура ПроверитьПодписьPKCS7ПослеПолученияЗначенияСвойстваСписокОшибок(ЗначениеСвойства, ВходящийКонтекст) Экспорт
	...
	Результат = ПодготовитьРезультат(Ложь, "ОписаниеОшибки", ВходящийКонтекст.ОписаниеОшибки, ВходящийКонтекст);
	Результат.Вставить("Подписанты", ВходящийКонтекст.Подписанты);
	ВыполнитьОбработкуОповещения(
		ВходящийКонтекст.ОповещениеОЗавершении,
		Результат);
...
Процедура ЗашифроватьPKCS7(ОповещениеОЗавершении, ИсходныйФайл, Сертификаты, ИмяФайлаИлиРасширениеИлиАдрес, ВыводитьСоообщения, МенеджерКриптографии, КлючевойКонтейнер = Неопределено) Экспорт
	...
	ТипыПодходятДляЭПСервиса = (ТипЗнч(ИсходныйФайл) = Тип("Строка") И ЭтоАдресВоВременномХранилище(ИсходныйФайл)) Или ТипЗнч(ИсходныйФайл) = Тип("ДвоичныеДанные");
...
Процедура ЗашифроватьPKCS7ПослеШифрованияСервисомКриптографии(Результат, ВходящийКонтекст) Экспорт
	...
	Если ТипЗнч(Результат)=Тип("Структура") Тогда
		...
		Если ИмяФайлаИлиРасширениеИлиАдрес = Неопределено Тогда //Результат необходимо переложить по адресу/пути исходных данных
			...
			Если ЭтоАдресВоВременномХранилище(ИсходныйФайл) Тогда 
				
				Если ЭтоАдресВоВременномХранилище(ЗашифрованныеДанные) Тогда 
			...
			Иначе //Путь к файлу на диске
				...
				Если Не ЭтоАдресВоВременномХранилище(ЗашифрованныеДанные) Тогда //Двоичные данные
		...
		ИначеЕсли ЭтоАдресВоВременномХранилище(ИмяФайлаИлиРасширениеИлиАдрес) Тогда	//Результат необходимо поместить по указанному адресу
...
Процедура ПодписатьИЗашифроватьPKCS7(ОповещениеОЗавершении, СертификатПодписанта, ИмяФайлаДанных, ВключатьИсходныеДанные, СертификатыПолучателей,
	...
	Если СертификатВЗащищенномХранилищеНаСервере(СертификатПодписанта) Тогда
...
Процедура ПодписатьИЗашифроватьPKCS7ПослеПодписания(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		РезультатОшибки = ПодготовитьРезультат(Ложь, "ОписаниеОшибки", КраткоеПредставлениеОшибкиКриптосервиса(Результат.ИнформацияОбОшибке));
...
Процедура РасшифроватьPKCS7(ОповещениеОЗавершении, ЗашифрованныйФайл, ИмяФайлаИлиРасширениеИлиАдрес, Сертификат, ВыводитьСоообщения, МенеджерКриптографии) Экспорт
	...
	Если Сертификат = Неопределено Или СертификатВЗащищенномХранилищеНаСервере(Сертификат) Тогда
		
		Если НЕ ЭтоАдресВоВременномХранилище(ЗашифрованныйФайл) И ТипЗнч(ЗашифрованныйФайл)<>Тип("ДвоичныеДанные") Тогда
			...
			ВыполнитьОбработкуОповещения(
				ОповещениеОЗавершении,
				ПодготовитьРезультат(Ложь, "ОписаниеОшибки", ОшибкаТекстом));
	...
	Если ТипЗнч(Результат)=Тип("Структура") Тогда
		...
		Если ИмяФайлаИлиРасширениеИлиАдрес = Неопределено Тогда //Результат необходимо переложить по адресу исходного файла / пути нового файла
			...
			Иначе //Путь к файлу на диске
				...
				Если Не ЭтоАдресВоВременномХранилище(РасшифрованныеДанные) Тогда //Двоичные данные
...
Процедура ПолучитьПодписантовPKCS7(ОповещениеОЗавершении, ИмяФайлаПодписи, КаталогСертификатов, ВыводитьСоообщения, МенеджерКриптографии) Экспорт
...
Процедура ПолучитьПодписантовPKCS7ПослеВыполнения(Результат, Параметры, ВходящийКонтекст) Экспорт
	...
	Если Подписанты <> Неопределено И Подписанты.Количество() > 0 Тогда
		ВыполнитьОбработкуОповещения(
			ВходящийКонтекст.ОповещениеОЗавершении,
			ПодготовитьРезультат(Истина, "Подписанты", Подписанты, ВходящийКонтекст));
	Иначе
...
Процедура ПодписатьПослеПодписиСервисомКриптографии(Результат, ВходящийКонтекст) Экспорт
	...
	Если ТипЗнч(Результат)=Тип("Структура") Тогда
		...
		Если ЗначениеЗаполнено(ИмяФайлаПодписи) Тогда 
			...
			Если ЭтоАдресВоВременномХранилище(ИмяФайлаПодписи) Тогда 
				Если Результат.Выполнено Тогда 
		...
		Иначе //Поместим результат по адресу исходных данных / запишем в новый файл
			...
			Если ЭтоАдресВоВременномХранилище(ИсходныйФайл) Тогда 
				Если ЭтоАдресВоВременномХранилище(Подпись) Тогда 
			...
			Иначе //Исходные данные переданы как полный путь к файлу на диске
				...
				Если Не ЭтоАдресВоВременномХранилище(Подпись) Тогда //Двоичные данные
...
Процедура ПодписатьПослеТрансформацииМассиваСертификатов(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено и Результат.Сертификаты.Количество() > 0 Тогда
		...
		ВходящийКонтекст.МенеджерКриптографии.НачатьВызовПодписать(
			...
			?(ВходящиеПараметры[2] <> Неопределено, ВходящиеПараметры[1].АлгоритмХеширования, 32798) // идентификатор алгоритма хеширования
		);
...
Процедура ХешироватьДанныеСервисПередЗавершением(Результат, ВходящийКонтекст) Экспорт
	...
	ВыполнитьОбработкуОповещения(ВходящийКонтекст.ОповещениеОЗавершении, ПодготовитьРезультат(Результат.Выполнено, "Хеш", Хеш, ВходящийКонтекст));
