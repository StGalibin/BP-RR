...
Процедура ОпределитьПоставщиковПоСписаннымПартиямНДС(СписанныеПартии, Период)
	...
	ПараметрыСчетовФактур = ПолучитьПараметрыСчетовФактурПоНДСПредъявленный(ТаблицаСчетовФактур, Период);
...
Функция ПолучитьПараметрыСчетовФактурПоНДСПредъявленный(ТаблицаСчетовФактур, Период) Экспорт
	...
	|	МИНИМУМ(НДСПредъявленный.Период) КАК Период
	|ПОМЕСТИТЬ ПервыеЗаписи
	|ИЗ
	|	ТаблицаСчетовФактур КАК ТаблицаСчетовФактур
	...
	|			И (НДСПредъявленный.Период < &Период)
	|ГДЕ
	|	НЕ НДСПредъявленный.СчетФактура ЕСТЬ NULL 
	|
	|СГРУППИРОВАТЬ ПО
	|	НДСПредъявленный.СчетФактура
	|
	...
	|	НДСПредъявленный.Поставщик
	|ИЗ
	|	ПервыеЗаписи КАК ПервыеЗаписи
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.НДСПредъявленный КАК НДСПредъявленный
	|		ПО ПервыеЗаписи.СчетФактура = НДСПредъявленный.СчетФактура
	|			И ПервыеЗаписи.Период = НДСПредъявленный.Период";
...
Процедура ПодготовитьТаблицуРублевыеСуммыПоОтгрузкеВУЕ(ТаблицаРублевыеСуммы, ТаблицаРеквизиты) Экспорт
	...
	Если Реквизиты.РасчетыВУсловныхЕдиницах
	   И Реквизиты.НДСНачисленПриОтгрузке Тогда
