...
Процедура ЗарегистрироватьОтложенныеРасчетыСКонтрагентамиПриВключении(Организации, Отказ) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ПериодыОтложенногоПроведения Цикл
		...
		ДатаВводаНачальныхОстатков = РегистрыСведений.ДатыВводаНачальныхОстатков.ДатаВводаНачальныхОстатков(СтрокаТаблицы.Организация);
		...
		Если КонецМесяца(ДатаВводаНачальныхОстатков) + 1 = СтрокаТаблицы.ДатаНачала Тогда
			ДатаНачала = '0001-01-01';
		Иначе
			ДатаНачала = СтрокаТаблицы.ДатаНачала;
		КонецЕсли;
...
Функция ВыполнитьОтложенныеРасчеты(ПараметрыРасчета) Экспорт
	...
	// Используется только в документа поступления / реализации.
...
//
// Параметры:
//	Форма - УправляемаяФорма - Форма, из которой вызвана актуализация.
...
//
// Параметры:
//	Форма - УправляемаяФорма - Форма, из которой вызываны отложенные расчеты.
...
//
// Параметры:
//	Форма - УправляемаяФорма - Форма, из которой вызывается актуализация.
...
//
// Параметры:
//	Форма - УправляемаяФорма - Форма, из которой вызывалась актуализация.
...
Процедура ПодготовитьДоговорыДляРасчетов(Параметры)
	...
	|	ЗНАЧЕНИЕ(Перечисление.СостоянияОтложенныхРасчетов.КВыполнению)
	|ИЗ
	|	ВТ_Проводки КАК ВТ_Проводки
	...
	|	ВТ_ДоговорыДляРасчета КАК ВТ_ДоговорыДляРасчета
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РасчетыСКонтрагентамиОтложенноеПроведение КАК РасчетыСКонтрагентамиОтложенноеПроведение
	|		ПО ВТ_ДоговорыДляРасчета.ДоговорКонтрагента = РасчетыСКонтрагентамиОтложенноеПроведение.ДоговорКонтрагента
	|			И (РасчетыСКонтрагентамиОтложенноеПроведение.Организация = &Организация)
	|			И (РасчетыСКонтрагентамиОтложенноеПроведение.СостояниеРасчетов = ЗНАЧЕНИЕ(Перечисление.СостоянияОтложенныхРасчетов.КИсключениюИзРасчетов))
...
Процедура ОчиститьПараметрыПоДокументу(Параметры)
	...
	// Для документов поступления и реализации запоминаем значения реквизита "Способ зачет авансов".
...
Процедура СдвинутьГраницуАктуальностиРегламентныхОперацийНазад(Параметры)
	...
	// Найдем первый месяц, который не был исключен из перепроведения, чтобы сбросить актуальность рег.операции в этом месяце.
	ДатаНачала = НачалоМесяца(Параметры.ДатаНачала);
	Пока ДатаНачала <= Параметры.ДатаОкончания Цикл
	...
	Если ДатаНачала > КонецМесяца(Параметры.ДатаОкончания) Тогда
		// Все месяцы расчета исключены из перепроведения документов, 
...
Функция ПодготовитьТаблицуВзаиморасчетовЗачетАвансов(Параметры, ТаблицаДокумента, ТаблицаДокументыАвансов, ТаблицаРеквизиты, Отказ) Экспорт
	...
	Если ТаблицыЗачетАвансов.ТаблицаДокумента.Количество() = 0 Тогда
		Возврат ТаблицаВзаиморасчетов;
	КонецЕсли;

	СчетаРасчетов 	= ВыгрузитьЗаполненныеЗначенияКолонки(ТаблицыЗачетАвансов.ТаблицаДокумента, "СчетРасчетов");
	СчетаАвансов 	= ВыгрузитьЗаполненныеЗначенияКолонки(ТаблицыЗачетАвансов.ТаблицаДокумента, "СчетАвансов");
...
Функция ПолучитьОстаткиАвансов(Параметры, СчетаАвансов)
	...
	// Приведем таблицу к виду, используемому в УчетВзаиморасчетов.ПустаяТаблицаОстаткиАвансов().
	
	// Добавление колонки СуммаВзаиморасчетовПоВсемСчетамУСН по всем счетам УСН.
	ОписаниеТиповЧисло = БухгалтерскийУчетКлиентСервер.ТипСумма();
	ОстаткиАвансов.Колонки.Добавить("СуммаВзаиморасчетовПоВсемСчетамУСН", ОписаниеТиповЧисло);
	ЗаписьУчетнойПолитики = УчетнаяПолитикаИзКеша(Параметры);
	Если ЗаписьУчетнойПолитики.ПрименяетсяУСН ИЛИ ЗаписьУчетнойПолитики.ПрименяетсяУСНПатент Тогда
		Для Каждого СтрокаТаблицы Из ОстаткиАвансов Цикл
			СтрокаТаблицы.СуммаВзаиморасчетовПоВсемСчетамУСН = СтрокаТаблицы.СуммаВзаиморасчетовЕНВД
				+ СтрокаТаблицы.СуммаВзаиморасчетовПатент
				+ СтрокаТаблицы.СуммаВзаиморасчетовКомитента
				+ СтрокаТаблицы.СуммаВзаиморасчетовТорговыйСбор;
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого СтрокаВзаиморасчетов Из ТаблицаВзаиморасчетов Цикл
		...
		Если (СтрокаВзаиморасчетов.ДокументРасчетов <> Параметры.Регистратор) Тогда
			...
			Если НайденнаяСтрока <> Неопределено Тогда
				...
				Иначе
					НомерПлатежноРасчетногоДокумента	= ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(
...
Функция ПодготовитьТаблицуВзаиморасчетовПогашениеЗадолженности(Параметры, РасшифровкаПлатежа, ТаблицаРеквизиты, Отказ, ВидДокумента = Неопределено) Экспорт
	...
	Если ТаблицыПогашениеЗадолженности.РасшифровкаПлатежа.Количество() = 0 Тогда
		Возврат ТаблицаВзаиморасчетов;
	КонецЕсли;

	СчетаРасчетов 	= ВыгрузитьЗаполненныеЗначенияКолонки(ТаблицыПогашениеЗадолженности.РасшифровкаПлатежа, "СчетРасчетов");
	СчетаАвансов	= ВыгрузитьЗаполненныеЗначенияКолонки(ТаблицыПогашениеЗадолженности.РасшифровкаПлатежа, "СчетАвансов");
...
Функция ПодготовитьТаблицуВзаиморасчетовБезналичнаяРозничнаяВыручка(Параметры, ТаблицаБезналичныхОплат, ТаблицаРеквизиты, Отказ) Экспорт
	...
	Если ТаблицыБезналичныхОплат.Реквизиты.Количество() = 0
		ИЛИ ТаблицыБезналичныхОплат.ТаблицаОплат.Количество() = 0 Тогда
		Возврат ТаблицаВзаиморасчетов;
	КонецЕсли;
	...
	СчетаРасчетов 	= ВыгрузитьЗаполненныеЗначенияКолонки(ТаблицаОплат, "СчетРасчетов");
	СчетаАвансов	= ВыгрузитьЗаполненныеЗначенияКолонки(ТаблицаОплат, "СчетАвансов");
...
Функция ПодготовитьТаблицуВзаиморасчетовСписаниеЗадолженности(Параметры, РасшифровкаПлатежа, ТаблицаРеквизиты, Отказ) Экспорт
	...
	Если ТаблицыСписаниеЗадолженности.РасшифровкаПлатежа.Количество() = 0 Тогда
		Возврат ТаблицаВзаиморасчетов;
	КонецЕсли;
	...
	СчетаРасчетов = ВыгрузитьЗаполненныеЗначенияКолонки(ТаблицыСписаниеЗадолженности.РасшифровкаПлатежа, "СчетРасчетов");
...
Функция РегистрыУСНФормируютсяПоДоговорам(Параметры)
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Графа4_УСН", МетаданныеДокумента) Тогда
		...
		Если ВидОперацииДДС = "ПрочееДДС"
			...
			ИЛИ ВидОперацииДДС = "ПокупкаВалюты" Тогда
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'Отложенные расчеты с контрагентами'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
#Область ПереключениеРежимамПроведения

Процедура ЗарегистрироватьОтложенныеРасчетыСКонтрагентамиПриВключенииПоОрганизации(Организация, ДатаНачала, ДатаОкончания, МенеджерВременныхТаблиц, Отказ)
	...
	// Будем записывать порциям по 1000 записей в наборе.
	НаборЗаписей.Отбор.Организация.Установить(Организация);
