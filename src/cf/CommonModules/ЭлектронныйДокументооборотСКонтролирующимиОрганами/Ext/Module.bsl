...
Процедура ОтметитьКакПрочтенное(Ссылка) Экспорт
	...
	Иначе
		ОтметитьСсылкуПоказаннойПользователю(Ссылка);
	КонецЕсли;
...
Функция Это1СБизнесСтарт() Экспорт
	...
	Если ВРЕГ(Метаданные.Имя) = ВРЕГ("БухгалтерияПредприятияБазовая1") Тогда
		Если Метаданные.Константы.Найти("ПредпринимательВСервисе") <> Неопределено Тогда
			Возврат Константы["ПредпринимательВСервисе"].Получить();
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтослатьПисьмоПартнеру(Заявление, Тариф) Экспорт
	...
	Попытка
		...
		|	ЗаявлениеАбонентаСпецоператораСвязиЭлектронныеДокументы.Ссылка.ИНН КАК ИНН,
		|	ЗаявлениеАбонентаСпецоператораСвязиЭлектронныеДокументы.Ссылка.КПП КАК КПП,
		|	ЗаявлениеАбонентаСпецоператораСвязиЭлектронныеДокументы.Ссылка.КраткоеНаименование КАК КраткоеНаименование,
		|	ЗаявлениеАбонентаСпецоператораСвязиЭлектронныеДокументы.Документ КАК Документ,
		|	ЗаявлениеАбонентаСпецоператораСвязиЭлектронныеДокументы.Ссылка.ТелефонМобильный,
		|	ЗаявлениеАбонентаСпецоператораСвязиЭлектронныеДокументы.Ссылка.ТелефонОсновной,
		|	ЗаявлениеАбонентаСпецоператораСвязиЭлектронныеДокументы.Ссылка.ТелефонМобильныйДляАвторизации
		|ИЗ
		...
		Пока Выборка.Следующий() Цикл
			...
			СведенияДляПисьма.Вставить("ИмяАбонента", 	Выборка.КраткоеНаименование);
			СведенияДляПисьма.Вставить("ИНН", 			Выборка.ИНН);
			СведенияДляПисьма.Вставить("КПП", 			Выборка.КПП);
			СведенияДляПисьма.Вставить("Тариф", 		Тариф);

			Телефон = "";
			Если ЗначениеЗаполнено(Выборка.ТелефонМобильныйДляАвторизации) Тогда
				Телефон = Выборка.ТелефонМобильныйДляАвторизации;
			ИначеЕсли ЗначениеЗаполнено(Выборка.ТелефонМобильный) Тогда
				Телефон = Выборка.ТелефонМобильный;
			ИначеЕсли ЗначениеЗаполнено(Выборка.ТелефонОсновной) Тогда
				Телефон = Выборка.ТелефонОсновной;
			КонецЕсли;
			...
			ДанныеФайла = ПрисоединенныеФайлы.ПолучитьДанныеФайла(Выборка.Файл, Неопределено, Истина);
...
// Возвращает параметры подключения к веб-сервису обмена с ФСС по электронным листкам нетрудоспособности.
//
// Параметры:
...
//  Неопределено                   - ошибка или недостаточно прав.
//
Функция НастройкиОбменаФССЭЛН(Организация) Экспорт
	...
	Возврат КонтекстЭДОСервер.СерверПриемаОтчетностиФССЭЛН(ИспользоватьТестовыйСервер);
...
Процедура ОтметитьДеревоПросмотренным(Новое) Экспорт
	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		Если ЗначениеЗаполнено(СтрокаДерева.Ссылка) Тогда
			ОтметитьСсылкуПоказаннойПользователю(СтрокаДерева.Ссылка);
		КонецЕсли;
		...
		Для Каждого ПодстрокаДерева Из ПодстрокиДерева Цикл
			Если ЗначениеЗаполнено(ПодстрокаДерева.Ссылка) Тогда
				ОтметитьСсылкуПоказаннойПользователю(ПодстрокаДерева.Ссылка);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
