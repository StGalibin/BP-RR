...
//  База - коллекция значений, по которой разносится сумма;
//  Колонка - Строка - Имя колонки с суммой в базе;
...
Процедура РазнестиСуммуПоБазе(Знач Сумма, Знач База, Колонка = "Сумма", Знач Точность = 2) Экспорт
	...
	// При нулевом новом итоге просто обнуляем колонку коллекции
	...
	// Определяем исходный итог по колонке и строку с максимальным значением по модулю
	...
	Иначе	
		// Изменяем размер в колонке пропорционально изменению итога
		Коэффициент = Сумма / Итог;
		Для Каждого Строка Из База Цикл
			Строка[Колонка] = Окр(Строка[Колонка] * Коэффициент, Точность, 1);
		КонецЦикла;	
	КонецЕсли;
	
	// Определяем получившийся итог по колонке
	Итог = 0;
	Для Каждого Строка Из База Цикл
		Итог = Итог + Строка[Колонка]
	КонецЦикла;	
...
// Получает сведения о сотруднике см. ЭлектронныйДокументооборотСКонтролирующимиОрганамиПереопределяемый.ПолучитьСведенияОСотруднике
//
...
//	ЭлектронныйДокументооборотСКонтролирующимиОрганамиПереопределяемый.ОпределитьНаличиеДанныхДляЗаявленияНаСертификат
//
...
// Подробнее см. РегламентированнаяОтчетностьПереопределяемый
//
...
Функция ОписаниеРегистраРассчитанныхПериодовСостояний() Экспорт 
	...
	Описание.Вставить("ОтборРасчитанныхДанныхРавно");
	Описание.Вставить("ОтборРасчитанныхДанныхНеРавно");
...
//
Процедура ДобавитьОтборЗаписейРегистраРассчитанныхПериодовСостоянийРавно(ОписаниеРегистраРассчитанныхПериодовСостояний, ЛевоеЗначение, ПравоеЗначение) Экспорт 
	Если ОписаниеРегистраРассчитанныхПериодовСостояний.ОтборРасчитанныхДанныхРавно = Неопределено Тогда
		ОписаниеРегистраРассчитанныхПериодовСостояний.ОтборРасчитанныхДанныхРавно = Новый Структура;	
	КонецЕсли;		
	
	ОписаниеРегистраРассчитанныхПериодовСостояний.ОтборРасчитанныхДанныхРавно.Вставить(ЛевоеЗначение, ПравоеЗначение);	
КонецПроцедуры	
...
//      ПравоеЗначение - значение отбора
//
Процедура ДобавитьОтборЗаписейРегистраРассчитанныхПериодовСостоянийНеРавно(ОписаниеРегистраРассчитанныхПериодовСостояний, ЛевоеЗначение, ПравоеЗначение) Экспорт 
	Если ОписаниеРегистраРассчитанныхПериодовСостояний.ОтборРасчитанныхДанныхНеРавно = Неопределено Тогда
		ОписаниеРегистраРассчитанныхПериодовСостояний.ОтборРасчитанныхДанныхНеРавно = Новый Структура;	
	КонецЕсли;		
	
	ОписаниеРегистраРассчитанныхПериодовСостояний.ОтборРасчитанныхДанныхНеРавно.Вставить(ЛевоеЗначение, ПравоеЗначение);	
КонецПроцедуры	
...
Процедура ОбновитьРегистрРассчитанныхПериодовСостояний(ОписаниеРегистраПервичныхДанных, ОписаниеРегистраВторичныхДанных, КлючиОбновляемыхДанных, РежимЗагрузки = Ложь) Экспорт
	...
	РасчитанныеДанныеВторичногоРегистра = НовыйРасчитанныеДанныеВторичногоРегистраСостяонияй(ОписаниеРегистраПервичныхДанных);
	...
	Пока ВыборкаРассчитанныхПериодов.Следующий() Цикл
		Если ЗначенияИзмеренийЗаписейСостоянийОтличны(ДанныеПредыдущегоСостояния, ВыборкаРассчитанныхПериодов, ИзмеренияВторичногоРегистра)
			И РасчитанныеДанныеВторичногоРегистра.Количество() > 0 Тогда
			
			ЗаписатьНаборыВторичногоРегистраСостояний(ОписаниеРегистраВторичныхДанных, РасчитанныеДанныеВторичногоРегистра, КлючиОбновленныхНаборов, РежимЗагрузки);
			
			ДанныеПредыдущегоСостояния = Неопределено;
			РасчитанныеДанныеВторичногоРегистра.Очистить();
		КонецЕсли;		
		...
		Если НЕ ПродолжатьДействиеТекущегоСостояния(ДанныеПредыдущегоСостояния, ВыборкаРассчитанныхПериодов, ИзмеренияРасчета, ОписаниеРегистраВторичныхДанных.Ресурсы) Тогда
			...
			ДанныеПредыдущегоСостояния = ДобавитьЗаписьВРассчитанныеПериодыСостояний(РасчитанныеДанныеВторичногоРегистра, ВыборкаРассчитанныхПериодов);
		КонецЕсли;			
	КонецЦикла;		
	
	Если РасчитанныеДанныеВторичногоРегистра.Количество() > 0 Тогда
		ЗаписатьНаборыВторичногоРегистраСостояний(ОписаниеРегистраВторичныхДанных, РасчитанныеДанныеВторичногоРегистра, КлючиОбновленныхНаборов, РежимЗагрузки);
	КонецЕсли;	
...
Процедура ЗаписатьНаборыВторичногоРегистраСостояний(ОписаниеРегистраВторичныхДанных, РасчитанныеДанные, КлючиОбновленныхНаборов, РежимЗагрузки)
	Если РасчитанныеДанные.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;	
	...
	Для Каждого Измрение Из ОписаниеРегистраВторичныхДанных.Измерения Цикл
		Набор.Отбор[Измрение].Установить(РасчитанныеДанные[0][Измрение]);
	КонецЦикла;
	
	ОтборПоГоду = ОписаниеРегистраВторичныхДанных.ОтборПоГоду;
	ГодТекущейЗаписи = Неопределено;
	
	Для Каждого ДанныеПериода Из РасчитанныеДанные Цикл
		Если ОтборПоГоду  
...
Функция НовыйРасчитанныеДанныеВторичногоРегистраСостяонияй(ОписаниеРегистраПервичныхДанных)
	РасчитанныеДанные = Новый ТаблицаЗначений;
	РасчитанныеДанные.Колонки.Добавить("ДатаНачала", Новый ОписаниеТипов("Дата"));
	РасчитанныеДанные.Колонки.Добавить("ДатаОкончания", Новый ОписаниеТипов("Дата"));
	РасчитанныеДанные.Колонки.Добавить("СостояниеДействует", Новый ОписаниеТипов("Булево"));
	
	Для Каждого ИзмерениеРасчета Из ОписаниеРегистраПервичныхДанных.Измерения Цикл
		РасчитанныеДанные.Колонки.Добавить(ИзмерениеРасчета);
	КонецЦикла;	
	
	Для Каждого Ресурс Из ОписаниеРегистраПервичныхДанных.Ресурсы Цикл
		РасчитанныеДанные.Колонки.Добавить(Ресурс);
		РасчитанныеДанные.Колонки.Добавить("ИспользованиеРесурса" + Ресурс);
	КонецЦикла;	
	
	Возврат РасчитанныеДанные;
КонецФункции	

Функция ДобавитьЗаписьВРассчитанныеПериодыСостояний(РасчитанныеДанныеВторичногоРегистра, ДанныеТекущегоСостояния)	
	НоваяСтрока = РасчитанныеДанныеВторичногоРегистра.Добавить();
...
Процедура ЭлектронноеВзаимодействиеПриОпределенииТекущегоКаталогаВременныхФайлов(ТекущийКаталог) Экспорт
...
Функция ЭлектронноеВзаимодействиеПриОпределенииСсылкиНаОбъект(ТипОбъекта, ИдОбъекта = "", ДополнительныеРеквизиты = Неопределено, ИДЭД = Неопределено) Экспорт
...
Процедура ЗаполнитьПоОснованиюСотрудником(ДокументОбъект, ДанныеЗаполнения, ПроверкаОформленностиНаРаботу = Ложь, ОтключитьПроверкуЗанятости = Ложь, ТолькоГоловнаяОрганизация = Ложь) Экспорт
	...
	КадровыеДанные =  КадровыйУчет.КадровыеДанныеСотрудников(Истина, ДанныеЗаполнения, "ГоловнаяОрганизация,Организация,ФизическоеЛицо,ДатаПриема,ОформленПоТрудовомуДоговору,ДатаЗавершенияРаботы");
	Если КадровыеДанные.Количество() > 0 Тогда
...
Процедура ПриСозданииНаСервереФормыСДинамическимСписком(Форма, НаименованиеРеквизитаСписок, ГруппаНастроек = "СписокНастройкиОтбора",
	...
	Если ОбщегоНазначения.ЭтоЖурналДокументов(МетаданныеОбъекта) Тогда
		Для Каждого РегистрируемыйДокумент Из МетаданныеОбъекта.РегистрируемыеДокументы Цикл
			Форма.ТипыОбъектовОповещения.Добавить(ТипЗнч(ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(РегистрируемыйДокумент.ПолноеИмя()).ПустаяСсылка()));
		КонецЦикла;
	КонецЕсли;
...
// Процедура обновляет динамический список на форме списка журнала, при этом обновляются массивы документов для критериев отбора
//
// Параметры:
...
Процедура ЗаполнитьСписокТабличныхЧастейИсключаемыхИзКраткогоСостава(Источник, МассивТабличныхЧастей) Экспорт
	ЗарплатаКадрыВнутренний.ЗаполнитьСписокТабличныхЧастейИсключаемыхИзКраткогоСостава(Источник, МассивТабличныхЧастей);
КонецПроцедуры
...
Процедура ИзменитьНаименованиеФизическихЛиц(ДанныеФИО)
	...
	Пока ДанныеФИО.Следующий() Цикл
		...
		НаименованиеФизЛица = Строка(СтруктураФИО.Фамилия) + " " + СтруктураФИО.Имя + " " + СтруктураФИО.Отчество;
...
Функция ЗапросВТПервичныеДанные(МенеджерВременныхТаблиц, ОписаниеРегистра, ЗначенияИзмерений)
	ИмзмеренияОтбора = Новый Массив;
	
	Для Каждого КолонкаФильтар Из ЗначенияИзмерений.Колонки Цикл
		Если ВРег(КолонкаФильтар.Имя) <> "ГОД" Тогда 
			ИмзмеренияОтбора.Добавить(КолонкаФильтар.Имя);
		КонецЕсли;	
	КонецЦикла;	
	...
	//ВТЗначенияИзмерений
	ЗаменитьШаблонНаПоляИсточника(ТекстЗапроса, "ЗначенияИзмерений.ИзмерениеОтбора_", "ИзмерениеОтбора_", ИмзмеренияОтбора);
	...
	ЗаменитьШаблонНаПоляИсточника(ТекстЗапроса, ШаблонСоединения, "ИзмерениеОтбора_", ИмзмеренияОтбора, РазделительСоединения);
...
Функция ЗапросРассчитанныеДаты(МенеджерВременныхТаблиц, ОписаниеРегистра)
	...
	ТекстЗапроса = 
		...
		|					ТОГДА ЛОЖЬ
		|				КОГДА СостоянияДействующиеНаДаты.ДатаНачалаСобытия > СостоянияДействующиеНаДатыВытесняющие.ДатаНачалаСобытия
		|						И СостоянияДействующиеНаДаты.Начало < СостоянияДействующиеНаДатыВытесняющие.Начало
...
Функция ЗаписьВторичныхДанныхСостоянийСоответствуетОтбору(Запись, ОписаниеРегистра)
	Если Не ЗначениеЗаполнено(ОписаниеРегистра.ОтборРасчитанныхДанныхРавно) 
		И Не ЗначениеЗаполнено(ОписаниеРегистра.ОтборРасчитанныхДанныхНеРавно)  Тогда
	...
	Если ОписаниеРегистра.ОтборРасчитанныхДанныхРавно <> Неопределено Тогда
		Для Каждого ЭлементОтбора Из ОписаниеРегистра.ОтборРасчитанныхДанныхРавно Цикл
			Если Запись[ЭлементОтбора.Ключ] = ЭлементОтбора.Значение Тогда
	...
	Если ОписаниеРегистра.ОтборРасчитанныхДанныхНеРавно <> Неопределено Тогда
		Для Каждого ЭлементОтбора Из ОписаниеРегистра.ОтборРасчитанныхДанныхНеРавно Цикл
			Если Запись[ЭлементОтбора.Ключ] <> ЭлементОтбора.Значение Тогда
...
Функция ПродолжатьДействиеТекущегоСостояния(ДанныеПредыдущегоСостояния, ДанныеТекущегоСостояния, ИзмеренияРасчета, Ресурсы)
	...
	Для Каждого ОписаниеРесурса Из Ресурсы Цикл
		Если ДанныеПредыдущегоСостояния[ОписаниеРесурса] <> ДанныеТекущегоСостояния[ОписаниеРесурса]
			ИЛИ ДанныеПредыдущегоСостояния["ИспользованиеРесурса" + ОписаниеРесурса] <> ДанныеТекущегоСостояния["ИспользованиеРесурса" + ОписаниеРесурса] Тогда
...
Процедура ЗаполнитьГодЗаписиРегистраСведений(ИмяРегистра, ИзмерениеОтбора, ИмяПоляПериод, ПараметрыОбновления = Неопределено) Экспорт
	...
	"ВЫБРАТЬ
	|	*,
	|	НАЧАЛОПЕРИОДА(РегистрСведений.#ИмяПоляПериод, ГОД) КАК ГодЗаполнение
	|ИЗ
	|	РегистрСведений.#ИмяРегистра КАК РегистрСведений
	|ГДЕ
	|	РегистрСведений.Год = ДАТАВРЕМЯ(1, 1, 1)
	|УПОРЯДОЧИТЬ ПО
	|	РегистрСведений.#ИзмерениеОтбора";
...
Функция ИмяВТПоУмолчанию(ПолноеИмяТаблицы)
...
Процедура ОсвободитьМенеджерВременныхТаблиц(МенеджерВременныхТаблиц, Знач ИсключаяВТ = Неопределено) Экспорт
	...
	Если ИсключаяВТ <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СократитьМассив(УдалитьВТ, ИсключаяВТ);
	КонецЕсли;
...
//		...
//	МенеджерВТ - менеджер временных таблиц,
...
Процедура ЗаполнитьИменаПолейВТ(ТекстЗапроса, МенеджерВТ, ИмяВТ, ПсевдонимТаблицы, ПоляИсключения) Экспорт
	...
	ИменаПолейВТ = ОбщегоНазначенияКлиентСервер.СократитьМассив(ИменаПолейВТ, ПоляИсключения);
...
Функция СостоянияСотрудниковДляРасчетаОценочныхОбязательствОтпусков(Сотрудники, Период) Экспорт
	...
	Возврат ЗарплатаКадрыВнутренний.СостоянияСотрудниковДляРасчетаОценочныхОбязательствОтпусков(Сотрудники, Период);
