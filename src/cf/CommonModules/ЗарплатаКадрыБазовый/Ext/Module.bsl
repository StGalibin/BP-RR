...
Функция ПолучитьСведенияОбОрганизации(Знач Организация, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено) Экспорт
	...
	Для Каждого Показатель Из СписокПоказателей Цикл
		...
		ИначеЕсли ИмяПоказателя = "КодТарифаПлательщикаПоУмолчаниюДляРСВ" Тогда
			...
			Если ОбщийНалоговыйРежим <> Неопределено Тогда 
				Значение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбщийНалоговыйРежим, "КодФНС");
			Иначе
				Значение = "01";
			КонецЕсли	
...
Процедура ЗаполнитьПоказателиРегламентированногоОтчета(ПоказателиОтчета, ИДОтчета, ИДРедакцииОтчета) Экспорт
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчет4ФСС" Тогда
		...
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2017Кв1" Тогда
			УчетСтраховыхВзносов.ОписаниеПоказателей_4ФСС_2017Кв1(ПоказателиОтчета);
...
Процедура ЭлектронноеВзаимодействиеПриОпределенииСоответствияСправочников(СоответствиеСправочников) Экспорт
	...
	СоответствиеСправочников.Вставить("Банки",       Метаданные.Справочники.КлассификаторБанковРФ.Имя);
...
Процедура ЗаполнитьСписокТабличныхЧастейИсключаемыхИзКраткогоСостава(Источник, МассивТабличныхЧастей) Экспорт
	...
	МассивТабличныхЧастей.Добавить("ФизическиеЛица");
...
//		Ведомости         - оплачиваемые ведомости, 
//                              (ДокументСсылка.ВедомостьНаВыплатуЗарплаты),
//                              (ДокументСсылка.ВедомостьНаВыплатуЗарплатыВКассу),
//								(ДокументСсылка.ВедомостьНаВыплатуЗарплатыВБанк).
//		ФизическиеЛица    - физические лица (СправочникСсылка.ФизическиеЛица), получившие зарплату по ведомостям.
...
Процедура ЗарегистрироватьОплатуВедомостей(ПлатежныйДокумент, Организация, Ведомости, ФизическиеЛица, ДатаОперации, Отказ) Экспорт
	...
	// Для ведомостей, полученных из внешнего учета, регистрируем только факт выдачи денег работнику
	Если ПолученныеВедомости.Количество() > 0 Тогда
		ВзаиморасчетыССотрудниками.ЗарегистрироватьОплатуВедомостей(ПлатежныйДокумент, Организация, ПолученныеВедомости, ФизическиеЛица, Неопределено, Отказ);
	КонецЕсли;	
	
	Если ВнутренниеВедомости.Количество() = 0 Тогда
		Возврат
	КонецЕсли;	
		
	// Ведомости на выплату начисленной зарплаты регистрируем во всех необходимых учетах
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("Организация",	Организация);
	Запрос.УстановитьПараметр("Ведомости",		ВнутренниеВедомости);
	
	// Данные ведомостей 	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЗарплатаКВыплате.Регистратор КАК Ведомость,
	|	НАЧАЛОПЕРИОДА(ЗарплатаКВыплате.Регистратор.ПериодРегистрации, МЕСЯЦ) КАК ПериодРегистрации,
	|	ВЫБОР
	|		КОГДА ЗарплатаКВыплате.Регистратор.СпособВыплаты.ХарактерВыплаты = ЗНАЧЕНИЕ(Перечисление.ХарактерВыплатыЗарплаты.Зарплата)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОкончательныйРасчет,
	...
	|	НАЧАЛОПЕРИОДА(ЗарплатаКВыплате.Регистратор.ПериодРегистрации, МЕСЯЦ),
	|	ВЫБОР
	|		КОГДА ЗарплатаКВыплате.Регистратор.СпособВыплаты.ХарактерВыплаты = ЗНАЧЕНИЕ(Перечисление.ХарактерВыплатыЗарплаты.Зарплата)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|ИЗ
	|	ВТДанныеВедомостей КАК Ведомости";

	ОплатаВедомостей = Запрос.Выполнить();
	Если ОплатаВедомостей.Пустой() Тогда
		Возврат
	КонецЕсли;	
	ВзаиморасчетыССотрудниками.ЗарегистрироватьОплатуВедомостейДокументом(ПлатежныйДокумент, Отказ, ОплатаВедомостей.Выгрузить());
	
	// Регистрация удержанного НДФЛ
	Если НЕ Отказ Тогда
		
		// Итоговый характеры выплаты
		Запрос.Текст =
		"ВЫБРАТЬ
		|	МАКСИМУМ(Ведомости.ОкончательныйРасчет) КАК ОкончательныйРасчет
		|ИЗ
		|	ВТДанныеВедомостей КАК Ведомости";
		ОкончательныйРасчет = Запрос.Выполнить().Выгрузить()[0].ОкончательныйРасчет;
		
		// Итоговый период регистрации
		// Это максимальный период регистрации из ведомостей под расчет,
		// а если ведомостей под расчет нет, то просто максимальный из имеющихся
		Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	МАКСИМУМ(Ведомости.ПериодРегистрации) КАК ПериодРегистрации
		|ИЗ
		|	ВТДанныеВедомостей КАК Ведомости
		|
		|ГДЕ
		|	&УсловиеНаОкончательныйРасчет";
		Если ОкончательныйРасчет Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеНаОкончательныйРасчет", "Ведомости.ОкончательныйРасчет = ИСТИНА") 
		Иначе
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеНаОкончательныйРасчет", "ИСТИНА") 
		КонецЕсли;	
		ПериодРегистрации = Запрос.Выполнить().Выгрузить()[0].ПериодРегистрации;
		...
		|	Ведомости.СтатьяФинансирования КАК СтатьяФинансирования,
		|	Ведомости.СтатьяРасходов КАК СтатьяРасходов,
		|	СУММА(Ведомости.СуммаВыплаты) КАК СуммаВыплаты
		|ПОМЕСТИТЬ ВТВыплаты
		|ИЗ
		|	ВТДанныеВедомостей КАК Ведомости
		|
		...
		|	Ведомости.СтатьяФинансирования,
		|	Ведомости.СтатьяРасходов";
		Запрос.Выполнить();
		
		// дополняем выплаты их состоянием
		ОписательВТ = 
			ВзаиморасчетыССотрудниками.ОписательВременныхТаблицДляСоздатьВТСостояниеВыплат(
				Запрос.МенеджерВременныхТаблиц, "ВТВыплаты");
...
Функция СостоянияСотрудниковДляРасчетаОценочныхОбязательствОтпусков(Сотрудники, Период) Экспорт
	...
	КадровыеДанные =  КадровыйУчет.КадровыеДанныеСотрудников(Истина, Сотрудники, "ДатаУвольнения");
	КадровыеДанные.Колонки.Добавить("Уволен", Новый ОписаниеТипов("Булево"));
	Для каждого СтрокаТЗ Из КадровыеДанные Цикл
		СтрокаТЗ.Уволен = (СтрокаТЗ.ДатаУвольнения >= Период);
	КонецЦикла;
