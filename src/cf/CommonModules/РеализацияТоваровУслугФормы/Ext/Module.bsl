...
Процедура ОтражениеДоходовПриИзменении(Форма) Экспорт
...
Процедура ЗаполнитьСписокВыбораОтражениеДоходов(Форма) Экспорт
	
	Объект   = Форма.Объект;
	Элементы = Форма.Элементы;
	
	Если Объект.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ПродажаКомиссия
		...
		ФункциональныеОпцииФормы = Новый Структура(
			"РаздельныйУчетТорговыйСборПриУСН, ПрименяетсяУСНПатент, ПрименяетсяТолькоУСНПатент",
			Ложь, Ложь, Ложь);
			
		ЗаполнитьЗначенияСвойств(ФункциональныеОпцииФормы, Форма);
		...
		СтрокаОсновнойСистемы = СтрШаблон(НСтр("ru = '%1%2'"),
			?(ПрименяетсяУСН, НСтр("ru = 'УСН'"), НСтр("ru = 'Доход ИП'")),
			?(ПлательщикЕНВД, НСтр("ru = ' или ЕНВД'"), ""));
			
		Если НЕ ФункциональныеОпцииФормы.ПрименяетсяТолькоУСНПатент Тогда
			СписокВыбора.Добавить("Основная", СтрокаОсновнойСистемы);
		КонецЕсли;
		
		Если ФункциональныеОпцииФормы.РаздельныйУчетТорговыйСборПриУСН Тогда
			СписокВыбора.Добавить("ТорговыйСбор", НСтр("ru = 'Деятельность на торговом сборе'"));
		КонецЕсли;
		
		Если ФункциональныеОпцииФормы.ПрименяетсяУСНПатент Тогда
		...
		Если ЗначениеЗаполнено(Форма.ОтражениеДоходов)
			И СписокВыбора.НайтиПоЗначению(Форма.ОтражениеДоходов) = Неопределено
			И СписокВыбора.Количество() > 0 Тогда
			
			Форма.ОтражениеДоходов = СписокВыбора[0].Значение;
			ОтражениеДоходовПриИзменении(Форма);
...
Процедура УстановитьОтражениеДоходов(Форма) Экспорт
	...
	Объект   = Форма.Объект;
	Элементы = Форма.Элементы;
	ИспользоватьТорговыйСбор = Объект.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ПродажаКомиссия
		Или Объект.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.Товары
		Или Объект.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ОтгрузкаБезПереходаПраваСобственности;
	
	СписокВыбора = Элементы.ОтражениеДоходов.СписокВыбора;
	
	Если ПустаяСтрока(Форма.ОтражениеДоходов) И СписокВыбора.Количество() > 0 Тогда
		Если Объект.ДеятельностьНаПатенте Тогда
			ОтражениеДоходов = Объект.Патент;
		ИначеЕсли Объект.ДеятельностьНаТорговомСборе И ИспользоватьТорговыйСбор Тогда
			ОтражениеДоходов = "ТорговыйСбор";
		Иначе
			ОтражениеДоходов = "Основная";
		КонецЕсли;
		
		Если СписокВыбора.НайтиПоЗначению(ОтражениеДоходов) = Неопределено Тогда
			Форма.ОтражениеДоходов = СписокВыбора[0].Значение;
		Иначе
			Форма.ОтражениеДоходов = ОтражениеДоходов;
		КонецЕсли;
	КонецЕсли;
...
Процедура СохранитьВидЭДПоУмолчанию(ВидОперации, ВидЭД) Экспорт
	
	ВидыЭДПоУмолчанию = БухгалтерскийУчетПереопределяемый.ПолучитьЗначениеПоУмолчанию("ВидЭДРеализации");
	ВидыЭДПоУмолчанию.Вставить(ВидОперации, ВидЭД);
	ОбщегоНазначенияБПВызовСервера.УстановитьЗначениеПоУмолчанию("ВидЭДРеализации", ВидыЭДПоУмолчанию);
...
Функция СписокАдресовДоставки(Контрагент, Грузополучатель, Дата)
	...
	ЮрФизЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КонтрагентДоставки, "ЮридическоеФизическоеЛицо");
	...
	Если ЮрФизЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
		...
		Если УправлениеКонтактнойИнформациейБП.АдресаСовпадают(КонтрагентДоставки,
				Справочники.ВидыКонтактнойИнформации.ЮрАдресКонтрагента,
				Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента) Тогда
				
			ВидФактАдреса = Справочники.ВидыКонтактнойИнформации.ЮрАдресКонтрагента;
		Иначе
			ВидФактАдреса = Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента;
		КонецЕсли;
		
	Иначе
		ВидФактАдреса = Справочники.ВидыКонтактнойИнформации.АдресПоПропискеФизическиеЛица;
	КонецЕсли;
...
Процедура УстановитьВидЭД(Форма) Экспорт
	
	Объект = Форма.Объект;
	Элементы = Форма.Элементы;
	
	ДоступныеЗначения = Документы.РеализацияТоваровУслуг.ПолучитьДоступныеВидыЭдПоВидуОперации(
		Объект.ВидОперации, Объект.Организация, Объект.Контрагент, Объект.ДоговорКонтрагента);
	...
	Если ДоступныеЗначения.Количество() = 0 Тогда
		...
		Элементы.ВидЭД.Видимость = Ложь;
		
	Иначе
		
		Элементы.ВидЭД.Видимость = Истина;
		...
		Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
			Если ДоступныеЗначения.НайтиПоЗначению(Объект.ВидЭлектронногоДокумента) = Неопределено Тогда
				Объект.ВидЭлектронногоДокумента = ДоступныеЗначения[0].Значение;
			КонецЕсли;
		КонецЕсли
