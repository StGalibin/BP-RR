...
Процедура ЗаполнитьДанныеПолученныеИзСервисаОбменаЭЛН(Документ, РезультатВыполненияОбмена)
	...
	Если ДанныеИзСервиса.LN_RESULT <> Неопределено Тогда
		...
		Если Значения.MSE_RESULT <> Неопределено Тогда
			Документ.НовыйСтатусНетрудоспособного = Значения.MSE_RESULT;
		КонецЕсли;
		Если Значения.OTHER_STATE_DT <> Неопределено Тогда
			Документ.ДатаНовыйСтатусНетрудоспособного = XMLЗначениеСПроверкойТипа(Значения.OTHER_STATE_DT, Тип("Дата"));
		КонецЕсли;
		Если Значения.RETURN_DATE_LPU <> Неопределено Тогда
			Документ.ПриступитьКРаботеС = XMLЗначениеСПроверкойТипа(Значения.RETURN_DATE_LPU, Тип("Дата"));
		КонецЕсли;
		Если Значения.NEXT_LN_CODE <> Неопределено Тогда
			Документ.НовыйСтатусНетрудоспособного = Значения.NEXT_LN_CODE;
		КонецЕсли;
	КонецЕсли;
	...
	Если ДанныеИзСервиса.HOSPITAL_BREACH <> Неопределено Тогда
		...
		Если Значения.HOSPITAL_BREACH_CODE <> Неопределено Тогда
			Документ.НовыйСтатусНетрудоспособного = Значения.HOSPITAL_BREACH_CODE;
		КонецЕсли;
		Если Значения.HOSPITAL_BREACH_DT <> Неопределено Тогда
			Документ.ДатаНарушенияРежима = XMLЗначениеСПроверкойТипа(Значения.HOSPITAL_BREACH_DT, Тип("Дата"));
		КонецЕсли;
	КонецЕсли;
