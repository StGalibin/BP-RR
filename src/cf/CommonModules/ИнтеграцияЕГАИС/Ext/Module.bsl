...
//
Функция ПреобразоватьВXDTOАктПостановкиНаБаланс(ДокументСсылка, ТекстОшибкиГлобальный)
...
// Функция возвращает объект XDTO, соответствующий акту постановки на баланс.
//
Функция ПреобразоватьВXDTOАктПостановкиНаБаланс_v2(ДокументСсылка, ТекстОшибкиГлобальный)
...
//
Функция ПреобразоватьВXDTOАктПостановкиНаБалансВТорговомЗале(ДокументСсылка, ТекстОшибкиГлобальный)
...
//
Функция ПреобразоватьВXDTOАктСписания(ДокументСсылка, ТекстОшибкиГлобальный)
...
// Функция возвращает объект XDTO, соответствующий акту списания.
//
Функция ПреобразоватьВXDTOАктСписания_v2(ДокументСсылка, ТекстОшибкиГлобальный)
...
//
Функция ПреобразоватьВXDTOАктСписанияИзТорговогоЗала(ДокументСсылка, ТекстОшибкиГлобальный)
...
//
Функция ПреобразоватьВXDTOПередачуВТорговыйЗал(ДокументСсылка, ТекстОшибкиГлобальный)
...
//
Функция ПреобразоватьВXDTOВозвратИзТорговогоЗала(ДокументСсылка, ТекстОшибкиГлобальный)
	...
	Для Каждого СтруктураДокумента Из МассивДокументов Цикл
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ReplyFormA = Неопределено
			...
			Если НЕ ОбъектXDTO.Document.ReplyFormA = Неопределено Тогда
				ЗагрузитьСправку(
					...
					Перечисления.ВидыДокументовЕГАИС.СправкаА,
					РезультатОбработкиДокумента,
					ДополнительныеПараметры);
			Иначе
				ЗагрузитьСправку(
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ReplyFormB = Неопределено
			...
			Если НЕ ОбъектXDTO.Document.ReplyFormB = Неопределено Тогда
				ЗагрузитьСправку(
					...
					Перечисления.ВидыДокументовЕГАИС.СправкаБ,
					РезультатОбработкиДокумента,
					ДополнительныеПараметры);
			Иначе
				ЗагрузитьСправку(
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ReplyRests = Неопределено
			...
			Если НЕ ОбъектXDTO.Document.ReplyRests_v2 = Неопределено Тогда
				ЗагрузитьОстатки(ОбъектXDTO.Document.ReplyRests_v2, РезультатОбработкиДокумента, СтруктураДокумента, ДополнительныеПараметры);
			Иначе
				ЗагрузитьОстатки(ОбъектXDTO.Document.ReplyRests, РезультатОбработкиДокумента, СтруктураДокумента, ДополнительныеПараметры);
			КонецЕсли;
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ReplyRestsShop_v2 = Неопределено Тогда
			...
			ЗагрузитьОстаткиВТорговомЗале(ОбъектXDTO.Document.ReplyRestsShop_v2, РезультатОбработкиДокумента, СтруктураДокумента, ДополнительныеПараметры);
...
// Добавляет новые справки Б в общую таблицу загрузки.
//
Процедура ДобавитьСправкиВТаблицуЗагрузки(КвитанцияXDTO, СправкиБ, СтруктураДокумента, РезультатОбработкиДокумента, ДополнительныеПараметры)
...
Процедура ЗарегистрироватьСправкиБПоТТН(СправкиБ, Результат)
	...
	|			И (ТТНВходящаяЕГАИСТовары.Ссылка.ИдентификаторЕГАИС = СправкиБ.ИдентификаторТТН
	...
	|			И (ТТНИсходящаяЕГАИСТовары.Ссылка.ИдентификаторЕГАИС = СправкиБ.ИдентификаторТТН
...
Процедура ЗарегистрироватьСправкиПоАктуПостановкиНаБаланс(КвитанцияXDTO, РезультатОбработкиДокумента, ДополнительныеПараметры)
	...
	Для Каждого СтрокаКвитанцииXDTO Из КвитанцияXDTO.Content.Position Цикл
		...
		НоваяСправкаА = СоздатьСправку(ДанныеСправкиА, Перечисления.ВидыДокументовЕГАИС.СправкаА, ДополнительныеПараметры, РезультатОбработкиДокумента.ОписаниеОшибки);
		...
		Для Каждого СтрокаСправокБ Из СписокСправокБXDTO Цикл
			...
			ДанныеСправкиБ.ДокументОснование    = РезультатОбработкиДокумента.ДокументОснование;
			...
			НоваяСправкаБ = СоздатьСправку(ДанныеСправкиБ, Перечисления.ВидыДокументовЕГАИС.СправкаБ, ДополнительныеПараметры, РезультатОбработкиДокумента.ОписаниеОшибки);
...
//
Процедура ЗагрузитьОстатки(ОтветОстаткиXDTO, РезультатОбработкиДокумента, СтруктураДокумента, ДополнительныеПараметры)
...
// Загружает остатки ЕГАИС в базу данных.
//
Процедура ЗагрузитьОстаткиВТорговомЗале(ОтветОстаткиXDTO, РезультатОбработкиДокумента, СтруктураДокумента, ДополнительныеПараметры)
...
Процедура ЗагрузитьСправку(СправкаXDTO, ВидСправки, РезультатОбработкиДокумента, ДополнительныеПараметры)
	...
	Если ПустаяСтрока(ТекстОшибки) Тогда
		...
		ИначеЕсли ВидСправки = Перечисления.ВидыДокументовЕГАИС.СправкаБ Тогда
...
Функция ДокументСоответствуетТипуЕГАИС(ДокументСсылка, ТипЕГАИС)
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ВозвратИзТорговогоЗалаЕГАИС") Тогда
...
Функция ДанныеИсходящегоЗапроса(Идентификатор, ТипЕГАИС)
	...
	Иначе
		...
		ИначеЕсли Результат.ВидДокумента = Перечисления.ВидыДокументовЕГАИС.АктПостановкиНаБаланс
			ИЛИ Результат.ВидДокумента = Перечисления.ВидыДокументовЕГАИС.АктПостановкиНаБалансВТорговомЗале Тогда
			ИмяТаблицы = "АктПостановкиНаБалансЕГАИС";
			
		ИначеЕсли Результат.ВидДокумента = Перечисления.ВидыДокументовЕГАИС.АктСписания
			ИЛИ Результат.ВидДокумента = Перечисления.ВидыДокументовЕГАИС.АктСписанияИзТорговогоЗала Тогда
			ИмяТаблицы = "АктСписанияЕГАИС";
			
		ИначеЕсли Результат.ВидДокумента = Перечисления.ВидыДокументовЕГАИС.ВозвратИзТорговогоЗала Тогда
			ИмяТаблицы = "ВозвратИзТорговогоЗалаЕГАИС";
			
		ИначеЕсли Результат.ВидДокумента = Перечисления.ВидыДокументовЕГАИС.ПередачаВТорговыйЗал Тогда
			ИмяТаблицы = "ПередачаВТорговыйЗалЕГАИС";
		Иначе
			Возврат Результат;
		КонецЕсли;
