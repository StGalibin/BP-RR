...
Функция НовыйКонтекстРасчета(Организация, КонецПериода, ДатаПодписи = '0001-01-01', КППОбособленногоПодразделения = "", Периодичность = Неопределено) Экспорт
	...
	КонтекстРасчета.Вставить("НачалоПериода",                 НалоговыйУчет.НачалоНалоговогоПериода(КонецПериода));
...
Процедура ПолучитьКапитальныеВложения(ПроцессорПолученияДанных)
	...
	|	СУММА(Модернизация.СуммаНУ) / СУММА(Модернизация.СуммаМодернизацииНУ) <= &ВерхнийЛимитПроцента
	|
	|УПОРЯДОЧИТЬ ПО
	|	СуммаНУ УБЫВ";
