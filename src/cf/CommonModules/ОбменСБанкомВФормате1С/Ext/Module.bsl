...
//
Функция ПрочитатьФайл(ИмяФайла, Кодировка, ОдиночныйРазделительСтрок = Ложь) Экспорт
...
Функция НовыйДанныеИзБанка(ВерсияФормата = "1.02") Экспорт
...
Функция ЭлементыРеквизитовУчастника()
	...
	ЭлементыРеквизитовУчастника.Добавить("БИК");
	ЭлементыРеквизитовУчастника.Добавить("КорСчет");
	ЭлементыРеквизитовУчастника.Добавить("Банк");       // ПлательщикБанк1, ПолучательБанк1
	ЭлементыРеквизитовУчастника.Добавить("ГородБанка"); // ПлательщикБанк2, ПолучательБанк2
	ЭлементыРеквизитовУчастника.Добавить("РасчСчет");
...
Процедура ПеренестиВДанныеБанка(ДанныеИзБанка, СтекСекций, ОписаниеФайлаОбмена)
	...
	ИначеЕсли Секция.Описание.Имя = "Документ" Тогда
		...
		// Исправим ошибку, которую допускает банк ВТБ24 и некоторые другие:
		// теряют информацию о полном номере банковского счета плательщика,
		// если расчеты выполнялись через кор.счета филиальной сети банка плательщика.
		Если БанковскиеПравила.СтрокаСоответствуетФорматуБанковскогоСчета(СтрокаДанныхИзБанка.ПлательщикСчет) 
			И СтрокаДанныхИзБанка.ПлательщикРеквизиты.Свойство("Наименование") Тогда
			...
			Если БанковскиеПравила.ЭтоСчетРасчетовФилиалаБанкаСКлиентамиБанка(БалансовыйСчет) Тогда
				...
				Если ЗначениеЗаполнено(НомерСчета) Тогда
					...
					СтрокаДанныхИзБанка.ПлательщикРеквизиты.Удалить("БИК"); // Часто в файле содержится "головной" БИК банка, а не того банка, где открыт счет с этим номером
				КонецЕсли;
