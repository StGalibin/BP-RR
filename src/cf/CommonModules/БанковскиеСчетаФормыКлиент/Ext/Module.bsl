...
Функция ПодсказкаВводаПоляНомерСчета(НомерСчета, БИК) Экспорт
	...
	СклоняемыйТекст = СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
		ДлинаСчетаПоЗакону - КоличествоЦифрВСчете, Нстр("ru = 'цифру,цифры,цифр'"));
...
//               - СправочникСсылка.КлассификаторБанковРФ   -  ссылка на банк, для которого требуется получить реквизиты.
//
// Возвращаемое значение:
...
Функция ПолучитьДанныеБанка(СсылкаНаБанк) Экспорт
	...
	ИначеЕсли ТипЗнч(СсылкаНаБанк) = Тип("СправочникСсылка.КлассификаторБанковРФ") Тогда
