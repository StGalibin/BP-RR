...
Процедура ЗаполнитьРегламентированныйОтчетОперацииСДенежнымиСредствамиНФО2015Кв3(ПараметрыОтчета, Контейнер) Экспорт
	...
	ОперацииНФО = ПодготовитьОперацииСДенежнымиСредствамиНФО(ПараметрыОтчета);
...
Функция ПодготовитьОперацииСДенежнымиСредствамиНФО(ПараметрыОтчета) Экспорт
	...
	Для каждого СтрокаБанк Из ДанныеОтчета.Банки.Строки Цикл
		...
		Для каждого СтрокаВалюта Из СтрокаБанк.Строки Цикл
			...
			СтрокаСтраница11.Данные.ОстаткиНачПериода = Окр(СтрокаВалюта.НачОстаток / 1000, 3);
			СтрокаСтраница11.Данные.ОборотыСписание   = Окр(СтрокаВалюта.Расход / 1000, 3);
			СтрокаСтраница11.Данные.ОборотыЗачисление = Окр(СтрокаВалюта.Приход / 1000, 3);
			СтрокаСтраница11.Данные.ОстаткиКонПериода = Окр(СтрокаВалюта.КонОстаток / 1000, 3);
			...
			Для каждого СтрокаТаблицы Из ТаблицаДанные Цикл
				...
				ЗаполнитьСтрокуТаблицыОперации(НоваяСтрока, СтрокаТаблицы, "111");
			КонецЦикла;
			...
			Для каждого СтрокаТаблицы Из ТаблицаДанные Цикл
				...
				ЗаполнитьСтрокуТаблицыОперации(НоваяСтрока, СтрокаТаблицы, "112");
			КонецЦикла;
			...
			Для каждого СтрокаТаблицы Из ТаблицаДанные Цикл
				...
				ЗаполнитьСтрокуТаблицыОперации(НоваяСтрока, СтрокаТаблицы, "113");
			КонецЦикла;
			...
			Для каждого СтрокаТаблицы Из ТаблицаДанные Цикл
				...
				ЗаполнитьСтрокуТаблицыОперации(НоваяСтрока, СтрокаТаблицы, "114");
			КонецЦикла;
			...
			Для каждого СтрокаТаблицы Из ТаблицаДанные Цикл
				...
				ЗаполнитьСтрокуТаблицыОперации(НоваяСтрока, СтрокаТаблицы, "115");
			КонецЦикла;
			...
			Для каждого СтрокаТаблицы Из ТаблицаДанные Цикл
				...
				ЗаполнитьСтрокуТаблицыОперации(НоваяСтрока, СтрокаТаблицы, "116");
			КонецЦикла;
		...
		Для каждого СтрокаТаблицы Из ТаблицаДанные Цикл
			...
			ЗаполнитьСтрокуНерезидентыБанк(НоваяСтрока, СтрокаТаблицы, "121");
		КонецЦикла;
		...
		Для каждого СтрокаТаблицы Из ТаблицаДанные Цикл
			...
			ЗаполнитьСтрокуНерезидентыБанк(НоваяСтрока, СтрокаТаблицы, "122");
		КонецЦикла;
	...
	СтрокаСтраница21.Данные.ОстаткиНачПериода = Окр(ДанныеОтчета.Касса.Итог("НачОстаток") / 1000, 3);
	СтрокаСтраница21.Данные.ОборотыСписание   = Окр(ДанныеОтчета.Касса.Итог("Расход") / 1000, 3);
	СтрокаСтраница21.Данные.ОборотыЗачисление = Окр(ДанныеОтчета.Касса.Итог("Приход") / 1000, 3);
	СтрокаСтраница21.Данные.ОстаткиКонПериода = Окр(ДанныеОтчета.Касса.Итог("КонОстаток") / 1000, 3);
	...
	Для каждого СтрокаТаблицы Из ТаблицаДанные Цикл
		...
		ЗаполнитьСтрокуТаблицыОперации(НоваяСтрока, СтрокаТаблицы, "211");
	КонецЦикла;
	...
	Для каждого СтрокаТаблицы Из ТаблицаДанные Цикл
		...
		ЗаполнитьСтрокуТаблицыОперации(НоваяСтрока, СтрокаТаблицы, "212");
	КонецЦикла;
	...
	Для каждого СтрокаТаблицы Из ТаблицаДанные Цикл
		...
		ЗаполнитьСтрокуТаблицыОперации(НоваяСтрока, СтрокаТаблицы, "213");
	КонецЦикла;
	...
	Для каждого СтрокаТаблицы Из ТаблицаДанные Цикл
		...
		ЗаполнитьСтрокуТаблицыОперации(НоваяСтрока, СтрокаТаблицы, "214");
	КонецЦикла;
	...
	Для каждого СтрокаТаблицы Из ТаблицаДанные Цикл
		...
		ЗаполнитьСтрокуТаблицыОперации(НоваяСтрока, СтрокаТаблицы, "215");
	КонецЦикла;
	...
	Для каждого СтрокаТаблицы Из ДанныеОтчета.НерезидентыКасса Цикл
		...
		ЗаполнитьСтрокуНерезидентыКасса(НоваяСтрока, СтрокаТаблицы, "220");
	КонецЦикла;
...
Процедура ЗаполнитьСтрокуТаблицыОперации(ЗаполняемаяСтрока, СтрокаДанные, КодРаздела)

	ЗаполняемаяСтрока.Данные["П" + КодРаздела + "0001"] = СтрокаДанные.КодОперации;
	ЗаполняемаяСтрока.Данные["П" + КодРаздела + "0002"] = Окр(СтрокаДанные.Расход / 1000, 3);
	ЗаполняемаяСтрока.Данные["П" + КодРаздела + "0003"] = Окр(СтрокаДанные.Приход / 1000, 3);
	
КонецПроцедуры

Процедура ЗаполнитьСтрокуНерезидентыБанк(ЗаполняемаяСтрока, СтрокаДанные, КодРаздела)
	...
	ЗаполняемаяСтрока.Данные["П" + КодРаздела + "0004"] = Окр(СтрокаДанные.Расход / 1000, 3);
	ЗаполняемаяСтрока.Данные["П" + КодРаздела + "0005"] = Окр(СтрокаДанные.Приход / 1000, 3);

КонецПроцедуры

Процедура ЗаполнитьСтрокуНерезидентыКасса(ЗаполняемаяСтрока, СтрокаДанные, КодРаздела)

	ЗаполняемаяСтрока.Данные["П" + КодРаздела + "0001"] = СтрокаДанные.КодСтраны;
	ЗаполняемаяСтрока.Данные["П" + КодРаздела + "0002"] = СтрокаДанные.КодОперации;
	ЗаполняемаяСтрока.Данные["П" + КодРаздела + "0003"] = Окр(СтрокаДанные.Расход / 1000, 3);
	ЗаполняемаяСтрока.Данные["П" + КодРаздела + "0004"] = Окр(СтрокаДанные.Приход / 1000, 3);
