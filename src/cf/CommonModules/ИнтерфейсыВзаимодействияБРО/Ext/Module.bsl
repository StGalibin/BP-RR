...
Процедура ЗаполнитьСсылкиНаРеглОтчеты(ПараметрыРеглОтчетов) Экспорт
	...
	|	ПараметрыРеглОтчетов.КорректирующийОтчет,
	...
	|	ЕСТЬNULL(РегламентированныйОтчет.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК Дата,
	|	ПараметрыРеглОтчетов.ИсточникОтчета,
	|	ПараметрыРеглОтчетов.Организация,
	|	ПараметрыРеглОтчетов.ДатаОкончания,
	|	ПараметрыРеглОтчетов.КорректирующийОтчет,
	...
	|			И (ВЫБОР
	|				КОГДА ПараметрыРеглОтчетов.КорректирующийОтчет
	|					ТОГДА РегламентированныйОтчет.Вид > 0
	|				ИНАЧЕ РегламентированныйОтчет.Вид = 0
	|			КОНЕЦ)
	...
	ТаблицаДокументы.Колонки.Добавить("Ссылка", Новый ОписаниеТипов("ДокументСсылка.РегламентированныйОтчет"));
...
Функция ПоляКоторыеНеВсегдаТребуетсяЗаполнять(ИмяРеглОтчета, ИмяФормы, Период) Экспорт
	...
	ИначеЕсли ИмяРеглОтчета = "РегламентированныйОтчетСтатистикаФормаП6"
			И (ИмяФормы = "ФормаОтчета2017Кв1") Тогда
...
// Выгрузка без открытия формы возможна только при условии, что данный отчет ее поддерживает - см. ВыгрузкаОтчетаПоддерживается().
//
// Параметры:
//   СохраненныйОтчет - ДокументСсылка.РегламентированныйОтчет - ссылка на сохраненный регл. отчет.
//
// Возвращаемое значение:
//   Структура - результат выгрузки (см. НовыйРезультатВыгрузкиРеглОтчета())
//
Функция ВыгрузитьРегламентированныйОтчет(СохраненныйОтчет) Экспорт
	
	Результат = НовыйРезультатВыгрузкиРеглОтчета();
	
	Если ТипЗнч(СохраненныйОтчет) <> Тип("ДокументСсылка.РегламентированныйОтчет")
		ИЛИ СохраненныйОтчет.Пустая() Тогда
		
		ДобавитьОшибкуФормированияРеглОтчета(Результат,
	...
	РеквизитыОтчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СохраненныйОтчет,
		"Организация, ИсточникОтчета, ВыбраннаяФорма, КодНалоговогоОргана, ДатаНачала, ДатаОкончания, ДатаПодписи, НаименованиеОтчета, ПредставлениеПериода");
	
	Если НЕ ВыгрузкаОтчетаПоддерживается(РеквизитыОтчета.ИсточникОтчета, РеквизитыОтчета.ВыбраннаяФорма) Тогда
	...
	Если ДанныеВыгрузки.ОтчетВыгружен Тогда
		
		АдресХранилища = ПоместитьВоВременноеХранилище(ДанныеВыгрузки.ТекстВыгрузки, Новый УникальныйИдентификатор);
		
		Результат.Вставить("ОтчетВыгружен", Истина);
		Результат.Вставить("АдресХранилищаТекстаВыгрузки", АдресХранилища);
		Результат.Вставить("ИмяФайла", ДанныеВыгрузки.ИмяФайла);
		Результат.Вставить("КодировкаФайлаВыгрузки", ДанныеВыгрузки.КодировкаВыгрузки);
		
	Иначе
		
		Для каждого СтрокаОшибки Из ДанныеВыгрузки.ТаблицаСообщений Цикл
			ДобавитьОшибкуФормированияРеглОтчета(Результат, СтрокаОшибки.Описание);
		КонецЦикла;
...
// Возвращает признак поддержки выгрузки регл. отчета без открытия формы.
//
// Параметры:
//   ИсточникОтчета - Строка - вид отчета, например, "РегламентированныйОтчетУСН".
//   ВыбраннаяФорма - Строка - форма отчета, например, "ФормаОтчета2015Кв1".
//
// Возвращаемое значение:
//   Булево - если Истина, выгрузка поддерживается.
//
Функция ВыгрузкаОтчетаПоддерживается(ИсточникОтчета, ВыбраннаяФорма) Экспорт
	
	ПолноеИмя = ВРег(ИсточникОтчета) + "." + ВРег(ВыбраннаяФорма);
	
	Возврат РегламентированнаяОтчетность.ОтчетыПоддерживающиеИнтерфейсВыгрузки()[ПолноеИмя] = Истина;
...
Функция ЗаписатьРеглОтчетУСНФормаОтчета2015Кв1(ПараметрыЗаписи)
	...
	Если СохраняемыйРеглОтчет <> Неопределено Тогда
		...
		Иначе
			...
			ДобавитьОшибкуФормированияРеглОтчета(ПараметрыЗаписи,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1'"),
			РегламентированнаяОтчетностьКлиентСервер.СформироватьТекстСообщения(ОписаниеОшибки()));
		ДобавитьОшибкуФормированияРеглОтчета(ПараметрыЗаписи, ТекстСообщения);
...
Процедура ЗаполнитьЗначенияСвойствСтруктурыПоИсточнику(СтруктураПриемник, СтруктураИсточник)
	...
	Если ТипЗнч(СтруктураИсточник) <> Тип("Структура")
		И ТипЗнч(СтруктураИсточник) <> Тип("Структура") Тогда
		// Ничего не получится
		Возврат;
	КонецЕсли;
...
Функция НовыйРезультатВыгрузкиРеглОтчета()
	
	Результат = Новый Структура;
	
	Результат.Вставить("ОтчетВыгружен", Ложь);
	Результат.Вставить("АдресХранилищаТекстаВыгрузки", "");
...
Процедура ДобавитьОшибкуФормированияРеглОтчета(РезультатВыгрузки, ТекстОшибки)
