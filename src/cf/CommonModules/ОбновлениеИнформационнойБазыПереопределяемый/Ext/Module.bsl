...
Процедура ПослеОбновленияИнформационнойБазы(Знач ПредыдущаяВерсияИБ, Знач ТекущаяВерсияИБ,
	...
	Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		Если Константы.НачалоРаботы.Получить() = Истина 
		 ИЛИ Константы.ИнтерфейсТаксиПростой.Получить() = Истина Тогда
			ВыводитьОписаниеОбновлений = Ложь;
		КонецЕсли;
	КонецЕсли;
	
	Если НЕ ТарификацияБПВызовСервераПовтИсп.РазрешенУчетРегулярнойДеятельности() Тогда
		// При работе в режиме сдачи "нулевой" отчетности описание обновлений не показываем.
		ВыводитьОписаниеОбновлений = Ложь;
	КонецЕсли;
