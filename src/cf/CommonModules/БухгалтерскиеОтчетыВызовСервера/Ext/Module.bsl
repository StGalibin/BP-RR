			...
			Для Каждого ЭлементСтруктуры Из ДополнительныеСвойства Цикл
				...
				Если ФормаОтчета.Отчет.Свойство(ЭлементСтруктуры.Ключ) Тогда
					...
					ИначеЕсли ЭлементСтруктуры.Ключ = "Организация"
						...
						Если ДоступныеОрганизации.Найти(ЭлементСтруктуры.Значение) = Неопределено Тогда
							// Доступ к организации с момента последнего запуска отчета может быть изменен.
							// В этом случае не будет восстанавливать организацию из настроек.
							Продолжить;
						КонецЕсли;
