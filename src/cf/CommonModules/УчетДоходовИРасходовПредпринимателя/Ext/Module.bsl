...
Функция ПодготовитьТаблицыПоступленияОтПродажПоПлатежнымКартам(ТаблицаРасшифровкаПлатежа, ТаблицаРеквизитов, Движения, Отказ) Экспорт
	...
	Если Параметры.Реквизиты.Количество() = 0 ИЛИ Параметры.РасшифровкаПлатежа.Количество() = 0 Тогда
		Возврат Результат;
	КонецЕсли;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ПрочиеРасчетыОбщие.СчетУчета,
	|	ВТ_ПрочиеРасчетыОбщие.Контрагент,
	|	ВТ_ПрочиеРасчетыОбщие.РасчетныйДокумент,
	|	ВТ_ПрочиеРасчетыОбщие.ДатаРасчетногоДокумента,
	|	ВТ_ПрочиеРасчетыОбщие.ДоговорКонтрагента,
	|	ВТ_ПрочиеРасчетыОбщие.Сумма,
	|	ВТ_ПрочиеРасчетыОбщие.Покупатель,
	|	ВТ_ПрочиеРасчетыОбщие.ДоговорПокупателя,
	|	ВТ_ПрочиеРасчетыОбщие.ТипСклада
	|ИЗ
	...
	ОстаткиОтгруженныхМПЗ = Результаты[4].Выгрузить();
	...
	Для Каждого Платеж Из РасшифровкаПлатежа Цикл
		...
		Для Каждого ПрочиеРасчеты Из НайденныеПрочиеРасчеты Цикл
			...
			Иначе
				...
				НайденныеОтгруженныеМПЗ.Сортировать("ПорядокПогашения");
...
Функция ПодготовитьТаблицуПоступленияОС(ТаблицаОС, ТаблицаВзаиморасчетов, ТаблицаРеквизитов) Экспорт
	...
	Если НЕ Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ОсновныеСредства Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Процедура РегистрацияСведенийОбОплатеОСиНМА(ТаблицаОСиНМА, ТаблицаРеквизитов, Движения, Отказ) Экспорт
	...
	Если Параметры.Реквизиты.Количество() = 0
		ИЛИ Параметры.СписокСведений.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Функция ПодготовитьТаблицуОплатыОСиНМА(ТаблицыМПЗ, ТаблицаРеквизитов) Экспорт
	...
	Если ТаблицыМПЗ = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Процедура СформироватьДвиженияСписанияРБП(ТаблицаРБП, ТаблицаПрочихРасходов, ТаблицаРеквизитов, Движения, Отказ) Экспорт
	...
	Если Параметры.Реквизиты.Количество() = 0 ИЛИ Параметры.СписокРБП.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Функция ПодготовитьТаблицуВключенияАвансовВПрочиеДоходы(ТаблицаРеквизитов, Отказ) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(СтатьяДоходов) Тогда
		...
		ТекстСообщения  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не указана статья доходов по авансам в налоговом учете предпринимателя ""%1"". 
		...
		БухгалтерскийУчетПереопределяемый.СообщитьОбОшибкеРегОперацииСНавигацией(
			ТекстСообщения, 
			БухгалтерскийУчетКлиентСерверПереопределяемый.ПолучитьНавигационнуюСсылкуНаУчетнуюПолитикуПоНДФЛ(), 
			Отказ, 
...
Функция ДоляДоходовЕНВД(Организация, НачалоПериода, КонецПериода) Экспорт
	...
	Если ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Организация)
		ИЛИ НЕ УчетнаяПолитика.ПлательщикЕНВД(Организация, КонецПериода) Тогда
		Возврат 0;
	КонецЕсли;
...
Функция ПодготовитьТаблицуРасходовНаУплатуСтраховыхВзносов(ТаблицаРеквизитов) Экспорт
	...
	ЕНВДУменьшаетсяНаФиксированныеВзносы = ПлательщикЕНВД
		И (УчетЕНВД.НалогУменьшаетсяНаФиксированныеВзносыИПРаботодателей(Реквизиты.Период)
			Или Не УчетЗарплаты.ИПИспользуетТрудНаемныхРаботников(Реквизиты.Организация));
	...
	Для каждого СтраховойВзнос Из ТаблицаРасходов Цикл
		...
		Если ВестиУчетПоВидамДеятельностиИП И ПлательщикЕНВД Тогда
		...
		ИначеЕсли ВестиУчетПоВидамДеятельностиИП И Не ПлательщикЕНВД Тогда
...
Функция СуммаДоходаДляРасчетаСтраховыхВзносов(Организация, ДатаНачала, ДатаОкончания) Экспорт
	...
	СтруктураПараметров.Вставить("НачалоПериода", НачалоГода(ДатаНачала));
...
Функция РасходыНаУплатуСтраховыхВзносовЗаТекущийПериод(Организация, Период) Экспорт
	...
	СтатьяЗатрат = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент(
		"Справочник.ПрочиеДоходыИРасходы.РасходыПоУплатеСтраховыхВзносовИП");
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Организация",   Организация);
	Запрос.УстановитьПараметр("ОблагаетсяНДС", Не УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС(Организация, Период));
	Запрос.УстановитьПараметр("ДатаНачГода",   НачалоГода(Период));
	Запрос.УстановитьПараметр("ДатаКон",       Новый Граница(КонецМесяца(Период), ВидГраницы.Включая));
	...
	|			КОНЕЦ < &ДатаНачГода) КАК ВложенныйЗапрос";
...
Процедура ЗаполнитьЛистВ3НДФЛ2015(НомерСтроки, Контейнер, ДанныеЛистаРаздела61, Организация, ВидДеятельности)
	...
	Если СтруктураДанныхЛистаВ.ИтогоДоходов <> 0 ИЛИ СтруктураДанныхЛистаВ.ИтогоРасходов <> 0 Тогда
		...
		ПолучитьСведенияОПоказателяхОтчетаРегламентированныйОтчет3НДФЛ2015(ЗаполняемыеПоказатели);
		...
		Иначе
			...
			// Нужно перенести итоговые данные введенные вручную на последней странице
...
Процедура ЗаполнитьРегламентированныйОтчет3НДФЛ2015(ПараметрыОтчета, Контейнер) Экспорт
	...
	ЗаполнитьКонтейнер3НДФЛ(ПараметрыОтчета, Контейнер);
	
КонецПроцедуры

Процедура ЗаполнитьРегламентированныйОтчет3НДФЛ2016(ПараметрыОтчета, Контейнер) Экспорт
...
Процедура ЗаполнитьКонтейнер3НДФЛ(ПараметрыОтчета, Контейнер) Экспорт
...
Процедура ПолучитьСведенияОПоказателяхОтчетаРегламентированныйОтчет3НДФЛ2015(ПоказателиОтчета) Экспорт
	...
	РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "П000100001030", Истина, Ложь);
	РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "П000100001050", Истина, Ложь);
	
	РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "П000500001010", Истина, Ложь);
	...
	РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "П000500001090", Истина, Ложь);
...
Функция ПолучитьСтатьюЗатрат(СтруктураПараметров)
	...
	Если БухгалтерскийУчетПовтИсп.СчетВИерархии(СчетУчета, ПланыСчетов.Хозрасчетный.АрендныеОбязательства) 
		...
		ПредопределеннаяСтатьяЗатрат = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.СтатьиЗатрат.ПрочиеЗатраты");
		
		Если ПредопределеннаяСтатьяЗатрат = Неопределено Тогда
			СтатьяЗатрат = Справочники.СтатьиЗатрат.ПредопределенныйЭлементПрочиеЗатраты();
		Иначе
			СтатьяЗатрат = ПредопределеннаяСтатьяЗатрат;
		КонецЕсли;
...
Процедура СформироватьДвиженияВключенияСтоимостиНМАВСоставРасходовИП(ТаблицаРасходов, ТаблицаРеквизитов, Движения, Отказ) Экспорт
	...
	Если Параметры.Реквизиты.Количество() = 0
		ИЛИ Параметры.ТаблицаРасходов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
