...
Функция ПериодичностьУплатыФиксированныхСтраховыхВзносов(Организация, Период) Экспорт
	...
	ФиксированныеВзносыУменьшаютЕНВД = УчетЕНВД.НалогУменьшаетсяНаФиксированныеВзносыИПРаботодателей(Период)
		ИЛИ НЕ УчетЗарплаты.ИПИспользуетТрудНаемныхРаботников(Организация);
	
	Если ПрименяетсяУСН ИЛИ (ПлательщикЕНВД И ФиксированныеВзносыУменьшаютЕНВД) Тогда
...
Функция РассчитатьФиксированныеСтраховыеВзносы(Знач Период, Организация = Неопределено) Экспорт
	...
	Иначе
		...
		// За неполный месяц деятельности фиксированный размер страховых взносов определяется
		// пропорционально количеству календарных дней этого месяца
		ДнейВНеполномМесяце = День(КонецМесяца(ДатаРегистрации));
		ПолныхДней = ДнейВНеполномМесяце - День(ДатаРегистрации);
		Если УчитыватьДеньРегистрацииПриРасчетеСтраховыхВзносовИП(Период) Тогда
			// Обязанность по уплате страховых взносов возникает с даты регистрации индивидуального предпринимателя.
			ПолныхДней = ПолныхДней + 1;
		Иначе
			...
			// расчете страховых взносов не учитывается.
			// Данный вариант не применяется с 2017 года.
		КонецЕсли;
...
Функция ТарифыСтраховыхВзносов(Период)
	...
	Возврат Новый Структура("ПФР, ФФОМС, ФСС", ПФР, ФФОМС, ФСС);
...
Функция ПериодичностьНачисления(Организация, Период) Экспорт
	...
	Если Период >= ДатаАвтоНачисленияСтраховыхВзносовИП() Тогда
		...
		Если УчетнаяПолитика.ПрименяетсяУСНДоходыМинусРасходы(Организация, Период) Тогда
			...
			Периодичность = Перечисления.Периодичность.Квартал;
		ИначеЕсли УчетнаяПолитика.ПлательщикНДФЛ(Организация, Период)
...
//
Функция ТаблицаНачисленияСтраховыхВзносов(Организация, Период)
	...
	ПорядокУчетаРасходов.СтатьяЗатрат = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент(
...
//
Функция ПодготовитьТаблицуРасходовНаУплатуСтраховыхВзносов(Организация, Период) Экспорт
	
	Если Период < ДатаНачалаУчетаСтраховыхВзносовИП() Или ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Организация) Тогда
		Возврат Неопределено;
	КонецЕсли;
...
//    ТаблицаЗначений, Неопределено - см. УчетСтраховыхВзносовИП.НоваяТаблицаСтраховыхВзносов();
//
Функция ПодготовитьТаблицуРасходовУменьшающихНалог(Организация, Период) Экспорт
	
	Если Период < ДатаНачалаУчетаСтраховыхВзносовИП() Или ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Организация) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	// Расчет расходов выполняется раз в квартал
	НачалоПериода = НачалоКвартала(Период);
	КонецПериода  = КонецМесяца(Период);
...
Функция ТаблицаРасходовНаУплатуСтраховыхВзносов(Организация, НачалоПериода, КонецПериода)
	...
	ГраницыПериодаУплатыПрошлогоГода = ГраницыПериодаУплатыСтраховыхВзносов(НачалоПрошлогоГода, Перечисления.ВидыПлатежейВГосБюджет.ВзносыСвышеПредела);
	Если НачалоПериода <= ГраницыПериодаУплатыПрошлогоГода.КонецПериода Тогда
		// До 01 апреля могут уплачиваться взносы, которые относятся к прошлому году
	...
	Для Каждого СтраховойПериод Из ВсеСтраховыеПериоды Цикл
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаНачисленияСтраховыхВзносов(Организация, ГраницаНачисления), ТаблицаСтраховыхВзносов);
	КонецЦикла;
