...
Процедура ЗаписатьРеализацияТоваровУслуг(ОбъектXDTO, УзелОбмена, РегистрироватьПриИзменении = Истина, НомерБП = "", НовыйСчетФактура = Ложь) Экспорт
	...
	Для каждого ЗагружаемаяСтрокаТЧ Из ОбъектXDTO.ТоварыУслуги Цикл
		...
		Если ЗагружаемаяСтрокаТЧ.СтавкаНДС = "НДС18" Тогда
	...
	Если НовыйОбъект Тогда
		Если ЕстьНДС Тогда
			Если УчетНДСВызовСервера.СоздатьСчетФактуруВыданныйНаОсновании(Объект.Ссылка) <> Неопределено Тогда
...
Функция ПолучитьОбъектXDTOИзДанныхВыборкиИзменений(Данные, МассивФайлов, УзелОбмена)
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("СправочникОбъект.Номенклатура") Тогда
		...
		Если НЕ Данные.ЭтоГруппа Тогда
			...
			ПередаваемыйОбъект.СтавкаНДС            = СериализоватьСтавкуНДС(Данные.СтавкаНДС);
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("СправочникОбъект.Валюты") Тогда
		...
		ПередаваемыйОбъект.ПараметрыПрописиНаРусском = Данные.ПараметрыПрописиНаРусском;
...
Функция ПолучитьОбъектXDTOФайл(ФайлСсылка)
	...
	Если ЗначениеЗаполнено(ФайлСсылка) Тогда
		ДанныеКартинки = ПрисоединенныеФайлы.ПолучитьДанныеФайла(ФайлСсылка);
...
Функция ОстаткиНоменклатуры()
	...
	СчетаУчетаТоваров = ОбщегоНазначенияКлиентСервер.СократитьМассив(СчетаУчетаТоваров, СчетаИсключаемые);
...
Процедура ДобавитьНовуюСтрокуКИ(Объект, ОбъектXDTOЗагружаемаяСтрокаКИ)
	...
	НоваяСтрокаКИ.Тип = ДесериализоватьТипКИ(ОбъектXDTOЗагружаемаяСтрокаКИ.Тип);
	Если ОбъектXDTOЗагружаемаяСтрокаКИ.ВидПредопределенный Тогда
		НоваяСтрокаКИ.Вид = Справочники.ВидыКонтактнойИнформации[ОбъектXDTOЗагружаемаяСтрокаКИ.ВидИмяПредопределенныхДанных]
	Иначе
		НоваяСтрокаКИ.Вид = ДесериализоватьСсылкуНаСправочник(ОбъектXDTOЗагружаемаяСтрокаКИ.Вид, "ВидыКонтактнойИнформации");
	КонецЕсли;
	НоваяСтрокаКИ.Представление = ОбъектXDTOЗагружаемаяСтрокаКИ.Представление;
	...
	ОбъектКИ = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияИзXML(НоваяСтрокаКИ.ЗначенияПолей, НоваяСтрокаКИ.Вид);
	
	Если УправлениеКонтактнойИнформациейСлужебный.XDTOКонтактнаяИнформацияЗаполнена(ОбъектКИ) Тогда
		Если НоваяСтрокаКИ.Тип = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты Тогда
			УправлениеКонтактнойИнформациейБП.ЗаполнитьРеквизитыТабличнойЧастиДляАдресаЭлектроннойПочты(НоваяСтрокаКИ, ОбъектКИ);
		ИначеЕсли НоваяСтрокаКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			УправлениеКонтактнойИнформациейБП.ЗаполнитьРеквизитыТабличнойЧастиДляАдреса(НоваяСтрокаКИ, ОбъектКИ);
		ИначеЕсли НоваяСтрокаКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
			УправлениеКонтактнойИнформациейБП.ЗаполнитьРеквизитыТабличнойЧастиДляТелефона(НоваяСтрокаКИ, ОбъектКИ);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаписатьНоменклатуру(ОбъектXDTO, УзелОбмена) Экспорт
	...
	Если НЕ ОбъектXDTO.ЭтоГруппа Тогда
		...
		Объект.СтавкаНДС 			= ДесериализоватьСтавкуНДС(ОбъектXDTO.СтавкаНДС);
