...
// Возвращает перечень регламентированных отчетов, для которых реализован интерфейс выгрузки в XML без открытия формы.
//
// Возвращаемое значение:
//   Соответствие - ключ содержит полное имя отчета в формате <Источник>.<Форма> в верхнем регистре;
//                  в значения помещается Истина для простого использования в условиях.
//
Функция ОтчетыПоддерживающиеИнтерфейсВыгрузки() Экспорт
	
	ДоступныеОтчеты = Новый Соответствие;
	ДоступныеОтчеты.Вставить(ВРег("РегламентированныйОтчетУСН.ФормаОтчета2015Кв1"), Истина);
...
Процедура ВставитьПоказатели_ЕдиныйНалогНаВмененныйДоход(ТаблицаПоказателей, ОписаниеОтчета)
	...
	Если ОписаниеОтчета.ВыбраннаяФорма = "ФормаОтчета2017Кв1"
...
Функция ПолучитьСсылкуНаРеглОтчет(ИсточникОтчета, Организация, ДатаОкончания, КорректирующийОтчет = Ложь, РегистрацияВНалоговомОргане = Неопределено) Экспорт
	...
	РезультатЗапроса.Сортировать("Дата Убыв, Вид Убыв");
...
Процедура ОтключитьВнешнийМодульДокументооборотаСФНС() Экспорт
	...
	Если ОбменДаннымиПовтИсп.АвтономнаяРаботаПоддерживается() И ОбщегоНазначенияПовтИсп.ЭтоАвтономноеРабочееМесто() Тогда
