...
//
Процедура ЗаполнитьТемуПолучателяПисьма(МассивОбъектов, КоллекцияПечатныхФорм, ПараметрыПисьма, КоличествоЭД = 0) Экспорт
	...
	Если ЗаполнитьТемуПолучателяПисьма Тогда
		...
		ИначеЕсли Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(МассивОбъектов[0])) Тогда
			ЗаполнитьТемуТелоПисьмаСДокументом(МассивОбъектов, ПараметрыПисьма, КоллекцияПечатныхФорм, КоличествоЭД);
		Иначе
...
Процедура ПослеОтправкиПисьма(ПараметрыПисьма) Экспорт
	...
	Для Каждого ПолучательОтправленногоПисьма Из ПараметрыПисьма.Кому Цикл
		...
		Если ТребуетсяСохранитьАдрес Тогда
			Адрес = Новый Структура("КонтактнаяИнформация, Представление",
						"", ПолучательОтправленногоПисьма.Адрес);
...
Процедура УстановитьПолучателяПоУмолчанию(Контакты) Экспорт
	...
	Контрагент = Контакты[0].ИсточникКонтактнойИнформации;
...
Процедура ЗаполнитьТемуТелоПисьмаСКарточкойОрганизации(МассивОбъектов, ПараметрыПисьма, ТаблицаПечатныхФорм)
	...
	Если ВсегоОрганизаций = 1 Тогда
		...
		ТемаПисьма = СтрШаблон(ТемаПисьма, МассивОбъектов[0].НаименованиеСокращенное);
	ИначеЕсли ВсегоОрганизаций > 1 И ВсегоОрганизаций < 10 Тогда
		...
		КонецЦикла;
		ТемаПисьма = СтрШаблон(ТемаПисьма, НаименованияОрганизаций);
...
Процедура СохранитьАдресаЭлектроннойПочты(Контрагент, Адреса)
	...
	ИначеЕсли НЕ ДоступенВвводНесколькихЗначений Тогда
		УправлениеКонтактнойИнформацией.ЗаполнитьКонтактнуюИнформациюОбъекта(Контрагент, ВидКИ, Адреса[0]);
		Возврат;
	КонецЕсли;
	...
	Для Каждого Адрес Из Адреса Цикл
		...
		ЗначенияПолей = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВXML(Адрес, , ВидКИ);
		...
		ЗаполнитьРеквизитыТабличнойЧастиДляАдресаЭлектроннойПочты(СтрокаКИ);
		
	КонецЦикла;
	
	Отбор = Новый Структура("Вид", ВидКИ);
	КоличествоСтрокСАдресамиПочты = Приемник.КонтактнаяИнформация.НайтиСтроки(Отбор).Количество();
	
	Если КоличествоСтрокСАдресамиПочты > 0 Тогда
		Для Каждого СтрокаКИ Из Приемник.КонтактнаяИнформация Цикл
			Если СтрокаКИ.ВидДляСписка = ВидКИ И СтрокаКИ.Вид = ВидКИ Тогда
				СтрокаКИ.ВидДляСписка = Неопределено;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыТабличнойЧастиДляАдресаЭлектроннойПочты(СтрокаКИ)
	
	Результат = ОбщегоНазначенияКлиентСервер.РазобратьСтрокуСПочтовымиАдресами(СтрокаКИ.Представление, Ложь);
	
	Если Результат.Количество() > 0 Тогда
		СтрокаКИ.АдресЭП = Результат[0].Адрес;
...
Процедура ЗаполнитьТемуТелоПисьмаСДокументом(МассивОбъектов, ПараметрыПисьма, КоллекцияПечатныхФорм, КоличествоЭД)
	
	СтруктураТиповОбъектыПечати = ОбщегоНазначенияБП.РазложитьСписокПоТипамОбъектов(МассивОбъектов);
