...
Процедура ПереносКИОсновныхСредствПриОбновленииС20()
	...
	Пока Выборка.Следующий() Цикл
		...
		// Конвертацию в XML проведем через промежуточный служебный метод,
		// т.к. штатный метод конвертации структуры в XML не отрабатывает до конца правильно. В частности, не заполняет тип дома.
		ОбъектКИXDTO = УправлениеКонтактнойИнформациейСлужебный.АдресXMLВXDTO(АдресСтруктурой, "", Перечисления.ТипыКонтактнойИнформации.Адрес);
		ЗначениеПолей =  УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияXDTOВXML(ОбъектКИXDTO);
		
		УправлениеКонтактнойИнформацией.ЗаполнитьКонтактнуюИнформациюОбъекта(ОбъектОС, Справочники.ВидыКонтактнойИнформации.АдресМестонахожденияОсновныеСредства, ЗначениеПолей);
