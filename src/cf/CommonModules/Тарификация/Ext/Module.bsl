...
//  ИдентификаторПоставщика - Строка - уникальный строковый идентфикатор (код) поставщика услуг в сервисе.
//  ИдентификаторУслуги - Строка - уникальный строковый идентфикатор (код) услуги в сервисе.
//  ИдентификаторЛицензии - УникальныйИдентификатор - идентификатор лицензии.
//
// Возвращаемое значение:
//  Булево - результат проверки (Истина = лицензия зарегистрирована).
//
Функция ЗарегистрированаЛицензияУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, ИдентификаторЛицензии) Экспорт
	...
	Попытка
		...
		Кэш = ЗначениеКэшаЛицензийУникальныхУслуг(ИдентификаторПоставщика, ИдентификаторУслуги, ИдентификаторЛицензии);
		Если Кэш <> Неопределено Тогда
			ВремяЖизниКэша = ТарификацияСлужебный.ВремяЖизниКэшаЛицензийУникальныхУслугВСекундах();
			Если Кэш.ДатаИзменения + ВремяЖизниКэша < ТекущаяДатаСеанса() Тогда
				ЗаписатьИнформацию(ИмяСобытия, ЗаголовокКомментария, НСтр("ru = 'Лицензия доступна: '") + Строка(Кэш.ЛицензияЗарегистрирована));
				ЗафиксироватьТранзакцию();
				Возврат Кэш.ЛицензияЗарегистрирована;
			КонецЕсли;
		КонецЕсли;
		...
		Ответ = ПолучитьОтветОтHTTPСервиса(ПараметрыПодключения, "/UniqueServiceLicense", "GET", ПараметрыЗапроса);
		
		СтруктураОтветаHTTP = СтруктураОтветаHTTP(Ответ.ПолучитьТелоКакСтроку());
		
		Если Ответ.КодСостояния = 200 Тогда
			ЛицензияДоступна = СтруктураОтветаHTTP.LicenseAvailable;
			ЗаписатьИнформацию(ИмяСобытия, ЗаголовокКомментария, НСтр("ru = 'Лицензия доступна: '") + Строка(ЛицензияДоступна));
			УстановитьПривилегированныйРежим(Истина);
			ЗаполнитьЗначениеКэшаЛицензийУникальныхУслуг(ИдентификаторПоставщика, ИдентификаторУслуги, ИдентификаторЛицензии, ЛицензияДоступна);
			УстановитьПривилегированныйРежим(Ложь);
			ЗафиксироватьТранзакцию();
			Возврат ЛицензияДоступна;
...
//  ИдентификаторПоставщика - Строка - уникальный строковый идентфикатор (код) поставщика услуг в сервисе.
//  ИдентификаторУслуги - Строка - уникальный строковый идентфикатор (код) услуги в сервисе.
//  ИдентификаторЛицензии - УникальныйИдентификатор - уникальный идентификатор лицензии.
...
//
Функция ЗанятьЛицензиюУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, ИдентификаторЛицензии, ИдентификаторОперации) Экспорт
...
//
Функция ОсвободитьЛицензиюУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, ИдентификаторЛицензии, ИдентификаторОперации, КодОбластиДанных = Неопределено) Экспорт
...
Функция ЗанятьЛицензииЛимитированнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, КоличествоЛицензий, КодОбластиДанных = Неопределено) Экспорт
	...
	Попытка
		...
		УстановитьПривилегированныйРежим(Ложь);
		ОтветЗанятияУслуги.ЗанятоЛицензий = ОтветЗанятияУслуги.ЗанятоЛицензий + КоличествоЛицензий;
...
Функция КоличествоЛицензийУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги) Экспорт
	...
	Попытка
		...
		ИдентификаторЛицензии = Новый УникальныйИдентификатор;
...
Функция ПараметрыПодключенияКHTTPСервисуТарификацииДляПриложений()
	...
	ПараметрыПодключения.Вставить("URL", РаботаВМоделиСервиса.ВнутреннийАдресМенеджераСервиса()+ "/hs/ApplicationTariffControl_1_0_1_2");
...
//
Функция ПараметрыЗапросаЛицензииУникальнойУслуги(ИдентификаторПоставщикаУслуги, ИдентификаторУслуги, ИдентификаторЛицензии, ИдентификаторОперации, Пользователь)
...
// Возвращает значение кэша по лицензии уникальной услуги: зарегистрирована ли данная лицензия или нет.
// Время жизни кэша хранится в константе "ВремяЖизниКэшаЛицензийУникальныхУслуг".
//
// Параметры:
//  ИдентификаторПоставщикаУслуги - Строка - идентификатор поставщика услуги.
//  ИдентификаторУслуги - Строка - идентификатор услуги.
...
//
// Возвращаемое значение:
//  Структура - значение кэша.
//
Функция ЗначениеКэшаЛицензийУникальныхУслуг(ИдентификаторПоставщика, ИдентификаторУслуги, ИдентификаторЛицензии)
	
	Услуга = УслугаПоИдентификаторуИИдентификаторуПоставщика(ИдентификаторУслуги, ИдентификаторПоставщика);
	
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	КэшЛицензийУникальныхУслуг.ЛицензияЗарегистрирована КАК ЛицензияЗарегистрирована,
		|	КэшЛицензийУникальныхУслуг.ДатаИзменения КАК ДатаИзменения
		|ИЗ
		|	РегистрСведений.КэшЛицензийУникальныхУслуг КАК КэшЛицензийУникальныхУслуг
		|ГДЕ
		|	КэшЛицензийУникальныхУслуг.Услуга = &Услуга
		|	И КэшЛицензийУникальныхУслуг.ИдентификаторЛицензии = &ИдентификаторЛицензии");
	Запрос.УстановитьПараметр("ИдентификаторЛицензии", ИдентификаторЛицензии);
	Запрос.УстановитьПараметр("Услуга", Услуга);
	Выборка = Запрос.Выполнить().Выбрать();
	...
	Возврат Новый Структура("ЛицензияЗарегистрирована, ДатаИзменения", Выборка.ЛицензияЗарегистрирована, Выборка.ДатаИзменения);
	
КонецФункции

// Перезаписывает значение кэша по лицензии уникальной услуги.
//
// Параметры:
//  ИдентификаторПоставщикаУслуги - Строка - идентификатор поставщика услуги.
//  ИдентификаторУслуги - Строка - идентификатор услуги.
//  ИдентификаторЛицензии - Строка - идентификатор лицензии.
//  ЛицензияЗарегистрирована - <ektdj - kbwtypbz pfhtubcnhbhjdfyf bkb ytn/
//
Процедура ЗаполнитьЗначениеКэшаЛицензийУникальныхУслуг(ИдентификаторПоставщика, ИдентификаторУслуги, ИдентификаторЛицензии, ЛицензияЗарегистрирована)
	
	Услуга = УслугаПоИдентификаторуИИдентификаторуПоставщика(ИдентификаторУслуги, ИдентификаторПоставщика);
	
	Запись = РегистрыСведений.КэшЛицензийУникальныхУслуг.СоздатьМенеджерЗаписи();
	Запись.Услуга = Услуга;
	Запись.ИдентификаторЛицензии = ИдентификаторЛицензии;
	Запись.ЛицензияЗарегистрирована = ЛицензияЗарегистрирована;
	Запись.ДатаИзменения = ТекущаяДатаСеанса();
	Запись.Записать();
