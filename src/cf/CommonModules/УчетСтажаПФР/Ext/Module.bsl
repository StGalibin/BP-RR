...
Процедура ЗаполнитьПолеСотрудникВРегистрахУчета(ПараметрыОбновления = Неопределено) Экспорт 
	...
	|		ПО ПараметрыПериодовСтажаПФР.ФизическоеЛицо = Сотрудники.ФизическоеЛицо
	|			И ПараметрыПериодовСтажаПФР.ГоловнаяОрганизация = Сотрудники.ГоловнаяОрганизация
	|			И (ПараметрыПериодовСтажаПФР.Сотрудник = ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	ЗаполнитьПолеСотрудникВРегистре("ПараметрыПериодовСтажаПФР", "Регистратор", ПараметрыОбновления);
КонецПроцедуры	

Процедура ЗаполнитьПолеСотрудникВРегистре(ИмяРегистра, ИмяПоляРегистратор, ПараметрыОбновления = Неопределено) Экспорт 
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	ЗарплатаКадрыПериодическиеРегистры.СоздатьВТИмяРегистраСрезПоследних("КадроваяИсторияСотрудников", Запрос.МенеджерВременныхТаблиц, Ложь, ОписаниеФильтра);	
	...
	|	МАКСИМУМ(ЕСТЬNULL(КадроваяИсторияСотрудников.Сотрудник, ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка))) КАК Сотрудник
	...
	|		ПО ПериодыПолученияВидовЗанятости.ГоловнаяОрганизация = КадроваяИсторияСотрудников.ГоловнаяОрганизация
	|			И ПериодыПолученияВидовЗанятости.ФизическоеЛицо = КадроваяИсторияСотрудников.ФизическоеЛицо
	|			И ПериодыПолученияВидовЗанятости.Период = КадроваяИсторияСотрудников.Период
	|			И (КадроваяИсторияСотрудников.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы))
	|
	|СГРУППИРОВАТЬ ПО
	|	ПериодыПолученияВидовЗанятости.ГоловнаяОрганизация,
	|	ПериодыПолученияВидовЗанятости.ФизическоеЛицо,
	...
	|	СотрудникиСОсновнымВидомЗанятостиПредварительно.Сотрудник
	...
	|	ЗаписиКОбновлению.ТипДоговора,
	|	ЗаписиКОбновлению.Начало,
	|	ЗаписиКОбновлению.Окончание,
	|	ЗаписиКОбновлению.Приоритет,
	|	ЗаписиКОбновлению.Сторно,
	|	ЗаписиКОбновлению.ДокументОснование,
	|	ЗаписиКОбновлению.ДатаНачалаСобытия,
	|	ВЫБОР
	|		КОГДА ЗаписиКОбновлению.ТипДоговора <> ЗНАЧЕНИЕ(Перечисление.ТипыДоговоровСЗВ63.Трудовой)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)
	|		КОГДА ЗаписиКОбновлению.Сотрудник = ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)
