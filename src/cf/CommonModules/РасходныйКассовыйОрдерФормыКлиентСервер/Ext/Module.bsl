...
Процедура ЗаполнитьРеквизитыПлатежаВБюджет(Форма, НастройкаЗаполнения = Неопределено) Экспорт
...
Функция СвойстваСтрокРасшифровкиПлатежа(ПараметрыОбъекта, ПолучатьДоговор) Экспорт
	...
	Иначе
		...
		ПараметрыПлатежа = Новый Структура("ВидОперации, Организация, Дата, Контрагент, ДоговорКонтрагента, Сделка, ПрименениеУСН,
		...
		Возврат РасходныйКассовыйОрдерВызовСервера.СвойстваСтрокРасшифровкиПлатежаСервер(ПараметрыПлатежа, ПолучатьДоговор);
...
//
// Параметры:
//  Форма                    - УправляемаяФорма - однострочная форма РКО
