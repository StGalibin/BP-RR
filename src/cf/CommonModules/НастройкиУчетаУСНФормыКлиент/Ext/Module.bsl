...
Процедура СтавкаНалогаПриИзменении(Форма) Экспорт
	
	// Проверка на превышение максимально допустимой ставки
	...
	Если Запись.СтавкаНалога > ПределСтавки Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстПредупреждения,,"СтавкаНалогаУСН", "НастройкиУчетаУСН");
		
		Запись.СтавкаНалога = Форма.ТекущаяСтавкаНалога;
