...
ИначеЕсли ТипЗнч(Источник) = Тип ("ДокументОбъект.ВозвратТоваровОтПокупателя") 
	и ЗначениеЗаполнено(Источник.Сделка)
	и ТипЗнч(Источник.Сделка) = Тип ("ДокументСсылка.РеализацияТоваровУслуг")
	и ЗначениеЗаполнено(Источник.Сделка.СпецификацияКДоговору)
	Тогда
	...
	Для Каждого ТекСтрокаТовары Из Источник.Товары Цикл
		...
		Движение.Период = Источник.Дата;
		Движение.Спецификация = Источник.Сделка.СпецификацияКДоговору;
		Движение.Номенклатура = ТекСтрокаТовары.Номенклатура;
		Движение.Цена= ТекСтрокаТовары.Цена;
		Движение.Количество = ТекСтрокаТовары.Количество;
		Движение.Организация = Источник.Организация;
	...
	ИначеЕсли ТипЗнч(Источник) = Тип ("ДокументОбъект.КорректировкаРеализации") и ЗначениеЗаполнено(Источник.ДокументРеализации) Тогда
	
	ДокументРеализации = УчетНДСПереопределяемый.ПолучитьИсправляемыйДокументРеализации(Источник.ДокументРеализации, Истина);
	
	Если ТипЗнч(ДокументРеализации) = Тип ("ДокументСсылка.РеализацияТоваровУслуг")
	и ЗначениеЗаполнено(ДокументРеализации.СпецификацияКДоговору) Тогда
	
	Для Каждого ТекСтрокаТовары Из Источник.Товары Цикл
		
		Количество = ТекСтрокаТовары.Количество-ТекСтрокаТовары.КоличествоДоИзменения;
		...
		Движение.ВидДвижения = ВидДвиженияНакопления.Расход;
		Движение.Период = Источник.Дата;
		Движение.Спецификация = ДокументРеализации.СпецификацияКДоговору;
		Движение.Номенклатура = ТекСтрокаТовары.Номенклатура;
		Движение.Цена= ТекСтрокаТовары.Цена;
