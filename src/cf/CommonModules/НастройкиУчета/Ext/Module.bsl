...
Процедура ПриЗаписи(НаборЗаписей, Отказ, Замещение) Экспорт
	...
	Если ИмяРегистра = "НастройкиСистемыНалогообложения" Тогда
		...
		Для Каждого Запись ИЗ НаборЗаписей Цикл
			СинхронизацияНастроекУчета(Запись.Период, Запись.Организация, ИмяРегистра);
		КонецЦикла;
...
Процедура СинхронизацияНастроекУчета(Период, Организация, ИмяРегистра)
		...
		Иначе
			...
			Иначе
				Если КлючЗаписиПредыдущий.Период <> КлючЗаписиТекущий.Период Тогда
					...
					СистемаНалогообложенияПредыдущая = РегистрыСведений.НастройкиСистемыНалогообложения.Получить(КлючЗаписиПредыдущий.Период, Отбор).СистемаНалогообложения;
					СистемаНалогообложенияТекущая    = РегистрыСведений.НастройкиСистемыНалогообложения.Получить(КлючЗаписиТекущий.Период, Отбор).СистемаНалогообложения;
					Если СистемаНалогообложенияПредыдущая <> СистемаНалогообложенияТекущая Тогда
						СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("УчетнаяПолитика", Организация, КлючЗаписиНастройки.Период, Период);
						КлючЗаписиНастройки = КлючЗаписиДействующейУчетнойПолитики("УчетнаяПолитика", Организация, Период);
					КонецЕсли;
				КонецЕсли;
		...
		Если УчетнаяПолитика.ПлательщикНалогаНаПрибыль(Организация, Период) Тогда
			...
			Если НастройкиНалога.Количество() = 0
				...
				Если НастройкиНалога.Количество() = 0 Тогда
					...
					РегистрыСведений.НастройкиУчетаНалогаНаПрибыль.УстановкаНастроекПоУмолчанию(Запись, ДанныеЗаполнения);
					Запись.Записать(Истина);
				Иначе
		...
		Если Константы.УчетЗарплатыИКадровСредствамиБухгалтерии.Получить() Тогда
			...
			Если НастройкиНалога.Количество() = 0 Тогда
				...
				Если НастройкиНалога.Количество() = 0 Тогда
					...
					Запись.Организация = Организация;
					УчетЗарплаты.УстановкаНастроекПоУмолчанию("СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев", Запись, ДанныеЗаполнения);
					Запись.Записать(Истина);
				Иначе
...
Процедура СоздатьНастройкиУчетнойПолитикиПоУмолчанию(ДанныеЗаполнения)
	...
	НастройкиПоУмолчаниюСистемаНалогообложения(НастройкиСистемыНалогообложения,  ДанныеЗаполнения, СтруктураУчетнойПолитики);
	...
	НастройкиПоУмолчаниюУчетСтраховыхВзносовИП(НастройкиУчетаСтраховыхВзносовИП, ДанныеЗаполнения, СтруктураУчетнойПолитики);
...
Процедура НастройкиПоУмолчаниюУчетНалогаНаПрибыль(НастройкиУчетаНалогаНаПрибыль, ДанныеЗаполнения, СтруктураУчетнойПолитики)
	...
	// На 2017-2020 годы вводятся "нестандартные" ставки налога на прибыль (401-ФЗ от 30.11.2016)
	...
	Если ДанныеЗаполнения.Период < '2017-01-01' Тогда
		...
		ПериодЗаписи = ДанныеЗаполнения.Период;
		ДанныеЗаполнения.Период = '2017-01-01';
		...
		// Возвращаем обратно значение периода в ДанныеЗаполнения
		ДанныеЗаполнения.Период = ПериодЗаписи;
...
Процедура СоздатьНастройкиУчетаПоДаннымУчетнойПолитики() Экспорт
	...
	Пока ВыборкаУчетнаяПолитика.Следующий() Цикл
		...
		ЗаполнитьНастройкиСистемыНалогообложения(РСНастройкиСистемыНалогообложения,        ВыборкаУчетнаяПолитика, УплачиваетсяТорговыйСборБулево, УплачиваетсяТорговыйСбор);
		ЗаполнитьНастройкиБухгалтерскогоУчета(РСНастройкиБухгалтерскогоУчета,              ВыборкаУчетнаяПолитика, ВариантыБухгалтерскойОтчетности);
		...
		ЗаполнитьНастройкиУчетаСтраховыхВзносовИП(ТаблицаНастройкиУчетаСтраховыхВзносовИП, ВыборкаУчетнаяПолитика, СтруктураУчетнойПолитики);
		ЗаполнитьНастройкиУчетаНалогаНаПрибыль(НастройкиУчетаНалогаНаПрибыль,              ВыборкаУчетнаяПолитика, СтруктураУчетнойПолитики);
	КонецЦикла;
