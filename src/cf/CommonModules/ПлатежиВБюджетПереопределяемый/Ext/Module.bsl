...
Функция КонтекстПлатежногоДокумента(ИсточникДанных) Экспорт
	...
	ПоказателиНалоговогоПериода = ПоказателиНалоговогоПериода(ИсточникДанных.Организация, ВидНалога, Период, ИсточникДанных.ВидНалоговогоОбязательства);
	...
	Если ПериодПлатежаЗадан Тогда
		...
		Если ПоказателиПериода.Периодичность <> ПлатежиВБюджетКлиентСервер.ПлатежПоКонкретнойДате() Тогда
			ПоказательПериода = ПлатежиВБюджетКлиентСервер.НалоговыйПериод(Контекст.ПериодПлатежа, ПоказателиПериода.Периодичность);
	...
	Если ПлатежиВБюджетКлиентСервер.ПрименяетсяОКТМО8Символов(Контекст.Период) Тогда
		Контекст.КодТерритории = СокрП(Лев(Контекст.КодТерритории, 8));
		Контекст.КодТерриторииПоУмолчанию = СокрП(Лев(Контекст.КодТерриторииПоУмолчанию, 8));
	КонецЕсли;
...
Функция ПоказателиНалоговогоПериода(Организация, ВидНалога, Период, ВидНалоговогоОбязательства = Неопределено) Экспорт
	
	Показатели = ПоказателиПериода();
	
	ПорядокУплатыНалога = РегистрыСведений.ЗадачиБухгалтера.ПорядокУплатыНалогаЗаПериод(Организация, ВидНалога, Период);
