...
Функция РеквизитыХранящиесяВБазе(НаборРеквизитов, Организация = Неопределено) Экспорт
	...
	НаборРеквизитов.Вставить(Перечисления.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПГражданство,      Истина);
	...
	Если Организация <> Неопределено Тогда
		...
		Если ЭтоОбособленноеПодразделение Тогда
			НаборРеквизитов.Вставить(ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ДополнительныйКодФСС"), Истина);
		КонецЕсли;
		Если РеквизитыОрганизации.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
			НаборРеквизитов.Вставить(ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ЭлектроннаяПочта"), Истина);
		КонецЕсли;
	КонецЕсли;
...
Функция ОрганизацияПодключаемогоОборудованияПоУмолчанию(ТипыОборудования) Экспорт
	...
	СписокОборудования = Справочники.ПодключаемоеОборудование.ПолучитьСписокОборудования(ТипыОборудования);
...
Функция СобытиеОбработкаДвижений()
	...
	Возврат НСТр("ru = 'Обработка движений'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура СообщитьОЦиклическойСсылкеВОбъекте(МетаданныеОбъекта, ЭлементДанных)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'ЦиклическаяСсылка'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура УстановитьПростойИнтерфейс() Экспорт
