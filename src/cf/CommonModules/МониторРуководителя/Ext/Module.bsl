...
Процедура ОбновитьПоказателиДвиженияДенежныхСредств() Экспорт
	...
	РазделДвижениеДенежныхСредств = Перечисления.РазделыМонитораРуководителя.УдалитьДвижениеДенежныхСредств;
	РазделПоступлениеДенежныхСредств = Перечисления.РазделыМонитораРуководителя.ПоступлениеДенежныхСредств;
	РазделРасходДенежныхСредств = Перечисления.РазделыМонитораРуководителя.РасходДенежныхСредств;
	...
	Пока СписокРазделовИзНастроек.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(СписокРазделов) Тогда
			...
			СписокСодержитРаздел = СписокРазделов.НайтиПоЗначению(РазделРасходДенежныхСредств);
			Если СписокСодержитРаздел = Неопределено Тогда
				РазделСпискаДвижениеДенежныхСредств = СписокРазделов.НайтиПоЗначению(РазделДвижениеДенежныхСредств);
				ИндексРаздела = СписокРазделов.Индекс(РазделСпискаДвижениеДенежныхСредств);
				Если ИндексРаздела = Неопределено Тогда
					ИндексРаздела = 0;
				КонецЕсли;
				СписокРазделов.Вставить(ИндексРаздела, РазделРасходДенежныхСредств, , Истина);
			КонецЕсли;
			
			СписокСодержитРаздел = СписокРазделов.НайтиПоЗначению(РазделПоступлениеДенежныхСредств);
			Если СписокСодержитРаздел = Неопределено Тогда
				РазделСпискаРасходДенежныхСредств = СписокРазделов.НайтиПоЗначению(РазделРасходДенежныхСредств);
				ИндексРаздела = СписокРазделов.Индекс(РазделСпискаРасходДенежныхСредств);
				Если ИндексРаздела = Неопределено Тогда
					ИндексРаздела = 0;
				КонецЕсли;
				СписокРазделов.Вставить(ИндексРаздела, РазделПоступлениеДенежныхСредств, , Истина);
			КонецЕсли;
			
			РазделСпискаДвижениеДенежныхСредств = СписокРазделов.НайтиПоЗначению(РазделДвижениеДенежныхСредств);
			Если РазделСпискаДвижениеДенежныхСредств <> Неопределено Тогда
				СписокРазделов.Удалить(РазделСпискаДвижениеДенежныхСредств);
			КонецЕсли;
	...
	ДанныеМонитора = РегистрыСведений.ДанныеМонитораРуководителя.СоздатьНаборЗаписей();
	ДанныеМонитора.Отбор.РазделМонитора.Установить(Перечисления.РазделыМонитораРуководителя.УдалитьДвижениеДенежныхСредств);
	ДанныеМонитора.Прочитать();
	ДанныеМонитора.Очистить();
	ОбновлениеИнформационнойБазы.ЗаписатьНаборЗаписей(ДанныеМонитора);
	
	Отбор = Новый Структура;
	Отбор.Вставить("КлючОбъекта", "МониторРуководителя");
	
	НастройкиМонитора = ХранилищеОбщихНастроек.Выбрать(Отбор);
	
	Пока НастройкиМонитора.Следующий() Цикл
		
		Пользователь = НастройкиМонитора.Пользователь;
	...
	ЗаполнитьДанныеМонитораРуководителя();
