...
Функция ПериодичностьРегламентнойОперации(ВидОперации) Экспорт
	...
	Если ВидОперации = Перечисления.ВидыРегламентныхОпераций.НачислениеНДС 
		...
		Или ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетЕНВД Тогда
...
Функция ПредставлениеОперацииПерепроведениеДокументов(КодЯзыка = "") Экспорт
	...
	Возврат НСтр("ru = 'Перепроведение документов за месяц'", КодЯзыка);
...
//
Процедура СоздатьДетальныйСтатусЗакрытияМесяца(МенеджерВременныхТаблиц, Период, Организация, Знач ОтборОрганизаций = Неопределено, Знач ВидыОпераций = Неопределено) Экспорт
...
Процедура СоздатьДанныеРегламентныхОпераций(МенеджерВременныхТаблиц, Период, Организация, ОтборОрганизаций) Экспорт
...
Процедура СоздатьОперацииКВыполнению(МенеджерВременныхТаблиц, ВидыОпераций, Период, Организация, ОтборОрганизаций)
	...
	Для Каждого СтрокаВидОперации Из ВидыОпераций Цикл
		...
		Условие = УсловиеВыполненияРегламентнойОперации(
			...
			Период,
			ОтборОрганизаций.Количество() = 1, // если организация одна, то запрос можно выполнить оптимальнее
			КешУсловийОпераций);
...
Функция УсловиеВыполненияРегламентнойОперации(ВидОперации, Организация, Период, ОднаОрганизация, КешНастроек)
	...
	Если ВидОперации = Перечисления.ВидыРегламентныхОпераций.НачислениеЗарплаты Тогда
		...
		Иначе
			Возврат Не ЗначениеНастройки(КешНастроек, "УчетЗарплатыИКадровСредствамиБухгалтерии") 
				Или ЗначениеНастройки(КешНастроек, "НеобходимоНачислятьЗарплату", Организация, Период);
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.НачислениеРезерваОтпусков Тогда
		...
		Если НЕ ЗначениеНастройки(КешНастроек, "ИспользуетсяПодсистемаУчетаЗарплатыИКадров") Тогда
			Возврат Ложь;
		Иначе
			Возврат ЗначениеНастройки(КешНастроек, "ВедетсяУчетРезервовОтпусков", Организация, Период);
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.НачислениеНДС Тогда
		...
		Возврат ЗначениеНастройки(КешНастроек, "СистемаНалогообложенияОбщая", Организация, Период);
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.ПризнаниеВНУЛизинговыхПлатежей Тогда
		...
		Если Не ЗначениеНастройки(КешНастроек, "ПлательщикНалогаНаПрибыль", Организация, Период)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетНалогаНаИмущество Тогда
		...
		ОСНО = ЗначениеНастройки(КешНастроек, "СистемаНалогообложенияОбщая", Организация, Период);
		...
		Если Месяц(Период) <> 12 И Месяц(Период)%3 = 0 Тогда
			...
			КонецЕсли;
			ОрганизацииУплачивающиеАвансыПоНалогуНаИмущество = ЗначениеНастройки(КешНастроек, ИмяНастройки, Организация, Период);
		...
		Если ОСНО Тогда
			...
			|	ИЗ
			|		РегистрБухгалтерии.Хозрасчетный.Остатки(&НачалоГода, Счет В (&СчетаОсновныхСредств), , Организация В (&Организации)) КАК АмортизацияОС
			|	ГДЕ
			|		АмортизацияОС.СуммаОстаток <> 0
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			...
			|		ХозрасчетныйОбороты.Организация
			|	ИЗ
			|		РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоГода, &КонецПериода, , Счет В (&СчетаОсновныхСредств), , Организация В (&Организации), , ) КАК ХозрасчетныйОбороты
			|	ГДЕ
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетТранспортногоНалога Тогда
		...
		Если Месяц(Период) <> 12 И Месяц(Период)%3 = 0 Тогда
			...
			КонецЕсли;
			ОрганизацииУплачивающиеАвансыПоТранспортномуНалогу = ЗначениеНастройки(КешНастроек, ИмяНастройки, Организация, Период);
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетЗемельногоНалога Тогда
		...
		Если Месяц(Период) <> 12 И Месяц(Период)%3 = 0 Тогда
			...
			КонецЕсли;
			ОрганизацииУплачивающиеАвансыПоЗемельномуНалогу = ЗначениеНастройки(КешНастроек, ИмяНастройки, Организация, Период);
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.ИсключениеНЗПИзСоставаМатериальныхРасходовДляУСН Тогда
		...
		Возврат ЗначениеНастройки(КешНастроек, "УСНДоходыМинусРасходы", Организация, Период)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.ПризнаниеРасходовНаПриобретениеОСДляУСН Тогда
		...
		Если НЕ ЗначениеНастройки(КешНастроек, "УСНДоходыМинусРасходы", Организация, Период) Тогда
			Возврат Ложь;
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетТорговогоСбора Тогда
		...
		СложноеУсловие.ТекстЗапроса = // Текст запроса может быть модифицирован - см. ОптимизироватьЗапросДляОднойОрганизации
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЗарегистрированныеТорговыеТочки.Организация,
		|	ЗНАЧЕНИЕ(Перечисление.ВидыРегламентныхОпераций.РасчетТорговогоСбора)
		|ИЗ
		|	РегистрСведений.ПараметрыТорговыхТочек КАК ЗарегистрированныеТорговыеТочки
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПараметрыТорговыхТочек КАК ТорговыеТочкиСнятыеСУчета
		|		ПО ТорговыеТочкиСнятыеСУчета.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийТорговыеТочки.СнятиеСУчета)
		|			И ТорговыеТочкиСнятыеСУчета.Период <= &КонецПредыдущегоКвартала
		|			И ЗарегистрированныеТорговыеТочки.ТорговаяТочка = ТорговыеТочкиСнятыеСУчета.ТорговаяТочка
		|			И ЗарегистрированныеТорговыеТочки.Организация = ТорговыеТочкиСнятыеСУчета.Организация
		|ГДЕ
		|	ЗарегистрированныеТорговыеТочки.Организация В(&Организации)
		|	И ЗарегистрированныеТорговыеТочки.Период <= &КонецПериода
		|	И ТорговыеТочкиСнятыеСУчета.Период ЕСТЬ NULL 
		|	И НЕ ЗарегистрированныеТорговыеТочки.Организация.ОбособленноеПодразделение";
		
		ОптимизироватьЗапросДляОднойОрганизации(СложноеУсловие.ТекстЗапроса, ОднаОрганизация);
		
		СложноеУсловие.ПараметрыЗапросов.Вставить("КонецПредыдущегоКвартала", НачалоКвартала(Период) - 1);
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.ЗакрытиеСчетов20_23_25_26 Тогда
		...
		Если ЗначениеНастройки(КешНастроек, "ВедетсяПроизводственнаяДеятельность", Организация, Период) Тогда
			Возврат Истина;
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетРезервовПоСомнительнымДолгам Тогда
		 ...
		 Возврат ЗначениеНастройки(КешНастроек, "ФормироватьРезервыПоСомнительнымДолгам", Организация, Период);
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.РаспределениеРасходовПоВидамДеятельностиДляУСН Тогда
		...
		ИначеЕсли НЕ ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Организация)
			И ЗначениеНастройки(КешНастроек, "ПлательщикЕНВД", Организация, Период) Тогда
		...
		Если НачалоГода(Период) <> НачалоКвартала(Период)
			И ЗначениеНастройки(КешНастроек, "МетодРаспределенияРасходовУСН", Организация, Период) = Перечисления.МетодыРаспределенияРасходовУСНПоВидамДеятельности.НарастающимИтогомСНачалаГода Тогда
			...
			|ГДЕ
			|	НЕ РегламентнаяОперация.ПометкаУдаления
			|	И РегламентнаяОперация.Дата МЕЖДУ &НачалоГода И &КонецПредыдущегоКвартала
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.РаспределениеРасходовПоВидамДеятельностиДляИП Тогда
		...
		Возврат ЗначениеНастройки(КешНастроек, "ПлательщикНДФЛ", Организация, Период);
		
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.ВключениеАвансовВПрочиеДоходыДляИП Тогда
		
		Возврат ЗначениеНастройки(КешНастроек, "ПлательщикНДФЛ", Организация, Период); // Периодичность задана в ПериодичностьРегламентнойОперации()
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетРасходовУменьшающихОтдельныеНалоги Тогда
		...
		// периодичность задана в ПериодичностьРегламентнойОперации()
		Возврат УчетнаяПолитика.ПрименяетсяУСНДоходы(Организация, Период)
			Или (Период >= УчетЕНВД.ДатаНачалаПоддержкиЕНВД() И ЗначениеНастройки(КешНастроек, "ПлательщикЕНВД", Организация, Период));
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетНалогаНаПрибыль Тогда
		...
		Возврат ЗначениеНастройки(КешНастроек, "ПлательщикНалогаНаПрибыль", Организация, Период);
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетЕНВД Тогда
		...
		Возврат Период >= УчетЕНВД.ДатаНачалаПоддержкиЕНВД()
			И УчетнаяПолитика.ПлательщикЕНВД(Организация, Период); // Периодичность задана в ПериодичностьРегламентнойОперации()
		
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.СписаниеУбытковПрошлыхЛет Тогда
		
		Если НЕ ЗначениеНастройки(КешНастроек, "ПлательщикНалогаНаПрибыль", Организация, Период) Тогда
			Возврат Ложь;
		КонецЕсли;
...
Функция ЗначениеНастройки(КешНастроек, ИмяНастройки, Организация = Неопределено, Период = Неопределено)
	...
	ИначеЕсли ИмяНастройки = "ВедетсяПроизводственнаяДеятельность" Тогда
		Значение = БухгалтерскийУчетПереопределяемый.ВедетсяПроизводственнаяДеятельность(Организация, Период);
	ИначеЕсли ИмяНастройки = "ПлательщикНалогаНаПрибыль" Тогда
		Значение = УчетнаяПолитика.ПлательщикНалогаНаПрибыль(Организация, Период);
	ИначеЕсли ИмяНастройки = "ПлательщикНДФЛ" Тогда
		Значение = УчетнаяПолитика.ПлательщикНДФЛ(Организация, Период);
	ИначеЕсли ИмяНастройки = "ПлательщикЕНВД" Тогда
		Значение = УчетнаяПолитика.ПлательщикЕНВД(Организация, Период);
	ИначеЕсли ИмяНастройки = "СистемаНалогообложенияОбщая" Тогда
		Если Не КешНастроек.Свойство("СистемаНалогообложения") Тогда
			КешНастроек.Вставить("СистемаНалогообложения", УчетнаяПолитика.СистемаНалогообложения(Организация, Период));
		КонецЕсли;
		Значение = (КешНастроек.СистемаНалогообложения = Перечисления.СистемыНалогообложения.Общая);	
	ИначеЕсли ИмяНастройки = "УСНДоходыМинусРасходы" Тогда
		Если Не КешНастроек.Свойство("СистемаНалогообложения") Тогда
			КешНастроек.Вставить("СистемаНалогообложения", УчетнаяПолитика.СистемаНалогообложения(Организация, Период));
		КонецЕсли;
		Значение = (КешНастроек.СистемаНалогообложения = Перечисления.СистемыНалогообложения.Упрощенная
...
Функция ТекстОшибкиРаноВыполнятьОперацию(ВидОперации, Период, Устарела, Организация = Неопределено) Экспорт
	...
	Иначе
		ПараметрыТекста.Вставить("РазделИнтерфейса", НСтр("ru = 'Учет, налоги, отчетность'"));
	КонецЕсли;
...
Функция ЗапуститьВыполнениеВФоне(Знач ИдентификаторФормы, Знач ИмяЭкспортнойПроцедуры, Знач Параметры,
	...
	Если ОбщегоНазначенияКлиентСервер.РежимОтладки() Тогда // в командной строке /CРежимОтладки
...
Функция ПолучитьСписокСправокРасчетовПоВидуОперации(Знач ВидОперации,Знач Организация,Знач ПериодРегистрации = Неопределено) Экспорт
	...
	ПрименяетсяУСНДоходы                   = УчетнаяПолитика.ПрименяетсяУСНДоходы(Организация, ПериодРегистрации);
	ПрименяетсяЕНВД                        = УчетнаяПолитика.ПлательщикЕНВД(Организация, ПериодРегистрации);
	ПрименяютсяРазныеСтавкиНалогаНаПрибыль = УчетнаяПолитика.ПрименяютсяРазныеСтавкиНалогаНаПрибыль(Организация, ПериодРегистрации);
	...
	Если МассивОпераций.Найти(Перечисления.ВидыРегламентныхОпераций.РасчетНалогаНаПрибыль) <> Неопределено Тогда
		...
		Если ПлательщикНалогаНаПрибыль И ПрименяютсяРазныеСтавкиНалогаНаПрибыль Тогда
...
Функция РеквизитыЗаписиВоВспомогательныеРегистрыСведений(Период, Организация, Регистратор) Экспорт
	...
	РеквизитыДляЗаписиВоВспомогательныеРегистрыСведений.Колонки.Добавить("Регистратор"); // <ДокументСсылка.*>
...
Функция ПодготовитьТаблицуПересчетОНАиОНО(ТаблицаРеквизиты, СписокОрганизаций, Отказ, Реформация = Истина) Экспорт
	...
	Для каждого СтрокаВидАктиваОбязательства Из ТаблицаВидовАктивовИОбязательств Цикл
		...
		// Временные разницы не формируются по прочим доходам и расходам.
...
Функция ПолучитьТекстСообщенияОНеобходимостиРеформации(Период, Организация = Неопределено) Экспорт
	...
	|	И УчетнаяПолитикаСрезПоследних.ПоддержкаПБУ18
	...
	Иначе
		...
		Иначе
			РазделИнтерфейса = НСтр("ru = 'Учет, налоги, отчетность'");
		КонецЕсли;
