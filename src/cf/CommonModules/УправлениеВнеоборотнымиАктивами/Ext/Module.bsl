...
Процедура ЗаполнитьОтчетАвансыИмуществоФормаОтчета2017Кв1(ПараметрыОтчета, Контейнер) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;
		СтраницаРаздел21.П000210004003 = Выборка.КодПоОКОФ;
...
Процедура ЗаполнитьОтчетТранспортныйНалогФормаОтчета2016Кв1(ПараметрыОтчета, Контейнер) Экспорт
	...
	|	И РегистрацияТранспортныхСредств.Период <= &ДатаОкончанияОтчета
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ПО ВТ_Регистрация.ОсновноеСредство = ВТ_Снятые.ОсновноеСредство
	|			И ВТ_Регистрация.ИФНС = ВТ_Снятые.ИФНС
	...
	|	ВТ_ПредыдущееСнятиеСУчета.ИФНС КАК ИФНС,
	...
	|		ПО ВТ_ПредыдущееСнятиеСУчета.ОсновноеСредство = ВТ_Регистрация.ОсновноеСредство
	|			И ВТ_ПредыдущееСнятиеСУчета.ИФНС = ВТ_Регистрация.ИФНС
	...
	|	ВТ_ПредыдущееСнятиеСУчета.ИФНС,
	...
	|	ВТ_Регистрация.ОсновноеСредство КАК ОсновноеСредство,
	|	ВТ_Регистрация.ИФНС КАК ИФНС,
	|	МАКСИМУМ(ВТ_ПервоначальнаяРегистрация.ПериодПервоначальнойРегистрации) КАК ПериодРегистрации,
	|	МАКСИМУМ(ВТ_СнятиеСУчета.ПериодСнятияСУчета) КАК ПериодСнятияСУчета,
	|	ВТ_Регистрация.Период КАК ДатаРегистрационныхДанных
	|ПОМЕСТИТЬ ВТ_РегистрацияТранспортногоСредства
	|ИЗ
	|	ВТ_Регистрация КАК ВТ_Регистрация
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ПервоначальнаяРегистрация КАК ВТ_ПервоначальнаяРегистрация
	|		ПО ВТ_Регистрация.ОсновноеСредство = ВТ_ПервоначальнаяРегистрация.ОсновноеСредство
	|			И ВТ_Регистрация.ИФНС = ВТ_ПервоначальнаяРегистрация.ИФНС
	|			И ВТ_Регистрация.Период = ВТ_ПервоначальнаяРегистрация.Период
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СнятиеСУчета КАК ВТ_СнятиеСУчета
	|		ПО ВТ_Регистрация.ОсновноеСредство = ВТ_СнятиеСУчета.ОсновноеСредство
	|			И ВТ_Регистрация.ИФНС = ВТ_СнятиеСУчета.ИФНС
	...
	|	ЕСТЬNULL(РегистрацияТранспортногоСредства.ПериодРегистрации, ДАТАВРЕМЯ(1, 1, 1)) КАК ПериодРегистрации,
	|	ЕСТЬNULL(РегистрацияТранспортногоСредства.ПериодСнятияСУчета, ДАТАВРЕМЯ(1, 1, 1)) КАК ПериодСнятияСУчета
	|ИЗ
	|	ВТ_ТаблицаРасчетовТранспортногоНалога КАК ТаблицаРасчетовТранспортногоНалога
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_РегистрацияТранспортногоСредства КАК РегистрацияТранспортногоСредства
	|		ПО ТаблицаРасчетовТранспортногоНалога.ОсновноеСредство = РегистрацияТранспортногоСредства.ОсновноеСредство
	|			И ТаблицаРасчетовТранспортногоНалога.ИФНС = РегистрацияТранспортногоСредства.ИФНС
	|			И ТаблицаРасчетовТранспортногоНалога.ДатаРегистрационныхДанных = РегистрацияТранспортногоСредства.ДатаРегистрационныхДанных
	|ГДЕ
	|	ТаблицаРасчетовТранспортногоНалога.ИФНС = &НалоговыйОрган
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Пока ВыборкаОКТМО.Следующий() Цикл
		...
		Пока ВыборкаТранспортныхСредств.Следующий() Цикл
			...
			СтраницаРаздел2.П000020005003 = ВыборкаТранспортныхСредств.Марка;
			СтраницаРаздел2.П000020006003 = ВыборкаТранспортныхСредств.РегистрационныйЗнак;
			СтраницаРаздел2.П000020007003 = ВыборкаТранспортныхСредств.ПериодРегистрации;
			СтраницаРаздел2.П000020008003 = ВыборкаТранспортныхСредств.ПериодСнятияСУчета;
