...
Процедура ЗаполнитьСтавкуНДСТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ИмяТабличнойЧасти, МетаданныеДокумента) Экспорт
	...
	Если ЕстьСчетУчетаНДСПоРеализации 
		...
		ИначеЕсли ЕстьСчетДоходов И БухгалтерскийУчетПовтИсп.СчетОтноситсяКДеятельностиЕНВД(СтрокаТабличнойЧасти.СчетДоходов) Тогда
			// ЕНВД
			СтавкаБезНДС	= Истина;
		КонецЕсли;
...
// Процедура распределяет сумму скидки пропорционально общей сумме по строкам табличной части
//
// Параметры:
//  ТабличнаяЧасть              - табличная часть по которой распределяем скидку
//  ДанныеОбъекта               - данные необходимые для расчета скидки.
//
Процедура РаспределитьСкидкуПоСтрокамТабЧасти(ТабличнаяЧасть, ДанныеОбъекта) Экспорт
	...
	Если ДанныеОбъекта.СуммаСкидки <> 0 Тогда
		...
		Всего = ТабличнаяЧасть.Итог("Сумма") + ?(ДанныеОбъекта.СуммаВключаетНДС, 0, ТабличнаяЧасть.Итог("СуммаНДС"));
		
		Если ДанныеОбъекта.СуммаСкидки >= Всего Тогда
			ТабличнаяЧасть.ЗаполнитьЗначения(0, "Цена, Сумма, СуммаНДС");
			Возврат;
		КонецЕсли;
		
		ВсегоСоСкидкой = Всего - ДанныеОбъекта.СуммаСкидки;
		
		ВсегоПоСтрокам = Новый Массив;
		Для Индекс = 0 По ТабличнаяЧасть.Количество()-1 Цикл
			СтрокаТабличнойЧасти = ТабличнаяЧасть[Индекс];
			
			ВсегоПоСтрокам.Добавить(СтрокаТабличнойЧасти.Сумма + ?(ДанныеОбъекта.СуммаВключаетНДС, 0, СтрокаТабличнойЧасти.СуммаНДС));
		КонецЦикла;
