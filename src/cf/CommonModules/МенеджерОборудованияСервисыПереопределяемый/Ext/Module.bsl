...
Процедура ЗаполнитьНастройкиУстройства(ИДУстройства, Настройки) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПодключаемоеОборудование.Ссылка КАК Устройство
	|ИЗ
	...
	Устройство = Выборка.Устройство;
	Настройки = ПодключаемоеОборудованиеOfflineВызовСервера.ПолучитьНастройкиДляККМОффлайн(Устройство);
...
Процедура ЗаполнитьПрайсЛист(ИДУстройства, СтруктураПрайсЛиста) Экспорт
	...
	Для Каждого СтрокаТЧ Из ТаблицаТоваров Цикл
		...
		ЭлементМассива.Вставить("ИмеетХарактеристики",	Ложь);
		ЭлементМассива.Вставить("ИмеетУпаковки",		Ложь);
		ЭлементМассива.Вставить("СтавкаНДС",			УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(СтрокаТЧ.СтавкаНДС));
		ЭлементМассива.Вставить("УникальныйИдентификатор",СтрокаТЧ.УникальныйИдентификатор);
		
		СтруктураПрайсЛиста.Товары.Добавить(ЭлементМассива);
