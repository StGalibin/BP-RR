...
#Область РазделыС8По9
	...
	Если Контейнер.Свойство("Раздел8_Прил1")
		...
		ПараметрыРаздел81.Вставить("НачалоПериода",                      НачалоКвартала(ПараметрыОтчета.мДатаНачалаПериодаОтчета));
		ПараметрыРаздел81.Вставить("НачалоНалоговогоПериода",            НачалоКвартала(ПараметрыОтчета.мДатаНачалаПериодаОтчета));
	...
	Если Контейнер.Свойство("Раздел9_Прил1")
		...
		ПараметрыРаздел91.Вставить("КонецПериода",
			...
			ПараметрыОтчета.ДатаПодписи, 
			КонецДня(ПараметрыОтчета.ДатаПодписи)));
		ПараметрыРаздел91.Вставить("НачалоПериода",                      НачалоКвартала(ПараметрыОтчета.мДатаНачалаПериодаОтчета));
		ПараметрыРаздел91.Вставить("НачалоНалоговогоПериода",            НачалоКвартала(ПараметрыОтчета.мДатаНачалаПериодаОтчета));
...
Функция ТекстЗапросаРаздел3Приложение1()
	...
	|	Реквизиты.ОбъектНедвижимости.КодРегиона КАК КодРегиона,
	|	ВЫБОР
	...
	|	Реквизиты.ОбъектНедвижимости.КодРегиона,
	|	ВЫБОР
	|		КОГДА ПОДСТРОКА(Реквизиты.ОбъектНедвижимости.НаименованиеПолное, 1, 250) = """"
	|			ТОГДА Реквизиты.ОбъектНедвижимости.Наименование
	|		ИНАЧЕ ПОДСТРОКА(Реквизиты.ОбъектНедвижимости.НаименованиеПолное, 1, 250)
...
Функция ТекстЗапросаРаздел4_6()
	...
	|		ПО НалоговаяБазаПоОтгрузкам.СчетФактура = ПодтверждениеНулевойСтавки.СчетФактураВыданный
	|ГДЕ
	|	НалоговаяБазаПоОтгрузкам.СчетФактура ССЫЛКА Документ.СчетФактураВыданный
