...
Функция ПолучитьМассивВидовДоговоров(СПоставщиком = Ложь, СПокупателем = Ложь, 
	...
	СКомитентомНаЗакупку = Ложь, СКомиссионеромНаЗакупку = Ложь, Прочее = Ложь) Экспорт
...
Функция ПолучитьСведенияОСпискеНоменклатуры(СписокНоменклатуры, ПараметрыОбъекта, ОпределятьСчета = Истина, ОпределятьЦены = Истина) Экспорт
	...
	//ИнтеграцияГИСМ
	ВедетсяУчетМаркированнойПродукции = ПолучитьФункциональнуюОпцию("ВестиУчетМаркируемойПродукцииГИСМБП");
	...
	Для Каждого Номенклатура Из СписокНоменклатуры Цикл
		...
		//ИнтеграцияГИСМ
		Если НЕ ВедетсяУчетМаркированнойПродукции
...
Процедура ЗаполнитьРеквизитыКонтрагента(Реквизиты, Контрагент, Период)
	...
	Если Реквизиты.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		Реквизиты.ФамилияИнициалыФизлица = ФизическиеЛицаКлиентСервер.ФамилияИнициалы(Реквизиты.ПолноеНаименование);
	КонецЕсли;
...
Функция ВидыКонтактнойИнформацииКомпании(ТипКомпании)
	...
	ВидыКонтактнойИнформацииКомпании = Новый Структура("ЮридическийАдрес, ФактическийАдрес, ПочтовыйАдрес, Телефоны, Факс, Email");
...
Процедура ЗаполнитьКонтактнуюИнформациюВРеквизитахКомпании(Реквизиты, Компания, Период)
	...
	// Если фактический и почтовый адреса совпадают с текущим юридическим адресом, то заменяем их на юр. адрес из истории
	УправлениеКонтактнойИнформациейБП.ЗаменитьАдресаНаЮрИзИстории(Компания, КонтактнаяИнформация);
...
Функция ПоказыватьСчетаУчетаВДокументахПоУмолчанию()
	...
	Если ПолучитьФункциональнуюОпцию("ВестиУчетПоПодразделениям") Тогда
		Возврат Истина; // 1С:Бухгалтерия КОРП
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ВестиУчетПоОрганизациям") Тогда
		Возврат Истина; // 1С:Бухгалтерия ПРОФ
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ПредпринимательВСервисе") Тогда
		Возврат Ложь; // 1С:Предприниматель 2015 в сервисе
	КонецЕсли;
	
	Возврат (Перечисления.СистемыНалогообложения.ОсновнаяСистемаНалогообложения() = Перечисления.СистемыНалогообложения.Общая);
