...
Процедура СформироватьДвиженияОказаниеУслуг(ТаблицаРеализованныеТоварыУслуги, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если НЕ ПлательщикНДС Тогда // Организация на УСН
		...
		Если ТаблицаНачисленийДляНДС.Итог("СуммаНДСРуб") <> 0 Тогда
...
Процедура СформироватьДвиженияРозничнаяВыручка(ТаблицаНачислениеНДС, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если НЕ ПлательщикНДС Тогда // Организация на УСН
		...
		Если Параметры.ТаблицаНачислениеНДС.Итог("НДС") <> 0 Тогда
...
Процедура СформироватьДвиженияВозвратТоваровПоставщику(ТаблицаРеализованныеТоварыУслуги, ТаблицаТовары, ТаблицаСписаниеГТД, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если НЕ ПлательщикНДС Тогда // Организация на УСН
		...
		Если Параметры.РеализованныеТовары.Итог("СуммаНДСРуб") <> 0 Тогда 
...
Процедура СформироватьДвиженияПоступлениеТоваровУслугОтПоставщика(ТаблицаТовары, ТаблицаУслуги, ТаблицаНомераГТД, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если ДанныеДвижений.Итог("НДС") <> 0 ИЛИ Реквизиты.ПредъявляетНДС Тогда
		СформироватьДвиженияНДСПредъявленныйПоступлениеТоваровУслуг(ДанныеДвижений, Движения, Отказ);
		Если Реквизиты.НДСВключенВСтоимость Тогда
			СформироватьДвиженияНДСПредъявленныйВключениеВСтоимость(ДанныеДвижений, Движения,, Отказ);
		КонецЕсли;
	...
	Если ДвиженияРаздельныйУчетНДС Тогда
		...
		Если НЕ НТТПоПродажнойСтоимости 
			  И Реквизиты.ВидОперации <> Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ОсновныеСредства Тогда
...
// Возвращает признак ведения регистров для заполнения раздела 7 Декларации НДС
//
// Параметры
// Дата     - тип дата, в данном параметре передается
//            дата на которую необходимо определить признак ведения 
//            регистров для заполнения раздела 7 Декларации НДС
// Возвращаемое значение:
//  Булево -  признак применения
//            Истина - ведутся регистры для раздела 7 Декларации НДС
//            Ложь   - не ведутся регистры для раздела 7 Декларации НДС
//
Функция ЗаполняетсяРаздел7ДекларацииНДС(Дата) Экспорт
	
	НачалоЗаполненияРаздела7Декларации = '20170401';
	
	Если Дата < НачалоЗаполненияРаздела7Декларации Тогда
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
...
Функция ПодготовитьТаблицуСторноНаВыданныйАванс(ТаблицаРеквизиты, Отказ) Экспорт
	...
	|ВЫБРАТЬ
	|	СторноВычета.Период,
	|	СторноВычета.Регистратор,
	|	СторноВычета.Содержание,
	|	СторноВычета.Организация,
	|	СторноВычета.Контрагент,
	|	СторноВычета.СчетФактура,
	|	СторноВычета.ВидЦенности,
	|	СторноВычета.СтавкаНДС,
	|	СторноВычета.ДатаОплаты,
	|	СторноВычета.ДокументОплаты,
	|	СторноВычета.Событие,
	|	СторноВычета.ДатаСобытия,
	|	ВЫБОР
	...
	|	СторноВычета.ДоговорКонтрагента,
	|	СторноВычета.ИсправленныйСчетФактура,
	|	СторноВычета.СуммаБезНДС,
	|	СторноВычета.СуммаНДС,
	|	СторноВычета.СчетУчетаНДС,
	...
	Для Каждого СтрокаВзаиморасчетов Из Параметры.Взаиморасчеты Цикл
		...
		Если СтрокаВзаиморасчетов.РасчетыВУсловныхЕдиницах = Истина
			...
			Если НЕ ЗначениеЗаполнено(СтрокаТаблицыСуммовыхРазниц.СтавкаНДС) Тогда
				СтрокаТаблицыСуммовыхРазниц.СтавкаНДС = Перечисления.СтавкиНДС.НДС18;
			КонецЕсли;
...
Процедура СформироватьДвиженияСписаниеНДС(ТаблицаРеквизиты, ТаблицаДокумента, Движения, Отказ) Экспорт
	...
	Если Реквизиты.СписатьНДС Тогда // Для документа "Восстановление НДС" списание НДС - опция.
...
Процедура СформироватьКорректировочныеДвиженияКорректировкаРеализации(Реквизиты, ТаблицаНДСВычет, ТаблицаНДСВосстановление, Движения, Отказ)
	...
	СформироватьДвиженияПредъявленНДС(ТаблицаНДСВычет, Движения, Отказ, Истина);
	
	СформироватьДвиженияЗаписьКнигиПродаж(ТаблицаНДСВосстановление, Движения, Отказ);
	
КонецПроцедуры

Процедура СформироватьДвиженияПредъявленНДС(ТаблицаПредъявленногоНДС, Движения, Отказ, ЗаполнятьВидЦенности = Ложь, Сторно = Ложь)
	...
	Для каждого СтрокаДвижения Из ТаблицаПредъявленногоНДС Цикл
		...
		Если ЕстьСчетДоходов И БухгалтерскийУчетПовтИсп.СчетОтноситсяКДеятельностиЕНВД(СтрокаДвижения.СчетДоходов) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СформироватьИсправительныеДвиженияКорректировкаРеализации(Реквизиты, ТаблицаНДСНачислениеНДС, ТаблицаНДСВычет, ТаблицаНДСВосстановление, ТаблицаНДССторноКнигиПокупок, ТаблицаНДСОстаткиНДСПредъявленный, Движения, Отказ)
	...
	Если ТипЗнч(ИсправляемыйДокумент) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		...
		// Сторно движений исправляемого документа
		СформироватьДвиженияПредъявленНДС(ТаблицаНДСОстаткиНДСПредъявленный, Движения, Отказ, , Истина);
...
Процедура СформироватьДвиженияНДСРеализация0РеализацияТоваровУслуг(ТоварыУслуги, Реквизиты, Движения, Отказ)
	...
	ТоварыУслуги0 = ТоварыУслуги.Скопировать(
		Новый Структура("СтавкаНДС", Перечисления.СтавкиНДС.НДС0), 
		"Контрагент,ВидЦенности,СтавкаНДС,ЭтоНесырьевойТовар,СуммаБезНДСРуб,СуммаБУ");
...
Процедура СформироватьДвиженияНДСРеализация0РеализацияТоваровУслугКомиссионером(ТоварыУслуги, Реквизиты, Движения, Отказ)

	ТоварыУслуги.Индексы.Добавить("СтавкаНДС");
	ТоварыУслуги0 = ТоварыУслуги.Скопировать(
		Новый Структура("СтавкаНДС", Перечисления.СтавкиНДС.НДС0), 
		"СчетФактура,Контрагент,Покупатель,ВыставленСФ,ВидЦенности,СтавкаНДС,ЭтоНесырьевойТовар,СуммаБезНДСРуб,СуммаБУ");
...
Процедура СформироватьДвиженияНДСЗаписиКнигиПродажРеализацияТоваровУслуг(ТоварыУслуги, Реквизиты, Движения, Отказ)
	...
	Для каждого СтрокаТаблицы Из ТоварыУслуги Цикл
		...
		Если ПлательщикНДС
			И БухгалтерскийУчетПовтИсп.СчетОтноситсяКДеятельностиЕНВД(СтрокаТаблицы.СчетДоходов) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СформироватьДвиженияНДСЗаписиКнигиПродажРеализацияТоваровУслугКомиссионером(ТоварыУслуги, Реквизиты, Движения, Отказ)
	...
	Для каждого СтрокаТаблицы Из ТоварыУслуги Цикл
		...
		Если БухгалтерскийУчетПовтИсп.СчетОтноситсяКДеятельностиЕНВД(СтрокаТаблицы.СчетДоходов) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СформироватьДвиженияОшибочныеРеквизитыКонтрагентов(ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Параметры.ТаблицаОшибочныеРеквизитыКонтрагентов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОтразитьКорректировкуПоступленияВКнигеПродаж(Реквизиты, ТаблицаВосстановление, Движения, Отказ)
	...
	Если ТаблицаВосстановление <> Неопределено Тогда
...
Процедура ОтразитьКорректировкуПоступленияВКнигеПокупок(
	...
	Если ВычетНДСНаОснованииДокумента <> Неопределено 
		...
		ИЛИ Реквизиты.ПоставщикПредъявляетНДС) Тогда
		...
		Если Реквизиты.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.ИсправлениеСобственнойОшибки 
			И СтруктураТаблиц.ТаблицаНДСВычетСУчетомПропорции.Количество() > 0 Тогда
...
Функция ПодготовитьТаблицыВычетНДССУчетомПропорции(Реквизиты, ВычетНДСНаОснованииДокумента, ТаблицаСторноВычета)
	...
	Если ТаблицаСторноВычета.Количество() > 0 Тогда
		...
		ТаблицаНДСВычетСУчетомПропорции = ТаблицаНДСПредъявленыйСУчетомПропорции.Скопировать();
		...
		ТаблицаНДСВычетСУчетомПропорции.ЗаполнитьЗначения(
		...
		ТаблицаНДСВычетСУчетомПропорции.ЗаполнитьЗначения(НСтр("ru='НДС'"), "Содержание");
		ТаблицаНДСВычетСУчетомПропорции.ЗаполнитьЗначения(Перечисления.СобытияПоНДСПокупки.ПредъявленНДСКВычету, "Событие");
		ТаблицаНДСВычетСУчетомПропорции.ЗаполнитьЗначения(СтрокаСторнирующейЗаписи.ДатаСобытия, "ДатаСобытия");
...
Функция ПодготовитьДанныеШапкиСчетаФактуры1137(ВыборкаСФ, Реквизиты, ПлатежноРасчетныеДокументы, ДополнитьДаннымиУПД)
	...
	ЭтоСчетФактураНаАванс = ВыборкаСФ.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаАванс;
	...
	Если ВыборкаСФ.ЭтоСчетФактураВыданный Тогда
		...
		КонецЕсли;
		Если Не ЭтоСчетФактураНаАванс И ДополнитьДаннымиУПД Тогда
	...
	Если Не ЭтоСчетФактураНаАванс И ДополнитьДаннымиУПД Тогда
		...
		Если Реквизиты.ПеревозкаАвтотранспортом И ЗначениеЗаполнено(Реквизиты.ДокументОснование) Тогда
			...
			НомерОснования     = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(РеквизитыОснования.Номер, Истина, Ложь);
...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц)
	...
	|	ЕСТЬNULL(Номенклатура.КодТНВЭД.Код, """") КАК ТоварКодТНВЭД,
	...
	|	ТаблицаДокумента.ПериодичностьУслуги
	|ИЗ
	|	ТаблицаДокумента КАК ТаблицаДокумента
...
Функция ТекстЗапросаДанныеДляПечатиКорректировочныхСчетовФактур(НомераТаблиц)
	...
	|	Номенклатура.КодТНВЭД.Код КАК ТоварКодТНВЭД,
...
Функция ПолучитьДанныеДляПечатиСчетаФактуры1137(МассивОбъектов, ТекстЗапросаПоСчетамФактурам, ДополнитьДаннымиУПД = Ложь) Экспорт
	...
	Пока ВыборкаСФ.Следующий() Цикл
		...
		Если ВыборкаСФ.Дата >= Дата(2016, 07, 01) И ВыборкаСФ.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаРеализацию Тогда
			Для Каждого СтрокаДокумента Из ТаблицаДокумента Цикл
...
Функция ПолучитьДанныеДляПечатиКорректировочныхСчетовФактур1137(МассивОбъектов, ТекстЗапросаПоСчетамФактурам, ДополнитьДаннымиУПД = Ложь) Экспорт
	...
	Пока ВыборкаСФ.Следующий() Цикл
		...
		Если ВыборкаСФ.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.Корректировочный
		   И ВыборкаСФ.ДатаИсходногоДокумента >= Дата(2016, 07, 01) Тогда
			Для Каждого СтрокаДокумента Из ТаблицаДокумента Цикл
				Если СтрокаДокумента.СтавкаНДС = Перечисления.СтавкиНДС.НДС0 И СокрЛП(СтрокаДокумента.ТоварКодТНВЭД) <> "" Тогда
					МассивСтрок = Новый Массив();
...
Процедура ПечатьКомплектаСчетовФактур(КоллекцияПечатныхФорм, МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Для Каждого МакетСчетаФактуры Из МакетыСчетовФактур Цикл
		...
		Если МакетСчетаФактуры.Ключ = "СчетФактура" Тогда
			...
			ТабДокумент = УчетНДСБП.ПечатьСчетовФактур(МассивОбъектовПечатьСчетовФактур, ОбъектыПечатиСчетаФактуры,
				Документы.СчетФактураВыданный.ТекстЗапросаПечатьСчетовФактур(Ложь));
		ИначеЕсли МакетСчетаФактуры.Ключ = "СчетФактура1137" Тогда
			...
			ТабДокумент = УчетНДС.ПечатьСчетовФактур1137(МассивОбъектовПечатьСчетовФактур, ОбъектыПечатиСчетаФактуры,
...
Функция ПечатьКомплектаУПД(КоллекцияПечатныхФорм, МассивОбъектов, ОбъектыПечати, ИменаФайлов) Экспорт
	...
	МассивСтрокКоллекции = КоллекцияПечатныхФорм.НайтиСтроки(Новый Структура("ИмяМакета","УниверсальныйПередаточныйДокументКомплект"));
	Для Каждого СтрокаКоллекции Из МассивСтрокКоллекции Цикл
		СтрокаКоллекции.ИмяМакета = "УниверсальныйПередаточныйДокумент";
	...
	Для Каждого МакетУПД Из МакетыУПД Цикл
		...
		ИначеЕсли МакетУПД.Ключ = "УПДСтатус2" Тогда
			...
			Если НЕ ПустаяСтрока(ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов) Тогда
				ТабДокумент = УчетНДС.ПечатьУниверсальныхПередаточныхДокументов(МассивОбъектовПечатьУПД, ОбъектыПечатиУПД, 
...
Процедура СформироватьПроводкиСписаниеНДСПоАренднымОбязательствам(ДанныеДвижений, РеквизитыПроводок, НДСВключенВСтоимость, Движения, Отказ)
	...
	Для каждого СтрокаТаблицы Из ДанныеДвижений Цикл
		...
		Если НДСВключенВСтоимость Тогда
			...
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт,
				1, ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.СписаниеВыделенногоНДСНаПрочиеРасходы"));
...
Процедура СформироватьДвиженияПоступлениеПредметовЛизингаОтПоставщика(ТаблицаПредметыЛизинга, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Параметры.ПредметыЛизинга.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Функция ФорматныйКонтрольИННиКППКонтрагентаПройден(Контрагент, ТипСчетФактуры, ВыводитьСообщенияАдресно = Истина) Экспорт
	...
	// Не выполняем проверку для контрагентов-иностранцев
	Если СтруктураДляПроверки.СтранаРегистрации <> ПредопределенноеЗначение("Справочник.СтраныМира.Россия") Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ФорматныйКонтрольИННиКПППоСчетуФактуреПройден(Объект) Экспорт
	...
	// Не выполняем проверки для Корректировочных счет-фактур
	...
	ИначеЕсли ТипЗнч(Объект.ВидСчетаФактуры) = Тип("ПеречислениеСсылка.ВидСчетаФактурыПолученного") 
		И Объект.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.Корректировочный Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура СформироватьДвиженияТаможеннаяДекларацияЭкспорт(ТаблицаСведения, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Параметры.ТаблицаСведения.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Функция ПравоРегистрироватьВидСчетаФактуры(ВидСФ) Экспорт
	...
	ИначеЕсли ВидСФ = Перечисления.ВидСчетаФактурыВыставленного.НаАванс
		...
		Или ВидСФ = Перечисления.ВидСчетаФактурыВыставленного.НаАвансКомитентаНаЗакупку Тогда
