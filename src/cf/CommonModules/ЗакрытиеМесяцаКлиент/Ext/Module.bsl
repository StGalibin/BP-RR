...
Процедура ОбработатьРезультатАктуализации(Форма, Организация, Период = Неопределено) Экспорт
	...
	Если РезультатАктуализации.Выполнено Тогда
		ПараметрыОповещения = Новый Структура("Организация", Организация);
		Если РезультатАктуализации.ПолнаяАктуализация Тогда
			// После повторного формирования данные всех открытых отчетов станут актуальны
			// (т.е. без необходимости ещё раз актуализировать).
		...
		Иначе
			...
			БухгалтерскийУчетКлиентПереопределяемый.ОбработкаОповещенияАктуализации(
				Форма, Организация, РезультатАктуализации.ПределАктуализации, "АктуализацияЗавершенаУспешно", ПараметрыОповещения, Форма);
		КонецЕсли;
	Иначе 
...
//	ФормаОтчета                   - УправляемаяФорма - Форма отчета, в котором проводилась проверка актуальности.
...
Процедура ПроверитьАктуальностьДанных(Форма, Организация, Период = Неопределено)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "Отчет") Тогда
		...
		ОтборыРасчетыСКонтрагентами = БухгалтерскиеОтчетыКлиентСервер.ОпределитьПараметрыОтчетаРасчетыСКонтрагентами(Форма);
		ПараметрыПроверки.Вставить("ОтборыРасчетыСКонтрагентами", ОтборыРасчетыСКонтрагентами);
...
//
// Параметры:
//  ФормаСоставнойДлительнойОперации - УправляемаяФорма - форма с прогресс-баром.
//
// Возвращаемое значение:
...
// 
// Параметры:
//  ФормаСоставнойДлительнойОперации - УправляемаяФорма - форма, из которой производилось открытие. 
//
