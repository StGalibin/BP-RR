...
//
Функция НалоговаяСтавкаФедеральныйБюджет(Период, Организация) Экспорт
...
Функция ПолучитьСтавкуНалогаНаПрибыль(СтруктураШапкиДокумента, ИФНС = Неопределено, СтавкаФБ = 0, СтавкаРФ = 0, СтавкаМестный = 0) Экспорт   
	...
	СтавкиНалога = РегистрыСведений.НастройкиУчетаНалогаНаПрибыль.ПолучитьПоследнее(СтруктураШапкиДокумента.Дата, Новый Структура("Организация", СтруктураШапкиДокумента.Организация));
	...
	Если СтавкиНалога.ПрименяютсяРазныеСтавкиНалогаНаПрибыль Тогда
		...
		Если ИФНС <> Неопределено Тогда
			НалоговыйУчетОбособленныхПодразделений.ОпределитьНалоговуюСтавку(
				СтавкаНалогаПоИФНС, СтруктураШапкиДокумента.Организация, ИФНС, СтруктураШапкиДокумента.Дата); 
		КонецЕсли;
...
Функция ОстаткиВременныхРазницПоВидуАктивовОбязательств(СтрокаВидАктиваОбязательства,
	...
	Счета84 = Новый Массив;
	Счета84.Добавить(ПланыСчетов.Хозрасчетный.НераспределеннаяПрибыль);
	Счета84.Добавить(ПланыСчетов.Хозрасчетный.ПрибылиИУбыткиОтПересчетаОНА_ОНО);
	...
	Запрос.УстановитьПараметр("МассивСчетов",           СтрокаВидАктиваОбязательства.Счета); // счета с субсчетами
	...
	Запрос.УстановитьПараметр("Счета84",                БухгалтерскийУчет.СформироватьМассивСубсчетовПоОтбору(Счета84));
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&КонецГода, Счет В (&МассивСчетов), &ВидСубконто, Организация В (&Организации)) КАК ОстаткиВР
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ДоляТранспортныхРасходовТекущегоМесяца(Период, Организация)
	...
	СчетаТоваров = ОбщегоНазначенияКлиентСервер.СократитьМассив(СчетаТоваров, СчетаТарыПодТоваром); // удаляем счета тары
...
Процедура ПолучитьСведенияОПоказателяхОтчетаПрибыльФормаОтчета2015Кв1(ПоказателиОтчета, ИДРедакцииОтчета) Экспорт
...
Процедура ЗаполнитьОтчетПрибыльФормаОтчета2015Кв1(ПараметрыОтчета, Контейнер, ТаблицаРасшифровки, ИДРедакцииОтчета) Экспорт
	...
	|ВЫБРАТЬ
	|	ПрочиеДоходыИРасходы.Ссылка,
	|	ПрочиеДоходыИРасходы.ВидДеятельностиДляНалоговогоУчетаЗатрат = &ЗатратыРаспределяемые
	|ИЗ
	|	Справочник.ПрочиеДоходыИРасходы КАК ПрочиеДоходыИРасходы
	|ГДЕ
	|	(ПрочиеДоходыИРасходы.ВидДеятельностиДляНалоговогоУчетаЗатрат = &ЗатратыОсновнаяСистема
	|			ИЛИ ПрочиеДоходыИРасходы.ВидДеятельностиДляНалоговогоУчетаЗатрат = &ЗатратыРаспределяемые)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СтатьяРасходов
	|;
	...
	|			ТОГДА ВЫРАЗИТЬ(ХозрасчетныйОбороты.Субконто1 КАК Справочник.ОсновныеСредства).АмортизационнаяГруппа
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК АмортизационнаяГруппа,
	|	ВЫБОР
	|		КОГДА &НелинейныйМетод
	|			ТОГДА ВЫРАЗИТЬ(ХозрасчетныйОбороты.Субконто1 КАК Справочник.ОсновныеСредства).ГруппаОС
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ГруппаОС,
	|	СУММА(ХозрасчетныйОбороты.СуммаНУОборотКт) КАК АмортизацияОсновныхСредств,
	|	0 КАК АмортизацияНематериальныхАктивов
...
Процедура ОпределитьРазделыСВариантамиОтчетов(Разделы) Экспорт
	...
	Разделы.Добавить(Метаданные.Подсистемы.ПростойИнтерфейс.Подсистемы.Отчеты.Подсистемы.СправкиРасчеты, НСтр("ru = 'Справки-расчеты'"));
	Разделы.Добавить(Метаданные.Подсистемы.ПростойИнтерфейс.Подсистемы.Отчеты.Подсистемы.РегистрыБУСубъектовМалогоПредпринимательства, НСтр("ru = 'Регистры бухгалтерского учета субъектов малого предпринимательства'"));
...
Процедура СписокОтчетовПоддерживаемыхПодсистемой(СписокОтчетов) Экспорт
	...
	СписокОтчетов.Добавить("Отчет.СправкаРасчетРезервыОтпусков");
	...
	СписокОтчетов.Добавить("Отчет.СправкаРасчетТорговогоСбора");
	СписокОтчетов.Добавить("Отчет.СправкаРасчетПризнаниеРасходовПоОСПоступившимВЛизинг");
	СписокОтчетов.Добавить("Отчет.СправкаРасчетЕдиногоНалогаНаВмененныйДоход");
	СписокОтчетов.Добавить("Отчет.СправкаРасчетРасходовУменьшающихОтдельныеНалоги");
