...
//  	    * РегистрацияВНалоговомОргане - необязательная, может отсутствовать.
...
//
Процедура СформироватьДоходыНДФЛПоКодамДоходовИзТаблицыЗначений(Движения, Отказ, Организация, ДатаОперации, Начисления, Записывать = Ложь, ОкончательныйРасчет = Истина, Регистратор = Неопределено) Экспорт
	...
	Если Начисления.Колонки.Найти("РегистрацияВНалоговомОргане") = Неопределено Тогда
		НачисленияСРегистрациями = ТаблицаДвиженийСРегистрациейВНалоговомОргане(Начисления, "ДатаПолученияДохода");
	Иначе
		НачисленияСРегистрациями = Начисления;
	КонецЕсли;
	РегистрацияОрганизации = ЗарплатаКадры.РегистрацияВНалоговомОргане(Организация, ДатаОперации);
	
	Для Каждого СтрокаДанных Из НачисленияСРегистрациями Цикл
...
//
Процедура СформироватьДоходыНДФЛПоНачислениям(Движения, Отказ, Организация, ДатаОперации, ДатаВыплаты, МенеджерВременныхТаблиц, МесяцНачисления = Неопределено, Записывать = Ложь, ОкончательныйРасчет = Истина, ИмяВТНачисления = "ВТНачисления", Регистратор = Неопределено, МенеджерКадровогоУчета = Неопределено) Экспорт
	...
	СформироватьВТРегистрацийПодразделенийВНалоговомОргане(Запрос.МенеджерВременныхТаблиц, "ВТДатыПолученияДохода", "ДатаПолученияДохода");
		
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ДатыПолученияДохода.ФизическоеЛицо,
	|	ДатыПолученияДохода.КодДохода,
	|	ДатыПолученияДохода.ДатаПолученияДохода,
	|	ДатыПолученияДохода.МесяцНалоговогоПериода,
	|	ДатыПолученияДохода.СуммаДохода,
	|	ДатыПолученияДохода.КодВычета,
	|	ДатыПолученияДохода.СуммаВычета,
	|	ДатыПолученияДохода.Подразделение,
	...
	|	ДатыПолученияДохода.Сотрудник,
	|	ДатыПолученияДохода.Начисление
	|ИЗ
	...
	|		ПО ДатыПолученияДохода.Подразделение = ИсторияРегистрацийВНалоговомОрганеСрезПоследних.СтруктурнаяЕдиница
	|			И ДатыПолученияДохода.ДатаПолученияДохода = ИсторияРегистрацийВНалоговомОрганеСрезПоследних.Период";
	...
	ТаблицыКУдалению.Добавить("ВТИсторияРегистрацийВНалоговомОрганеСрезПоследних");
...
//		Регистратор - ДокументСсылка - 
//
Процедура СформироватьДоходыНДФЛПоНачислениямСПланируемойДатойВыплаты(Движения, Отказ, Организация, ДатаОперации, МенеджерВременныхТаблиц, МесяцНачисления = Неопределено, Записывать = Ложь, ОкончательныйРасчет = Истина, ИмяВТНачисления = "ВТНачисления", Регистратор = Неопределено) Экспорт
	
	СформироватьДоходыНДФЛПоНачислениям(Движения, Отказ, Организация, ДатаОперации, Неопределено, МенеджерВременныхТаблиц, МесяцНачисления, Записывать, ОкончательныйРасчет, ИмяВТНачисления, Регистратор); 
...
Процедура СформироватьПредоставленныеИмущественныеВычетыПоВременнойТаблице(Движения, Отказ, Организация, ДатаОперации, МенеджерВременныхТаблиц, Записывать = Ложь, ОкончательныйРасчет = Истина) Экспорт
	...
	Запрос.УстановитьПараметр("ДатаОперации",ДатаОперации);
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	СформироватьВТРегистрацийПодразделенийВНалоговомОргане(МенеджерВременныхТаблиц, "ВТДанныеОбИмущественныхВычетах", "МесяцНалоговогоПериода");
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	|			И Вычеты.МесяцНалоговогоПериода = ИсторияРегистрацийВНалоговомОрганеСрезПоследних.Период
	|ГДЕ
	|	Вычеты.Сумма <> 0";
	Результат = Запрос.Выполнить();
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТИсторияРегистрацийВНалоговомОрганеСрезПоследних");
...
Процедура СформироватьСтандартныеВычетыПоВременнойТаблице(Движения, Отказ, Организация, ДатаОперации, МенеджерВременныхТаблиц, Записывать = Ложь, ОкончательныйРасчет = Истина) Экспорт
	...
	СформироватьВТРегистрацийПодразделенийВНалоговомОргане(МенеджерВременныхТаблиц, "ВТДанныеОСтандартныхВычетах", "МесяцНалоговогоПериода");
	РегистрацияВНалоговомОргане = ЗарплатаКадры.РегистрацияВНалоговомОргане(Организация, ДатаОперации);
...
Процедура СформироватьИсчисленныйНалогПоВременнойТаблице(Движения, Отказ, Организация, ДатаОперации, ИсчисленныйНалог, Записывать = Ложь, ОкончательныйРасчет = Истина) Экспорт
	...
	СформироватьВТРегистрацийПодразделенийВНалоговомОргане(ИсчисленныйНалог, "ВТНДФЛИсчисленный", "МесяцНалоговогоПериода");
	РегистрацияВНалоговомОргане = ЗарплатаКадры.РегистрацияВНалоговомОргане(Организация, ДатаОперации);
	...
	|			И Налоги.МесяцНалоговогоПериода = ИсторияРегистрацийВНалоговомОрганеСрезПоследних.Период
	|ГДЕ
	|	Налоги.Сумма <> 0";
	...
	ЗарплатаКадры.УничтожитьВТ(ИсчисленныйНалог, "ВТИсторияРегистрацийВНалоговомОрганеСрезПоследних");
...
Процедура СформироватьУдержанныйНалогПоВременнойТаблице(Движения, Отказ, Организация, ДатаОперации, УдержанныйНалог, РегистрацияВНалоговомОргане = Неопределено, Записывать = Ложь) Экспорт
	...
	|	Налоги.Сумма,
	|	Налоги.ВключатьВДекларациюПоНалогуНаПрибыль,
	|	Налоги.СрокПеречисления,
	|	Налоги.СуммаВыплаченногоДохода,
	|	Налоги.СуммаОтпускныхБольничных,
	|	Налоги.СуммаОблагаемогоДохода,
	|	Налоги.ДокументОснование,
	|	Налоги.УчитыватьВыплаченныйДоходВ6НДФЛ,
	|	ВЫБОР
	|		КОГДА Налоги.КодДохода = ЗНАЧЕНИЕ(Справочник.ВидыДоходовНДФЛ.Код1010)
	|			ТОГДА Налоги.КодДохода
	...
	|		ИНАЧЕ ИсторияРегистрацийВНалоговомОрганеСрезПоследних.РегистрацияВНалоговомОргане
	|	КОНЕЦ КАК РегистрацияВНалоговомОргане
	...
	|			И Налоги.КодДохода = СтавкаНДФЛПоСтавкеРезидента.КодДохода
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТИсторияРегистрацийВНалоговомОрганеСрезПоследних КАК ИсторияРегистрацийВНалоговомОрганеСрезПоследних
	|		ПО Налоги.Подразделение = ИсторияРегистрацийВНалоговомОрганеСрезПоследних.СтруктурнаяЕдиница
	|			И (ВЫБОР
	...
	|				ИНАЧЕ &ДатаОперации
	|			КОНЕЦ = ИсторияРегистрацийВНалоговомОрганеСрезПоследних.Период)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Если ИмеющиесяКолонки.Найти("Ставка") = Неопределено Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Налоги.Ставка,", "СтавкаНДФЛПоСтавкеРезидента.СтавкаНДФЛ КАК Ставка,");
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	Налоги.ФизическоеЛицо КАК ФизическоеЛицо,
		|	Налоги.СтавкаНалогообложенияРезидента,
	...
	КонецЕсли;
	Если ИмеющиесяКолонки.Найти("ВключатьВДекларациюПоНалогуНаПрибыль") = Неопределено Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Налоги.ВключатьВДекларациюПоНалогуНаПрибыль", "Неопределено КАК ВключатьВДекларациюПоНалогуНаПрибыль")
	КонецЕсли;
	Если ИмеющиесяКолонки.Найти("ДокументОснование") = Неопределено Тогда
		...
		Если ИмеющиесяКолонки.Найти("УчитыватьВыплаченныйДоходВ6НДФЛ") = Неопределено Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Налоги.УчитыватьВыплаченныйДоходВ6НДФЛ", "ЛОЖЬ КАК УчитыватьВыплаченныйДоходВ6НДФЛ")
		КонецЕсли;
	Иначе 	
		Если ИмеющиесяКолонки.Найти("УчитыватьВыплаченныйДоходВ6НДФЛ") = Неопределено Тогда
			Запрос.Текст = 
	...
	Если ИмеющиесяКолонки.Найти("СрокПеречисления") = Неопределено Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Налоги.СрокПеречисления", "Неопределено КАК СрокПеречисления")
	Иначе 	
		ВычислятьСрокПеречисления = Ложь;
	КонецЕсли;
	Если ИмеющиесяКолонки.Найти("СуммаВыплаченногоДохода") = Неопределено Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Налоги.СуммаВыплаченногоДохода", "0 КАК СуммаВыплаченногоДохода")
	КонецЕсли;
	Если ИмеющиесяКолонки.Найти("СуммаОблагаемогоДохода") = Неопределено Тогда
		Если ИмеющиесяКолонки.Найти("СуммаВыплаченногоДохода") = Неопределено Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Налоги.СуммаОблагаемогоДохода", "0 КАК СуммаОблагаемогоДохода")
		Иначе
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Налоги.СуммаОблагаемогоДохода", "Налоги.СуммаВыплаченногоДохода КАК СуммаОблагаемогоДохода")
		КонецЕсли;
	КонецЕсли;
	Если ИмеющиесяКолонки.Найти("СуммаОтпускныхБольничных") = Неопределено Тогда
		Если ИмеющиесяКолонки.Найти("СуммаВыплаченногоДохода") = Неопределено Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Налоги.СуммаОтпускныхБольничных", "0 КАК СуммаОтпускныхБольничных")
		Иначе
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Налоги.СуммаОтпускныхБольничных", "Налоги.СуммаВыплаченногоДохода КАК СуммаОтпускныхБольничных")
		КонецЕсли;
	КонецЕсли;
	Если ИмеющиесяКолонки.Найти("РегистрацияВНалоговомОргане") = Неопределено Тогда
		...
		ДатаОперацииТекст = "ДАТАВРЕМЯ(" + Формат(Год(ДатаОперации),"ЧЦ=4; ЧГ=0") + "," + Месяц(ДатаОперации) + "," + День(ДатаОперации) + ", 0, 0, 0)";
		СформироватьВТРегистрацийПодразделенийВНалоговомОргане(УдержанныйНалог, "ВТНалогУдержанный", "ВЫБОР
																									|	КОГДА НалогУдержанный.МесяцНалоговогоПериода > " + ДатаОперацииТекст + "
																									|		ТОГДА НалогУдержанный.МесяцНалоговогоПериода
																									|		ИНАЧЕ " + ДатаОперацииТекст + "
																									|	КОНЕЦ");
	...
	Иначе
		...
		"ВЫБРАТЬ ПЕРВЫЕ 0
		|	ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0) КАК Период,
		|	ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка) КАК СтруктурнаяЕдиница,
		|	ЗНАЧЕНИЕ(Справочник.РегистрацииВНалоговомОргане.ПустаяСсылка) КАК РегистрацияВНалоговомОргане
		|ПОМЕСТИТЬ ВТИсторияРегистрацийВНалоговомОрганеСрезПоследних";
		Запрос.Выполнить()
	КонецЕсли;
	Запрос.Текст = ТекстЗапроса;
	...
	Для каждого СтрокаТЗ Из Результаты Цикл
		...
		НоваяСтрокаРасчетовНалоговыхАгентов = СтрокаРасчетовНалоговыхАгентовСБюджетом(Движения, Организация, ДатаОперации, ВидДвиженияНакопления.Приход, НоваяСтрокаРасчетовНалогоплательщика, СтрокаТЗ, НоваяСтрокаРасчетовНалогоплательщика.КодДохода);
	...
	ТаблицыКУдалению = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("ВТИсторияРегистрацийВНалоговомОрганеСрезПоследних");
	ТаблицыКУдалению.Добавить("ВТСтавкаНДФЛПоСтавкеРезидента");
...
Процедура СформироватьНалогиВычеты(Движения, Отказ, Организация, ДатаОперации, ДанныеДокумента, Записывать = Ложь, ОкончательныйРасчет = Истина) Экспорт
	...
	Если КолонкиДанных.Найти("НалогПоСтавке13") <> Неопределено Тогда
		СформироватьИсчисленныйНалогПоТаблицеЗначений(Движения, Отказ, Организация, ДатаОперации, НалогиИзДанныхДокумента(ДанныеСРегистрациями, "НалогПоСтавке13", Перечисления.НДФЛСтавкиНалогообложенияРезидента.Ставка13), , РегистрацияВНалоговомОргане, Ложь, ОкончательныйРасчет);
	КонецЕсли;
	...
	Если КолонкиДанных.Найти("НалогПоСтавке09") <> Неопределено Тогда
		СформироватьИсчисленныйНалогПоТаблицеЗначений(Движения, Отказ, Организация, ДатаОперации, НалогиИзДанныхДокумента(ДанныеСРегистрациями, "НалогПоСтавке09", Перечисления.НДФЛСтавкиНалогообложенияРезидента.Ставка09), , РегистрацияВНалоговомОргане, Ложь, ОкончательныйРасчет);
	КонецЕсли;
	...
	Если КолонкиДанных.Найти("НалогПоСтавке35") <> Неопределено Тогда
		СформироватьИсчисленныйНалогПоТаблицеЗначений(Движения, Отказ, Организация, ДатаОперации, НалогиИзДанныхДокумента(ДанныеСРегистрациями, "НалогПоСтавке35", Перечисления.НДФЛСтавкиНалогообложенияРезидента.Ставка35), , РегистрацияВНалоговомОргане, Ложь, ОкончательныйРасчет);
	КонецЕсли;
...
Процедура СформироватьИспользованныеАвансовыеПлатежиПоВременнойТаблице(Движения, Отказ, Организация, ДатаОперации, МенеджерВременныхТаблиц, Записывать = Ложь, ОкончательныйРасчет = Истина) Экспорт
	...
	|ГДЕ
	|	Вычеты.Сумма <> 0";
	СформироватьВТРегистрацийПодразделенийВНалоговомОргане(МенеджерВременныхТаблиц, "ВТДанныеОЗачтенныхПлатежах", "МесяцНалоговогоПериода");
	Результат = Запрос.Выполнить();
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТИсторияРегистрацийВНалоговомОрганеСрезПоследних");
	Если Результат.Пустой() Тогда
		Возврат
...
Функция РассчитатьНалогПоОсновнойСтавке(Регистратор, Организация, МесяцРасчета, МенеджерВременныхТаблиц, Знач ПервыйМесяцНалоговогоПериода = Неопределено, Знач ПоследнийМесяцНалоговогоПериода = Неопределено, ИсключатьДоходыРегистратора = Истина, ОкончательныйРасчет = Истина, ДатаУдержанияИсчисленногоНалога = Неопределено) Экспорт
	...
	Если ЭтоРасчетПоДоходамТекущегоМесяца Тогда
		...
		|	
		|	ВЫБРАТЬ
		|		МАКСИМУМ(СведенияОДоходахНДФЛ.МесяцНалоговогоПериода),
		|		МИНИМУМ(СведенияОДоходахНДФЛ.МесяцНалоговогоПериода)
		|	ИЗ
		...
		|				ИЗ
		|					ВТФизическиеЛица КАК СписокСотрудников)
		|		И СведенияОДоходахНДФЛ.ДоходМежрасчетногоПериода) КАК МесяцыНалоговогоПериода";
	...
	|ВЫБРАТЬ
	|	Вычеты.ФизическоеЛицо КАК ФизическоеЛицо,
	|	НАЧАЛОПЕРИОДА(Вычеты.Период, МЕСЯЦ) КАК МесяцНалоговогоПериода,
	|	Вычеты.КодВычета КАК КодВычета,
	|	Вычеты.Год КАК НалоговыйПериод,
	|	СУММА(Вычеты.Сумма) КАК Размер
	|ПОМЕСТИТЬ ВТПравоНаИмущественныеВычеты
	|ИЗ
	|	РегистрНакопления.ИмущественныеВычетыНДФЛ КАК Вычеты
	|ГДЕ
	|	Вычеты.Регистратор <> &Регистратор
	|	И Вычеты.Период МЕЖДУ &НачалоГодаПериодаРасчета И &КонецМесяцаРасчета
	|	И Вычеты.ГоловнаяОрганизация = &ГоловнаяОрганизация
	|	И Вычеты.ФизическоеЛицо В
	|			(ВЫБРАТЬ
	|				ФизЛица.ФизическоеЛицо
	|			ИЗ
	|				ВТФизическиеЛица КАК ФизЛица)
	|	И Вычеты.Год = ГОД(Вычеты.Период)
	|	И Вычеты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|
	|СГРУППИРОВАТЬ ПО
	|	Вычеты.ФизическоеЛицо,
	|	НАЧАЛОПЕРИОДА(Вычеты.Период, МЕСЯЦ),
	|	Вычеты.КодВычета,
	|	Вычеты.Год
	|
	|ИМЕЮЩИЕ
	|	СУММА(Вычеты.Сумма) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СУММА(ВЫБОР
	|			КОГДА ПравоНаИмущественныеВычеты.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код329)
	|				ТОГДА ПравоНаИмущественныеВычеты.Размер
	|			ИНАЧЕ 0
			...
			Пока ВыборкаРасчетов.СледующийПоЗначениюПоля("НалоговыйПериод") Цикл
				...
				Пока ВыборкаРасчетов.СледующийПоЗначениюПоля("МесяцНалоговогоПериода") Цикл
							...
							Если ВыборкаРасчетов.ИсчислятьНалогНарастающимИтогом Тогда
								...
								Пока ВыборкаРасчетов.СледующийПоЗначениюПоля("ДатаПолученияДохода") Цикл
									...
									Если ВсегоВычетовЗаДату * ВычетНаДетейЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаДетей", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
									...
									Если ВсегоВычетовЗаДату * ВычетНаДетейДвойнойЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаДетейДвойной", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаДетейДвойнойВторойЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаДетейДвойнойВторой", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаВторогоРебенкаЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаВторогоРебенка", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаВторогоРебенкаДвойнойЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаВторогоРебенкаДвойной", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаВторогоРебенкаДвойнойВторойЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаВторогоРебенкаДвойнойВторой", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаТретьегоРебенкаЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаТретьегоРебенка", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаТретьегоРебенкаДвойнойЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаТретьегоРебенкаДвойной", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаТретьегоРебенкаДвойнойВторойЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаТретьегоРебенкаДвойнойВторой", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаДетейИнвалидовЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаДетейИнвалидов", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаДетейИнвалидовДвойнойЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаДетейИнвалидовДвойной", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаДетейИнвалидовДвойнойВторойЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаДетейИнвалидовДвойнойВторой", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаДетейОпекуновЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаДетейОпекунов", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаДетейДвойнойОпекуновЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаДетейДвойнойОпекунов", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаДетейДвойнойВторойОпекуновЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаДетейДвойнойВторойОпекунов", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаВторогоРебенкаОпекуновЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаВторогоРебенкаОпекунов", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаВторогоРебенкаДвойнойОпекуновЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаВторогоРебенкаДвойнойОпекунов", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаВторогоРебенкаДвойнойВторойОпекуновЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаВторогоРебенкаДвойнойВторойОпекунов", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаТретьегоРебенкаОпекуновЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаТретьегоРебенкаОпекунов", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаТретьегоРебенкаДвойнойОпекуновЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаТретьегоРебенкаДвойнойОпекунов", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаТретьегоРебенкаДвойнойВторойОпекуновЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаТретьегоРебенкаДвойнойВторойОпекунов", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаДетейИнвалидовОпекуновЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаДетейИнвалидовОпекунов", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаДетейИнвалидовДвойнойОпекуновЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаДетейИнвалидовДвойнойОпекунов", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетНаДетейИнвалидовДвойнойВторойОпекуновЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетНаДетейИнвалидовДвойнойВторойОпекунов", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетЛичныйЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетЛичный", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетРасходыНаДорогостоящееЛечениеЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетРасходыНаДорогостоящееЛечение", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетВзносыНаДобровольноеСтрахованиеЖизниЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетВзносыНаДобровольноеСтрахованиеЖизни", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетСтраховыеВзносыНаМедУслугиЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетСтраховыеВзносыНаМедУслуги", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетРасходыНаЛечениеЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетРасходыНаЛечение", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетРасходыНаОбучениеДетейЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетРасходыНаОбучениеДетей", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетРасходыНаСвоеОбучениеЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетРасходыНаСвоеОбучение", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетИмущественныйПроцентыПоКредитамЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетИмущественныйПроцентыПоКредитам", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетИмущественныйПроцентыПриПерекредитованииЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетИмущественныйПроцентыПриПерекредитовании", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
										ВсегоВычетовЗаДату = ВсегоВычетовЗаДату - Учтено;
									...
									Если ВсегоВычетовЗаДату * ВычетИмущественныйРасходыЗаМесяц > 0 Тогда
										...
										ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, '00010101', Учтено, "ПримененныйВычетИмущественныйРасходы", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
					...
					Если ЕстьВычетыЗаМесяц Тогда
						...
						ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, ДатаУчетаВычетов, ВычетНаДетейОпекуновЗаМесяц, "ПримененныйВычетНаДетейОпекунов", СтруктураПоиска, ДоходыДляРаспределенияВычетов, КоэффициентыДляРаспределенияВычетов);
						ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, ДатаУчетаВычетов, ВычетНаДетейДвойнойОпекуновЗаМесяц, "ПримененныйВычетНаДетейДвойнойОпекунов", СтруктураПоиска, ДоходыДляРаспределенияВычетов, КоэффициентыДляРаспределенияВычетов);
						ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, ДатаУчетаВычетов, ВычетНаДетейДвойнойВторойОпекуновЗаМесяц, "ПримененныйВычетНаДетейДвойнойВторойОпекунов", СтруктураПоиска, ДоходыДляРаспределенияВычетов, КоэффициентыДляРаспределенияВычетов);
						ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, ДатаУчетаВычетов, ВычетНаТретьегоРебенкаОпекуновЗаМесяц, "ПримененныйВычетНаТретьегоРебенкаОпекунов", СтруктураПоиска, ДоходыДляРаспределенияВычетов, КоэффициентыДляРаспределенияВычетов);
						ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, ДатаУчетаВычетов, ВычетНаТретьегоРебенкаДвойнойОпекуновЗаМесяц, "ПримененныйВычетНаТретьегоРебенкаДвойнойОпекунов", СтруктураПоиска, ДоходыДляРаспределенияВычетов, КоэффициентыДляРаспределенияВычетов);
						ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, ДатаУчетаВычетов, ВычетНаТретьегоРебенкаДвойнойВторойОпекуновЗаМесяц, "ПримененныйВычетНаТретьегоРебенкаДвойнойВторойОпекунов", СтруктураПоиска, ДоходыДляРаспределенияВычетов, КоэффициентыДляРаспределенияВычетов);
						ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, ДатаУчетаВычетов, ВычетНаВторогоРебенкаОпекуновЗаМесяц, "ПримененныйВычетНаВторогоРебенкаОпекунов", СтруктураПоиска, ДоходыДляРаспределенияВычетов, КоэффициентыДляРаспределенияВычетов);
						ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, ДатаУчетаВычетов, ВычетНаВторогоРебенкаДвойнойОпекуновЗаМесяц, "ПримененныйВычетНаВторогоРебенкаДвойнойОпекунов", СтруктураПоиска, ДоходыДляРаспределенияВычетов, КоэффициентыДляРаспределенияВычетов);
						ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, ДатаУчетаВычетов, ВычетНаВторогоРебенкаДвойнойВторойОпекуновЗаМесяц, "ПримененныйВычетНаВторогоРебенкаДвойнойВторойОпекунов", СтруктураПоиска, ДоходыДляРаспределенияВычетов, КоэффициентыДляРаспределенияВычетов);
						...
						ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, ДатаУчетаВычетов, ВычетВзносыНаДобровольноеСтрахованиеЖизниЗаМесяц, "ПримененныйВычетВзносыНаДобровольноеСтрахованиеЖизни", СтруктураПоиска, ДоходыДляРаспределенияВычетов, КоэффициентыДляРаспределенияВычетов);
	...
	Для каждого СтрокаТЗ Из РезультатыРасчетов Цикл
		Если ЗначениеЗаполнено(СтрокаТЗ.ПримененныйВычетЛичный) И Не ЗначениеЗаполнено(СтрокаТЗ.КодВычетаЛичный) Тогда
			...
			Для каждого СтрокаВычетов Из СтандартныхВычетовРанее.НайтиСтроки(СтруктураПоиска) Цикл
				Если ЗначениеЗаполнено(СтрокаВычетов.КодВычетаЛичный) Тогда
					СтрокаТЗ.КодВычетаЛичный = СтрокаВычетов.КодВычетаЛичный;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	...
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТДатыПоМесяцам");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТПериодыПоГодам");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТПериодыНалогаНаДоходы");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТПериодыСрезаВычетов");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТСтандартныеВычетыПоНДФЛЗаПериод");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТПраваНаВычетыНаДетей");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТПраваНаЛичныеВычеты");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТРезидентствоФизлицНаКонецГода");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТРезидентствоФизлицНаКонецМесяца");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТПериодыФизлиц");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТРазмерыСтандартныхВычетов");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТПрименениеСтандартныхВычетов");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТПравоНаИмущественныеВычеты");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТСведенияОДоходахНДФЛ");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТДоходыФизлиц");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТПредоставленныеСтандартныеИСоциальныеВычеты");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТПредоставленоИмущественныхВычетов");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТСоциальныеВычетыПомесячно");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТДоходыСУчетомВычетовКДоходам");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТДоходыРезидентовПоМесяцам");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТДоходыРезидентовНарастающимЗаГод");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТПраваНаСтандартныеВычетыПоМесяцам");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТНалогНерезидентов");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТСтрокиУчета");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТСтавкаНДФЛПоСтавкеРезидента");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТДанныеУчета");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТДоходыСоСтавками");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТДоходыНалоги");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТСтавкиНалогаЧислом");
...
Процедура ВписатьСуммыВыплаченногоДоходаВУдержанныеНалоги(МенеджерВременныхТаблиц, Регистратор) Экспорт
	...
	|		КОНЕЦ
	|	ИЗ
	|		ВТЗарегистрированоРанее КАК ЗарегистрированоРанее
	|	ГДЕ
	|		ЗарегистрированоРанее.ДоляВыплаты > 0
	|		И НЕ ЗарегистрированоРанее.УчитыватьСтатьиФинансирования) КАК НалогиИДоходы
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДокументыОснования КАК ДокументыОснования
...
Функция РассчитатьУдержанныеНалоги(Ссылка, Организация, ДатаОперации, МенеджерТаблиц, ПериодРегистрации = Неопределено, ОкончательныйРасчет = Истина) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеУчета.ДатаНачисления,
	|	ДанныеУчета.Период,
	|	ДанныеУчета.МесяцНалоговогоПериода,
	|	ДанныеУчета.ФизическоеЛицо,
	|	ДанныеУчета.СтавкаНалогообложенияРезидента,
	|	ДанныеУчета.КодДохода,
	|	ДанныеУчета.РегистрацияВНалоговомОргане,
	|	ДанныеУчета.Организация,
	|	ДанныеУчета.Подразделение,
	|	ДанныеУчета.ВключатьВДекларациюПоНалогуНаПрибыль,
	...
	|			КОНЕЦ
	|	КОНЕЦ КАК СуммаОстаток,
	|	ДанныеУчета.СуммаВыплаты,
	|	ДанныеУчета.НачисленоНалога
	...
	ТаблицыКУдалению = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("ВТДанныеУчета");
	...
	Если ЗначениеЗаполнено(ПериодРегистрации) Тогда
		...
		|	СуммыНДФЛПоСотрудникам.НачисленоНалога,
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТПоФизическомуЛицуНаДатуПолученияДохода
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
...
Процедура РасчетПоказателей_6НДФЛ_2016Кв1(НачалоПериода, ОкончаниеПериода, ДатаПодписи, Организация, РегистрацияВНалоговомОргане, ОписаниеТаблиц, ПодробныеДанные = Ложь)
	...
	Если ОписаниеТаблиц.Свойство("Раздел1Доходы") Или ОписаниеТаблиц.Свойство("Раздел1Вычеты") Или ОписаниеТаблиц.Свойство("КоличествоПолучателейДохода") Тогда
		...
		СоздатьВТСтавкаНДФЛПоСтавкеРезидента(Запрос.МенеджерВременныхТаблиц, "ВТДоходы", "УчитыватьИзмененияСтатусаДляОтчетности", Месяц(ОкончаниеПериода));
	...
	Если ОписаниеТаблиц.Свойство("Раздел1Налоги") Или ОписаниеТаблиц.Свойство("Раздел2") Тогда
		...
		Если ОписаниеТаблиц.Свойство("Раздел1Налоги") Тогда
			...
			СоздатьВТСтавкаНДФЛПоСтавкеРезидента(Запрос.МенеджерВременныхТаблиц, "ВТНалоги", "УчитыватьИзмененияСтатусаДляОтчетности", Месяц(ОкончаниеПериода));
		...
		Если ОписаниеТаблиц.Свойство("Раздел2") Тогда
			...
			СоздатьВТСтавкаНДФЛПоСтавкеРезидента(Запрос.МенеджерВременныхТаблиц, "ВТСтрокиДоходов", "УчитыватьИзмененияСтатусаДляОтчетности", Месяц(ОкончаниеПериода));
...
Функция ТаблицаНДФЛ() Экспорт
	...
	НДФЛ.Колонки.Добавить("Подразделение", Метаданные.ОпределяемыеТипы.ТерриторииВыполненияРаботВОрганизации.Тип);
...
Функция ДанныеОПереплатеПоНДФЛ(Организация, ДатаАктуальности, ФизическиеЛица = Неопределено, ДокументСсылка = Неопределено) Экспорт
	...
	ТаблицаПереплат.Сортировать("Сотрудник, НалоговыйПериод, МесяцНалоговогоПериода, РегистрацияВНалоговомОргане");
	ТаблицаПереплат.Свернуть("Сотрудник, НалоговыйПериод, МесяцНалоговогоПериода, РегистрацияВНалоговомОргане", "ПоСтавке13,ПоСтавке09,ПоСтавке35");
...
//			Подразделение
//			КодВычетаЛичный
//			колонки рассчитанных ресурсов: ПримененныйВычетЛичный, 
...
Функция ТаблицаДвиженийСРегистрациейВНалоговомОргане(ТаблицаДвижений, ИмяПоляДаты) Экспорт
	...
	СформироватьВТРегистрацийПодразделенийВНалоговомОргане(Запрос.МенеджерВременныхТаблиц, "ВТТаблицаДвижений", ИмяПоляДаты);
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТИсторияРегистрацийВНалоговомОрганеСрезПоследних КАК ИсторияРегистрацийВНалоговомОрганеСрезПоследних
	|		ПО ТаблицаДвижений.Подразделение = ИсторияРегистрацийВНалоговомОрганеСрезПоследних.СтруктурнаяЕдиница
	|			И ТаблицаДвижений.ИмяПоляДаты = ИсторияРегистрацийВНалоговомОрганеСрезПоследних.Период";
...
// Помещает данные о регистрациях в НО во временную таблицу.
//
// Параметры:
//	МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - менеджер временных таблиц, содержащий таблицу с полями:
//		Подразделение - ОпределяемыйТип.ТерриторииВыполненияРабот
...
//
// В результате в менеджер временных таблиц помещается таблица ВТИсторияРегистрацийВНалоговомОрганеСрезПоследних с полями
//		СтруктурнаяЕдиница - ОпределяемыйТип.ТерриторииВыполненияРабот
//		Период - Дата
//		РегистрацияВНалоговомОргане - СправочникСсылка.РегистрацииВНалоговомОргане.
//
Процедура СформироватьВТРегистрацийПодразделенийВНалоговомОргане(МенеджерВременныхТаблиц, ИмяТаблицыПодразделений, Знач ПолеДаты) Экспорт 
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ИмяТаблицыПодразделений.ПолеДаты КАК Период,
	|	ИмяТаблицыПодразделений.Подразделение КАК СтруктурнаяЕдиница
	|ПОМЕСТИТЬ ВТПодразделения
	|ИЗ
	|	ВТИмяТаблицыПодразделений КАК ИмяТаблицыПодразделений";
	Если СтрНайти(ПолеДаты, ".") = 0 Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПолеДаты", ПолеДаты);
	Иначе 	
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ИмяТаблицыПодразделений.ПолеДаты", ПолеДаты);
	КонецЕсли;
	Если СтрНачинаетсяС(ВРег(ИмяТаблицыПодразделений), "ВТ") Тогда
		Запрос.Текст = СтрЗаменить(ТекстЗапроса, "ИмяТаблицыПодразделений", Сред(ИмяТаблицыПодразделений, 3));
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВТИмяТаблицыПодразделений", ИмяТаблицыПодразделений);
		Запрос.Текст = СтрЗаменить(ТекстЗапроса, "ИмяТаблицыПодразделений", ИмяТаблицыПодразделений);
	КонецЕсли;
	Запрос.Выполнить();
	...
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТПодразделения");
...
Функция НалогиИзДанныхДокумента(ДанныеДокумента, ИмяКолонки, Ставка)
	...
	ИсчисленныйНалог = ДанныеДокумента.Скопировать(,"ФизическоеЛицо, МесяцНалоговогоПериода, Подразделение, РегистрацияВНалоговомОргане, " + ИмяКолонки);
...
//				с полями, соответствующими полям регистра (в структуре могут отсутствовать необязательные поля)
Функция СтрокаРасчетовНалоговыхАгентовСБюджетом(Движения, Организация, ДатаОперации, ВидДвижения, ДанныеДокумента, ДанныеРаспределения = Неопределено, КодДохода = Неопределено) 
...
Процедура ОбработатьНаборыЗаписейРегистраНакопления(ИмяРегистра, ТекстЗапроса, ПараметрыЗапроса = Неопределено, МенеджерВременныхТаблиц = Неопределено, ПараметрыОбновления = Неопределено) Экспорт
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.СледующийПоЗначениюПоля("Регистратор") Цикл
			Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(ПараметрыОбновления, "РегистрНакопления." + ИмяРегистра, "Регистратор", Выборка.Регистратор) Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.РегистрыСведений.РазмерВычетовНДФЛ) Тогда
		...
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.1.2.74";
		Обработчик.Процедура = "РегистрыСведений.РазмерВычетовНДФЛ.ЗаполнитьРазмерыВычетовНДФЛ";
		Обработчик.ОбщиеДанные = Истина;
		Обработчик.НачальноеЗаполнение = Истина;
...
Процедура СформироватьВТФизическиеЛицаСУчетомПереездаВИнойНО(КППОтправки, МенеджерВременныхТаблиц, ИмяТаблицыПодразделений, ПолеДаты) Экспорт 
	...
	СформироватьВТРегистрацийПодразделенийВНалоговомОргане(МенеджерВременныхТаблиц, ИмяТаблицыПодразделений, ПолеДаты);
	...
	|	Доходы.ФизическоеЛицо,
	|	Доходы.РегистрацияВНалоговомОргане,
	|	ИсторияРегистрацийВНалоговомОрганеСрезПоследних.РегистрацияВНалоговомОргане КАК НоваяРегистрацияВНалоговомОргане
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТИсторияРегистрацийВНалоговомОрганеСрезПоследних КАК ИсторияРегистрацийВНалоговомОрганеСрезПоследних
	|		ПО Доходы.Подразделение = ИсторияРегистрацийВНалоговомОрганеСрезПоследних.СтруктурнаяЕдиница
	|			И Доходы.КонецГода = ИсторияРегистрацийВНалоговомОрганеСрезПоследних.Период
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НовыеРегистрацииФизическогоЛица.ФизическоеЛицо,
	|	НовыеРегистрацииФизическогоЛица.РегистрацияВНалоговомОргане
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РегистрацииФизическогоЛица.ФизическоеЛицо
...
Функция СправкиНДФЛДанныеДляПечати(МассивОбъектов, ПолучатьИтоговыеСуммы = Ложь) Экспорт
	...
	|	ДанныеДокументов.НомерКорректировки,
	...
	|	ДанныеДокументов.КодНалоговогоОрганаПолучателя,
