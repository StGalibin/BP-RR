...
// Обработчик обновления 3.0.43.7
//
Процедура УстановитьСрокиОплатыСчетовЗначениеНеЗаполнять() Экспорт
	
	Если НЕ Константы.УдалитьИспользоватьСрокиОплатыСчетовПокупателю.Получить() Тогда
		УстановитьПривилегированныйРежим(Истина);
		Константы.СрокОплатыСчетовПокупателю.Установить(0);
	КонецЕсли;
