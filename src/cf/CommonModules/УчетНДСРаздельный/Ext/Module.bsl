...
Процедура СформироватьДвиженияВозвратТоваровПоДокументуОтгрузки(ТаблицаВозвращаемыеТовары, Реквизиты, Движения, Отказ)
	...
	ТаблицаПартий.Колонки.Добавить("СчетЗатрат", Новый ОписаниеТипов("ПланСчетовСсылка.Хозрасчетный"));
	ТаблицаПартий.Колонки.Добавить("Субконто1");
	ТаблицаПартий.Колонки.Добавить("Субконто2");
	ТаблицаПартий.Колонки.Добавить("Субконто3");
	ТаблицаПартий.Колонки.Добавить("Подразделение", Новый ОписаниеТипов("СправочникСсылка.ПодразделенияОрганизаций"));
	Для Каждого СтрокаТаблицы Из ТаблицаПартий Цикл
		
		Если СтрокаТаблицы.Регистр = "НДСРаздельныйУчет" Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СформироватьДвиженияКурсовыеРазницыНалоговыйАгент(ТаблицаРеквизитов, ТаблицаВзаиморасчетов, Движения, Отказ) Экспорт
	...
	ДанныеПроводокВключениеВСтоимость = ПодготовитьДанныеПроводокВключениеВСтоимостьКурсовыеРазницыНалоговыйАгент(
		СтруктураТаблицДляДвижений.НДСРаздельныйУчет, Реквизиты.Период);
...
Функция ПодготовитьДанныеПроводокВключениеВСтоимостьКурсовыеРазницыНалоговыйАгент(ДвиженияНДСРаздельныйУчет, Период)
	...
	ОпределитьПоставщиковПоСписаннымПартиямНДС(ДанныеПроводок, Период);
...
Функция ПодготовитьДанныеДвиженийНДСНеоблагаемыеОперации(СписанныеПартииНДС, РеализованныеТоварыУслуги, Реквизиты)
	...
	Для Каждого СтрокаТаблицы Из РеализованныеТоварыУслуги Цикл
		...
		Если ЗначениеЗаполнено(Реквизиты.КодОперацииПоСделке)
			...
			Если ЗначениеЗаполнено(Реквизиты.КодОперацииПоСделке) Тогда 
				...
				НоваяСтрока.СуммаНДСНеПодлежащаяВычету = СписанныеПартииНДС.Итог("НДС");
				СписанныеТоварыНеоблагаемыеНДС      = СписанныеПартииНДС.Скопировать(
...
Функция ПодтверждающиеДокументыПоРегистратору(ОперацииКПодтверждению, ДокументРегистратор)
	...
	Если ТипЗнч(ДокументРегистратор) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда 
		...
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументРегистратор, 
			"ВидОперации, Организация, Контрагент, ДоговорКонтрагента, ВидЭлектронногоДокумента");
		...
		Если ОбменСКонтрагентамиСлужебныйВызовСервера.НастройкаЭДСуществует(ДокументРегистратор)
			...
			ДоступныеЗначения = Документы.РеализацияТоваровУслуг.ПолучитьДоступныеВидыЭдПоВидуОперации(
				РеквизитыДокумента.ВидОперации, 
			...
			Если ДоступныеЗначения.Количество() <> 0 Тогда 
				ТекущееЗначение = ДоступныеЗначения.НайтиПоЗначению(РеквизитыДокумента.ВидЭлектронногоДокумента);
				Если ТекущееЗначение <> Неопределено
					И ТекущееЗначение.Представление = "Универсальный передаточный документ" Тогда 
...
Функция ПолучитьОстаткиПартийНДС(СписанныеТовары, Реквизиты)
	...
	|	НДСРаздельныйУчетОстатки.СуммаБезНДСОстаток КАК СуммаБезНДС,
	|	НДСРаздельныйУчетОстатки.НДСОстаток КАК НДС,
	|	ВТРеквизитыНоменклатуры.ЭтоСырье,
	...
	|		ПО НДСРаздельныйУчетОстатки.АналитикаУчетаЗатрат = ВТРеквизитыНоменклатуры.АналитикаУчетаЗатрат
	|ГДЕ
	...
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ВТНДСРаздельныйУчетОстатки.ЭтоСырье
	|	КОНЕЦ КАК БлокироватьДоПодтвержденияСтавки0,
	|	ВТНДСРаздельныйУчетОстатки.КодРаздел7ДекларацииНДС,
	|	ВТНДСРаздельныйУчетОстатки.Номенклатура
	|ИЗ
...
Процедура ОпределитьПоставщиковПоСписаннымПартиямНДС(СписанныеПартии, Период)
	...
	ПараметрыСчетовФактур = УчетНДСБП.ПолучитьПараметрыСчетовФактурПоНДСПредъявленный(ТаблицаСчетовФактур, Период);
	...
	Для Каждого СтрокаТаблицы Из СтрокиПартийНДС Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ДатаПартии) Тогда
			Если ЗначениеЗаполнено(СтрокаТаблицы.Партия) Тогда
				СтрокаТаблицы.ДатаПартии = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицы.Партия, "Дата");
			Иначе
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
Функция СчетаПартионногоУчетаЦенностейДляЦелейНДС()
	...
	СчетаУчетаЦенностей = ОбщегоНазначенияКлиентСервер.СократитьМассив(СчетаУчетаЦенностей,
