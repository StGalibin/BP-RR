...
Функция ВозможнаОтправкаОтчета(Знач Организация, Знач Банк, Знач ПараметрыКлиента, ПодключенаИнтернетПоддержка = Ложь, СоглашениеПринято = Ложь) Экспорт
	...
	ПодключенаИнтернетПоддержка = ОбщегоНазначенияПовтИсп.РазделениеВключено()
		ИЛИ ИнтернетПоддержкаПользователей.ДанныеАутентификацииПользователяИнтернетПоддержки() <> Неопределено;
...
Функция ТикетАутентификации()
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		Возврат "005d413a-f6ff-4a5f-8bd0-28d9eb765373";
	КонецЕсли;
	...
	ЗаписьJSON.ЗаписатьЗначение(ДанныеАутентификации.Логин);
	ЗаписьJSON.ЗаписатьИмяСвойства("password");
	ЗаписьJSON.ЗаписатьЗначение(ДанныеАутентификации.Пароль);
	ЗаписьJSON.ЗаписатьИмяСвойства("serviceNick");
	ЗаписьJSON.ЗаписатьЗначение("reportBank");
	ЗаписьJSON.ЗаписатьКонецОбъекта();
	ЗаписьJSON.Закрыть();
	...
	Результат = ОтчетностьВБанкиСлужебный.ОтправитьЗапросНаСервер(
		"https://login.1c.ru", "/rest//public/ticket/get", Заголовки, Данные, Истина, 30);
	...
	Иначе
		Если ЗначениеЗаполнено(Результат.Тело) Тогда
			...
			Если НЕ ДанныеОтвета = Неопределено Тогда
				...
				КонецЕсли;
			КонецЕсли;
			ТекстОшибки = НСтр("ru = 'Ошибка аутентификации на сервисе 1С:Логин.
								|Код состояния: %1
								|%2'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, Результат.КодСостояния, Результат.Тело);
		Иначе
