...
Функция ПечатьТОРГ12(СведенияТОРГ12, ОбъектыПечати, ИмяПараметровПечати) Экспорт
	...
	Для Каждого СведенияОДокументе Из СведенияТОРГ12 Цикл
		...
		ОбластьМакетаПодвал.Параметры.ДолжностьРуководителя = СведенияОДокументе.РуководительДолжностьНаименование;
...
Функция ПечатьСчетаНаОплату(СведенияСчетаНаОплату, ОбъектыПечати, СПечатью = Ложь) Экспорт
	...
	Для Каждого СведенияОДокументе Из СведенияСчетаНаОплату Цикл
		...
		Если СПечатью Тогда
			...
			ДвоичныеДанныеФаксимиле           = Справочники.Организации.ДвоичныеДанныеКартинкиОрганизации(СведенияОДокументе.Получатель, "ФайлФаксимильнаяПечать");
			ДвоичныеДанныеПечати              = Справочники.Организации.ДвоичныеДанныеКартинкиОрганизации(СведенияОДокументе.Получатель, "ФайлПечать");
			ДвоичныеДанныеПодписьРуководителя       = Справочники.Организации.ДвоичныеДанныеКартинкиОрганизации(СведенияОДокументе.Получатель, "ФайлПодписьРуководителя");
			ДвоичныеДанныеПодписьГлавногоБухгалтера = Справочники.Организации.ДвоичныеДанныеКартинкиОрганизации(СведенияОДокументе.Получатель, "ФайлПодписьГлавногоБухгалтера");
		...
		Если ЗначениеЗаполнено(ДвоичныеДанныеПечати)
			...
			Иначе
				...
				Если ЗначениеЗаполнено(Макет.Области.Найти("ПодвалСчетаСПодписьюИПечатью")) Тогда
					...
					// В случае если используется факсимильная подпись выводим только данные ответственных лиц
...
Функция ПечатьПредварительныйПросмотрСчетаНаОплату(Организация, ОбъектыПечати, ИмяМакета) Экспорт
	...
	Если ЗначениеЗаполнено(ФайлЛоготип) Тогда
		ДанныеКартинки = ПрисоединенныеФайлы.ПолучитьДанныеФайла(ФайлЛоготип);
	...
	Если ЗначениеЗаполнено(ДвоичныеДанныеПечать)
		...
		Иначе
			Если ЗначениеЗаполнено(Макет.Области.Найти("ПодвалСчетаПредпринимательСПодписьюИПечатью")) Тогда
				ОбластьМакета = Макет.ПолучитьОбласть("ПодвалСчетаПредпринимательСПодписьюИПечатью");
				ОбластьМакета.Рисунки.ПечатьПредпринимателя.Картинка     = Новый Картинка(ДвоичныеДанныеПечать);
				ОбластьМакета.Рисунки.ПодписьПредпринимателя.Картинка    = Новый Картинка(ДвоичныеДанныеПодписьРуководителя);
			Иначе
				ОбластьМакета = Макет.ПолучитьОбласть("ПодвалСчетаПредприниматель");
			КонецЕсли;
