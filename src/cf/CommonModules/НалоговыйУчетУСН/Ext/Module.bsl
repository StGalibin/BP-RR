...
Процедура ЗаполнитьСписокВыбораПорядкаОтраженияАвансовДляТЧ(ПолеОтражениеАванса, СоответствиеОтражениеАванса = Неопределено, Объект) Экспорт
	...
	РаздельныйУчетТорговыйСборПриУСН = УчетнаяПолитика.РаздельныйУчетТорговыйСборПриУСН(Организация, Дата);
	...
	Если УчетУСН.ЕстьАвансыПокупателей(ВидОперации) Тогда
		...
		Если ПрименяетсяУСН Тогда
			...
			Если РаздельныйУчетТорговыйСборПриУСН Тогда
				Представление = НСтр("ru = 'Доход от деятельности на торговом сборе'");
				СоответствиеПоиска.Вставить(СписокВыбора.Количество(), Перечисления.ПорядокОтраженияАвансов.ДоходТорговыйСбор);
				СписокВыбора.Добавить(Представление);
			КонецЕсли;
			...
			Если НЕ Результат.Пустой() Тогда
				...
				Пока Выборка.Следующий() Цикл
					...
					Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Доход по патенту ""%1""'"), Выборка.Наименование);
	...
	// Передаем соответствие значений и их представлений для помещения в реквизит формы
	СоответствиеОтражениеАванса = Новый ФиксированноеСоответствие(СоответствиеПоиска);
	
КонецПроцедуры

Функция ПорядокОтраженияАванса(Организация, Знач Дата) Экспорт
	
	// Порядок отражения аванса по умолчанию
	СтруктураОтражениеАвансов = ПорядокОтраженияАвансовПоУмолчанию(Организация, Дата);
	Возврат ?(СтруктураОтражениеАвансов.ПорядокОтраженияАванса = Перечисления.ПорядокОтраженияАвансов.ДоходПатент,
...
Функция ПорядокОтраженияАвансовПоУмолчанию(Организация, Знач Дата)
...
Функция СодержаниеОперацииДляКУДиР(ПараметрыУСН) Экспорт
	...
	ИначеЕсли (ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога
		...
		НазначениеПлатежа = ОписаниеНалоговогоПлатежа(ПараметрыУСН.Налог, ПараметрыУСН.СубконтоДт1, ПараметрыУСН.НалоговыйПериод);
		Содержание_УСН    = ?(Не ПустаяСтрока(НазначениеПлатежа), НазначениеПлатежа, НСтр("ru = 'Уплата налога'"));
...
Функция РеквизитыПервичногоДокумента(Документ, ТЧ = "", Строка = 0, УказыватьВид = Ложь) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(НомерНаПечать) Тогда
		...
		НомерНаПечать	= ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Документ.Номер, Истина, Ложь);
...
Процедура СформироватьДвиженияУСН(ДокументОбъект, СтруктураДопПараметров = Неопределено, ЭтоОбъект = Истина, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") Тогда
		...
		ОтразитьВУСН(ДокументСсылка, "Розница", СтруктураШапкиДокумента, 0, СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
		
		Если СтруктураДопПараметров.Свойство("ПроданныеСертификаты") Тогда
			ОтразитьВУСН(ДокументСсылка, "ПродажаПодарочныхСертификатов", СтруктураШапкиДокумента, 1, СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КорректировкаДолга") Тогда
		...
		ИначеЕсли ДокументСсылка.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.Прочее
			И СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН Тогда
			...
			ИначеЕсли СчетаДтКт и Субконто1ДтКт и Субконто2ДтКт и Субконто3ДтКт и СуммаДтКт Тогда
				
				ОтразитьВУСН(ДокументСсылка, "ПроведениеВзаимозачетаДебитор", СтруктураШапкиДокумента,0,СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
				ОтразитьВУСН(ДокументСсылка, "ПроведениеВзаимозачетаКредитор", СтруктураШапкиДокумента,1,СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
				
			ИначеЕсли НЕ СчетаДтКт и НЕ СчетДтПустой
				И (НЕ ЕстьСубконтоКонтрагентыПоДт Или НЕ ЕстьСубконтоДоговорыКонтрагентовПоДт) Тогда
				
				ОтразитьВУСН(ДокументСсылка, "СписаниеДебиторскойЗадолженности", СтруктураШапкиДокумента,0,СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
				
			ИначеЕсли НЕ СчетаДтКт и НЕ СчетКтПустой
			...
			ИначеЕсли СчетаДтКт и Субконто1ДтКт и Субконто2ДтКт и Субконто3ДтКт
...
Процедура ОтразитьВУСН(ДокументСсылка, ВидОперации = Неопределено, Параметры = Неопределено, ЭтапПроведения = 0, СтруктураДопПараметров = Неопределено, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	ИначеЕсли (ВидОперации = "Списание")
		...
		Если ВидОперации = "Реализация" Тогда
			...
			Выручка.НДС			 = РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(Выручка.НДС, ВалютаДокумента,
																СтруктураШапкиДокумента.ВалютаВзаиморасчетов,
																СтруктураШапкиДокумента.КурсДокумента, КурсВзаиморасчетов,
																СтруктураШапкиДокумента.КратностьДокумента, КратностьВзаиморасчетов);
			...
			Выручка.Патент		 = РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(Выручка.Патент, ВалютаДокумента,
																...
																СтруктураШапкиДокумента.КурсДокумента, КурсВзаиморасчетов,
																СтруктураШапкиДокумента.КратностьДокумента, КратностьВзаиморасчетов);
			Выручка.ТорговыйСбор = РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(Выручка.ТорговыйСбор, ВалютаДокумента,
																СтруктураШапкиДокумента.ВалютаВзаиморасчетов,
																СтруктураШапкиДокумента.КурсДокумента, КурсВзаиморасчетов,
																СтруктураШапкиДокумента.КратностьДокумента, КратностьВзаиморасчетов);
	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН ИЛИ СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСНДоходы Тогда
			...
			Если (ДвиженияРегистров.КУДиР.Количество() > 0)
				...
				Для каждого Расход Из ДеревоРасходов Цикл
					...
					Если ВидОперации = "НачислениеСтраховыхВзносовИП" Тогда
						ЗаписьКУДиР.РеквизитыПервичногоДокумента = РеквизитыПервичногоДокумента(Расход.ДокументОплаты);
					Иначе
		...
		ИначеЕсли КУДиР.Графа4 <> 0
			...
			РеквизитыПервичногоДокумента = РеквизитыПервичногоДокумента(ДокументСсылка);
	...
	Для Каждого Строка Из Расход.Строки Цикл
		...
		ИначеЕсли Строка.ВидРасхода = Перечисления.ВидыРасходовУСН.НДС_ТС Тогда
			...
			Если ЗначениеЗаполнено(Строка.РасчетныйДокумент)
				...
				СтрокаЗаявление = " (заявление о ввозе " + РеквизитыПервичногоДокумента(Строка.РасчетныйДокумент) + ")";
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(Строка.РеквизитыДокументаОплаты)
				...
				СтрокаУплаты = " Уплачен в адрес """ + Контрагент + """, документ уплаты "
					+ РеквизитыПервичногоДокумента(Строка.РеквизитыДокументаОплаты,,, Истина) + ".";
...
Процедура ПеремещениеРасходовУСН(СтруктураШапкиДокумента, ТаблицаПартий, ДвиженияРегистров, Договор, СтатусПартии, ВидОперации = "Списание")
	...
	Для Каждого СтрокаПартии ИЗ ТаблицаПартий Цикл
		...
		// Подлежащее погашению при списании количество
		КоличествоОсталосьПогасить     = СтрокаПартии.Количество;
		КоличествоОсталосьОприходовать = СтрокаПартии.КорКоличество;
		Для Каждого Строка Из НайденныеСтроки Цикл
			Если КоличествоОсталосьПогасить <= 0 Тогда
				Прервать;
			КонецЕсли;
			...
			КоличествоПриход = ?(СтрокаПартии.Количество = 0,
				0,
				Окр(НоваяСтрока.Количество * СтрокаПартии.КорКоличество / СтрокаПартии.Количество, 3));
			КоличествоПриход = Мин(КоличествоПриход, КоличествоОсталосьОприходовать);

			НоваяСтрока.КоличествоПриход = КоличествоПриход;

			КоличествоОсталосьОприходовать = КоличествоОсталосьОприходовать - КоличествоПриход;
	...
	// Переместим расходы по НДС, уплаченному при ввозе из Таможенного союза
	Для Каждого СтрокаПартии ИЗ ТаблицаПартий Цикл
...
Процедура СписаниеТЗР(СтруктураШапкиДокумента, ДвиженияРегистров)
	...
	Если ТранзакцияАктивна() Тогда
		...
		ЭлементБлокировки.УстановитьЗначение("Период",
			Новый Диапазон(НачалоКвартала(СтруктураШапкиДокумента.Дата), КонецКвартала(СтруктураШапкиДокумента.Дата)));
		...
		Если Не МетаданныеДокумента.ТабличныеЧасти.Найти("РасшифровкаПлатежа") = Неопределено Тогда
			...
			ИначеЕсли ДокументСсылка.РасшифровкаПлатежа.Количество() > 1 Тогда
				...
				СтрокаСодержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСодержания,
					...
					СтрокаДоговоров);
			Иначе
				СтрокаСодержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 от покупателя ""%2""",
					СпособОплаты,
...
Функция СведенияОВидахНоменклатурыВСчетахУчета(СчетаУчета)
	...
	Для каждого СчетУчета Из СчетаУчета Цикл
		...
		Если ЕстьТовары И ЕстьМатериалы Тогда // Незачем больше проверять счета
			Прервать;
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат Новый Структура("ЕстьТовары, ЕстьМатериалы", ЕстьТовары, ЕстьМатериалы);
...
Функция НазваниеНалогаПоСчетуУчета(СчетУчета)
...
Функция ОписаниеЗачетаНалоговПриНачислении(СчетУчета)
	...
	НазваниеНалога = НазваниеНалогаПоСчетуУчета(СчетУчета);
...
Функция ОписаниеНалоговогоПлатежа(СчетУчета, ВидПлатежаВГосБюджет, ПериодУплаты = Неопределено)
	...
	Иначе
		...
		ЭтоСтраховыеВзносы = ПлатежиВБюджетКлиентСерверПереопределяемый.ЭтоОбязательныеСтраховыеВзносы(ВидНалога);
		ЭтоНДФЛ            = ПлатежиВБюджетКлиентСерверПереопределяемый.ЭтоНДФЛ(ВидНалога);
		...
		Событие = ?(ЭтоСтраховыеВзносы, "Уплачены", "Уплачен");
		
		ПредставлениеПериода = "";
		Если ЗначениеЗаполнено(ПериодУплаты)
			И (ЭтоСтраховыеВзносы Или ЭтоНДФЛ) Тогда // Для страховых взносов и НДФЛ укажем месяц уплаты
			ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(" за %1 г",
				Формат(ПериодУплаты, "ДФ='MMMM yyyy'"));
		КонецЕсли;
		
		Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 %2%3.",
			Событие, НазваниеНалога, ПредставлениеПериода);
...
Функция ПодготовитьТаблицуОплатыОСДляУСН(ТаблицаОплатыОС)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ОплатыОСДляУСН.ОсновноеСредство,
	|	ОплатыОСДляУСН.Организация КАК Организация,
	|	ОплатыОСДляУСН.СуммаОплаты,
	|	ОплатыОСДляУСН.РеквизитыДокументаОплаты КАК РеквизитыДокументаОплаты
	...
	|	ДанныеПервичныхДокументов.Дата КАК ДатаОплаты,
	...
	|			И ТаблицаОплатыОС.РеквизитыДокументаОплаты = ДанныеПервичныхДокументов.Документ";
...
Процедура СвернутьРасшифровкуПлатежаПоДоговорам(КУДиР, СтруктураШапкиДокумента, ВидОперации, МетаданныеДокумента)
	...
	КУДиР.РасшифровкаПоДоговорам.Свернуть(
		...
		"Графа4, Графа5, Графа6, Графа7, НДС, ДоходЕНВД, ДоходТорговыйСбор, ДоходКомитента, ДоходПатент, СуммаНаЗакупкуДляКомитента");
