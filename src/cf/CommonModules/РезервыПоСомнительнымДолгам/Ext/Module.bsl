...
Функция НачислитьРезерв(ОтчетныйПериод, Организация) Экспорт
	...
	ПрименятьПункт70Приказа34н = ФормироватьБУ И ОтчетнаяДата = НачалоГода(ОтчетнаяДата); // см. ниже - выполнение формальных требований приказа
...
Процедура СформироватьПроводки(НаборЗаписейХозрасчетный, НачислениеРезерва, Период, Организация) Экспорт
	...
	СтатьяДоходовРасходов = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.РезервыПоСомнительнымДолгам");
	...
	Для Каждого Запись Из НачислениеРезерва Цикл
		...
		Если Запись.НачисленоБУ <> 0 Или Запись.НачисленоНУ <> 0 Тогда
			...
			НалоговыйУчет.ЗаполнитьНалоговыеСуммыПроводки(
				Запись.НачисленоНУ, Запись.НачисленоНУ,
				НачисленоПР,        НачисленоПР,
				0,                  0, // Временных разниц нет
				Проводка,
				ПрименениеПБУ18);
		...
		Если Запись.ВосстановленоБУ <> 0 Или Запись.ВосстановленоНУ <> 0 Тогда
			...
			ВосстановленоПР = Запись.ВосстановленоБУ - Запись.ВосстановленоНУ;
			
			НалоговыйУчет.ЗаполнитьНалоговыеСуммыПроводки(
				Запись.ВосстановленоНУ, Запись.ВосстановленоНУ,
				ВосстановленоПР,        ВосстановленоПР,
				0,                      0, // Временных разниц нет
				Проводка,
				ПрименениеПБУ18); 
		...
		Если Запись.ПрисоединеноБУ <> 0 Тогда
			...
			НалоговыйУчет.ЗаполнитьНалоговыеСуммыПроводки(
				0,                     0, // Не отражается в налоговом учете
				Запись.ПрисоединеноБУ, Запись.ПрисоединеноБУ,
				0,                     0,
				Проводка,
				ПрименениеПБУ18);
...
Функция ДанныеДляНачисленияРезерва(ОтчетнаяДата, Организации, МенеджерВременныхТаблиц, ПолучатьСостояниеРезерва = Ложь, ПолучатьСостояниеРезерваНачалоГода = Ложь)
	...
	Иначе
		...
		Если ПолучатьСостояниеРезерваНачалоГода Тогда
			...
			Запрос.УстановитьПараметр("НачалоГода", НачалоГода(ОтчетнаяДата - 1));
...
Функция РассчитатьРезерв(ДанныеДляНачисленияРезерва, ОтчетнаяДата, МенеджерВременныхТаблиц)
	...
	Пока ДанныеДляНачисленияРезерва.Следующий() Цикл
		...
		Запись.СрокЗадолженности = (ОтчетнаяДата - Запись.ДатаПризнанияЗадолженности) / 86400 - 1; // В календарных днях
		...
		Запись.ДатаПризнанияЗадолженностиПросроченной = Запись.ДатаПризнанияЗадолженности + Запись.СрокОплаты * 86400;
