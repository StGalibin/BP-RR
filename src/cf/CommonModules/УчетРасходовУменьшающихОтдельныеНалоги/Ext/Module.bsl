...
//  ДанныеСНачалаГода - Булево - если Истина то расходы будут получены с начала года.
// 
// Возвращаемое значение:
//  ТаблицаЗначений -  Таблица с расходами, см. функцию ПустаяТаблицаРасходов()
//
Функция РасчетРасходовУменьшающихОтдельныеНалоги(ТаблицаРеквизитов, ДанныеСНачалаГода=Ложь) Экспорт
	...
	Если Не (Реквизиты.ПрименяетсяУСНДоходы Или Реквизиты.ПлательщикЕНВД) Тогда
		Возврат ТаблицаРасходов;
	КонецЕсли;
	...
	УстановитьПараметрыЗапросаРасходыУменьшающиеНалог(Запрос, Реквизиты, ДанныеСНачалаГода);
...
Процедура СформироватьДвиженияРасходыУменьшающиеОтдельныеНалоги(ТаблицаРасходов, ТаблицаРеквизитов, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаЗаписи Из ТаблицаДвижений Цикл
		...
		Если СтрокаЗаписи.СуммаРасхода > 0 Или СтрокаЗаписи.СуммаРасходаЕНВД > 0 Тогда
...
Функция СчетаСтраховыхВзносовУменьшающихНалог() Экспорт
	...
	Возврат ОбщегоНазначенияКлиентСервер.СократитьМассив(СчетаУчетаУплачено, ИсключаемыеСчета);
...
Функция КоэффициентДоходовЕНВДДляРаспределенияФиксированныхВзносов(Организация, Период) Экспорт
	...
	Если ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Организация)
		ИЛИ НЕ УчетнаяПолитика.ПлательщикЕНВД(Организация, Период) Тогда
		Возврат 0;
	КонецЕсли;
...
Функция ПодготовитьПараметрыДвиженийРасходовПоНалогам(ТаблицаРасходов, ТаблицаРеквизитов)
	...
	СписокОбязательныхКолонок = ""
		...
		+ "СуммаРасходаЕНВД,"              // <Число, 15, 2> - сумма расхода на ЕНВД по доле начисления
		...
		+ "НачисленоЕНВД,"                 // <Число, 15, 2> - сумма текущей задолженности, начисленной по страховому взносу для расчета доли ЕНВД
		+ "ДоляЕНВД,"                      // <Число, 15, 2> - рассчитанная доля ЕНВД
		...
		+ "СуммаРасходаЕНВДРаспределяемая,"// <Число, 15, 2> - сумма расхода, которая должна быть распределена
...
Функция ПустаяТаблицаРасходов()
	...
	ТаблицаРасходов.Колонки.Добавить("НачисленоЕНВД", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
	ТаблицаРасходов.Колонки.Добавить("ДоляЕНВД", ОбщегоНазначения.ОписаниеТипаЧисло(7, 4));
	ТаблицаРасходов.Колонки.Добавить("СуммаРасхода", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
	ТаблицаРасходов.Колонки.Добавить("СуммаРасходаЕНВД", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
	ТаблицаРасходов.Колонки.Добавить("СуммаРасходаЕНВДРаспределяемая", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
...
Процедура ДобавитьРасходыПоВзносамСФОТ(РасходыПоВзносам, Реквизиты, ТаблицаРасходов)
	...
	Для каждого СтрокаВзноса Из РасходыПоВзносам Цикл
		...
		Если Уплата > 0 Тогда
			Разность = Уплата - НачальнаяЗадолженность;
			...
			Иначе
				СуммаРасхода = Мин(НачальнаяЗадолженность, Уплата)
			КонецЕсли;
		ИначеЕсли Начисление > 0 Тогда
			
			СуммаРасхода = Мин(Начисление, НачальнаяПереплата);
		...
		// Учтем распределение между основной системой и ЕНВД
		Если СтрокаВзноса.НачисленоЕНВД > 0 Тогда
			ДоляНачисленияЕНВД = ?(СтрокаВзноса.НачисленоВсего = 0, 0, СтрокаВзноса.НачисленоЕНВД/СтрокаВзноса.НачисленоВсего);
			СуммаРасходаЕНВД = Окр(СуммаРасхода * ДоляНачисленияЕНВД, 2, РежимОкругления.Окр15как20);
			СуммаРасхода = СуммаРасхода - СуммаРасходаЕНВД;
		КонецЕсли;
		...
		Если СуммаРасхода > 0 Или СуммаРасходаЕНВД > 0 Тогда
			...
			СтрокаРасхода.НачисленоЕНВД       = СтрокаВзноса.НачисленоЕНВД;
			СтрокаРасхода.СуммаРасхода        = СуммаРасхода;
			СтрокаРасхода.СуммаРасходаЕНВД    = СуммаРасходаЕНВД;
			СтрокаРасхода.ДоляЕНВД            = ДоляНачисленияЕНВД;
...
Процедура ДобавитьРасходыПоВыплатеБольничных(РасходыПоБольничным, Реквизиты, ТаблицаРасходов)
	...
	Для каждого СтрокаБольничного Из РасходыПоБольничным Цикл
		...
		СуммаРасходаЕНВД = Окр(СтрокаБольничного.БольничныйЗаСчетРаботодателяЕНВД*КоэффВыплаченности, 2, РежимОкругления.Окр15как20);
		...
		Если СуммаРасхода > 0 Или СуммаРасходаЕНВД > 0 Тогда
			...
			СтрокаРасхода.НачисленоЕНВД       = СтрокаБольничного.БольничныйЗаСчетРаботодателяЕНВД;
			СтрокаРасхода.СуммаРасхода        = СуммаРасхода;
			СтрокаРасхода.СуммаРасходаЕНВД    = СуммаРасходаЕНВД;
...
Процедура ДобавитьРасходыПоВзносамИП(Реквизиты, ТаблицаРасходов)
	...
	ПрименяетсяТолькоЕНВД = Реквизиты.ПлательщикЕНВД
		И УчетнаяПолитика.ПрименяетсяОсобыйПорядокНалогообложения(Реквизиты.Организация, Реквизиты.Период);
	
	ЕНВДУменьшаетсяНаВзносыИПРаботодателей = УчетЕНВД.НалогУменьшаетсяНаФиксированныеВзносыИПРаботодателей(Реквизиты.Период);
	...
	Если НЕ УменьшатьУСН И НЕ УменьшатьЕНВД Тогда
		Возврат;
	КонецЕсли;
	
	ТаблицаРасходовНаУплатуСтраховыхВзносов = УчетСтраховыхВзносовИП.ПодготовитьТаблицуРасходовУменьшающихНалог(Реквизиты.Организация, Реквизиты.Период);
	...
	Если ЕНВДУменьшаетсяНаВзносыИПРаботодателей Тогда
		...
		Иначе // Совмещение режимов
			КоэффициентЕНВД = КоэффициентДоходовЕНВДДляРаспределенияФиксированныхВзносов(Реквизиты.Организация, Реквизиты.Период);
		КонецЕсли;
	Иначе
		...
		КоэффициентЕНВД = ?(ПрименяетсяТолькоЕНВД, 1, 0);
	КонецЕсли;
	
	Для каждого Расход Из ТаблицаРасходовНаУплатуСтраховыхВзносов Цикл
		
		СуммаРасходаЕНВД = Окр(Расход.Сумма * КоэффициентЕНВД, 2, РежимОкругления.Окр15как20);
		СуммаРасхода     = ?(УменьшатьУСН, Расход.Сумма - СуммаРасходаЕНВД, 0);
		
		// Для вычитания из ЕНВД фиксированных взносов, уплачиваемых за прошлые годы,
		// требуется применение ЕНВД в периоде начисления.
		Если СуммаРасходаЕНВД > 0
		...
		Если СуммаРасхода = 0 И СуммаРасходаЕНВД = 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		НоваяСтрока.СуммаРасходаЕНВД = СуммаРасходаЕНВД;
		НоваяСтрока.ДоляЕНВД         = КоэффициентЕНВД;
		
		Если СуммаРасходаЕНВД > 0 Тогда
...
Процедура ВыявитьУчетныеОшибкиРасходовУменьшающихНалоги(Ошибки, Период, Организация) Экспорт
	...
	Если Не (УчетнаяПолитика.ПрименяетсяУСНДоходы(Организация, Период) Или УчетнаяПолитика.ПлательщикЕНВД(Организация, Период)) Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьПараметрыЗапросаРасходыУменьшающиеНалог(Запрос, Реквизиты, ДанныеСНачалаГода)
	
	// границы квартала
	Если ДанныеСНачалаГода Тогда
	...
	Запрос.УстановитьПараметр("КонецПрошлогоКварталаДата", НачалоПериода - 1);
...
Функция ТекстЗапросаРасходыПоВзносам(НомераТаблиц, ТаблицыДанных, Реквизиты)
	...
	Если Реквизиты.ПлательщикЕНВД Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_НачисленияВзносовДоляЕНВД.СчетВзноса КАК СчетВзноса,
		|	ВТ_НачисленияВзносовДоляЕНВД.ПериодНачисления КАК ПериодНачисления,
		|	СУММА(ВТ_НачисленияВзносовДоляЕНВД.НачисленоВсего) КАК НачисленоВсегоПоОрганизации
		|ПОМЕСТИТЬ ВТ_НачисленияИтоги
		|ИЗ
		|	ВТ_НачисленияВзносовДоляЕНВД КАК ВТ_НачисленияВзносовДоляЕНВД
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_НачисленияВзносовДоляЕНВД.СчетВзноса,
		|	ВТ_НачисленияВзносовДоляЕНВД.ПериодНачисления
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	СчетВзноса,
		|	ПериодНачисления
		...
		|	ВТ_НачисленияВзносовДоляЕНВД.СчетВзноса КАК СчетВзноса,
		|	ВТ_НачисленияВзносовДоляЕНВД.ПериодНачисления КАК ПериодНачисления,
		|	СУММА(ВТ_НачисленияВзносовДоляЕНВД.НачисленоЕНВД) КАК НачисленоЕНВД,
		|	ВТ_РегистрацииПодразделений.РегистрацияВНалоговомОргане,
		|	СУММА(ВТ_НачисленияВзносовДоляЕНВД.НачисленоВсего) КАК НачисленоВсего
		|ПОМЕСТИТЬ ВТ_НачисленияПоРегистрациям
		|ИЗ
		|	ВТ_НачисленияВзносовДоляЕНВД КАК ВТ_НачисленияВзносовДоляЕНВД
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_РегистрацииПодразделений КАК ВТ_РегистрацииПодразделений
		|		ПО ВТ_НачисленияВзносовДоляЕНВД.Подразделение = ВТ_РегистрацииПодразделений.Подразделение
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_НачисленияВзносовДоляЕНВД.СчетВзноса,
		|	ВТ_НачисленияВзносовДоляЕНВД.ПериодНачисления,
		...
		|	ЕСТЬNULL(ВТ_НачисленияПоРегистрациям.НачисленоЕНВД, 0) КАК НачисленоЕНВД,
	...
	Иначе
		...
		|	0 КАК НачисленоВсегоПоОрганизации
		|ИЗ
		|	ВТ_ОперацииПоВзносам КАК ВТ_ОперацииПоВзносам
		|ГДЕ
		|	ВТ_ОперацииПоВзносам.ДокументРасхода <> НЕОПРЕДЕЛЕНО
...
Функция ТекстЗапросаРасходыПоБольничным(НомераТаблиц, ТаблицыДанных, Реквизиты)
	...
	Если (Реквизиты.ЭтоИндивидуальныйПредприниматель И Не Реквизиты.ИПИспользуетТрудНаемныхРаботников)
		Или Не Реквизиты.ЕстьЗарплата Тогда
	...
	Если Реквизиты.ВедетсяУчетБольничных Тогда
		...
		|	СУММА(ВложенныйЗапрос.БольничныйЗаСчетРаботодателяЕНВД) КАК БольничныйЗаСчетРаботодателяЕНВД,
		|	ВложенныйЗапрос.Подразделение КАК Подразделение
		|ПОМЕСТИТЬ ВТ_НачисленияЗарплаты
		|ИЗ
		|	(ВЫБРАТЬ
		|		НАЧАЛОПЕРИОДА(ХозрасчетныйОборотыДтКт.Период, МЕСЯЦ) КАК ПериодНачисления,
		|		&ПолеРаботникОборотыДтКт КАК Сотрудник,
		|		ВЫБОР
		...
		|	&ПолеРаботникОбороты КАК Сотрудник
		...
		СтрПолеРаботникОборотыДтКт = ?(Реквизиты.ВедетсяУчетПоРаботникам, "ХозрасчетныйОборотыДтКт.СубконтоКт1", "НЕОПРЕДЕЛЕНО");
	...
	Иначе
		...
		СтрПолеФизЛицо = ?(Реквизиты.ВедетсяУчетПоРаботникам, "ТЧНачисленнаяЗарплатаИВзносы.ФизическоеЛицо", "НЕОПРЕДЕЛЕНО");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПолеФизЛицо", СтрПолеФизЛицо);

	КонецЕсли;
	
	СтрПолеРаботникОбороты = ?(Реквизиты.ВедетсяУчетПоРаботникам, "ХозрасчетныйОбороты.Субконто1", "НЕОПРЕДЕЛЕНО");
...
Функция ТекстЗапросаВыручкаПоРегистрациям(НомераТаблиц, ТаблицыДанных, Реквизиты)
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоПериодаДата, &КонецПериодаДата, , , , СчетКт = &СчетВыручки, , Организация = &Организация) КАК ХозрасчетныйОборотыДтКт
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_РегистрацииПодразделений КАК ВТ_РегистрацииПодразделений
...
Процедура РаспределитьПоРегистрациям (ВыручкаПоРегистрациям, Реквизиты, ТаблицаРасходов)
	...
	Если Реквизиты.УчетПоПодразделениям Тогда
		...
		Для каждого Расход Из РасходыДоРаспределения Цикл
			...
			Иначе
				...
				СуммыРасходаЕНВД    = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(Расход.СуммаРасходаЕНВД, КоэффициентыВыручки, 2);
				...
				СуммыНачисленоЕНВД  = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(Расход.НачисленоЕНВД, КоэффициентыВыручки, 2);
				...
				Для Счетчик = 0 По КоэффициентыВыручки.Количество() - 1 Цикл
					...
					СтрокаРасходов.СуммаРасходаЕНВДРаспределяемая = Расход.СуммаРасходаЕНВД;
					
					СтрокаРасходов.СуммаРасходаЕНВД = ?(СуммыРасходаЕНВД = Неопределено, 0, СуммыРасходаЕНВД[Счетчик]);
					СтрокаРасходов.СуммаРасхода     = ?(СуммыРасхода = Неопределено, 0, СуммыРасхода[Счетчик]);
					СтрокаРасходов.СуммаОплаты      = ?(СуммыЗадолженность = Неопределено, 0, СуммыОплаты[Счетчик]);
					...
					СтрокаРасходов.НачисленоЕНВД    = ?(СуммыНачисленоЕНВД = Неопределено, 0, СуммыНачисленоЕНВД[Счетчик]);
	...
	Иначе
		...
		Для каждого СтрокаРасхода Из ТаблицаРасходов Цикл
			Если СтрокаРасхода.СуммаРасходаЕНВД>0 И Не ЗначениеЗаполнено(СтрокаРасхода.РегистрацияВНалоговомОргане) Тогда
				Если ЗначениеЗаполнено(Реквизиты.РегистрацияЕНВДОрганизации) Тогда
