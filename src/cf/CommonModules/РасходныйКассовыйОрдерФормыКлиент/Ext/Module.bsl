...
Процедура ВедомостьЗарплатаРаботникуОчистка(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	РасходныйКассовыйОрдерФормыКлиентСервер.УстановитьПустоеЗначениеВыплатаЗарплатыВедомость(Элемент, 
...
Процедура РасшифровкаПлатежаПриНачалеРедактирования(Форма, Элемент, НоваяСтрока, Копирование) Экспорт
	...
	Если НоваяСтрока И Форма.ЕстьРасчетыВУсловныхЕдиницах Тогда
		Если Не РасходныйКассовыйОрдерФормыКлиентСервер.ФормаДокументаОднострочная(Форма) Тогда
			РасходныйКассовыйОрдерФормыКлиентСервер.ЗаполнитьНадписиВРасшифровке(Элементы.РасшифровкаПлатежа.ТекущиеДанные);
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыплатаДепонентовВедомостьОчистка(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	РасходныйКассовыйОрдерФормыКлиентСервер.УстановитьПустоеЗначениеВыплатаДепонентовВедомость(ТекущиеДанные.Ведомость, Форма.УчетЗарплатыИКадровВоВнешнейПрограмме);
...
Процедура ПолучитьПараметрыЗавершение(ПараметрыФискальногоУстройства, Параметры) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		МассивЧеков = РасходныйКассовыйОрдерВызовСервера.ПодготовитьДанныеДляПробитияЧека(Объект.Ссылка, НомерСекции, ВерсияФормата, ОписаниеОшибки);
	КонецЕсли;
...
Процедура НапечататьОдинЧек(Форма, МассивЧеков, ИдентификаторУстройства, ВерсияФормата)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ПараметрыПечатиЧека",     ОбщиеПараметры);
	ПараметрыФормы.Вставить("Организация",             Объект.Организация);
