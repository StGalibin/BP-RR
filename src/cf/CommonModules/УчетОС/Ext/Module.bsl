...
Процедура СформироватьДвиженияПоступлениеОсновныхСредств(ТаблицаОсновныеСредства, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Параметры.ОсновныеСредства.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для каждого СтрокаТаблицы Из Параметры.ОсновныеСредства Цикл
		...
		Если ОтражатьВНалоговомУчете Тогда
			...
			Если (СтрокаТаблицы.СуммаНУ + ?(НДСУчитываетсяВCтоимости, СтрокаТаблицы.СуммаНДСРуб, 0))
				...
				Проводка.Период      = Реквизиты.Период;
				Проводка.Организация = Реквизиты.Организация;
				Проводка.Содержание  = НСтр("ru = 'Включение стоимости в состав расходов (НУ)'");

				Проводка.СчетДт = ОсновнойСчетУчетаЗатрат;

				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 
					"НоменклатурныеГруппы", ОсновнаяНоменклатурнаяГруппа);
					
				СтатьяЗатрат = Справочники.СтатьиЗатрат.СтатьяЗатратПоНазначению("СписаниеМатериалов");
				Если СтатьяЗатрат = Неопределено Тогда
					СтатьяЗатрат = Справочники.СтатьиЗатрат.ПредопределенныйЭлементСписаниеМатериалов();
				КонецЕсли;
				...
				Проводка.СуммаНУДт = СтрокаТаблицы.СуммаНУ + ?(НДСУчитываетсяВCтоимости, СтрокаТаблицы.СуммаНДСРуб, 0);
				Проводка.СуммаПРДт = 0;
				Проводка.СуммаВРДт = - (СтрокаТаблицы.СуммаНУ + ?(НДСУчитываетсяВCтоимости, СтрокаТаблицы.СуммаНДСРуб, 0));
				
				Проводка.СуммаНУКт = СтрокаТаблицы.СуммаНУ + ?(НДСУчитываетсяВCтоимости, СтрокаТаблицы.СуммаНДСРуб, 0);
				Проводка.СуммаПРКт = 0;
				Проводка.СуммаВРКт = - (СтрокаТаблицы.СуммаНУ + ?(НДСУчитываетсяВCтоимости, СтрокаТаблицы.СуммаНДСРуб, 0));
		...
		Если ОтражатьВНалоговомУчете Или ПлательщикНДФЛ Тогда
			...
			Если ОтражатьВНалоговомУчете 
				...
				Движение.ПорядокВключенияСтоимостиВСоставРасходов = 
...
Функция ПодготовитьТаблицуПервоначальныеСведенияОСНУ(ТаблицаПервоначальныеСведенияОС, ТаблицаРеквизиты, Отказ) Экспорт
	...
	Если ОтражатьВНалоговомУчете Тогда
		...
		Если Реквизиты.СпособПоступления = Перечисления.СпособыПоступленияАктивов.ПоДоговоруЛизинга Тогда
			ТаблицаПервоначальныеСведенияОСНУ.ЗаполнитьЗначения(Реквизиты.ПервоначальнаяСтоимостьНУ, "СуммаНУ");
		КонецЕсли;
	Иначе
...
Функция ПодготовитьТаблицуАмортизацияОС(ТаблицаОС, ТаблицаРеквизиты, Отказ) Экспорт
	...
	Если ОтражатьВНалоговомУчете И НЕ ЗначениеЗаполнено(МетодНачисленияАмортизацииНУ) Тогда
		Если Реквизиты.ВыдаватьСообщения Тогда
			ШаблонСообщения = НСтр("ru = 'Не указан метод начисления амортизации в налоговом учете организации %1
...
Функция ПолучитьТекстЗапросаАмортизацияОСДанныеПроводок()
	...
	|	ВЫБОР
	|		КОГДА АмортизацияОСОбороты.СуммаНУОборотКт < 0
	|			ТОГДА 0
	|		ИНАЧЕ АмортизацияОСОбороты.СуммаНУОборотКт
...
Процедура СформироватьДвиженияПеремещениеОС(ТаблицаОС, ТаблицаРеквизиты, ПараметрыНачисленияАмортизации, Движения, Отказ) Экспорт
	...
	Если ЗначениеЗаполнено(Реквизиты.СчетУчета) Тогда
		...
		Для Каждого СтрокаТаблицы Из ТаблицаОстатковПоОС Цикл
			Если ЗначениеЗаполнено(СтрокаТаблицы.СчетУчета) 
			   И ЗначениеЗаполнено(СтрокаТаблицы.СчетНачисленияАмортизации)
...
Функция ПодготовитьТаблицыИзмененияПараметровАмортизацииОС(ТаблицаОС, ТаблицаРеквизиты, Отказ) Экспорт
	...
	ТаблицаАмортизацииБух = ПодготовитьТаблицуАмортизацияОС(СписокОС, ПараметрыАмортизации, Неопределено);
...
Процедура СформироватьДвиженияПоступлениеПредметыЛизинга(ТаблицаПредметыЛизинга, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Параметры.ТаблицаПредметыЛизинга.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура СформироватьДвиженияКорректировкиАмортизацииПоАренднымПлатежамНУ(ТаблицаКорректировкиАмортизации, 
	...
	Если Параметры.ТаблицаКорректировкиАмортизации.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
