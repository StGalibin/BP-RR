...
//	ПервыйНеисключенныйМесяц - Дата начала месяца, начиная с которого необходимо анализировать.
//
// Возвращаемое значение:
//	МоментВремени, с которого последовательность нарушена
//		или Неопределено, если нарушения последовательности не обнаружено
//
Функция МоментНачалаПерепроведенияСУчетомИсключенныхМесяцев(Организация, ДоДаты, ПервыйНеисключенныйМесяц = Неопределено) Экспорт

	// Если месяц был пропущен, то находим ближайщий следующий за ним,
	// который не является исключенным.

	Если ПервыйНеисключенныйМесяц <> Неопределено Тогда
		ДатаНачала = ПервыйНеисключенныйМесяц;
	Иначе
		ДатаНачала = РегистрыСведений.ИсключенныеИзПерепроведенияМесяцы.ПервыйНеисключенныйМесяц(Организация, ДоДаты);
	КонецЕсли;
...
//
Функция МоментНачалаПерепроведенияСУчетомИсключенныхМесяцевИОтложенныхРасчетов(Организация, ДоДаты) Экспорт

	ПервыйНеисключенныйМесяц = РегистрыСведений.ИсключенныеИзПерепроведенияМесяцы.ПервыйНеисключенныйМесяц(Организация, ДоДаты);

	Результат = МоментНачалаПерепроведенияСУчетомИсключенныхМесяцев(Организация, ДоДаты, ПервыйНеисключенныйМесяц);
...
//	Момент времени
//
Функция МоментВремениПервогоДокументаВПоследовательности(Организация, НачинаяСДаты, УчитыватьОтложенноеПроведение = Истина) Экспорт
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'РаботаСПоследовательностями'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СоздатьМоментВремени(Период, Регистратор)
	...
	// Иногда ссылки на документ в последовательности оказываются "битыми".
	// Если момент нарушения последовательности приходится на такую битую ссылку, то это может мешать 
	// нормальному восстановлению хронологической последовательности. 
	// Поэтому если обнаруживается такая битая ссылка, то момент нарушения последовательсти определяем просто по дате,
	// а саму битую ссылку постараемся удалить из последовательности.
			...
			Если ПравоДоступа("Изменение", Метаданные.Последовательности.ДокументыОрганизаций) Тогда
				...
				Исключение
					...
					ТекстСообщения = СтрШаблон(
						НСтр("ru = 'Ошибка при удалении отсутствующей ссылки на документ из набора последовательности ДокументыОрганизаций.
			                  |%1'"),
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	// Регистратор в последовательности не задан либо его ссылка "битая", определяем момент времени только по дате.
	Если Результат = Неопределено Тогда
...
//
Процедура ЗарегистрироватьУстареваниеОперации(Объект, УстаревшаяРегламентнаяОперация)
...
Процедура СброситьГраницыПоследовательностьВзаиморасчеты30Отложенно(Параметры) Экспорт

	СброситьГраницыПоследовательностиОтложенно("УдалитьВзаиморасчеты30", "ДоговорКонтрагента", Параметры);
...
Процедура СброситьГраницыПоследовательностьОбщаяПоследовательность30Отложенно(Параметры) Экспорт

	СброситьГраницыПоследовательностиОтложенно("УдалитьОбщаяПоследовательность30", "Организация", Параметры);
...
Процедура ИсключитьСчетаФактурыИзПоследовательностиДокументыОрганизаций(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось очистить последовательность ""Документы организаций"" по причине:
					|%1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), 
